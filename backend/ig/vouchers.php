<?php 

 return [
     "ONE_MONTH_STARTUP" => [
     "title" => "Atụmatụ mmalite n'efu otu ọnwa",
     "image" => "/images/email/vouchers/one-month-startup.png",
     "price" => 9.99,
     "currency" => "USD",
],
     "THREE_MONTHS_STARTUP" => [
     "title" => "Atụmatụ mmalite n'efu ọnwa atọ",
     "image" => "/images/email/vouchers/three-months-startup.png",
     "price" => 30,
     "currency" => "USD",
],
     "SIX_MONTHS_STARTUP" => [
     "title" => "Atụmatụ mmalite ọnwa isii efu",
     "image" => "/images/email/vouchers/six-months-startup.png",
     "price" => 60,
     "currency" => "USD",
],
     "TWELVE_MONTHS_STARTUP" => [
     "title" => "Atụmatụ mmalite n'efu ọnwa iri na abụọ",
     "image" => "/images/email/vouchers/twelve-months-startup.png",
     "price" => 120,
     "currency" => "USD",
],
     "ONE_MONTH_COMPANY" => [
     "title" => "Atụmatụ ụlọ ọrụ n'efu otu ọnwa",
     "image" => "/images/email/vouchers/one-month-company.png",
     "price" => 69,
     "currency" => "USD",
],
     "THREE_MONTHS_COMPANY" => [
     "title" => "Atụmatụ ụlọ ọrụ n'efu ọnwa atọ",
     "image" => "/images/email/vouchers/three-months-company.png",
     "price" => 207,
     "currency" => "USD",
],
     "SIX_MONTHS_COMPANY" => [
     "title" => "Atụmatụ ụlọ ọrụ n'efu ọnwa isii",
     "image" => "/images/email/vouchers/six-months-company.png",
     "price" => 414,
     "currency" => "USD",
],
     "TWELVE_MONTHS_COMPANY" => [
     "title" => "Atụmatụ ụlọ ọrụ n'efu ọnwa iri na abụọ",
     "image" => "/images/email/vouchers/twelve-months-company.png",
     "price" => 828,
     "currency" => "USD",
],
     "ONE_MONTH_ENTERPRISE" => [
     "title" => "Atụmatụ ụlọ ọrụ n'efu otu ọnwa",
     "image" => "/images/email/vouchers/one-month-enterprise.png",
     "price" => 299,
     "currency" => "USD",
],
     "THREE_MONTHS_ENTERPRISE" => [
     "title" => "Atụmatụ ụlọ ọrụ n'efu ọnwa atọ",
     "image" => "/images/email/vouchers/three-months-enterprise.png",
     "price" => 897,
     "currency" => "USD",
],
     "SIX_MONTHS_ENTERPRISE" => [
     "title" => "Atụmatụ ụlọ ọrụ n'efu nke ọnwa isii",
     "image" => "/images/email/vouchers/six-months-enterprise.png",
     "price" => "1,794",
     "currency" => "USD",
],
     "TWELVE_MONTHS_ENTERPRISE" => [
     "title" => "Atụmatụ ụlọ ọrụ n'efu nke ọnwa iri na abụọ",
     "image" => "/images/email/vouchers/twelve-months-enterprise.png",
     "price" => "3,588",
     "currency" => "USD",
],
     "register_voucher" => [
     "title" => "Ekele, deba aha ma nweta kaadị onyinye a",
     "image" => "/images/email/vouchers/one-month-startup.png",
     "delay" => 5000,
     "header" => "/images/popups/referral/m1.jpg",
     "color" => "#d83852",
     "color_btn" => "#C2185B",
     "contain" => true,
     "message" => "Ị banyere na saịtị ahụ site na njikọ nke onye na-arụsi ọrụ ike nke Selldone, ọ bụrụ na ị debanyere aha na Selldone ugbu a, ị ga-enweta kaadị onyinye bara uru $ 14.9. Ịdenye aha na iwu ụlọ ahịa na Selldone bụ 100% n'efu, mana ị nwere ike iji kaadị fanye ndị a iji weghachi ọrụ.",
],
];