export default {
  "currency_default": "USD",
  "language_default": "en",
  "selldone": "Վաճառված է",
  "store": "խանութ",
  "not_exist": "Գոյություն չունի",
  "error": "Սխալ",
  "active": "Ակտիվ",
  "inactive": "ոչ ակտիվ",
  "required_value": "Պահանջվող արժեքներ",
  "original_warranty": "Ապրանքի բնօրինակ երաշխիք",
  "return_warranty_days": "{days} վերադարձի երաշխիքի օր(ներ)",
  "no_return_warranty": "Ոչ վերադարձ",
  "reed_agree_terms": "Ես կարդացել եմ օգտագործման պայմանները և գաղտնիության քաղաքականությունը և համաձայն եմ դրանց հետ:",
  "commons": {
    "Currency": "Արժույթ",
    "COD": "COD",
    "State": "Պետություն",
    "Price": "գինը",
    "order_price": "Պատվերի գումարը",
    "VAT": "VAT",
    "Discount": "Զեղչ",
    "In_Stock": "Պահեստում",
    "Variants": "Տարբերակներ",
    "No_variants": "Ոչ մի տարբերակ",
    "all": "ԲՈԼՈՐ",
    "today": "Այսօր",
    "yesterday": "Երեկ",
    "license": "Լիցենզիա",
    "home": "տուն",
    "support": "Աջակցություն",
    "messages": "Հաղորդագրություններ",
    "no_message": "Ոչ մի հաղորդագրություն",
    "empty": "Դատարկ",
    "phone": "Հեռախոս",
    "address": "հասցեն",
    "name": "Անուն",
    "password": "Գաղտնաբառ",
    "email": "Էլ",
    "mobile": "Բջջային",
    "login": "մուտք",
    "register": "Գրանցվել",
    "select_account": "Ընտրեք հաշիվ",
    "online": "Առցանց",
    "offline": "Անցանց",
    "finish": "Ավարտել",
    "finished": "Ավարտված",
    "preview": "Նախադիտում",
    "free": "Անվճար",
    "free_completely": "Լիովին անվճար բաժանորդագրություն",
    "fee": "Վճար",
    "month": "Ամիս",
    "months": "Ամիսներ",
    "faq": "Հաճախակի տրվող հարցեր",
    "question": "Հարց",
    "answer": "Պատասխանել",
    "create_date": "Ստեղծել ամսաթիվ",
    "update_date": "Թարմացման ամսաթիվը",
    "start_date": "Մեկնարկի ամսաթիվ",
    "end_date": "Ավարտի ամսաթիվը",
    "social": "Հասարակական",
    "total": "Ընդամենը",
    "orders": "Պատվերներ",
    "sessions": "Նիստեր",
    "product_views": "Ապրանքի դիտումներ",
    "add_to_carts": "Ավելացնել զամբյուղներում",
    "checkouts": "Ստուգում",
    "payments": "Վճարում",
    "organic_search": "Օրգանական որոնում",
    "other": "Այլ",
    "landing_page": "Landing Page",
    "discount_code": "Զեղչի կոդը",
    "total_sell": "Ընդհանուր վաճառք",
    "details": "Մանրամասներ",
    "currency": "Արժույթ",
    "amount": "Գումարը",
    "published": "Հրապարակվել է",
    "draft": "Նախագիծ",
    "save_changes": "Պահպանել փոփոխությունները",
    "source": "Աղբյուր",
    "medium": "Միջին",
    "url": "URL",
    "revenue": "Եկամուտներ",
    "performance": "Կատարում",
    "term": "Ժամկետ",
    "content": "Բովանդակություն",
    "session": "Նիստ",
    "product": "Արտադրանք",
    "add": "Ավելացնել",
    "checkout": "Ստուգում",
    "payment": "Վճարում",
    "more_options": "Ավելի շատ տարբերակներ",
    "full_name": "Ամբողջական անուն",
    "tel": "Հեռ",
    "about_me": "Իմ մասին",
    "request_checking": "Ստուգման հարցում",
    "passport": "Անձնագիր",
    "birthday": "Ծննդյան օր",
    "country": "Երկիր",
    "city": "Քաղաք",
    "gender": "Սեռ",
    "offer": "Առաջարկ",
    "website": "Կայք",
    "apply_for_all_products": "∞ Դիմել բոլոր ապրանքների համար",
    "fix_commission": "Հաստատուն հանձնաժողով",
    "percent_commission": "Տոկոսային հանձնաժողով",
    "fixed_discount": "Ֆիքսված զեղչ",
    "discount_percentage": "Տոկոսային զեղչ",
    "discount_limit": "Զեղչի սահմանաչափ",
    "no_code": "Կոդ չկա",
    "max_count": "Առավելագույն հաշվարկ",
    "count_unit": "Նյութեր",
    "used": "Օգտագործված",
    "daily": "Ամենօրյա",
    "monthly": "Ամսական",
    "yearly": "Տարեկան",
    "state": "Կարգավիճակ",
    "disable": "Անջատել",
    "enable": "Միացնել",
    "activation_code": "Ակտիվացման կոդը",
    "quantity": "Քանակ",
    "no_club": "Ոչ մի ակումբ",
    "per_order": "Ըստ պատվերի",
    "count": "հաշվել",
    "seconds": "Վայրկյաններ",
    "auto": "Ավտոմատ",
    "title": "Կոչում",
    "message": "Հաղորդագրություն",
    "width": "Լայնությունը",
    "length": "Երկարություն",
    "height": "Բարձրություն",
    "label": "Պիտակ",
    "click": "Սեղմել",
    "bank": "Բանկ",
    "default": "Կանխադրված",
    "role": "Դեր",
    "user": "Օգտատեր",
    "users": "Օգտատերեր",
    "code": "Կոդ",
    "total_payment": "Ընդհանուր վճարում",
    "balance": "Հաշվեկշիռ",
    "chance": "Հնարավորություն",
    "free_for_new_users": "Անվճար նոր օգտվողների համար",
    "prize_amount": "Մրցանակի գումարը",
    "win": "Հաղթել",
    "gift_card": "Նվեր քարտ",
    "discount": "Զեղչ",
    "config": "Կազմաձև",
    "prize": "Մրցանակ",
    "Prizes": "Մրցանակներ",
    "result": "Արդյունք",
    "price": "գինը",
    "lottery": "Վիճակախաղ",
    "coupon": "Կտրոն",
    "customer_club": "Հաճախորդների Ակումբ",
    "shipping": "առաքում",
    "sum": "Գումար",
    "not_paid": "Չվճարված",
    "category": "Կարգավիճակ",
    "done": "Կատարած",
    "email_from": "Սկսած",
    "subject": "Առարկա",
    "schedule": "Ժամանակացույց",
    "files_list": "Ֆայլերի ցուցակ",
    "file_max_size": "Ֆայլի առավելագույն չափը",
    "value": "Արժեք",
    "help": "Օգնություն",
    "shipping_tax": "Առաքման հարկ",
    "net_price": "Զուտ գինը",
    "tax": "Հարկային",
    "tax_included": "Ներառված է գնի մեջ",
    "limit": "Սահման",
    "min_purchase": "Նվազագույն գնում",
    "eligible_for": "Իրավասու է",
    "only_for_first_buy": "Միայն առաջին գնման համար",
    "one_time_use": "Կարող է օգտագործվել մեկ անգամ",
    "period_of_qualify": "Ժամանակահատվածի օգտագործման սահմանափակում",
    "no_constraint": "Ոչ մի սահմանափակում",
    "timespan": "Ժամանակահատված",
    "days": "Օր(ներ)",
    "weight": "Քաշը",
    "distance": "Հեռավորությունը",
    "last_status": "Վերջին կարգավիճակը",
    "issued_at": "Թողարկվել է",
    "order": "Պատվեր",
    "order_detail": "Պատվերի տվյալներ",
    "percent": "տոկոս",
    "applied_count": "Կիրառված հաշվարկ",
    "campaign": "Քարոզարշավ",
    "os": "Օպերացիոն համակարգ",
    "browser": "Բրաուզերը",
    "device": "Սարք",
    "device_ip": "Սարքի IP հասցե",
    "location": "Գտնվելու վայրը",
    "user_agent": "Օգտագործողի գործակալ",
    "affiliate": "Գործընկերություն",
    "timeline": "Ժամանակացույց",
    "add_note": "Ավելացնել նշում",
    "record": "Գրառում",
    "unlimited": "Անսահմանափակ",
    "show_coupons": "Ցույց տալ կտրոնները",
    "hide_coupons": "Թաքցնել կտրոնները",
    "service_area": "Սպասարկման տարածք",
    "apps": "Հավելվածներ",
    "plugins": "Փլագիններ",
    "bots": "Բոտեր",
    "categories": "Կատեգորիաներ",
    "products": "Ապրանքներ",
    "business": "Բիզնես",
    "personal": "Անձնական",
    "size": "Չափը",
    "holiday": "Տոն",
    "sod": "Վճարեք առաքման ժամանակ",
    "cod": "Կանխիկ առաքում",
    "free_shipping": "Անվճար առաքում",
    "hours": "Ժամեր",
    "send_date": "Առաքման ամսաթիվը",
    "deliver_date": "Առաքման օրը",
    "return_date": "Վերադարձի ամսաթիվը",
    "pending": "Սպասվում է",
    "delivered": "Հանձնվել է",
    "returned": "Վերադարձել է",
    "canceled": "Չեղարկված է",
    "shipping_cost": "Առաքման ծախսերը",
    "action": "Գործողություն",
    "cancel_date": "Չեղարկել ամսաթիվը",
    "deliver_in_process": "Պատվերը առաքվում է..",
    "kind": "Տիպ",
    "detail": "Մանրամասներ",
    "map": "Քարտեզ",
    "actions": "Գործողություններ",
    "navigation": "Նավիգացիա",
    "navigation_app": "Դիմում",
    "payment_date": "Վճարման օր",
    "duration": "Տեւողությունը",
    "bill": "Բիլ",
    "delivery_persons": "Սուրհանդակներ",
    "delivery_services": "Առաքման ծառայություններ",
    "live_view": "Ուղիղ դիտում",
    "cart": "Սայլակ",
    "priority": "Առաջնահերթություն",
    "sufficient_credit": "Բավարար վարկ",
    "max_package_size": "Փաթեթի առավելագույն չափը",
    "delay": "Հետաձգում",
    "credit": "Վարկ",
    "livemode": "Ապրեք",
    "paused": "Դադարեցված է",
    "debug": "Վրիպազերծում",
    "token": "Նշան",
    "status": "Կարգավիճակ",
    "courier": "Սուրհանդակ",
    "track": "Հետևել",
    "track_order": "Պատվերների Հետևում",
    "cost": "Արժեքը",
    "receipt": "Անդորրագիր",
    "origin": "Ծագում",
    "destination": "Նպատակակետ",
    "sku": "SKU",
    "mpn": "MPN",
    "not_in_stock": "Բավարար պաշար չկա:",
    "sub_total": "Ենթագումար",
    "customer": "Հաճախորդ",
    "delivery": "Առաքում",
    "online_orders": "Առցանց պատվերներ",
    "pos_orders": "POS պատվերներ",
    "chips": "Չիպսեր",
    "subscribed": "Բաժանորդագրված է",
    "unsubscribed": "Չեղարկել բաժանորդագրությունը",
    "last_login": "Վերջին մուտք",
    "join_at": "Միանալու ամսաթիվը",
    "club": "Ակումբ",
    "buyer": "Գնորդ",
    "seller": "Վաճառող",
    "or": "Կամ",
    "type": "Տիպ",
    "condition": "Վիճակ",
    "brand": "Ապրանքանիշը",
    "attempt": "Փորձ",
    "error": "Սխալ",
    "success": "Հաջողություն",
    "fail": "Ձախողվել",
    "inventory": "Գույքագրում",
    "link": "Հղում",
    "visits": "Այցելություններ",
    "transactions": "Գործարքներ",
    "vat": "VAT",
    "pos_vat": "POS ԱԱՀ",
    "sort_by": "Դասավորել ըստ..",
    "from": "Սկսած",
    "from_lead_time": "Հաջորդ {hours} ընթացքում",
    "no_credit_card_info": "Քարտի տեղեկություններ չկան:",
    "active": "Ակտիվ",
    "not_active": "Ակտիվացված չէ:",
    "referrals": "Ուղղորդումներ",
    "dashboard": "Վահանակ",
    "more": "Ավելին",
    "inactive": "ոչ ակտիվ",
    "developer": "Մշակող",
    "setting": "Կարգավորում",
    "membership_conditions": "Անդամակցության պայմանները",
    "customers": "Հաճախորդներ",
    "created_at": "Ստեղծման ամսաթիվը",
    "updated_at": "Վերջին թարմացում",
    "register_at": "Գրանցման ամսաթիվը",
    "old": "Հին",
    "new": "Նոր",
    "start": "Սկսել",
    "end": "Վերջ",
    "pos": "POS",
    "out_of_stock": "Առկա չէ պահեստում",
    "fake": "Կեղծ",
    "low_inventory_warning": "Ուշադրություն, ցածր պաշար:",
    "webhook": "Webhook",
    "notification": "Ծանուցում",
    "sms": "SMS",
    "online_payment_link": "Առցանց վճարման հղում",
    "waiting_load_data": "Սպասում է տվյալների բեռնմանը..",
    "views": "Դիտումներ",
    "sales": "Վաճառք",
    "total_views": "Ընդհանուր դիտումներ",
    "total_sales": "Ընդհանուր վաճառք",
    "from_previous_n_days": "Վերջին {days} օրերից",
    "drop_shipping": "Dropshipping",
    "commission": "հանձնաժողով",
    "shipping_area": "Առաքման տարածք",
    "main_seller": "Հիմնական վաճառող",
    "domain": "Դոմեն",
    "contact": "Կոնտակտային տվյալները",
    "wallet": "Դրամապանակ",
    "reseller": "Վերավաճառող",
    "tracking_code": "Հետևման կոդը",
    "tracking_url": "Հետևման URL",
    "fulfillments": "Իրականացումներ",
    "reject": "Մերժել",
    "reserved": "Վերապահված է",
    "score": "Միավոր",
    "customer_price": "Հաճախորդի գինը",
    "options": "Ընտրանքներ",
    "show_details": "Ցույց տալ մանրամասները",
    "no_data": "Տվյալներ չկան..",
    "waiting": "Սպասում...",
    "waiting_connecting_payment": "Սպասում է վճարման ծառայությանը...",
    "creating_secure_payment": "Ապահով վճարումների ստեղծում:",
    "officer": "սպա",
    "ticket": "Տոմս",
    "ticket_url": "Տոմսի url",
    "closed_at": "Փակվել է",
    "close_ticket": "Փակել տոմսը",
    "reopen_ticket": "Վերաբացել տոմսը",
    "require": "Պահանջել*",
    "search": "Որոնել...",
    "language": "Լեզու",
    "tasks": "Առաջադրանքներ",
    "permissions": "Թույլտվություններ",
    "finish_date": "Ավարտի ամսաթիվը",
    "complete_date": "Ավարտման ամսաթիվ",
    "settle_date": "Հաշվարկի ամսաթիվը",
    "cost_span": "Գնային միջակայք",
    "industry": "Արդյունաբերություն",
    "rating": "Վարկանիշներ",
    "rates": "Վարկանիշ",
    "seen_at": "Տեսել է",
    "completed": "Ավարտված է",
    "you": "Դուք",
    "shop": "Խանութ",
    "store": "խանութ",
    "granted": "Տրված է",
    "deny": "Հերքել",
    "request": "Հայց",
    "expert": "Փորձագետ",
    "response": "Պատասխան",
    "requests": "հարցումներ",
    "experts": "Փորձագետներ",
    "responses": "Պատասխաններ",
    "negotiations": "Բանակցություններ",
    "all_chats": "Բոլոր զրույցները",
    "chats": "Զրույցներ",
    "tags": "Պիտակներ",
    "description": "Նկարագրություն",
    "rates_count": "Վարկանիշների քանակը",
    "rate": "Վարկանիշ",
    "comment": "Մեկնաբանություն",
    "chat": "Զրուցարան",
    "install": "Տեղադրեք",
    "edited": "Խմբագրվել է",
    "undo": "Հետարկել",
    "redo": "Կրկնել",
    "male": "Արական",
    "female": "իգական",
    "calculator": "Հաշվիչ",
    "catalog": "Կատալոգ",
    "max_height": "Առավելագույն բարձրություն",
    "images_compare": "Պատկերների համեմատություն",
    "canvas": "Կտավ",
    "table_of_contents": "Բովանդակություն",
    "comments": "Մեկնաբանություններ",
    "badge_codes": "Նշանակների կոդը",
    "need_more": "Պե՞տք է ավելին:",
    "invite_glow_caption": "Ստացեք 14,9 դոլար յուրաքանչյուր ընկերոջ համար:",
    "register_free": "100% անվճար գրանցում",
    "gift": "Նվեր",
    "google": "Google",
    "facebook": "Ֆեյսբուք",
    "instagram": "Instagram",
    "instashop": "Instashop",
    "show": "Ցուցադրում",
    "hide": "Թաքցնել",
    "waiting_review": "Սպասում է վերանայմանը",
    "not_selected": "Ընտրված չէ",
    "time": "Ժամանակը",
    "final_price": "Վերջնական գին",
    "starting_price": "Մեկնարկային գին",
    "day": "Օր",
    "paid": "Վճարված",
    "service_bill": "Հաշիվ-ապրանքագրի վճարում",
    "avocado": "Ավոկադո",
    "delivery_price": "Առաքման ծախսերը",
    "select_address": "Ընտրեք հասցեն",
    "image": "Պատկեր",
    "view": "Դիտել",
    "sell": "Վաճառք",
    "hyper": "Հիպեր",
    "cart_list": "Սայլակ",
    "barcode_scanner": "Շտրիխ կոդի սկաներ",
    "scan_qr_code": "Սկանավորեք QR կոդը",
    "new_carts": "Նոր սայլակներ",
    "pays": "Վճարումներ",
    "annually": "Տարեկան կտրվածքով",
    "sells": "Վաճառք",
    "articles": "Հոդվածներ",
    "jobs": "Աշխատանքներ",
    "job": "Աշխատանք",
    "blog": "Բլոգ",
    "deleted_at": "Ջնջվել է",
    "capacity": "Տարողություն",
    "download_at": "Վերջին ներբեռնման ամսաթիվը",
    "downloads": "Ներբեռնումներ",
    "usage": "Օգտագործումը",
    "select_upload_file": "Ընտրեք կամ բաց թողեք ֆայլը",
    "download_link": "Ներբեռնման հղումը",
    "uploading": "Վերբեռնվում է..",
    "files": "Ֆայլեր",
    "star": "Աստղ",
    "speed": "Արագություն",
    "feedback": "Հետադարձ կապ",
    "new_messages": "Նոր հաղորդագրություններ",
    "file": "Ֆայլ",
    "attachment": "Հավելված",
    "gallery": "Պատկերասրահ",
    "sync": "Համաժամեցում",
    "diff_time": "Ժամանակային տարբերություն",
    "network": "Ցանց",
    "account": "Հաշիվ",
    "configuration": "Կոնֆիգուրացիա",
    "blockchain": "Բլոկչեյն",
    "blog_link": "Բլոգի հղում",
    "community": "Համայնք",
    "critical_zone": "Կրիտիկական գոտի",
    "statistics": "Վիճակագրություն",
    "access": "Մուտք",
    "seo": "SEO",
    "followers": "Հետևորդներ",
    "followings": "Հետևյալներ",
    "private": "Նախադիտում",
    "soon": "Շուտով",
    "features": "Հատկություններ",
    "bot": "Բոտ",
    "sample": "Նմուշ",
    "how_it_work": "Ինչպես է դա աշխատում?",
    "page": "Էջ",
    "congratulations": "Շնորհավորում եմ",
    "recommend": "Խորհուրդ տալ",
    "recommended": "Առաջարկվում է",
    "recoms": "Առաջարկություններ",
    "likes": "Հավանումներ",
    "clicks": "Սեղմումներ",
    "platform": "Հարթակ",
    "cms": "CMS",
    "cash": "Կանխիկ",
    "dir_payment": "բանկային փոխանցում",
    "auto_complete": "Ավտոմատ լրացում",
    "compare": "Համեմատեք",
    "infinite": "Անսահման",
    "tips": "Խորհուրդներ",
    "technical": "Տեխնիկական",
    "finance": "Ֆինանսներ",
    "your_phone_input": "Մուտքագրեք հեռախոսահամար",
    "income": "Եկամուտ",
    "change": "Փոփոխություն",
    "new_users": "Նոր օգտվողներ",
    "success_orders": "Հաջողված պատվերներ",
    "direct": "Ուղղակի",
    "last_activity": "Վերջին գործունեությունը",
    "landing": "Վայրէջք",
    "now": "Հիմա",
    "accept_agreement": "Ես կարդացել և ընդունել եմ պայմանները։",
    "report": "Հաշվետվություն",
    "tablet": "Պլանշետ",
    "pc": "PC",
    "acquisition_channel": "Ձեռքբերման ալիք",
    "to": "Դեպի",
    "info": "Ինֆո",
    "general": "Գեներալ",
    "upload": "Վերբեռնել",
    "general_config": "Ընդհանուր կոնֆիգուրացիա",
    "limits_config": "Սահմանափակումներ",
    "design_config": "Դիզայն և ոճ",
    "duration_config": "Տևողության սահմանափակում",
    "constraints_config": "Իրավասության սահմանները",
    "clubs_config": "Հասանելի է ակումբներում",
    "additional_config": "Լրացուցիչ կոնֆիգուրացիաներ",
    "import": "Ներմուծում",
    "ribbon": "Ժապավեն",
    "countries": "Երկրներ",
    "edit": "Խմբագրել",
    "delete": "Ջնջել",
    "analytics": "Վերլուծություն",
    "rules": "Կանոններ",
    "unset": "Սահմանված չէ",
    "carts": "Սայլեր",
    "optional": "Ընտրովի",
    "design": "Դիզայն",
    "filter": "Զտել",
    "send": "Ուղարկել",
    "approved": "Հաստատված է",
    "unit": "Միավոր",
    "profile": "պրոֆիլը",
    "shipping_rates": "Առաքման սակագներ",
    "package": "Փաթեթ",
    "overview": "Ընդհանուր ակնարկ",
    "live": "Ապրեք",
    "never": "Երբեք",
    "always": "Միշտ",
    "mandatory": "Պարտադիր",
    "expire_at": "Ժամկետի ամսաթիվ",
    "position": "Դիրք",
    "channel": "Ալիք",
    "age": "Տարիք",
    "charge": "Լիցքավորում",
    "giftcard": "Նվեր քարտ",
    "acquisition": "Ձեռքբերում",
    "conversion": "Փոխակերպում",
    "total_payments": "Ընդհանուր վճարումներ",
    "community_ask": "Հարցրեք համայնքում",
    "children": "Երեխաներ",
    "export": "Արտահանում",
    "list": "Ցուցակ",
    "hierarchy": "Հիերարխիա",
    "images": "Պատկերներ",
    "demo": "Դեմո",
    "author": "Հեղինակ",
    "designer": "Դիզայներ",
    "max": "Առավելագույնը",
    "accepted": "Ընդունված է",
    "reply": "Պատասխանել",
    "reviewed": "Վերանայված",
    "restored": "Վերականգնվել է",
    "merchant": "Վաճառական",
    "per_product": "Մեկ ապրանքի համար",
    "journey": "Ճանապարհորդություն",
    "after": "հետո",
    "waiting_for_payment": "Խնդրում ենք լրացնել վճարումը...",
    "hour": "Ժամ",
    "minute": "Րոպե",
    "premium": "Հավելավճար",
    "light": "Լույս",
    "dark": "Մութ",
    "guest": "Հյուր",
    "shops": "Խանութներ",
    "vendor": "Վաճառող",
    "vendors": "Վաճառողներ",
    "marketplace": "Շուկա",
    "verify_date": "Ստուգեք ամսաթիվը",
    "note": "Նշում",
    "panel": "Վահանակ",
    "removed": "Հեռացված",
    "segments": "Հատվածներ",
    "personal_info": "Անձնական տվյալներ",
    "preferences": "Նախապատվություններ",
    "spec": "Տեսակ",
    "reviews": "Կարծիքներ",
    "constraints": "Սահմանափակումներ",
    "flexible": "Ճկուն",
    "usage_limit": "Օգտագործման սահմանափակում",
    "temporary": "Ժամանակավոր",
    "original": "Օրիգինալ",
    "saved": "Պահված է:",
    "sort": "Տեսակավորել",
    "documents": "Փաստաթղթեր",
    "document": "Փաստաթուղթ",
    "please_wait": "Խնդրում ենք սպասել...",
    "pickup": "Վերցնել",
    "customize": "հարմարեցնել",
    "difficulty": "Դժվարություն",
    "banned": "Արգելված",
    "company": "Ընկերություն",
    "tax_shipping": "Առաքման հարկ",
    "warranty": "Երաշխիք",
    "return_policy": "Վերադարձի քաղաքականություն",
    "guide": "Ուղեցույց",
    "translate": "Թարգմանել",
    "public": "Հանրային",
    "deleted": "Ջնջված է",
    "variant": "Տարբերակ",
    "secure": "Ապահով",
    "admin": "Ադմին",
    "tax_included_abstract": "ներառյալ",
    "payment_not_settled": "Վճարը չի մարվել:",
    "orders_list": "Պատվերների ցուցակ",
    "subscription_plan": "Բաժանորդագրման պլան",
    "contents": "Բովանդակություն",
    "pricings": "Գները",
    "static_page": "Ստատիկ էջ",
    "shop_button": "Խանութ կոճակ",
    "embed": "Տեղադրել",
    "over": "Ավարտվել է",
    "tax_vary_by_location": "տարբերվել. գտնվելու վայրը",
    "product_type": "Ապրանքի տեսակը",
    "tax_id": "Հարկային ID",
    "business_name": "Բիզնեսի անվանումը",
    "invoice": "Հաշիվ-ապրանքագիր",
    "included_in_the_product": "Ներառված է ապրանքի մեջ",
    "included_in_the_product_description": "Ապրանքը ներառում է հետևյալ բաղադրիչները",
    "cross_sell": "Խաչի վաճառք",
    "discount_off": "OFF",
    "extra_discount": "Լրացուցիչ զեղչ",
    "template": "Կաղապար",
    "in_cart": "Զամբյուղում",
    "pros_and_cons": "Կողմ եվ դեմ",
    "related_products": "Առնչվող ապրանքներ",
    "related_categories": "Առնչվող կատեգորիաներ",
    "badges": "Կրծքանշաններ",
    "explore": "Հետազոտել",
    "updated": "Թարմացվել է",
    "bank_name": "Բանկի անվանումը",
    "bank_account_number": "Բանկային հաշվի համարը",
    "bank_routing_number": "Բանկի երթուղային համարը",
    "bank_branch_address": "Բանկի մասնաճյուղի հասցեն",
    "bank_account_name": "Հաշվի սեփականատիրոջ լրիվ անվանումը",
    "valuation": "Գնահատում",
    "cluster": "Կլաստեր",
    "clusters": "Կլաստերներ",
    "subscriptions": "Բաժանորդագրություններ",
    "spam": "Սպամ",
    "video": "Տեսանյութ"
  },
  "dialogs": {
    "delete": {
      "title": "Ջնջել",
      "message": "Վստա՞հ եք ջնջելու այս տարրը:",
      "action": "Այո, ջնջել հիմա"
    }
  },
  "actions": {
    "add_to_cart": "Ավելացնել քարտին",
    "cancel": "Չեղարկել",
    "update": "Թարմացնել",
    "add": "Ավելացնել",
    "delete": "Ջնջել",
    "new": "Նոր",
    "save_continue": "Պահպանել և շարունակել",
    "save": "Պահպանել",
    "save_changes": "Պահպանել փոփոխությունները",
    "change": "Փոփոխություն",
    "continue": "Շարունակել",
    "close": "փակել",
    "finish": "Ավարտել",
    "poll": "Հարցում",
    "view": "Դիտել",
    "edit": "Խմբագրել",
    "next": "Հաջորդը",
    "previous": "Նախորդ",
    "create": "Ստեղծել",
    "set": "հավաքածու",
    "find": "Գտեք",
    "accept": "Ընդունել",
    "reject": "Մերժել",
    "back": "Ետ",
    "pay": "Վճարել",
    "pay_now": "Վճարե՛ք հիմա",
    "print": "Տպել",
    "return": "Վերադարձ",
    "publish": "Հրապարակել",
    "confirm": "Հաստատել",
    "copy": "Պատճենել",
    "submit": "Ներկայացնել",
    "send": "Ուղարկել",
    "yes": "Այո՛",
    "no": "ոչ",
    "meh": "Հմմ",
    "clear": "Պարզ",
    "dismiss": "Հեռացնել",
    "search": "Որոնել",
    "deselect_all": "Ապաընտրել բոլորը",
    "select_all": "Ընտրել բոլորը",
    "refresh": "Թարմացնել",
    "verify": "Հաստատել",
    "play": "Խաղալ",
    "open": "Բաց",
    "calculate": "Հաշվիր",
    "quick_buy": "Արագ գնումներ",
    "click_here": "Սեղմեք այստեղ",
    "schedule": "Ժամանակացույց",
    "send_now": "Ուղարկեք հիմա",
    "download": "Բեռնել",
    "reset_default": "Վերականգնել լռելյայն",
    "reset": "Վերականգնել",
    "check": "Ստուգեք",
    "cod_payment": "Կանխիկ առաքում",
    "dir_payment": "Դրամական փոխանցում",
    "online_payment": "Առցանց վճարում",
    "checkout": "Ստուգում",
    "show_all": "Ցույց տալ ամբողջը",
    "login": "մուտք",
    "logout": "Դուրս գալ",
    "more": "Ավելին..",
    "collapse": "Փլուզում",
    "apply_change": "Կիրառել փոփոխությունները",
    "apply": "Դիմել",
    "add_giftcard": "Ավելացնել նվեր քարտ",
    "charge": "Լիցքավորում",
    "send_invites": "Ուղարկեք հրավերներ",
    "past": "Անցյալ",
    "restore": "Վերականգնել",
    "cut": "Կտրել",
    "get_sms_code": "Ստացեք նույնականացման կոդը",
    "register_no_mail": "Շարունակեք առանց էլփոստի",
    "no_mail_register_message": "Էլփոստի հասցե չունե՞ք կամ մոռացե՞լ եք ձեր գաղտնաբառը:",
    "reset_by_mail": "Գաղտնաբառի վերականգնում էլփոստով",
    "reset_by_sms": "Գաղտնաբառի վերականգնում SMS-ով",
    "cash_payment": "Կանխիկ վճարում",
    "show_chart": "Ցույց տալ աղյուսակը",
    "open_app": "Բացեք հավելվածը",
    "login_now": "Մուտք գործեք հիմա",
    "watch_video": "Դիտեք տեսանյութը",
    "check_now": "Ստուգիր հիմա",
    "remove_image": "Հեռացնել պատկերը",
    "view_website": "Բացել հղումը",
    "explore": "Ուսումնասիրեք {name}-ը",
    "go": "Գնա",
    "read_more": "Կարդալ ավելին",
    "start_now": "Սկսեք հիմա — Դա անվճար է",
    "view_live": "Դիտեք ուղիղ եթերում",
    "resend_code": "Կրկին ուղարկել կոդը",
    "invite": "Հրավիրել",
    "add_more": "Ավելացնել ավելին",
    "install_now": "Տեղադրել Հիմա",
    "uninstall": "Տեղահանել",
    "show_preview": "Ցույց տալ նախադիտումը",
    "hide_preview": "Թաքցնել նախադիտումը",
    "download_now": "Ներբեռնել հիմա",
    "auto_fill": "Ինքնալրացում",
    "track": "Հետևել",
    "change_state": "Փոխել կարգավիճակը",
    "done": "Կատարած",
    "reply": "Պատասխանել",
    "watch_now": "Դիտեք հիմա",
    "unlock": "Ապակողպել",
    "lock": "Կողպեք",
    "create_clone": "Ստեղծեք կլոն",
    "regenerate": "Վերականգնել",
    "add_message": "Ավելացնել հաղորդագրություն",
    "continue_chat": "Շարունակել զրույցը",
    "add_new_message": "Ավելացնել նոր հաղորդագրություն",
    "change_setting": "Փոխել կարգավորումը",
    "delete_all": "Ջնջել բոլորը",
    "transfer_fund": "Փոխանցման ֆոնդ",
    "reverse_fund": "Հակադարձ ֆոնդ",
    "settlement": "Կարգավորում",
    "refund": "Փոխհատուցում",
    "partial_refund": "Մասնակի փոխհատուցում",
    "change_payment_method": "Փոխել վճարման եղանակը",
    "buy": "Գնել",
    "view_all": "Դիտել բոլորը",
    "view_now": "Դիտել հիմա",
    "paste": "Կպցնել"
  },
  "instance_message_title": "Ուղարկել ակնթարթային հաղորդագրություն",
  "instance_message": "Ողջույն, {name}, իմ անունը {author} է {company}-ից:",
  "errors": {
    "419": "Դուք երկար ժամանակ անգործուն եք եղել, խնդրում ենք <b>⟳ ԹԱՐՄԱՑԵՔ ԷՋԸ</b> ՝ անվտանգության նկատառումներից ելնելով:",
    "gateway_blockchain_sync_error": "Համակարգը չի համաժամանակացվում բլոկչեյնի հետ։ Մինչև մեր սերվերների համաժամացման ավարտը հնարավոր չի լինի ստեղծել նոր գործարք: Մենք խնդիրը լուծում ենք։",
    "gateway_network_error": "Վճարման դարպասի ծառայությունը կապի խնդրի հետ է հանդիպել:"
  },
  "receiver_info": {
    "map": {
      "title": "Աոաքման Հասցե",
      "confirm": "Սահմանել գտնվելու վայրը",
      "address_type": "Նպատակակետ",
      "receptor_type": "Ընդունիչ"
    },
    "receiver_address": "Ընտրեք առաքման հասցեն",
    "address": "հասցեն",
    "postcode": "Ինդեքս",
    "enter_value": "Լրացրեք այստեղ",
    "address_note": "Նշում",
    "tel": "Կոնտակտային համարը",
    "service_address": "Ընտրեք ձեր հասցեն"
  },
  "weight": {
    "g": "է",
    "oz": "ունցիա",
    "lb": "լբ",
    "kg": "կգ"
  },
  "mass_unit_name": {
    "g": "գրամ",
    "oz": "Ունցիա",
    "lb": "ֆունտ",
    "kg": "կիլոգրամ"
  },
  "length": {
    "cm": "սմ",
    "in": "մեջ",
    "ft": "ոտնաչափ",
    "mm": "մմ",
    "m": "մ",
    "yd": "յդ"
  },
  "size_unit_name": {
    "cm": "սանտիմետր",
    "in": "դյույմ",
    "ft": "Ոտք",
    "mm": "միլիմետր",
    "m": "մետր",
    "yd": "բակ"
  },
  "distance": {
    "m": "մետր",
    "km": "կմ"
  },
  "time": {
    "days": "Օրեր",
    "hours": "Ժամեր",
    "minutes": "Րոպեներ",
    "seconds": "Վայրկյաններ"
  },
  "units": [
    "Միավոր",
    "կետ",
    "յուրաքանչյուրը",
    "հավաքածու",
    "կտոր",
    "կիլոգրամ",
    "գրամ",
    "միլիգրամ",
    "մետր",
    "միլիմետր",
    "քառակուսի մետր",
    "քառակուսի ոտնաչափ",
    "խորանարդ մետր",
    "լիտր",
    "միլիլիտր",
    "ոտքերը",
    "գալոններ",
    "դյույմ",
    "ունցիա",
    "ֆունտ",
    "կվարտաներ",
    "բակերը",
    "սենյակ",
    "վայրկյան",
    "րոպե",
    "ժամեր",
    "օրեր",
    "շաբաթներ",
    "ամիսներ",
    "տարիներ"
  ],
  "region_types": {
    "State": "Պետություն",
    "Territory": "Տարածք",
    "Province": "Նահանգ"
  },
  "transaction_status": {
    "RequireAction": "Պահանջվում է գործողություն",
    "Processing": "Մշակում",
    "Canceled": "Չեղարկված է",
    "Succeeded": "Հաջողվել"
  },
  "placeholders": {
    "name": "Ջոն Քոքս",
    "email": "your-mail{'@'}mail.com",
    "phone": "+1 000 0000",
    "language": "Ընտրել լեզուն..",
    "friend_email": "Ձեր ընկերոջ էլ․․․",
    "friend_name": "Ընտրել Ձեր ընկերոջ անունը",
    "select_date": "Ընտրեք ամսաթիվ..."
  },
  "notification": {
    "confirm": "Հաստատել",
    "confirm_message": "Փոփոխությունները հաջողությամբ պահպանվեցին:",
    "error": "Սխալ",
    "message": "Հաղորդագրություն",
    "caution": "Զգուշություն",
    "warning": "Զգուշացում",
    "award": "Շնորհավորում եմ",
    "error_message": "Ցավոք, սխալ է տեղի ունեցել:",
    "congratulation": "Շնորհավորում եմ",
    "coming_soon": "Շուտով",
    "on_fetching_data": "Տեղեկատվության ստացում..",
    "new_pwa_version": "Նոր տարբերակը հասանելի է հիմա: Ցանկանու՞մ եք թարմացնել հավելվածը:",
    "over_license_limit": "Լիցենզիայի սահմանաչափի գերազանցում",
    "update_license": "Թարմացրեք ձեր լիցենզիան",
    "required": "Պահանջվում է",
    "invalid_email": "Սխալ Էլեկտրոնային փոստի հասցե",
    "invalid_domain": "Անվավեր տիրույթ",
    "invalid_min_length": "Նվազագույն երկարությունը պետք է լինի {length}",
    "invalid_min_array_length": "Առնվազն ընտրեք {length} տարր",
    "invalid_max_chars": "Առավելագույնը {maxCount} նիշ",
    "invalid_url": "Անվավեր URL",
    "country_invalid": "Ընտրեք նպատակակետ երկիրը:",
    "postal_code_invalid": "Սահմանեք վավեր փոստային կոդը:"
  },
  "colors": {
    "#D32F2F": "Կարմիր",
    "#C2185B": "Վարդագույն",
    "#7B1FA2": "Մանուշակագույն",
    "#512DA8": "Մուգ մանուշակագույն",
    "#303F9F": "Ինդիգո կապույտ",
    "#1976D2": "Կապույտ",
    "#0288D1": "Բաց կապույտ",
    "#0097A7": "Փիրուզագույն Կապույտ",
    "#00796B": "Սագի կանաչ",
    "#388E3C": "Կանաչ",
    "#689F38": "Բաց կանաչ",
    "#AFB42B": "Լիմուզին",
    "#FBC02D": "Դեղին",
    "#FFA000": "Սաթ",
    "#F57C00": "Նարնջագույն",
    "#E64A19": "Փայլուն նարնջագույն",
    "#5D4037": "Բրաուն",
    "#616161": "Մոխրագույն",
    "#455A64": "Մոխրագույն Կապույտ",
    "#FFD700": "Ոսկեգույն",
    "#C0C0C0": "Արծաթե",
    "#b76e79": "Ոսկե վարդ",
    "#383838": "Անփայլ սև",
    "#FFFFFF": "Սպիտակ",
    "#FFF": "Սպիտակ",
    "#000000": "Սեվ",
    "#000": "Սեվ",
    "#7d263a": "Շագանակագույն",
    "#FD9F17": "Ոսկի",
    "#229DBF": "Aqua",
    "#6DC9E2": "Aqua Triblend",
    "#413B2D": "Բանակ",
    "#474747": "Ասֆալտ",
    "#E6195C": "Բերի",
    "#B2184C": "Բերի Թրիբլենդ",
    "#FC7F5F": "Այրված նարնջագույն",
    "#80011F": "Կարդինալ",
    "#382829": "Շոկոլադ",
    "#F06336": "Մարջան",
    "#FAE7C9": "Կրեմ",
    "#6B2257": "Հաղարջ",
    "#222127": "Մուգ Մոխրագույն",
    "#0D5A74": "Deep Teal",
    "#00582F": "Մշտադալար",
    "#214C30": "Անտառ",
    "#DC2867": "Ֆուքսիա",
    "#38561F": "Քելլի",
    "#5C884D": "Տերեւ",
    "#558450": "Light Aqua",
    "#96B5C9": "Բաց կապույտ",
    "#DCAFCE": "Lilac",
    "#D3E1A4": "Լայմ",
    "#9DE7C2": "անանուխ",
    "#F6EAD2": "Բնական",
    "#1A1F35": "Նավատորմ",
    "#404624": "Ձիթապտղի",
    "#271020": "Սալոր",
    "#A8355C": "Ազնվամորու",
    "#D6BEA6": "Փափուկ կրեմ",
    "#BEA994": "Թան",
    "#0DB2AE": "Շագանակագույն",
    "#A86EA9": "Մանուշակ",
    "#36454F": "Ածուխ",
    "#b3b0ab": "ածխածնային մոխրագույն",
    "#463e3d": "Փայտածուխ Heather",
    "#282d3c": "Navy Blazer"
  },
  "address_info": {
    "name": "Անուն",
    "tel": "Հեռախոս",
    "email": "Էլ",
    "country": "Երկիր",
    "city": "Քաղաք",
    "state": "Պետություն",
    "address": "հասցեն",
    "building_number": "Շենքի համարը",
    "building_unit": "Միավոր",
    "postcode": "Ինդեքս",
    "note": "Նշում",
    "county": "կոմսություն"
  },
  "product_types": {
    "physical": "Ֆիզիկական",
    "physical_description": "Ֆիզիկական արտադրանք, ինչպիսիք են բջջային հեռախոսները, գրքերը, էլեկտրոնային բաղադրիչները:",
    "virtual": "Վիրտուալ",
    "virtual_description": "Ինտերնետ հարմար ապրանքներ, ինչպիսիք են՝ վերալիցքավորման քարտ, հյուրանոցի ամրագրում, խաղի վերալիցքավորում:",
    "file": "Ֆայլ",
    "file_description": "Բոլոր թույլատրված ֆայլերը, ինչպիսիք են առցանց դասընթացի տեսանյութը, ներբեռնվող փաստաթղթերը:",
    "service": "Ծառայություն",
    "service_description": "Ծառայություններ՝ վերանորոգում, տեղափոխում, ուսուցում։",
    "subscription": "Բաժանորդագրություն",
    "subscription_description": "Ապրանքների, ծառայությունների կամ բովանդակության բաժանորդագրություններ և հաշիվ-ապրանքագրեր:"
  },
  "license": {
    "free": "Անվճար",
    "startup": "Սկսնակ",
    "company": "Ընկերություն",
    "enterprise": "Ձեռնարկություն"
  },
  "currency": {
    "IRR": "IRR",
    "IRR_alt": "թուման",
    "USD": "USD",
    "BTC": "BTC",
    "ETH": "ETH",
    "NIOC": "NIOC",
    "SEL": "SEL",
    "EUR": "EUR",
    "ADA": "ADA",
    "GBP": "GBP",
    "AED": "AED",
    "AFN": "AFN",
    "CAD": "CAD",
    "TRY": "TRY",
    "CNY": "CNY",
    "RUB": "RUB",
    "SAR": "SAR",
    "LBP": "LBP",
    "QAR": "QAR",
    "EGP": "EGP",
    "INR": "INR",
    "DKK": "DKK",
    "PKR": "PKR",
    "ALL": "ALL",
    "AMD": "AMD",
    "ANG": "ANG",
    "AOA": "AOA",
    "ARS": "ARS",
    "AUD": "AUD",
    "AWG": "AWG",
    "AZN": "AZN",
    "BAM": "BAM",
    "BBD": "BBD",
    "BDT": "BDT",
    "BGN": "BGN",
    "BIF": "BIF",
    "BMD": "BMD",
    "BND": "BND",
    "BOB": "BOB",
    "BRL": "BRL",
    "BSD": "BSD",
    "BWP": "BWP",
    "BYN": "BYN",
    "BZD": "BZD",
    "CDF": "CDF",
    "CHF": "CHF",
    "CLP": "CLP",
    "COP": "COP",
    "CRC": "CRC",
    "CVE": "CVE",
    "CZK": "CZK",
    "DJF": "DJF",
    "DOP": "DOP",
    "DZD": "DZD",
    "ETB": "ETB",
    "FJD": "FJD",
    "FKP": "FKP",
    "GEL": "GEL",
    "GIP": "GIP",
    "GMD": "GMD",
    "GNF": "GNF",
    "GTQ": "GTQ",
    "GYD": "GYD",
    "HKD": "HKD",
    "HNL": "HNL",
    "HRK": "HRK",
    "HTG": "HTG",
    "HUF": "HUF",
    "IDR": "IDR",
    "ILS": "ILS",
    "ISK": "ISK",
    "JMD": "JMD",
    "JPY": "JPY",
    "KES": "KES",
    "KGS": "KGS",
    "KHR": "KHR",
    "KMF": "KMF",
    "KRW": "KRW",
    "KYD": "KYD",
    "KZT": "KZT",
    "LAK": "LAK",
    "LKR": "LKR",
    "LRD": "LRD",
    "LSL": "LSL",
    "MAD": "MAD",
    "MDL": "MDL",
    "MGA": "MGA",
    "MKD": "MKD",
    "MMK": "MMK",
    "MNT": "MNT",
    "MOP": "MOP",
    "MRO": "MRO",
    "MUR": "MUR",
    "MVR": "MVR",
    "MWK": "MWK",
    "MXN": "MXN",
    "MYR": "MYR",
    "MZN": "MZN",
    "NAD": "NAD",
    "NGN": "NGN",
    "NIO": "NIO",
    "NOK": "NOK",
    "NPR": "NPR",
    "NZD": "NZD",
    "PAB": "PAB",
    "PEN": "PEN",
    "PGK": "PGK",
    "PHP": "PHP",
    "PLN": "PLN",
    "PYG": "PYG",
    "RON": "RON",
    "RSD": "RSD",
    "RWF": "RWF",
    "SBD": "SBD",
    "SCR": "SCR",
    "SGD": "SGD",
    "SEK": "SEK",
    "SHP": "SHP",
    "SLL": "SLL",
    "SOS": "SOS",
    "SRD": "SRD",
    "STD": "STD",
    "SZL": "SZL",
    "THB": "THB",
    "TJS": "TJS",
    "TOP": "TOP",
    "TTD": "TTD",
    "TWD": "TWD",
    "TZS": "TZS",
    "UAH": "UAH",
    "UGX": "UGX",
    "UYU": "UYU",
    "UZS": "UZS",
    "VND": "VND",
    "VUV": "VUV",
    "WST": "WST",
    "XAF": "XAF",
    "XCD": "XCD",
    "XOF": "XOF",
    "XPF": "XPF",
    "YER": "YER",
    "ZAR": "ZAR",
    "ZMW": "ZMW",
    "GHS": "GHS",
    "ZWD": "ZWD",
    "IQD": "IQD",
    "SYP": "SYP"
  },
  "variants": {
    "color": "Գույն",
    "volume": "Չափը",
    "count_in_pack": "Հաշվեք փաթեթում",
    "weight": "Քաշը",
    "type": "Նյութ",
    "style": "Ոճ"
  },
  "delivery_state": {
    "check_queue": "Սպասում է վերանայմանը",
    "order_confirm": "Պատվերի հաստատում",
    "preparing_order": "Պատրաստում",
    "sent_order": "Առաքման ժամանակ",
    "to_customer": "Հանձնվել է"
  },
  "status": {
    "unpaid": "Չվճարված",
    "paid": "Վճարված",
    "canceled": "Չեղարկված է",
    "completed": "Տեղավորվել է",
    "reserved": "Վերապահված է",
    "COD": "COD",
    "sold": "Վաճառվել է",
    "open": "Բաց",
    "cancel": "Չեղարկված է",
    "active": "Ակտիվ",
    "inactive": "ոչ ակտիվ",
    "debug": "Վրիպազերծում",
    "live": "Ապրեք",
    "test": "Փորձարկում",
    "connected": "Միացված է",
    "disconnected": "Անջատված է",
    "accept": "Ընդունել",
    "reject": "Մերժել",
    "pending": "Սպասվում է",
    "rejected": "Մերժված է",
    "accepted": "Ընդունված է"
  },
  "dimensions": {
    "cm": "սմ",
    "km": "կմ",
    "kg": "կգ"
  },
  "transportation_type": {
    "motorbike": "Մոտոցիկլետ",
    "motorbike_description": "Միջին քաշը մի քանի կիլոմետրի ընթացքում:",
    "cargo_small": "Փոքր բեռ",
    "cargo_small_description": "Բազմակիլոմետրանոց թեթև բեռ.",
    "cargo": "Բեռներ",
    "cargo_description": "Միջին քաշը մի քանի կիլոմետրի ընթացքում:",
    "truck": "Բեռնատար մեքենա",
    "truck_description": "Մեծածախ կամ ծանր բեռներ երկրի ներսում:",
    "fast_post": "Արագ գրառում",
    "fast_post_description": "Փոստի միջոցով արագ տեղադրում:",
    "post": "Սովորական գրառում",
    "post_description": "Կանոնավոր փոստային բաժանմունքի միջոցով:",
    "airplane": "Ավիափոստ",
    "airplane_description": "Ներքին և արտասահմանյան թեթև և միջին բեռներ.",
    "bicycle": "Մաքուր ուղարկել",
    "bicycle_description": "Թեթև բեռ, ոչ շարժիչային տրանսպորտային միջոցներից մի քանի հարյուր յարդ հեռավորության վրա:",
    "global": "Համաշխարհային տրանսպորտ",
    "global_description": "Օդային, ծովային կամ ցամաքային բեռնափոխադրումներ արտերկիր:",
    "pickup": "Վերցնել",
    "pickup_description": "Հաճախորդները վերցնում են ապրանքները ձեր ֆիզիկական խանութից կամ պահեստից:"
  },
  "article": {
    "login_to_see_error": "Մուտք գործեք՝ հոդվածին ծանոթանալու համար",
    "create_new_action": "Ստեղծեք նոր բովանդակություն",
    "menu_title": "Մենյու",
    "menu": {
      "edit": "Խմբագրման ռեժիմ",
      "view_mode": "Դիտման ռեժիմ",
      "delete": "Ջնջել հոդվածը"
    },
    "schedule_action": "Ցանկանու՞մ եք պլանավորել հրապարակումը:",
    "schedule_title": "Պլանավորեք ինքնահրապարակումը",
    "edit_mode": "Խմբագրել",
    "public_view_mode": "Հանրային դիտում",
    "publish_at": "Հրապարակել ժամը",
    "published": "Հրապարակվել է",
    "draft": "Նախագիծ",
    "saving": "Խնայողություն",
    "save_and_publish_action": "Պահպանել և հրապարակել",
    "save_draft_action": "Պահպանել նախագիծը",
    "private": "Մասնավոր",
    "public": "Հանրային",
    "category_input": "Կարգավիճակ",
    "category_input_message": "Ընտրեք հոդվածի կատեգորիա",
    "parent_input": "Ծնողների կատեգորիա",
    "parent_input_message": "Ընտրեք հայրական բովանդակությունը",
    "schedule_input": "Ժամանակացույց",
    "schedule_input_placeholder": "Եթե ցանկանում եք, որ պատմությունը հրապարակվի կոնկրետ ժամին։",
    "schedule_view_action": "Դիտեք Հրապարակման ժամանակացույցը",
    "author_title": "Գրված է",
    "read_time": "Ուսումնասիրության ժամանակը",
    "read_time_unit": "Րոպեներ",
    "claps_action": "Ծափեր",
    "claps": "Ծափեր",
    "likes": "Հավանել",
    "about_author": "Հեղինակի մասին",
    "updated_at": "Թարմացվել է",
    "wrote_at": "Գրված է",
    "now": "Հիմա",
    "follow_action": "Հետևեք",
    "followed": "Հետևեց",
    "report_and_delete_action": "Հաղորդեք և ջնջեք այս հոդվածը",
    "report_action": "Հաղորդել չարաշահման մասին",
    "you_reported_this_article": "Այս հոդվածն արդեն ունի չարաշահման մասին հաղորդում:",
    "add_to_publication_action": "Ավելացնել հրապարակմանը",
    "author_socials": "Հեղինակ սոցիալական ցանցերում",
    "share": "Կիսվել",
    "report_dialog": {
      "report_and_delete_title": "Հաղորդել չարաշահման մասին և հեռացնել բովանդակությունը",
      "report_title": "Հաղորդել այս բովանդակության մասին որպես վիրավորական",
      "message": "Ո՞րն է այս բովանդակությունը ներկայացնելու պատճառը: Ընտրեք հետևյալ տարբերակներից մեկը.",
      "accept_and_delete_action": "Հաստատեք և ջնջեք բովանդակությունը"
    },
    "schedule_dialog": {
      "title": "Բովանդակության թողարկման ժամանակացույց",
      "publish_at": "Հրապարակել ժամը",
      "last_edit": "Վերջին խմբագրումը",
      "view_action": "Դիտել հոդվածը"
    },
    "notifications": {
      "update_success_title": "Ներկայացրեք Տեղեկատվություն",
      "update_success": "Հոդվածը բարեհաջող թարմացվեց:",
      "update_error": "Մուտքագրված տեղեկատվությունը վավեր չէ:",
      "delete_error": "Դուք չեք կարող ջնջել այս հոդվածը:",
      "report_and_delete_success_title": "Զեկուցել և ջնջել",
      "report_and_delete_success_message": "Հոդվածը հաջողությամբ ջնջվել է:",
      "report_success_title": "Հաղորդել հոդվածը",
      "report_success_message": "Այս հոդվածի վերաբերյալ ձեր զեկույցը տեղադրվել է:"
    },
    "alert": {
      "delete_article": {
        "title": "Ջնջել հոդվածը",
        "message": "Իսկապե՞ս ուզում եք ընդմիշտ ջնջել այս հոդվածը:",
        "action": "Ջնջել"
      }
    },
    "edit_window": {
      "title": "Ընտրեք Պատկերի տեքստ և ամփոփում",
      "digest": "Ամփոփում"
    },
    "editor": {
      "title_placeholder": "Կոչում..",
      "body_placeholder": "Գրեք հիանալի բովանդակություն..",
      "accept_file_types_error": "Այս ֆայլի ձևաչափն անընդունելի է.",
      "max_file_size_error": "Այս ֆայլը չափազանց մեծ է.",
      "notifications": {
        "image_uploaded_success": "Պատկերը հաջողությամբ պահպանվեց",
        "image_uploaded_failed": "Պատկերը բեռնելիս սխալ առաջացավ:"
      },
      "embed_placeholder": "Մուտքագրեք iframe կոդը, Facebook-ը, Youtube-ը կամ ցանկացած այլ URL և սեղմեք enter:",
      "embed_caption_placeholder": "Մուտքագրեք վերնագիր (ըստ ցանկության)",
      "images_caption_placeholder": "Մուտքագրեք վերնագիր այս պատկերի համար.."
    }
  },
  "article_report": {
    "inappropriate": "Այս հոդվածի բովանդակությունը անպատշաճ է:",
    "fake": "Սա կեղծ է և խաբուսիկ:",
    "spam": "Այս տարրը սպամ է ուղարկվել և պատճենվել:",
    "scam": "Կարելի է ակնկալել, որ դա կարող է հանգեցնել չարամիտ ծրագրերի, վիրուսների կամ համակարգի խափանումների:",
    "hacked": "Այս կայքը կարծես կոտրվել է:",
    "offensive": "Լեզվի բովանդակությունը և բառապաշարը բարոյական չէ և անհարգալից է:",
    "violent": "Կարծում եմ, որ այս բովանդակությունը պարունակում է պոռնոգրաֆիա և խախտում է օրենքները:",
    "harassment": "Ես կարծում եմ, որ սա ռասիստական է:",
    "hate": "Կարծում եմ՝ սա բռնությունը խթանելու մասին է:",
    "infringe": "Սա խախտում է իմ իրավունքները։"
  },
  "comments": {
    "new_action": "Սեղմեք մեկնաբանություն թողնելու համար..",
    "title": "Մեկնաբանություններ",
    "body_placeholder": "Ձեր մեկնաբանությունը կամ հաղորդագրությունը",
    "send_action": "Ուղարկել մեկնաբանություն",
    "body_is_empty_error": "Մեկնաբանությունը դատարկ է:",
    "comments": "Մեկնաբանություններ",
    "no_comments": "Գրեք առաջին մեկնաբանությունը...",
    "load_more_action": "Ստացեք ավելի շատ մեկնաբանություններ..",
    "cant_send_comment": "Դուք չեք կարող նոր գրառում ավելացնել:",
    "cant_remove_this_comment": "Դուք չեք կարող ջնջել այս մեկնաբանությունը:",
    "product_buyer": "Գնորդ",
    "delete_alert": {
      "title": "Ջնջել մեկնաբանությունը",
      "message": "Ցանկանու՞մ եք ջնջել այս մեկնաբանությունը:",
      "action": "Ջնջել"
    },
    "notifications": {
      "delete_success": "Ձեր մեկնաբանությունը հաջողությամբ ջնջվեց:"
    },
    "first_send": "Առաջին գրառումը",
    "comment_at": "Վրա",
    "edit_comment": "Խմբագրել մեկնաբանությունը",
    "my_comments": "Իմ մեկնաբանությունները"
  },
  "customer_club": {
    "bronze": "Բրոնզե",
    "silver": "Արծաթե",
    "gold": "Ոսկեգույն",
    "platinum": "Պլատին",
    "diamond": "Ադամանդ"
  },
  "application_categories": {
    "tracking": "Օգտագործողի վարքագծի ստուգում",
    "payment": "Վճարում",
    "seo": "SEO",
    "crm": "CRM",
    "transportation": "Տրանսպորտ",
    "automation": "Ավտոմատացում",
    "inventory": "Պահեստավորում",
    "accounting": "Հաշվապահություն",
    "marketing": "Մարքեթինգ",
    "advertising": "Գովազդ",
    "staff": "Անձնակազմ",
    "incentive": "Խրախուսանքներ",
    "shop": "Խանութ"
  },
  "social_network": {
    "instagram": "Instagram",
    "telegram": "Telegram",
    "googleplay": "GooglePlay",
    "appstore": "AppStore",
    "twitter": "Twitter",
    "linkedin": "Linkedin",
    "medium": "Միջին",
    "selldone": "Վաճառված է",
    "facebook": "Ֆեյսբուք",
    "pinterest": "Pinterest",
    "whatsapp": "WhatsApp",
    "skype": "Skype",
    "facebook_messenger": "Facebook Messenger"
  },
  "form_builder": {
    "add_item_action": "Ավելացնել նոր տարր",
    "title_input": "Կոչում",
    "name_input": "Փոփոխական անուն",
    "value_input": "Արժեքներ",
    "value_input_message": "Օգտագործողի ընտրանքների ցանկ",
    "default": "Կանխադրված արժեք",
    "types": {
      "simple": "Պարզ",
      "selective": "Ընտրովի",
      "switch": "Անջատիչ",
      "file": "Ֆայլ",
      "number": "Թիվ"
    },
    "placeholder_input": "Տեղապահ",
    "file_type_input": "Ընդունելի ֆայլերի տեսակներ",
    "file_type_message": "jpeg, png, zip, ..."
  },
  "virtual_order_info_summery": {
    "title": "ապրանքային անուն",
    "reserved_at": "Գրանցման ամսաթիվ",
    "total_price": "Ընդհանուր գումարը",
    "discount": "Զեղչ",
    "final_price": "Վերջնական գումար",
    "payment_status": "Վճարման կարգավիճակը",
    "payment_status_waiting": "Սպասում է վճարմանը",
    "payment_status_paid": "Վճարված",
    "buyer": "Գնորդ"
  },
  "virtual_item_info": {
    "title": "Վիրտուալ նյութ",
    "outputs": {
      "title": "Արդյունք տեղեկատվություն",
      "sub_title": "Այս կետի գաղտնի տեղեկատվությունը",
      "message": "Այս տեղեկատվությունը կցուցադրվի օգտագործողին ապրանքը գնելուց հետո:"
    },
    "inputs": {
      "title": "Պահանջվում է տեղեկատվություն",
      "sub_title": "Պահանջվում է գնորդի տեղեկատվությունը",
      "message": "Գնորդը պետք է մուտքագրի պահանջվող տեղեկատվությունը ձևաթղթում"
    },
    "notifications": {
      "inputs_set_success": "Տեղեկատվական ձևը հաջողությամբ գրանցվել է:",
      "confirm_by_admin_success": "Պատվերի ավարտը հաստատված է:",
      "confirm_by_buyer_success": "Պատվերի ավարտը հաստատված է:"
    }
  },
  "order_reject_reasons": {
    "cant_accept_cod": "Այս պատվերը չի կարող ընդունվել COD վճարման միջոցով:",
    "payment_returned": "Հաճախորդի կողմից վճարված գումարը վերադարձվում է:",
    "payment_fraud": "Վճարումը բացահայտվել է որպես խարդախություն:",
    "location_out_of_service": "Հաճախորդի գտնվելու վայրը սպասարկումից դուրս է:",
    "user_not_responding": "Հաճախորդը չի պատասխանել զանգերին:",
    "shop_owner_fault": "Խանութի աշխատակիցների կողմից սխալ է տեղի ունեցել տեղեկատվության լրացման ժամանակ:",
    "lack_of_inventory": "Առկա չէ պահեստում.",
    "legal_restriction": "Իրավական սահմանափակումների պատճառով հնարավոր չէ պատվեր կատարել:",
    "pricing_fault": "Վաճառողի կողմից գնագոյացման սխալ:",
    "system_fault": "Համակարգի սխալ:"
  },
  "basket_order_info_summery": {
    "receiver": "Ընդունիչ",
    "phone": "Հեռ",
    "reserved_date": "Գնման ամսաթիվը",
    "total_price": "Ընդհանուր գումարը",
    "discount": "Զեղչ",
    "final_price": "Վերջնական գումար",
    "address": "Առաքման հասցե",
    "payment_status": "Վճարման կարգավիճակը"
  },
  "basket_status": {
    "open": "Բաց",
    "reserved": "Վերապահված է",
    "paid": "Վճարված",
    "canceled": "Չեղարկված է",
    "cod": "Վճարեք առաքման ժամանակ",
    "receipt_added": "Անդորրագիրը ավելացվել է",
    "payment_processing": "Վճարումների մշակում"
  },
  "basket_delivery_state": {
    "rejected": "Մերժված է",
    "reject_reason_not_found": "Պատճառն անհայտ է։",
    "step_1": "Վերանայում",
    "step_2": "Հաստատում",
    "step_3": "Նախապատրաստում",
    "step_4": "Առաքման ժամանակ",
    "step_5": "Հանձնվել է"
  },
  "basket_return_request_view": {
    "title": "Վերադարձի հարցում",
    "delivery_date": "Առաքման օրը",
    "return_date": "Հարցման ամսաթիվը",
    "items_count": "Վերադարձված իրերը հաշվում են",
    "items_count_unit": "տարր(ներ)",
    "reason": "Պատճառ",
    "description": "Նկարագրություն",
    "status": "Կարգավիճակ",
    "download_image": "Ներբեռնեք պատկերը",
    "download_film": "Ներբեռնեք տեսանյութը",
    "download_voice": "Ներբեռնեք ձայնը",
    "accept_by_admin_action": "Ընդունել հարցումը",
    "reject_by_admin_action": "Մերժել հարցումը",
    "message_for_admin": "Համաձայն այն պայմանագրի, որը դուք ընդունել եք՝ օգտագործելով Seldone-ը, դուք պետք է կապ հաստատեք գնորդի հետ և տեղեկացնեք նրան իր պատվերը վերադարձնելու գործընթացի մասին: Դա պետք է արվի, եթե հարցումն ընդունվի կամ մերժվի, և գնորդի լիակատար համաձայնությունը ստացվի:",
    "notifications": {
      "update_status_success": "Հարցման կարգավիճակը թարմացվել է:"
    }
  },
  "basket_list": {
    "return_count": "Վերադարձ՝ {count} համար",
    "table": {
      "row": "Շարք",
      "image": "Պատկեր",
      "product_name": "Արտադրանք",
      "variant": "Տարբերակ",
      "warranty": "Երաշխիքներ",
      "count": "հաշվել",
      "price": "գինը"
    }
  },
  "return_request_list": {
    "count": "Հղման համարը",
    "return_warranty": "Գումարի վերադարձի երաշխիք",
    "return_warranty_unit": "Օր(ներ)",
    "table": {
      "code": "Պատվերի կոդը",
      "product": "Արտադրանք",
      "delivery_at": "Ստանալու ամսաթիվ",
      "state": "Կարգավիճակ",
      "reason": "Պատճառ",
      "return_warranty_period": "Երաշխիքային ժամկետ"
    }
  },
  "data_toolbar": {
    "search": "Որոնել",
    "sort_by": "Դասավորել ըստ",
    "count_item": "Ցույց տալ {item} տարր"
  },
  "sort": {
    "created_at": "Ստեղծել ամսաթիվ",
    "updated_at": "Խմբագրել ամսաթիվը",
    "title": "Կոչում",
    "description": "Նկարագրություն",
    "like": "Հանրաճանաչություն",
    "power": "Ծափեր",
    "product_type": "Ապրանքի տեսակը",
    "discount": "Զեղչ",
    "rate": "Վարկանիշ",
    "rate_count": "Վարկանիշների քանակը",
    "category": "Կարգավիճակ",
    "product": "Արտադրանք",
    "body": "Բովանդակություն",
    "card_type": "Քարտի տեսակը",
    "card_number": "Քարտի համար",
    "balance": "Հաշվեկշիռ",
    "paid": "Ծախսված գումարը",
    "currency": "Արժույթ",
    "expire_at": "Ժամկետի ամսաթիվ",
    "discount_code": "Զեղչի կոդը",
    "discount_percent": "Զեղչ",
    "discount_limit": "Զեղչի սահմանաչափ",
    "max_count": "Առավելագույն հաշվարկ",
    "used_count": "Օգտագործված",
    "start_date": "Մեկնարկի ամսաթիվ",
    "end_date": "Ավարտի ամսաթիվը",
    "total_discount_amount": "Ընդհանուր զեղչեր",
    "initial_charge": "Նախնական լիցքավորում",
    "total_count": "Ընդհանուր հաշվարկ",
    "amount_paid": "Վճարված գումարը",
    "name": "Անուն",
    "published": "Հրապարակվել է",
    "visits": "Այցելություններ",
    "color": "Գույն",
    "landing_page": "Landing Page",
    "enable": "Միացնել",
    "fix_commission": "Հաստատուն հանձնաժողով",
    "percent_commission": "Տոկոսային հանձնաժողով",
    "payment_at": "Վճարման օր",
    "issued_at": "Թողարկման ամսաթիվը",
    "payment": "Վճարում",
    "website": "Կայք",
    "email": "Էլ",
    "user": "Օգտատեր",
    "code": "Կոդ",
    "schedule_at": "Ժամանակացույցի ամսաթիվ",
    "sender": "Ուղարկող",
    "in_que": "Հերթի մեջ",
    "sent": "Ուղարկված",
    "shop": "Խանութ",
    "quantity": "Քանակ",
    "amount": "Գումարը",
    "jobs": "Աշխատանքները հաշվում են",
    "success": "Հաջողության հաշվարկ",
    "fail": "Անհաջողության հաշվարկ",
    "distance": "Հեռավորությունը",
    "duration": "Տեւողությունը",
    "reset_at": "Վերականգնել ամսաթիվը",
    "delivery_person": "Անձ",
    "delivery_service": "Ծառայություն",
    "date": "Ամսաթիվ",
    "order": "Պատվեր",
    "order_id": "Պատվերի ID",
    "status": "Կարգավիճակ",
    "price": "գինը",
    "count": "հաշվել",
    "final_price": "Վերջնական գին",
    "charge": "Լիցքավորում",
    "mention": "Նշել",
    "officer": "սպա",
    "registered": "Գրանցված է",
    "purchased": "Գնված",
    "sex": "Սեռ"
  },
  "search_box": {
    "result": "Որոնման արդյունքները",
    "tips": "Որոնեք ապրանքներ և կատեգորիաներ",
    "label": "Որոնել...",
    "category": "Կարգավիճակ"
  },
  "lead_status": {
    "physical_fast_delivery": "Ակնթարթային առաքում",
    "physical_under_24h_delivery": "Առաքում հաջորդ {lead} ժամվա ընթացքում",
    "physical_over_24h_delivery": "Պատրաստվում և ուղարկվում է հաջորդ {days} օրվա ընթացքում",
    "virtual_fast_prepare": "Ակնթարթային առաքում",
    "virtual_under_24h": "Կկատարվի հաջորդ {lead} ժամվա ընթացքում",
    "virtual_over_24h": "Կկատարվի հաջորդ {days} օրվա ընթացքում"
  },
  "return_request_state": {
    "pending": "Սպասվում է",
    "accepted": "Հաստատված է",
    "rejected": "Մերժված է"
  },
  "return_order_reason": {
    "physical": {
      "U1": "Ապրանքի անհամապատասխանություն",
      "U2": "Առաքման փաթեթում ապրանք չկա",
      "U3": "Փաթեթի կամ ապրանքի կոտրվածք",
      "U4": "Փաթեթի կամ սերիական համարի կեղծում",
      "U5": "Ծանրոցից որոշ իրեր բացակայում են",
      "U6": "Ապրանքը օրիգինալ չէ",
      "U7": "Ապրանքը չի բավարարում տեխնիկական պայմանները"
    }
  },
  "image_uploader": {
    "label": "Քաշել և թողնել կամ <u>Ընտրել ֆայլը</u>",
    "max_size": "Առավելագույն թույլատրելի չափը՝ {size}"
  },
  "timespan": {
    "Morning": "8-ից 12-ը",
    "Evening": "12-ից մինչև երեկոյան 6-ը",
    "Night": "18:00-12:00",
    "Instant": "Ակնթարթային գրառում"
  },
  "week_day": {
    "Saturday": "շաբաթ օրը",
    "Sunday": "Կիրակի",
    "Monday": "Երկուշաբթի",
    "Tuesday": "երեքշաբթի",
    "Wednesday": "չորեքշաբթի",
    "Thursday": "հինգշաբթի",
    "Friday": "Ուրբաթ"
  },
  "payment_form": {
    "title": "Ընտրեք վճարման եղանակը",
    "gift_cards_input": "Նվեր Քարտեր",
    "gift_cards_input_empty": "Նվեր քարտեր չեն գտնվել...",
    "more": "Այլ",
    "pay_amount_by_gift_cards": "Լիցքավորման քարտի վճարում",
    "select_payment_method": "Ընտրեք ձեր վճարման եղանակը հետևյալ տարբերակներից:",
    "payment_amount_remained": "Մնացած վճարման գումարը",
    "payment_amount": "Վճարված գումարը",
    "all_paid_by_gift_cards": "Շնորհավորում ենք, այս պատվերի համար վճարելու կարիք չկա: Սեղմեք «Վճարել նվեր քարտով» ստեղնը:",
    "pay_by_gift_cards_action": "Պատվերը վճարեք վճարային քարտով",
    "order_free_payment": "Շնորհավորում ենք, ձեր պատվերն անվճար է:",
    "order_free_payment_action": "Պատվիրեք անվճար հիմա",
    "qr": {
      "address": "Վճարման հասցե",
      "amount": "Գումարը"
    }
  },
  "discount_input": {
    "limit": "{percent}% զեղչ մինչև {limit} {currency}",
    "action": "Ավելացնել զեղչի կոդը",
    "code_input": "Զեղչի կոդը",
    "notifications": {
      "add_success": "Շնորհավորում եմ, վավեր զեղչի կոդը:"
    }
  },
  "gift_card": {
    "inactive": "Քարտն անջատված է",
    "expire": "Ժամկետը",
    "transfer": "Փոխանցում",
    "send_to_friend": "Ուղարկել ընկերոջը"
  },
  "count_down": {
    "days": "Օր",
    "hours": "Ժամ",
    "minutes": "Րոպեներ",
    "seconds": "Վայրկյաններ"
  },
  "gift_status": {
    "Unpaid": "Սպասում է ստացման",
    "Paid": "Վճարված",
    "Rejected": "Մերժված է",
    "Canceled": "Չեղարկված է"
  },
  "gift_program_status": {
    "Airdrop": "Վճարային համակարգ",
    "Advertising": "Գովազդ",
    "PersonalGift": "Անձնական նվեր",
    "ShopGift": "Նվերների խանութ"
  },
  "currency_selector": {
    "label": "Արժույթ"
  },
  "need_login": {
    "title": "մուտք",
    "login_sms": "SMS մուտք",
    "login_google": "Մուտք գործեք Google-ով",
    "login_facebook": "Մտնել Ֆեյսբուքով",
    "login_account": "Մուտք գործեք Selldone-ի կողմից",
    "register": "Նոր օգտատեր? Գրանցվիր հիմա.",
    "mobile_input": "Մուտքագրեք ձեր բջջային հեռախոսը",
    "verification_code": "Ստուգման կոդը",
    "msg_title": "Ողջու՜յն,",
    "msg_body": "Խնդրում ենք մուտք գործել: Դա տեւում է ընդամենը մի քանի վայրկյան:",
    "login_apple": "Մուտք գործեք Apple-ով",
    "login_email": "Մուտք գործեք էլփոստով",
    "email_login_step_1": "Խնդրում ենք տրամադրել վավեր էլփոստի հասցե, և մենք մուտքի կոդը կուղարկենք դրան: Ոչ մի գրանցում անհրաժեշտ չէ:",
    "email_login_step_2": "Համոզվա՞ծ եք, որ ճիշտ եք մուտքագրել ձեր էլ.փոստի հասցեն: Մենք մուտքի կոդը կուղարկենք տրամադրված էլ.փոստին, ուստի խնդրում ենք համոզվել դրա ճշգրտությունը:",
    "email_login_step_3": "Մենք կոդ ենք ուղարկել ձեր էլ. Խնդրում ենք նկատի ունենալ, որ կոդը կսպառվի 10 րոպեից:"
  },
  "shop_badges": {
    "GOLDEN_1YEAR": {
      "name": "5 տարվա ոսկու խանութ"
    },
    "GOLDEN_2YEAR": {
      "name": "4 տարվա ոսկու խանութ"
    },
    "GOLDEN_3YEAR": {
      "name": "3 տարվա ոսկու խանութ"
    },
    "GOLDEN_4YEAR": {
      "name": "2 տարվա ոսկու խանութ"
    },
    "GOLDEN_5YEAR": {
      "name": "1 տարվա ոսկու խանութ"
    },
    "TOKEN_OWNER_1M": {
      "name": "Ոսկե ներդրող"
    },
    "TOKEN_OWNER_10M": {
      "name": "Ադամանդի ներդրող"
    },
    "TOKEN_OWNER_100M": {
      "name": "Ռուբի ներդրող"
    },
    "COMMUNITY_LOYAL": {
      "name": "Կապի ցանցերի բարձրագույն համայնքի անդամ"
    },
    "GOLD_SUPPLIER": {
      "name": "Ոսկու մատակարար"
    },
    "GOLD_SELLER": {
      "name": "Ոսկու դիլեր"
    },
    "CONTENT_SUPERMAN": {
      "name": "Բովանդակության լավագույն արտադրող"
    },
    "NINJA": {
      "name": "Նինջա"
    },
    "STAR": {
      "name": "Աստղ"
    },
    "GUARDIAN": {
      "name": "Մեծ հաճախորդների սպասարկում"
    },
    "KING": {
      "name": "Շուկայի արքան"
    }
  },
  "cookie": {
    "title": "Այս կայքը օգտագործում է թխուկներ",
    "message": "Այս կայքը օգտագործում է թխուկներ՝ օգտատերերի փորձը բարելավելու համար: Օգտագործելով մեր կայքը, դուք համաձայնում եք բոլոր թխուկներին՝ համաձայն մեր թխուկների քաղաքականության:",
    "preferences_action": "Կառավարեք նախապատվությունները",
    "agree_action": "Համաձայն եմ"
  },
  "basket_item_message": {
    "title": "Պահանջվող տեղեկատվություն այս կետի համար",
    "save_actions": "Պահպանել հաղորդագրությունը",
    "notifications": {
      "success": "Ձեր մուտքային տվյալները պահպանվել են:"
    },
    "product_inputs_preview_message": "Այս հարմարեցման տարբերակները հասանելի են ձեր պատվիրած ապրանքի համար: Հենց որ ապրանքը ավելացվի ձեր զամբյուղին, դուք կկարողանաք ճշգրտումներ կատարել այստեղ:"
  },
  "faq": {
    "faq": "Հաճախակի տրվող հարցեր",
    "title": "Հարցեր եւ պատասխաններ",
    "sub_title": "Այն, ինչ դուք մեզ խնդրեցիք անել:",
    "message": "Ստորև տրված պատասխանների մեջ փնտրեք ձեր հարցի պատասխանը: Հարցրեք մեզ, եթե հարցերը մնում են անպատասխան:",
    "add_question_action": "Նոր Հարց տվեք",
    "read_more": "Կարդալ ավելին ..",
    "load_more": "Ավելի շատ հարցեր..",
    "main_page": "ՀՏՀ Գլխավոր էջ",
    "add_new": "Ներկայացրեք նոր հարց",
    "login_alert": "Մուտք գործեք՝ հարց տալու համար։",
    "question_input": "Հարց",
    "question_input_placeholder": "Հարցրեք ձեր հարցը.",
    "notifications": {
      "success_add": "Ձեր հարցը հաջողությամբ ուղարկվեց:"
    }
  },
  "rating_labels": [
    "Շատ վատ",
    "Վատ",
    "Նորմալ",
    "Լավ",
    "Գերազանց"
  ],
  "map_view": {
    "address_detail": "Հասցեի մանրամասները",
    "change": "Փոփոխություն",
    "select": "Ընտրեք",
    "not_found_address": "Ոչ մի արժեք չի գտնվել",
    "enter_address_hint": "Մուտքագրեք նպատակակետի հասցեն",
    "enter_your_address": "Մուտքագրեք ձեր հասցեն:",
    "postal_code": "Ինդեքս",
    "phone_input": "Հեռախոս {type}",
    "building_number": "Շենքի համարը",
    "building_unit": "Միավոր",
    "full_name_input": "Ամբողջական անուն {type}",
    "save_detail_action": "Պահպանեք մանրամասները",
    "more_detail_input": "Լրացուցիչ նկարագրություն",
    "add_address_to_list": "Ավելացրեք հասցե ցուցակին",
    "address_title_input": "Հասցեի պիտակ (պահանջվում է)",
    "notifications": {
      "save_in_list": "Այս հասցեն պահվել է ձեր ցուցակում՝ իր բոլոր մանրամասներով:",
      "edit_success": "Հասցեի նոր արժեքները պահպանված են {title}-ում:"
    },
    "delete_address_dialog": {
      "title": "Ջնջել հասցեն",
      "message": "Իսկապե՞ս ուզում եք ջնջել այս հասցեն:",
      "action": "Հեռացնել հասցեն"
    }
  },
  "comment_manager": {
    "can_not_edit": "Հնարավոր չէ խմբագրել այս մեկնաբանությունը:",
    "comment_input_empty": "Հաղորդագրության տեքստը դատարկ է:",
    "article_not_saved": "Պահպանե՛ք հոդվածը:"
  },
  "wheel_fortune": {
    "title": "Խաղալ և հաղթել",
    "message": "Յուրաքանչյուր խաղի արժեքը 10 չիպ է։ Դուք ներկայումս ունեք {chips} չիպ: Ստացեք ավելի շատ չիպեր՝ գնելով խանութից:",
    "win_message": "Շնորհավորում եմ, դու հաղթում ես:",
    "lost_message": "Կներեք, դուք պարտվել եք խաղը:",
    "try_again": "Կարող եք նորից փորձել"
  },
  "json": {
    "add": "Ավելացրեք կառուցվածքի տվյալները",
    "add_message": "Ստեղծեք նոր կառուցվածքային տվյալներ",
    "structures": "Կառուցվածքներ",
    "book": {
      "title": "Գիրք",
      "desc": "Գրքի գործողություններ, որոնք օգտատերերին հնարավորություն են տալիս գնել այն գիրքը, որը նրանք գտնում են անմիջապես Որոնման արդյունքներից:"
    },
    "occupation": {
      "title": "Մոտավոր աշխատավարձ",
      "desc": "Աշխատավարձի գնահատման մասին տեղեկությունները, ինչպիսիք են աշխատավարձի միջակայքերը և աշխատանքի տեսակների համար ըստ տարածաշրջանի միջին աշխատավարձերը, ցուցադրվում են Google-ում աշխատանք փնտրելու փորձի մեջ:"
    },
    "course": {
      "title": "Դասընթաց",
      "desc": "Կրթական դասընթացներ, որոնք հայտնվում են մատակարարի հատուկ ցանկում: Դասընթացները կարող են ներառել դասընթացի անվանումը, մատակարարը և կարճ նկարագրությունը:"
    },
    "howto": {
      "title": "Ինչպես",
      "desc": "«How-to»-ն օգտատերերին առաջնորդում է մի շարք քայլերով՝ առաջադրանքը հաջողությամբ ավարտելու համար՝ ներառելով տեսանյութեր, պատկերներ և տեքստ:"
    },
    "recipe": {
      "title": "Բաղադրատոմսը",
      "desc": "Բաղադրատոմսեր, որոնք ցուցադրվում են որպես անհատական հարուստ արդյունք կամ հյուրընկալող կարուսելի մաս:"
    }
  },
  "support": {
    "Financial": "Ֆինանսական",
    "Technical": "Տեխնիկական",
    "Advisor": "խորհրդական"
  },
  "condition": {
    "new": "Նոր",
    "refurbished": "Վերանորոգվել",
    "used": "Օգտագործված",
    "used_fair": "Օգտագործված արդար",
    "used_good": "Լավ օգտագործված",
    "used_like_new": "Օգտագործված է որպես նոր"
  },
  "variant_filter": {
    "title": "Ապրանքի տարբերակի զտիչ",
    "only_available": "Պահեստում"
  },
  "service": {
    "task": {
      "title": "Առաջադրանքներ",
      "message": "Այս ծառայության մեջ կատարված առաջադրանքների ցանկը:"
    },
    "appointment": {
      "title": "Նշանակում",
      "message": "Ընտրեք շաբաթվա ժամանակը հետևյալ ժամանակներից՝ հիմնվելով ձեզ համար հարմար օրվա և ժամի վրա:"
    },
    "booking": {
      "title": "Ամրագրում",
      "message": "Ստորև բերված օրացույցում նշեք այն օրերի ժամանակահատվածը, որոնք ցանկանում եք ամրագրել:"
    },
    "subscription": {
      "title": "Բաժանորդագրություն"
    },
    "charge": {
      "title": "Լիցքավորում"
    }
  },
  "PricingTypes": {
    "FIX": {
      "title": "Ֆիքսված գներ",
      "message": "Այս տեսակի գնագոյացման դեպքում ապրանքի գինը ֆիքսված գումար է և գնորդի կողմից գնի հաշվարկ պահանջելու կարիք չկա:"
    },
    "ESTIMATION": {
      "title": "Գնագոյացման գնահատում",
      "message": "Ապրանքների և ծառայությունների գինը որոշ չափով կհաշվարկվի վաճառողի կողմից պատվեր կատարելիս, ծառայության ընթացքում վճարման հաշիվ-ապրանքագրերը կարող են փոփոխվել կամ ավելացվել:"
    },
    "AGREEMENT": {
      "title": "Համաձայնագրի գնագոյացում",
      "message": "Ապրանքների կամ ծառայությունների վերջնական գինը չի փոխվի վաճառողի կողմից պատվերի տեղադրումից հետո:"
    },
    "BID": {
      "title": "Հայտ / Աճուրդ",
      "message": "Ապրանքը հնարավոր գնորդների կողմից կվաճառվի ամենաբարձր գնով:"
    }
  },
  "map": {
    "title": "Ապրանքներ ստանալու նպատակակետ",
    "confirm": "Սահմանել գտնվելու վայրը",
    "address": "Նպատակակետ",
    "receptor": "Ընդունիչ"
  },
  "ShopBots": {
    "telegram": "Telegram"
  },
  "article_tags": {
    "title": "Բովանդակության պիտակներ",
    "tags_input": "Պիտակներ",
    "tags_input_message": "Tags Այս հոդվածի ցանկը",
    "notification": {
      "add_success": "Ավելացված են պիտակներ"
    }
  },
  "difficulties": {
    "very_complicated": "Շատ բարդ",
    "hard": "Դժվար",
    "intermediate": "Միջանկյալ",
    "easy": "Հեշտ"
  },
  "billing_period": {
    "Daily": "Ամենօրյա",
    "Weekly": "Շաբաթական",
    "Monthly": "Ամսական",
    "Every3Months": "Ամեն 3 ամիսը մեկ",
    "Every6Months": "Ամեն 6 ամիսը մեկ",
    "Yearly": "Տարեկան"
  },
  "buyer_rating_statement": {
    "between1_2": "Օ՜, լավ չէ:",
    "between2_3": "Ամեն ինչ կարգին է!",
    "between3_4": "Մարդկանց դուր է գալիս:",
    "between4_5": "Մարդիկ սիրում են այն:"
  },
  "seasons": {
    "spring": "Գարուն",
    "summer": "Ամառ",
    "autumn": "Աշուն",
    "winter": "Ձմեռ"
  }
};