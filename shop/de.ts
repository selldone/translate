/*
 * Copyright (c) 2023. Selldone® Business OS™
 *
 * Author: M.Pajuhaan
 * Web: https://selldone.com
 * ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 *
 * All rights reserved. In the weave of time, where traditions and innovations intermingle, this content was crafted.
 * From the essence of thought, through the corridors of creativity, each word, and sentiment has been molded.
 * Not just to exist, but to inspire. Like an artist's stroke or a sculptor's chisel, every nuance is deliberate.
 * Our journey is not just about reaching a destination, but about creating a masterpiece.
 * Tread carefully, for you're treading on dreams.
 */

export default {
  /** @see SShopProductRatingView **/
  surveys: ["Sehr schlecht", "Schlecht", "Normal", "Gut", "Exzellent"],

  /** @see SFooterSection **/
  footer: {
    map: "Karte",
    email: "E-Mail",
    phone: "Telefon",
    address: "Adresse",

    copyright:
      "Die Nutzung der Inhalte ist nur für nichtkommerzielle Zwecke und mit Quellenangabe (Link mit {shop_title}) erlaubt. Alle Rechte sind {shop_title} vorbehalten.",
    powered_by: "Bereitgestellt von",
    privacy: "Datenschutzbestimmungen",
    term_of_use: "Nutzungsbedingungen",
    shop_app_setting: "Shop-Einstellung & DSGVO",

    dialog_setting: {
      title: "Shop-App-Einstellung",
      app_list: "Liste der Anwendungen",
    },
  },

  /** @see SShopProductCard **/
  product_card: {
    sold_out: "Ausverkauft",
    review_unit: "Bewertung",
    not_original: "Nicht original",
  },

  /** @see ShopPage **/
  /** @see Shop **/
  shop: {
    products_filter: "Filter",
    native: {
      title: "App-Anmeldung",
      message:
        "Diese App erfordert Zugriff auf Ihre Shop-Kontoinformationen. Dies umfasst den Zugriff auf Name, Kontaktnummer, Adresse, E-Mail, Bestellverlauf, Bestellungen und das Posten von Kommentaren in diesem Shop.",
      message_bold:
        "Verwenden Sie unbedingt die offizielle Shop-App, um diesen Shop zu betreten.",
    },
  },

  /** @see SortView **/
  sort: {
    title: "Sortieren nach",
    title_small: "Sortieren nach..",
    nothing: "Nichts",
    most_views: "Die meisten Aufrufe",
    most_likes: "Am beliebtesten",
    most_recent: "Neueste",
    most_sell: "Meistverkauft",
    cheapest: "Am günstigsten",
    most_expensive: "Am teuersten",
    only_available: "Verfügbar",
    view_mode: {
      normal: "Normal",
      grid: "Raster",
      list: "Liste",
      insta: "Insta",
    },
  },

  /** @see SProductOverview **/
  product_info: {
    fake: "Gefälscht",
    action: "Auktion",
    compare_limit: "Limit von 10 Artikeln",
    compare_add: "Zum Vergleich hinzufügen",
    compare_be_in_list: "In der Vergleichsliste",

    type: "Typ",
    brand: "Marke",
    category: "Kategorie",
    discount: "Rabatt",
    track_price: "Preis verfolgen",
    waiting_for_auction: "Warten auf die Auktion?",
    inform_auction: "Über Auktion informieren",
    return_in_days: "{days} Tage Rückgabegarantie",
    support24h7: "24/7-Support",
    support_normal: "Support während der Arbeitszeit",
    original_guarantee: "Originalgarantie",
    cod_payment: "Unterstützung per Nachnahme",

    pros: "Vorteile",
    cons: "Nachteile",

    external_link: "Weiterlesen",
    notifications: {
      congratulation: "Herzlichen Glückwunsch",
      waiting_list_add_success:
        "Wir benachrichtigen Sie, wenn dieses Produkt versteigert wird.",
      waiting_list_delete_success: "Sie haben die Auktionswarteliste verlassen.",
    },

    offer_message: "Kaufen Sie {min_quantity} und erhalten Sie {percent} % Rabatt auf diese Artikel",
    up_to: "Bis zu {per_order}x pro Bestellung",
    get_free: "KOSTENLOS ERHALTEN",
    get_off: "{percent} % RABATT ERHALTEN",
    select_a_vendor_message: "Bitte wählen Sie einen Anbieter aus.",
    product_description_header: "Wir stellen vor: {product}",
  },

  /** @see SShopBuyButton **/

  buy_button: {
    waiting_for_availability: "Warten auf Verfügbarkeit",
    inform_me_when_available: "Darauf warten?",
    pay_buy: "Bezahlen & Kaufen",
    quantity: "Anzahl",
    quantity_in_basket: "Artikel im Warenkorb",
    count_unit: "x",
    remove: "Entfernen",
    add_to_basket: "In den Warenkorb legen",
    buy: "Kaufen",
    subscribe_now: "Jetzt abonnieren",
    unsubscribe: "Abbestellen",
    select_a_subscription_plan: "Wählen Sie zuerst einen Plan aus!",
    manage_subscriptions: "Abonnements verwalten",
    notifications: {
      inform_add_success: "Wir informieren Sie, sobald es verfügbar ist.",
      inform_remove_success: "Sie wurden von der Warteliste gestrichen.",
    },
    zip_pin_input: "PLZ / PIN",
    zip_pin_not_available_msg: "An diesem Standort nicht verfügbar!",
    zip_pin_placeholder: "PLZ/PIN auswählen...",
  },

  /** @see StorefrontPageProduct **/

  product: {
    review: "Beschreibung",
    spec: "Spezifikationen",
    comments: "Bewertungen",
    price: "Preis",
    rate: "Bewertung",
    comment: "Kommentar",
    warranty: "Garantie",
    return_policy: "Rückgaberecht",
    guide: "Anleitung",
    shipping: "Versand",
    related_products_title: "Das könnte Ihnen auch gefallen",
    related_categories_title: "In Kollektionen entdecken",
  },

  /** @see StorefrontLayout **/
  layout_shop: {
    shop_menu: {
      orders_history: "Bestellverlauf",
      profile: "Mein Profil",
      favorites: "Lieblingsartikel",
      gift_cards: "Geschenkkarten",
      return_requests: "Rücksendungen",
      basket: "Warenkorb",
    },
    user_menu: {
      my_shops: "Meine Shops",
      go_to_admin: "{shop_name} verwalten",
      exit_from: "{shop_name} verlassen",
      chips: "{chips} Jetons",
    },
    login_to_shop: "Im Shop anmelden",
    login_to_shop_short: "Anmelden",
    item_in_basket_message: "{count} Dieser Artikel befindet sich in Ihrem Warenkorb.",
    search_title: "Im Shop {shop_name} suchen",
  },

  /** {@see BasketTopMenu} **/
  basket_top_menu: {
    basket: "Warenkorb",
    basket_is_empty: "Ihr Warenkorb ist leer.",
    total_price: "Gesamt",
    view_basket: "Ihr Warenkorb",
    more: "Mehr ...",
    accept_and_pay: "Zur Kasse",
    items: "Produkt(e)",
  },

  /** {@see StorefrontPageUserOrdersPhysical} **/
  history_orders_physical: {
    rejected: "Abgelehnt",
    table: {
      code: "Bestellcode",
      reserved: "Datum",
      delivery_state: "Lieferstatus",
      price: "Gesamtpreis",
      status: "Zahlungsstatus",
    },
  },
  /** {@see SOrdersList} **/
  history_orders: {
    table: {
      code: "Bestellcode",
      items: "Produkte",
      reserved: "Datum",
      delivery_state: "Letzter Status",
      price: "Gesamtpreis",
      status: "Zahlungsstatus",
    },
  },

  /** {@see StorefrontPageUser} **/
  user_page: {
    menu: {
      gift_card: "Geschenkkarte",
      comments: "Kommentare",
      favorites: "Favoriten",
      return_requests: "Rücksendungen",
      addresses: "Adressen",
      profile: "Profil",
      orders: "Bestellungen",
      wallets: "Geldbörsen",
    },
  },
  /**
   * @see StorefrontPageUserWallets
   */
  user_wallets: {
    title: "Meine Geldbörsen",
    subtitle: "Wenn Sie berechtigt sind und ein Cashback-Programm aktiv ist, wird ein Teil Ihrer Bestellung Ihrer Geldbörse gutgeschrieben. Sie können dieses Guthaben bei Ihrem nächsten Einkauf verwenden."
  },
  /**
   * @see SWalletInput
   */
  wallet_input:{
    my_wallet_balance:"Guthaben meiner Geldbörse",
  },


  /** {@see StorefrontPageBasketOrderPhysicalDashboard} **/
  order_page: {
    title: "Bestellung",

    /** {@see SShopCustomerOrderPaymentWidget} **/
    payment: {
      title: "Zahlung",
      payment_waiting: "Warten auf Zahlung",
      payment_completed: "Bezahlt",
      table: {
        title: "Titel",
        amount: "Betrag",
        description: "Beschreibung",
      },
      total_payment: "Bezahlter Betrag",
      discount: "Rabatt",
      discount_code: "Rabattcode",
      delivery_fee: "Versandkosten",
      delivery_fee_after: "Keine Versandkosten bezahlt",

      total_price: "Gesamtpreis der Artikel",
      total_items_discount: "Artikelrabatt",
      total_price_after_offers: "Gesamt nach Anwendung der Angebote",
      total_order_price_before_tax: "Gesamtpreis vor Steuern",

      buyer_payment: "Zahlungsbetrag des Käufers",

      payment_is_in_require_capture_message:
        "Wir haben Ihre Zahlung erhalten, sie wurde aber noch nicht erfasst.",
      settle_the_earlier_bill_first: "Begleichen Sie zuerst die frühere Rechnung!",
    },
    /** {@see SShopCustomerDeliveryInfoWidget} **/
    delivery: {
      title: "Lieferung",
      subtitle: "Käuferinformationen",
      confirm_received_action: "Produktempfang bestätigen",
      confirm_received_info:
        "Klicken Sie auf die Bestätigungsschaltfläche, wenn Sie die Bestellung erhalten haben. Bei Bedarf können Sie nach der Bestätigung eine Bewertung einreichen.",
      Recipient: "Empfänger",
      bill: "Rechnung",
      name: "Name",
      tel: "Telefon",
      email: "E-Mail",
      country: "Land",
      state: "Bundesland",
      city: "Stadt",
      address: "Adresse",
      building_no: "Hausnummer",
      unit_no: "Einheitsnummer",
      postal_code: "Postleitzahl",
      description: "Beschreibung",
      order_delivered: "Die Bestellung wurde geliefert.",
      delivery_time: "Lieferzeit",
      tracking_code: "Sendungsverfolgungsnummer",
      tracking_url: "Tracking-URL",

      notifications: {
        confirm_delivery: "Ihre Bestellung wird bestätigt.",
      },
    },
    /** {@see SShopBasketItemsList} **/
    basket_items: {
      original_warranty: "Echtheitsgarantie der Ware",
      return: "Rückgabe",
      table: {
        product: "Produktname",
        count: "Anzahl",
        unit_price: "Einzelpreis",
        total_price: "Gesamtpreis",
        discount: "Rabatt",
        final_price: "Endpreis",
        returned: "Zurückgegeben",
      },
    },
  },

  /** {@see SShopBasketItemReturnForm} **/
  return_request: {
    title: "Rückgabe",
    count_input: "Anzahl der zurückgegebenen Artikel",
    count_input_message: "Wie viele Artikel senden Sie zurück?",
    reason_input: "Grund der Rückgabe",
    reason_input_message: "Wählen Sie den Grund für die Rückgabe dieses Artikels aus.",
    note_input: "Beschreibung",
    note_input_hint: "Beschreiben Sie den Grund für die Rücksendung..",
    note_input_message:
      "Beschreiben Sie bei Bedarf den Grund für die Rücksendung dieses Artikels.",
    media_message:
      "Sie können hier Bilder, Audio oder Video des Produkts hochladen, um eine bessere Unterstützung für Produktstatusinformationen zu erhalten. Die Auswahl der Datei wird automatisch gesendet und bestätigt.",
    video: "Video",
    voice: "Stimme",
    add_action: "Rücksendeantrag senden",
    notifications: "Ihre Rücksendeanfrage wurde erfolgreich registriert.",
  },
  /** {@see StorefrontPageBasketOrderVirtualDashboard} **/
  virtual_order_page: {
    title: "Bestellung",
    info: {
      title: "Gekauftes virtuelles Produkt",
    },
  },

  /** {@see StorefrontPageUserProfile} **/
  user_profile: {
    title: "Meine persönlichen Informationen",
    name: "Name",
    email: "E-Mail",
    tel: "Telefon",
    identity: "Authentifizierung",
    success_kyc: "KYC verifiziert",
    no_kyc: "Keine KYC-Verifizierung",
    club: "Benutzerlevel",
    subscription: "Newsletter-Abonnement",
    leave_shop: "Diesen Shop verlassen und alle Kundenlevel entfernen",
    leave_shop_action: "Shop verlassen",
    edit_personal_info: "Persönliche Daten bearbeiten",
    leave_dialog: {
      title: "Shop verlassen",
      message:
        "Wenn Sie sich von diesem Shop abmelden, werden Ihre Bestellinformationen und -aufzeichnungen nicht gelöscht.",
      action: "Verlassen",
    },
    notifications: {
      leave_success: "Shop verlassen",
    },
    club_table: {
      discount: "Rabatt",
      limit: "Grenze",
      currency: "Währung",
      free_shipping: "Kostenloser Versand",
    },

    my_profile: {
      title: "Mein Profil",
      subtitle: "Dies sind meine Kundenprofilinformationen für den Shop.",
      edit_action: "Profil bearbeiten"
    },
    subscribe_status: {
      subscribed: "Ich bin angemeldet, um die neuesten Nachrichten und Aktionen per E-Mail zu erhalten.",
      unsubscribed: "Ich bin nicht angemeldet, um Neuigkeiten oder Aktionen per E-Mail zu erhalten."
    },
    my_club: {
      subtitle: "Meine Clubstufe und ihre Vorteile."
    },
    kyc: {
      subtitle: "Dies sind meine globalen Identifikationsinformationen."
    }

  },
  /** {@see StorefrontPageBasketCart} **/
  basket_page: {
    title: "Warenkorb",
    total_price: "Gesamtsumme",
    shipping: "Versandkosten",
    free_shipping: "Kostenloser Versand",
    shipping_not_pay: "Nicht enthalten",
    total_discount: "Gesamtrabatt",
    customer_club: "Kundenclub-Mitglied",
    customer_club_discount: "Mitgliederrabatt",
    final_price: "Restzahlung",
    lead_time: "Vorbereitungszeit",
    lead_time_unit: "Stunde",
    receive_time: "Bevorzugte Lieferzeit",
    select_time: "Lieferzeit auswählen",
    no_select_time: "So bald wie möglich",
    transportation_type: "Versandart",
    days_input: "Liefertage",
    days_input_label: "Tage",
    days_input_no_data: "Wählen Sie mindestens einen Tag aus",
    time_input: "Lieferzeit",
    time_input_label: "Lieferung um",
    time_input_label_no_data: "Wählen Sie mindestens ein Zeitintervall aus",
    support_cod: "Barzahlung bei Lieferung",
    not_support_cod: "Zahlung vor Ort nicht möglich!",
    final_confirm_action: "Endgültige Auftragsbestätigung",
    pay_and_complete_action: "Jetzt bezahlen",
    pre_confirm_service: "Anfrage einreichen",
    cross_selling_discount: "Abholrabatt",

    errors: {
      no_address: "Wählen Sie die Adresse aus, an die Ihre Bestellung versendet werden soll.",
      no_phone: "Geben Sie eine Kontaktnummer ein, um den Wareneingang zu koordinieren.",
      select_billing_country: "Wählen Sie Ihr Rechnungsland aus",
      select_billing_state: "Wählen Sie Ihre Steuerregion aus",
    },
    same_billing_address: "Rechnungsinformationen wie Empfängerinformationen",
    same_billing_address_desc:
      "Die Rechnungsadresse ist identisch mit der Lieferadresse.",
    custom_billing_info: "Unterschiedliche Rechnungsinformationen",
    custom_billing_info_desc:
      "Sie können Ihre Steuernummer eingeben und eine andere Adresse für die Rechnungsstellung festlegen.",
    basket_empty: "Warenkorb ist leer",
    extra_shipping_count: "Sie erhalten {count} Pakete.",
    select_location_error:
      "Finden Sie den Empfangsort der Sendung auf der Karte und klicken Sie auf den Indikator in der Mitte der Karte, um die genaue Adresse des Empfängers zu ermitteln.",

    billing_business_desc: "Es handelt sich um ein Geschäftskonto mit einer Steuernummer.",
    billing_personal_desc: "Es handelt sich um ein persönliches Konto.",

    no_billing_title: "Keine Rechnung",
    no_billing_desc: "Rechnungsadresse leer lassen.",

    can_pay_cod_message:"Sie können diese Bestellung bei Lieferung bar bezahlen.",
  },
  /** {@see SShopBasketItems} **/
  basket_items: {
    items_unit: "x",
    lead_unit: "Stunde",
    count_input: "Anzahl",

    price_changed: "Der Preis hat sich geändert. Neuer Preis:",
    offer: "Erhalte {count}x Artikel mit {percent} % Rabatt",
    offer_free: "Erhalte {count}x kostenlos",
  },

  /** {@see StorefrontUserAddresses} **/
  addresses_page: {
    title: "Meine Adressen",
    subtitle:"Sehen Sie hier Ihre gespeicherten Adressen ein und fügen Sie einfach eine neue hinzu.",
    new_action: "Neue Adresse",
    receiver: "Empfänger",
    address: "Adresse",
    building_no: "Hausnummer",
    building_unit: "Einheit",
    postcode: "Postleitzahl",
    tel: "Telefon",
    description: "Notiz",

    map: {
      title: "Meine Adressen",
      confirm: "Adressregistrierung",
      address_type: "Adresse",
      receptor_type: "Empfänger",
    },
    add_dialog: {
      title: "Neuer Adressname",
      message: "Geben Sie einen Titel für eine neue Adresse ein.",
      address_name_input: "Adresse (erforderlich)",
    },
  },
  /** {@see StorefrontPageUserReturns} **/
  return_requests: {
    title: "Rücksendeanfragen",
    subtitle:"Sehen Sie hier Ihre Rückgabeanfragen ein. Für Artikel mit Rückgaberecht können Sie auf der Bestellseite eine Rückgabeanfrage stellen.",
  },

  /** {@see StorefrontPageUserWishlist} **/
  user_favorites: {
    title: "Wunschzettel",
  },
  /** {@see StorefrontPageUserComments} **/
  user_comments: {
    title: "Meine Kommentare",
    first_post: "Erster Beitrag",
    last_edit: "Letzte Bearbeitung",
  },
  /** {@see StorefrontPageUserGiftcards} **/
  user_gift_cards: {
    title: "Meine Geschenkkarten",
    add_action: "Karte hinzufügen",
    add_card_dialog: {
      title: "Geschenkkarte hinzufügen",
      message:
        "Geben Sie die Kartennummer und den Sicherheitscode ein. Eine Geschenkkarte wird Ihrem Konto hinzugefügt.",
      card_number_input: "Kartennummer",
      info: "Hinweis: Beim Kauf im Geschäft wird Ihnen die Option angezeigt, mit Geschenkkarten zu bezahlen. Wenn Sie diese Option auswählen, wird der Betrag Ihres Aufladeguthabens vom Bestellbetrag abgezogen.",
      add_action: "Karte registrieren",
    },
  },

  /** {@see SComparisonButton} **/
  product_comparison: {
    title: "Produkt vergleichen",
    action_button: "Vergleichen",

    /** {@see SComparisonList} **/
    list: {
      no_data: "Es wurden keine Produkte zur Vergleichsliste hinzugefügt.",
      price: "Preis",
      type: "Typ",
      brand: "Marke",
      warranty: "Garantie",
      variants_comparison: "Produktvarianten vergleichen",
      spec: "Technische Spezifikationen",
      pros: "Vorteile",
      cons: "Nachteile",
      user_review: "Benutzerkommentare",
      rating: "Bewertung",
      rating_count: "Anzahl der Bewertungen",
      info: "Sie können diese Produktvergleichstabelle mit Ihren Freunden teilen, indem Sie den folgenden Link senden, oder später darauf zugreifen, indem Sie diesen Link speichern.",
      copy_action: "Vergleichslink kopieren",
      check_list: ["Ja", "Verfügt über"],
      cross_list: ["Nein", "Hat nicht"],
    },
  },
  /** {@see SCategoryFilter} **/
  product_filter_menu: {
    only_original: "Nur Originale",
    only_has_discount: "Nur rabattiert",
    price_range: "Preisspanne",
    brands: "Marke",
    main_shop_page: "Shop-Hauptseite",
    home: "Startseite",
  },

  /** {@see SShopProductRatingView} **/
  product_rating_view: {
    message:
      "Sehr geehrter {user}, Sie haben dieses Produkt gekauft. Was halten Sie von diesem Produkt?",
    notification: {
      title: "Glückwunsch",
      message: "Ihr Kommentar wurde aufgezeichnet.",
    },
  },

  /** @see StorefrontPageBlogsList **/
  blogs: {
    title: "Blog",
    add_new_blog: "Neuer Blog",
    popular: "Beliebt",
    topics: "Themen",
    suggestions: "Vorschläge",
  },

  /** @see StorefrontPageOfficial **/
  official_pages: {
    about_us: "Über uns",
    terms: "Nutzungsbedingungen",
    privacy: "Datenschutzbestimmungen",
    contact_us: "Kontaktieren Sie uns",
  },

  /** @see StorefrontContactUsPage **/
  contact_us_form: {
    title: "Kontaktformular",
    name: "Name",
    email: "E-Mail",
    phone: "Telefon",
    message: "Nachricht",
    notifications: {
      success: "Ihre Nachricht wurde gesendet.",
    },
    submitted_title: "Vielen Dank, dass Sie sich an uns gewandt haben!",
    submitted_message:
      "Wir freuen uns, dass Sie sich Zeit genommen haben, mit uns in Kontakt zu treten. Ihre Nachricht ist bei uns eingegangen und wird derzeit von unserem Team geprüft. Wir werden so schnell wie möglich antworten und danken Ihnen für Ihre Geduld. Ihr Interesse und Ihr Feedback sind uns wichtig und wir bemühen uns, Sie bestmöglich zu unterstützen.",
  },

  /** @see SStorefrontMasterPaymentDialog **/
  master_payment: {
    notifications: {
      payment_success_title: "Zahlungsbestätigung",
      payment_success: "Bezahlvorgang abgeschlossen.",
      buy_title: "Kaufen",
      pay_by_giftcards: "Ihre Bestellung wurde mit einer Geschenkkarte bezahlt.",
      free_order: "Ihre Bestellung ist kostenlos.",
      pay_by_cod: "Ihre Bestellung wurde als Nachnahme hinzugefügt.",
      pay_by_dir:
        "Ihre Bestellung wurde erfolgreich hinzugefügt. Jetzt können Sie per Sofortüberweisung bezahlen und anschließend den Zahlungsbeleg auf der Bestellseite hochladen.",
      pay_title: "Zahlung",
      qr_code_payment: "Zahlungs-QR-Code-Adresse erstellt.",
      connecting_to_online_gateway: "Verbindung zum Online-Zahlungsgateway herstellen.",
    },
  },

  /** {@see TimeProgressBar} **/
  time_progress_bar: {
    title_before_start: "Bis zum Start verbleibend",
    title_between_start_end: "Bis zum Ende verbleibend",
    title_after_end: "Fertig",
    title_no_start_end: "Ohne Zeitraum!",
  },
  /** {@see SStorefrontLotteryPrizesList} **/
  lottery_prizes: {
    message: "Sie können pro Bestellung ein Geschenk erhalten.",
  },

  /** {@see SStorefrontCouponsList} **/
  coupons: {
    coupon_code: "Gutscheincode",
    add_coupon: "Gutschein hinzufügen",
  },

  /** {@see SStorefrontAvocadoCustomerOrderForm} **/
  /** {@see SShopAvocadoCustomerOrderItems} **/
  /** {@see SStorefrontAvocadoCustomerOrderList} **/
  /** {@see StorefrontPageAvocado} **/
  /** {@see StorefrontPageAvocadoCart} **/

  avocado: {
    not_enable: "Das Avocado-Formular ist nicht aktiviert!",
    last_update: "Letzte Aktualisierung",
    step1:
      "1. Geben Sie den Namen und die Details des Produkts ein, das Sie bestellen möchten, und klicken Sie auf die Schaltfläche Hinzufügen.",
    title_plc: "Geben Sie den Namen des Produkts ein.",
    description_plc: "Haben Sie weitere Produktbeschreibungen?",
    link_plc: "Haben Sie einen Link zum Produkt?",
    image: "Produktbild",
    image_msg: "Sie können das Produktbild bei Bedarf senden.",
    add_item: "Artikel hinzufügen",
    items_list: "Liste der angeforderten Produkte",
    currency_msg: "Sie müssen den Betrag dieser Bestellung in dieser Währung bezahlen.",
    step2:
      "2. Nachdem Sie alle gewünschten Artikel hinzugefügt haben, klicken Sie auf die Schaltfläche zur Bestellprüfung. In diesem Stadium werden wir die Bestellung überprüfen und Sie über den Preis informieren. Wenn Sie auf diesen Link klicken, können Sie den Preis- und Zahlungslink sehen.",
    order_action: "Bestellung registrieren",
    show_all: "Alle anzeigen",
    show_accepted: "Bestätigte Artikel anzeigen",
    last_orders: "Liste Ihrer letzten Bestellungen",
    pending_status: "In Prüfung",
    check_now: "Bestellung prüfen",
    add_new_order: "Neue Bestellung erstellen",
    pay_online: "Online bezahlen",
    pricing_not_completed_message:
      "Ihre Bestellung wird überprüft. Ihre Bestellung wird in Kürze geprüft und mit einem Preis versehen, anschließend können Sie die Bestellung bezahlen.",

    select_address_message:
      "Bitte geben Sie Ihre Adresse ein, dann können Sie die Bestellung absenden.",

    delete_item_dialog: {
      title: "Artikel aus dem Warenkorb entfernen",
      message: "Möchten Sie diesen Artikel aus dem Warenkorb löschen?",
      action: "Ja, löschen",
    },

    notifications: {
      order_success: "Ihre Bestellung wurde erfolgreich aufgegeben.",
      delete_success: "Artikel gelöscht.",
    },
  },

  /** {@see SHyperCart} **/
  hyper: {
    title: "Bestellung erstellen",
    message:
      "Erstellen Sie die Liste der Produkte, die Sie kaufen möchten, indem Sie den Produktcode scannen.",

    currency_msg: "Die Währung, mit der Sie bezahlen möchten.",
    order_action: "Zahlung",
    add_new_order: "Neue Bestellung",
    continue_order: "Bestellung fortsetzen",

    delete_item_dialog: {
      title: "Artikel aus dem Warenkorb entfernen",
      message: "Möchten Sie diesen Artikel aus dem Warenkorb löschen?",
      action: "Ja, löschen",
    },
    notifications: {
      order_success: "Ihre Bestellung wurde erfolgreich aufgegeben.",
      delete_success: "Artikel gelöscht.",
    },
  },

  /** {@see SShopRowCustomerPendingPayment} **/
  pending_payment: {
    upload_payment_receipt: "Laden Sie ein Bild des Zahlungsbelegs hoch.",
    upload_payment_receipt_done:
      "✔ Sie haben das Belegbild bereits hochgeladen, warten Sie auf unsere Genehmigung. Wenn Sie Änderungen vornehmen müssen, können Sie das vorherige Bild durch ein neues ersetzen.",
    public_form_title: "Kontoinformationen für die Überweisung",
  },

  /** @see ServiceBillingItem **/
  service_bill: {
    task: {
      title: "Aufgabenliste",
      message: "Geben Sie die Titel der Arbeiten an, die Sie ausführen möchten, und teilen Sie dem Käufer mit, wie weit sie fortgeschritten sind.",
    },
    booking: {
      title: "Buchung / Reservierung",
      message: "Diese Bestellung wird zu einem bestimmten Zeitpunkt bearbeitet. Der Kunde hat den gewünschten Zeitraum ausgewählt. Wenn Sie Änderungen im ausgewählten Zeitraum vornehmen möchten, teilen Sie dies dem Kunden mit.",

      selected_checkin: "Ausgewählter Check-in durch den Kunden",
      selected_checkout: "Ausgewählter Check-out durch den Kunden",
      change_days_question:
        "Müssen Sie während der vom Kunden ausgewählten Tage wechseln?",
      show_calendar: "Bestellkalender anzeigen",
    },
    pricing: {
      title: "Preisgestaltung",
      message: "Wie viel verlangen Sie für diesen Service?",
    },
    subscription: {
      title: "Abonnement",
      message:
        "Sie können die Abonnementlaufzeit wählen, indem Sie diesen Service täglich kaufen. Teilen Sie dem Käufer mit, wenn Sie die bereits eingegebenen Standardwerte ändern möchten.",
      duration: "Abonnementzeitraum",
    },
    charge: {
      title: "Gebühr",
      message:
        "Sie haben für diesen Artikel eine Gebühr angegeben. Sie können den Belastungsbetrag für den vom Kunden gekauften Artikel ändern.",
      charge: "Gebühr",
    },
  },

  /** @see SAccessPrivateCheck **/
  check_access: {
    waiting_message: "Warten auf Zugriffsprüfung...",
    no_access_message:
      "Leider haben Sie keinen Zugang zu unserem Shop.<br/> Bitte kontaktieren Sie uns.",
  },

  /** @see SShopShareOrderButton **/
  share_order: {
    action: "Bestellung teilen",
    title: "Teilen Sie diese Bestellung mit anderen.",
    message:
      "Es ist ein sicherer Link. Teilen Sie ihn nur mit Personen, die Sie kennen. Jeder mit diesem Link kann Ihre Bestellung duplizieren und die ausgewählte Adresse sehen. Er enthält Informationen zu Ihrem aktuellen Warenkorb.",
    valid_period: "Dieser Link läuft nach 48 Stunden ab.",
    import_order_action: "Bestellung importieren",
    retrieve_dialog_title: "Externe Bestellung abrufen",
    address_included: "Warenkorbartikel + Adresse teilen",
    address_excluded: "Nur Warenkorbartikel teilen",
  },

  /** {@see ProductVariantsView} **/
  variants_view: {
    prices: "Preise",
    inventory: "Inventar",
    discount: "Rabatt",
  },

  rating: {
    product_rating_message:
      "Dieses Produkt hat eine Bewertung von {rate} von 5 Sternen erhalten und wurde von {rate_count} Personen bewertet.",
  },

  /** {@see SProductRelatedProducts} **/
  related_products: {
    card: {
      title: "Entdecken!",
      same_tags_subtitle: "Finden Sie weitere bemerkenswerte verwandte Artikel.",
      same_category_subtitle: "Weitere bemerkenswerte Artikel finden Sie in dieser Kategorie.",
    },
  },

  /** {@see SProductCrossSells} **/
  cross_selling: {
    title: "{product} zusammen bestellen mit",
  },

  /** {@see SSmartSelectVendor} **/
  select_vendor: {
    item_title: "Shop von {vendor} anzeigen",
  },

  login: {
    welcome: "Willkommen",
  },


};
