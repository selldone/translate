...
Use the English `en.ts` file as the only reference. For every target language `.ts` file, keep the structure exactly the same as `en.ts`: same comments, same `export default`, same object/array structure, same nesting, same key order, same key names, same formatting style, and same TypeScript syntax.
