export default {
  "categories": {
    "Baby": {
      "title": "Beebitarbed",
      "desc": "Söötmine, Rasedus, Vanker"
    },
    "Beauty": {
      "title": "Ilu",
      "desc": "Kosmeetika, parfüüm, juuksehooldus"
    },
    "Blog": {
      "title": "Blogi",
      "desc": "Spordiblogi, Teadusblogi, Tehnoloogiablogi"
    },
    "Fashion": {
      "title": "Mood",
      "desc": "Rõivad, jalanõud, käekellad"
    },
    "Crypto": {
      "title": "Krüpto",
      "desc": "Krüptovaluuta, plokiahel, NFT"
    },
    "Electronics": {
      "title": "Elektroonika",
      "desc": "Mobiil, videomäng, sülearvuti, televiisor"
    },
    "Food": {
      "title": "Söök ja jook",
      "desc": "Restoranid, pagariäri, joogid"
    },
    "Furniture": {
      "title": "Mööbel",
      "desc": "Kodumööbel, sisustus, pagas ja reisimine"
    },
    "Gift": {
      "title": "Kinkekaart",
      "desc": "App Store, Google Play, Netflix, Spotify"
    },
    "Groceries": {
      "title": "Toidukaubad",
      "desc": "Piim, puuvili, valmistoit, liha"
    },
    "Handcrafts": {
      "title": "Käsitöö",
      "desc": "Kollektsioneeritavad, maalitud kingad, skulptuurid, klaasikunst"
    },
    "Health": {
      "title": "Tervis",
      "desc": "Apteek, lemmikloomade hooldus, heaolu"
    },
    "Home": {
      "title": "Kodu ja elamine",
      "desc": "Köögitehnika, kööginõud, söögituba"
    },
    "Jewelry": {
      "title": "Ehted",
      "desc": "Kaelakee, Sõrmus, Käevõru"
    },
    "Sports": {
      "title": "Sport",
      "desc": "Treening, telkimine, jalgrattad, vaba aeg"
    },
    "Education": {
      "title": "Interneti-kursused",
      "desc": "Veebiarendus, Disain, Äri"
    },
    "Toy": {
      "title": "Mänguasi",
      "desc": "Mängu-, pusle-, märulifiguurid"
    },
    "Other": {
      "title": "muud",
      "desc": "Muud ametid..."
    }
  },
  "partners": {
    "title": "{name} teeb Selldone'iga koostööd",
    "subtitle": "võrguettevõtete järgmise põlvkonna avamisel.",
    "create_new_shop": "Looge uus pood",
    "select_a_shop": "Valige pood",
    "apply_deal_to": "Rakenda tehingut {shop}",
    "notifications": {
      "success_apply": "Tehing edukalt aktiveeritud!"
    }
  },
  "name": "Seadistamine",
  "shop": {
    "title": "Seadistage mu pood",
    "subtitle": "Minge järgmisele tasemele, täites need lihtsad sammud."
  },
  "open_website": "Ava minu veebisait",
  "add_product": "Lisa toode",
  "setup_shipping": "Kohaletoimetamise seadistamine",
  "add_domain": "Lisa domeen",
  "add_payment": "Ühendage makseväravaga",
  "product": {
    "step1": {
      "title": "Valik 1. Lisa käsitsi",
      "msg": "Saate oma poodi lisada füüsilisi tooteid, virtuaalseid üksusi, faile ja teenuseid ning alustada nende müüki.",
      "action_on": "Lisage oma esimene toode",
      "action_off": "Lisa rohkem tooteid",
      "category": "Kategooriad on teie veebipoodide veebisaidi meistriteos teistega võrreldes. Jaotage kõik kategooriatesse ja korraldage, lisades kategooriaid ja haldades pukseerimisega.",
      "add_category": "Lisa kategooria"
    },
    "step2": {
      "title": "Valik 2. Lisa Exceli abil",
      "msg": "Kas teil on palju tooteid, näiteks 1 000 või 10 000 toodet? Ärge muretsege, importige kõik ühe toiminguga.",
      "import_excel": "Import Exceliga",
      "download_template": "Laadige mall alla"
    },
    "step3": {
      "title": "Variant 3. Palga eksperte",
      "msg": "Kas teil on rohkem kui 100 toodet ja vajate kedagi, kes teie tooted lisaks? saatke päring aadressile expert@selldone.com ja broneerige aeg. Palkame teie jaoks eksperdid. Sellel on lihtne kindlasummaline hind, vaid 5,99 $ kauba kohta ja 2,99 $ täiendavate variantide kohta."
    }
  },
  "shipping": {
    "step1": {
      "title": "Samm 1. Määrake asukoht",
      "msg": "Esimesel sammul määrake oma ladu (saadetiste päritolu). Seda teavet kasutatakse vahemaa, saatmiskulude ja saatmisteenuse piirkonnas viibimise kontrollimiseks.",
      "edit_warehouse": "Redigeeri ladu",
      "add_warehouse": "Lisage oma ladu"
    },
    "step2": {
      "title": "Samm 2. Aktiveerige saatmisviisid",
      "msg": "Lisage oma poodi vähemalt üks tarneviis. Saate määrata hinna, lähtudes kaalust, vahemaast ja muudest parameetritest. Ärge unustage seada piiranguid vahemaa, kaalu ja pakkide suurusele, mida iga saatmisteenus toetab.",
      "add_delivery": "Lisage saatmisviisid"
    }
  },
  "domain": {
    "step1": {
      "title": "1. samm. Ostke domeen",
      "msg": "Kui teil pole veel isiklikku domeeni, on aeg osta oma domeen veebisaitidelt nagu namecheap.com ."
    },
    "step2": {
      "title": "2. toiming. Lubage SSL",
      "msg": "Kui kasutate selliseid teenuseid nagu Namecheap, mis pakub CDN-i ja SSL-i, peate lihtsalt oma domeenis aktiveerima 1.CDN ja 2.SSL."
    },
    "step3": {
      "title": "Samm 3. Määrake DNS-kirjed",
      "msg": "Õnnitleme! Nüüd pole vaja teha muud, kui konfigureerida DNS-i oma domeeniteenuse pakkujas ja lisada domeen oma kontole."
    }
  },
  "payment": {
    "step1": {
      "title": "Hinnakujundus valuutad",
      "msg": "Saate vastu võtta raha erinevates valuutades ja teie pood võib toetada rohkem kui ühte valuutat hinnakujunduses. Muide, kui määrate oma poe valuutaks USD, saavad kliendid teie käest ostmiseks maksta mis tahes valuutas (nt USD, EUR, GBP jne).",
      "manage_currencies": "Hinnakujunduse valuutade haldamine"
    },
    "step2": {
      "title": "Võta raha vastu",
      "msg": "Lisage oma poes sularaha, otseülekanne ja muud saadaolevad makseviisid.",
      "online_title": "Online makseviisid",
      "online_msg": "Aktsepteerige makseid kaartidega, uute pangarakendustega, osta-kohe-maksa-hiljem ja paljude muude veebimakseviisidega.",
      "add_online_gateway": "Lisage võrgumakseviis",
      "cod_title": "Kättetoimetamise makseviis",
      "cod_msg": "Nõustuge maksega pärast tellimuste kohaletoimetamist. Sularahamakseid saate käsitsi kinnitada.",
      "cash_title": "Sularahas makseviisid",
      "cash_msg": "Seda tüüpi makset kasutatakse Selldone® POS-is.",
      "dir_title": "Otsemakse viisid",
      "dir_msg": "Saate vastu võtta makseid otsese rahaülekandega, pangaülekandega ja muudel traditsioonilistel viisidel."
    },
    "step3": {
      "msg": "Ühendage Selldone oma Stripe'i kontoga vaid ühe klõpsuga 30 sekundi jooksul. Praegu toetame Stripe'i automaatse konfigureerimise protsessi. Minge oma lüüsi vahekaardile, lisage Stripe ja klõpsake nuppu \" <b>Automaatne ühendus Stripe'iga</b> \".",
      "my_gateways": "Minu makseväravad"
    }
  },
  "intro_inline_help_message": "Otsige seda punast abinuppu. See aitab teil harjutada end nullist oma ettevõtte OS-i eksperdiks."
};