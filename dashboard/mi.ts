/*
 * Copyright (c) 2023. Selldone® Business OS™
 *
 * Author: M.Pajuhaan
 * Web: https://selldone.com
 * ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 *
 * All rights reserved. In the weave of time, where traditions and innovations intermingle, this content was crafted.
 * From the essence of thought, through the corridors of creativity, each word, and sentiment has been molded.
 * Not just to exist, but to inspire. Like an artist's stroke or a sculptor's chisel, every nuance is deliberate.
 * Our journey is not just about reaching a destination, but about creating a masterpiece.
 * Tread carefully, for you're treading on dreams.
 */


export default {
  commons: {
    pay_today: "Utu i tenei ra",
    pay_yesterday: "Utu inanahi",
    pay_last30days: "Utu 30 ra whakamutunga",
    create_a_shop: "Waihangatia he toa",
    start_free: "Tīmata kore utu",
    featured_apps: "Ko nga taupānga kua tohua",
    more_apps: "Ētahi atu taupānga",
    my_public_profile: "Taku wharangi korero mo te iwi",
    product_admin: "Whakahaere hua",
    make_money_affiliate: "Hanga Moni",
    see_live_shop: "Tirohia te toa ora",
    see_live_product: "Tirohia te hua ora",
    has_inputs: "He tāuru āna",
    main_domain: "Rohe matua",
    domains: "Nga waahi",
    home_page: "Whārangi kāinga",
    google_icon_font_sample: "Tauira momotuhi Google",
    fontawesome_icon_font_sample: "Fontawesome (V6) tūemi",
  },

  numbers: {
    infinite: "He mutunga kore",
  },

  error: {
    not_fount_data: "Kaore i kitea nga raraunga!",
    license_max_limit: "Kua tae to raihana ki te tepe!",
    no_read_access: "kahore tūemi Urunga",
    no_write_access: "kahore tūemi Urunga",
    no_access: "kahore Urunga",
  },

  time_spans: {
    days_range: "Awhe Ra",
    today: "I tenei ra",
    yesterday: "Inanahi",
    last_7: "7 ra whakamutunga",
    last_14: "14 nga ra whakamutunga",
    last_30: "30 ra whakamutunga",
    last_90: "90 ra whakamutunga",
    custom: "Ritenga",
  },
  device_types: {
    desktop: "Papamahi",
    mobile: "Pukoro",
    tablet: "Papa",
  },

  create_website_action: "Waihangahia to paetukutuku — He kore utu",
  show_shops_action: "Rarangi o aku toa",

  // Companies:

  Youtube: "YouTube",
  Linkedin: "LinkedIn",
  Twitter: "Twitter",
  Facebook: "Facebook",
  Instagram: "Instagram",
  Reddit: "Reddit",
  Github: "GitHub",

  theme: {
    dark_mode: "Aratau pouri",
    dark_mode_description:
      "Kōwhiria tēnei kōwhiringa ki te hiahia koe ki te papamuri pouri me te tae kuputuhi ma.",
    light_mode: "Aratau Maama",
    light_mode_description:
      "Kōwhiria tēnei kōwhiringa ki te hiahia koe kia marama te papamuri me te tae pango te tuhinga.",
  },

  ai: {
    remove_bg: {
      title: "Tango papamuri",
      subtitle: "Waihanga mārama papamuri.",
    },
    reimagine: {
      title: "tūemi",
      subtitle: "Waihanga Ētahi atu.. Whakaahua tūemi Hou tūemi.",
    },
    upscale: {
      title: "tūemi",
      subtitle: "tūemi te tūemi o te Whakaahua.",
    },
  },

  // ------------------ Pages Title ------------------
  page_titles: {
    home: "Selldone ╏ Te Pūnaha Whakahaere Pakihi Mo nga Kamupene Tere Tipu",
    login: "takiuru",
    register: "Rehita",
    auth_2fa: "2 take Motuhēhē 🔐",
    academy: "Blog OS",
    help: "Pokapū Awhina",
    price_list: "Rarangi utu",
    my_stores: "Aku Toa",
    roles: "Ko aku mahi",
    wallet: "Taku Pukoro",
    personal_info: "Mōhiohio Whaiaro ╏ KYC",
    companies: "Aku Kamupene",
    access_control: "Mana Uru",
    security_center: "Pokapū Haumarutanga",
    gifts: {
      received: "Koha ╏ Kua riro",
      send: "Koha ╏ Tukuna",
    },
    monetization: {
      linked_accounts: "Monetization ╏ Nga Kaute Hononga",
      plans: "Monetization ╏ Mahere & Hononga",
    },
    affiliate: "Hononga",
    avocado: "Awhekātō — Hongere Hokohoko Hapori Hou Avocado",
    guild: "Guild — Hoera Hokohoko Huihuinga",
    partners_login: "Nga hoa ╏ Takiuru kiritaki",
  },

  /** NOT USED! */
  /* data: {
    shop: {
      users: "User",
      views: "View",
      likes: "Like",
      favorites: "Favorite",
      comments: "Comment",
      ratings: "Rating",
      wishlist: "Wishlist",
      new_visitors: "New Visitor",
      returning_visitors: "Return Visitor",
      page_views: "Page View",
      order_physical_added: "Physical Add to Cart",
      order_physical_checkout: "Physical Checkout",
      order_virtual_added: "Virtual Add to Cart",
      order_virtual_checkout: "Virtual Checkout",
      order_file_added: "File Add to Cart",
      order_file_checkout: "File Checkout",
      order_service_added: "Service Add to Cart",
      order_service_checkout: "Service Checkout",
    },
  },*/

  blank_is_all_products: "Waiho kia tuwhera ki te whakauru i nga hua katoa!",

  // ---------------------------------------------

  /** @see ShopCautionHelper **/
  cautions: {
    title:
      "Whakahaerehia to pakihi ma te mahi i nga tohutohu e whai ake nei. Kia mau ki te panui mo nga tohutohu mo te whakanui ake i te tawhita hua me te maha atu. Ko matou kei a koe i te huarahi.",
    no_payment_method: {
      title: "Tikanga utu",
      message:
        "Me taapiri he tikanga utu ki to toa.",
      action: "Kuaha toa",
    },
    link_account: {
      title: "Hononga pūkete",
      message:
        "Me hono koe i tetahi kaute ({currency}) ki to toa.",
      action: "Nga kaute toa",
    },
    no_domain: {
      title: "Tāpiri Rohe",
      message:
        "Ka taea e koe te hono i tetahi, neke atu ranei nga rohe whaiaro ki to toa. Ka whakauruhia to paetukutuku ki to rohe. He tere, he ngawari hoki tenei tukanga.",
    },
    domain_not_approved: {
      title: "Manatokonga Rohe",
      message:
        "Ko to rohe {domain} kaore ano kia manatokohia. Tirohia nga tautuhinga DNS me te whakaū i te rohe.",
    },
    no_currency: {
      title: "Tāpiri Moni",
      message:
        "Me tautuhi koe i tetahi moni mo to toa.",
      action: "Whakahaere Moni",
    },
    no_primary_domain: {
      title: "Ko te rohe matua",
      message:
        "Tīpakohia koa he rohe hei rohe tuatahi. Ma te panui i te paatene whetu i te taha o te ingoa rohe, ka taea e koe te kowhiri i te rohe hei rohe tuatahi o to toa. He mea nui tenei mo nga miihini rapu. Kia mahara ka watea tonu to toa ki nga rohe hohe katoa.",
    },
    shop_complete: {
      title: "Whakaotia nga korero",
      action: "Tautuhinga Toa",
      location_msg: "Tāpirihia to waahi",
      days_open_msg: "Whakaritea nga ra tuwhera",
      time_open_msg: "Whakaritea te wa tuwhera me te kati",
      description_msg: "Tuhia he korero poto mo to pakihi",
      icon_msg: "Whakaritehia to moko toa",
      shop_gateways_msg: "Tāpiri tikanga utu",
      socials_msg: "Tāpirihia o hononga hapori",
      clubs_msg: "Whakaritehia nga karapu kaihoko",
      domains_msg: "Tāpiri rohe ritenga",
      primary_msg: "Whakaritea he rohe hei tuatahi",
    },
  },

  /** @see BSupportMenu **/
  support: {
    request: "Tono tautoko",

    category: "Kāwai",
    title: "Tautoko",
    message:
      "Patai atu he patai, he patai ranei. Ma ta matou roopu tautoko koe e arahi. Ka whakaaro matou kia patai koe i to patai ki te wharangi e pa ana ki to tono. Hei tauira, mena he patai taau mo te tautuhinga rohe, tukuna mai ki te wharangi tautuhinga rohe o to toa.",

    close_at: "Katia i te {date}",
    close_ticket_by_user: "Mihi, kua whiwhi ahau i te whakautu",
  },

  /** @see SShopProductRatingView **/
  surveys: ["Tino rawakore", "Pohara", "Pūnoa", "Pai", "Tino pai"],

  /** @see SortView **/
  sort: {
    title: "Kōmaka mā",
    title_small: "Kōmaka mā..",
    nothing: "Kaore he mea",
    most_views: "Te nuinga o nga tirohanga",
    most_likes: "Tino rongonui",
    most_recent: "Hou",
    most_sell: "Te hokonga pai",
    cheapest: "Te iti rawa",
    most_expensive: "Te utu nui",
    only_available: "Kei te waatea",
    view_mode: {
      normal: "Pūnoa",
      grid: "Mātiti",
      list: "Rarangi",
      insta: "tūemi",
    },
  },

  /** {@see Helps} **/

  helps: {
    title: "Selldone Pokapū Awhina",
    subtitle:
      "Nau mai ki a Selldone. Mena kei te hanga koe i tetahi toa ipurangi hou i runga i te OS pakihi, ka kitea etahi atu korero i konei.",

    add_new_help: "Awhina Hou",
    resources: "Rauemi",

    categories: {
      Welcome: "Tīmatahia",
      Setup: "toa",
      Sell: "Hokohoko & Hokohoko",
      Developer: "Whakawhanake",

      Product: "Nga Hua me nga waahanga Whakahaere",
      Blog: "tūemi & tūemi",
      Order: "Te Poka Whakaoti Ota",
      Channel: "Nga Hoera Hoko",
      Accounting: "Kaute me nga tikanga utu",
      Incentives: "Nga whakatenatena",
      Marketing: "Hokohoko & Whakatairanga",
      Logistic: "tūemi & Te tuku",
      Customers: "Karapu Kiritaki me te Wehenga",
      Page: "Taunga me nga Wharangi Ritenga",
      Access: "Kaimahi / API Whakahaere Uru",
      Application: "Nga tono",
      POS: "POS",
    },
    faq: "FAQ",
    github: "GitHub tūemi",
    admin_api_app: "Ratonga paetukutuku API Kaiwhakahaere",
    shop_api_app: "Hoko ratonga paetukutuku API",
    design_kit: "Nga kete hoahoa",
    community: "Hapori",
  },

  /** {@see Blogs} **/
  /** {@see InterestTopicsView} **/

  blogs: {
    title: "tūemi",
    subtitle:
      "Nga akoranga mo te pakihi, te maia me te matauranga e hiahia ana koe ki te hanga pakihi ora.",
    add_new_blog: "Blog Hou",
    popular: "rongonui",
    topics: "Kaupapa",
    select_topics: "Tohua to mahi.",
    select_job_topics: "Tīpakohia ō kaupapa e pā ana ki te mahi",
    select_interested_topics: "He aha nga kaupapa e pirangi ana koe?",
  },

  /** {@see ApiDocs} **/
  api: {
    title: "API & Whakauru",
    message:
      "Ko tetahi o nga ahuatanga matua o Selldone ko te whakarato i nga API pakari me te haumaru ki te whakahaere me te hono atu i nga toa ki etahi atu ratonga penei i te whare putunga, kaute, me etahi atu. Ma te kore e awangawanga mo te whakahaere o te tūmau me te raraunga. Whakamahia noa nga API rohe. Ka whiwhi koe i nga mahi katoa mo te whakahaere i nga kaiwhakamahi, te motuhēhēnga me etahi atu waahanga rohe.",
  },

  /** {@see BPageShuttle} **/
  user_dashboard: {
    title: "Taku waka waka",
    subtitle: "Whakahaerehia to pakihi me nga korero hou me te tirohanga marama.",
    menu: {
      affiliate: "Hononga",
      monetize: "Whakamoni",
      gift: "Koha",
      security: "Haumarutanga",
      access: "Urunga",
      business: "Kamupene",
      wallet: "Pukoro",
      identification: "KYC",
      stores: "Toa",
      roles: "Ngā tūranga",
      home: "Kainga",
      expert: "Tohunga",
      deals: "Nga mahi",
      providers: "Kaituku",
    },
    shops: {
      create_new: "Waihanga toa hou",
      create_new_sub_need_initial_charge:
        "Ko te hanga i te toa tuatahi he kore utu. He nui pea te toa kotahi mo to pakihi, engari ki te hiahia koe ki te hanga toa ano, ko te utu mo te hanga toa hou he 25$. Ka tangohia e matou te moni mai i to putea putea.",

      menu: {
        title: "Aku Toa",
        shops: "Nga toa",
        new: "Hou",
      },

      accept_invite_question: "Ka whakaae koe ki tenei powhiri?",
      /** {@see BShopCard} **/

      shop_card: {
        menu_orders: {
          physical: "Nga ota hua tinana",
          virtual: "Ota hua mariko",
          files: "Nga ota kōnae",
          services: "Nga ota ratonga",
          subscriptions: "Nga ota ohaurunga",
        },
        status: "Tūnga",
        license: "Raihana",
        volume: "Te waahi kua whakamahia",
        expire: "Kua pau te Raihana",
        products: "Hua",
        page_views: "Tirohanga",
        user_behavior: "Te whanonga kaihoko",
        time_span: "mai i tenei wa ki tenei wa {time}",
        finance_report: "Pūrongo pūtea",
        buys: "Nga hokonga",
        payments: "Nga utu",
        orders: "Nga ota",
        currency: "Moni",
        no_payment: "kahore 💳 Utu!",
        no_purchase: "kahore Hokonga!",
      },
    },
  },

  widgets: {
    /** {@see BDashboardFeedback} **/
    overview: {
      title: "Nga urupare a nga kaihoko",
      title_small: "Tirohanga",
      wishlist: "Rarangi Hiahia",
      comments: "Nga korero",
      likes: "Ka pai",
      favorites: "Makau",
      powers: "Pakipaki",
      views: "Tirohanga",
      report_today: "I tenei ra te ripoata",
      report_last7days: "Ripoata 7 ra whakamutunga",
      report_last30days: "Purongo 30 ra whakamutunga",
    },
  },

  /** {@see BPageShop} **/
  admin_shop: {
    title: "toa",
    menu: {
      edit: "Tautuhinga",
      automation: "Aunoatanga",
      application: "Nga tono",
      access: "Urunga",
      pages: "Nga wharangi",
      channels: "Nga hongere",
      classification: "tūemi",
      marketing: "Te hokohoko",
      customers: "Kaihoko",
      logistic: "tūemi",
      auditing: "Kaute",
      incentives: "Nga whakatenatena",
      process_center: "Nga ota",
      blog: "tūemi",
      products: "Hua",
      shops: "Nga toa",
      dashboard: "Papatohu",
      autopilot: "KaiurungiAunoa",
      apps: "Taupānga",
      pos: "POS",
      seo: "SEO",
      localization: "Whakatakinga",
      crm: "CRM",
      listing: "rārangi",
    },

    /** {@see BPageShopDashboard} **/
    dashboard: {
      title: "Papatohu toa",
      online_staff: "(ng) kaimahi ipurangi",

      notify_referral: {
        message: "Tukuna to hononga powhiri ki o hoa ka whiwhi putea kore utu.",
        action: "Taku hononga tuku",
      },

      /** {@see BDashboardShopTechnical} **/
      info: {
        title: "Hangarau",
        subtitle: "Tirohanga Hangarau mo to OS pakihi.",

        shop_name: "Ingoa toa",
        shop_link: "Hononga toa",
        site_map: "Mahere pae",
        shop_domains: "Nga waahi",
        shop_domains_msg: "Rarangi rohe ritenga hono.",

        home_msg:
          "Tautuhia te whārangi kāinga taunoa o to paetukutuku. Ka taea te huri mo ia rohe ritenga.",

        add_domain: "Tāpiri rohe hōu",
        currencies: "Moni kaha",
        currencies_alert: "Tīpakohia kia kotahi moni mo te toa!",
        status: "Tūnga",
        license: "Raihana",
        expire: "Te ra paunga raihana",
        upgrade_license_action: "Whakapai ake ki {name}",
        wage: "Utu Fee",
        cpu_message:
          "Te maha o nga rauemi kua whakangaohia e koe ki te mahere kua hokona me nga rauemi ka taapirihia e koe ki to ratonga.",
        hide_domains: "Huna aku rohe",
        show_domains: "Whakaatuhia aku rohe",

        /** {@see BDomainsList} **/
        table: {
          primary: "Paraimere",
          url: "Toa – Papatohu – Nga korero – tūemi",
          site_map: "Mahere pae",
          enable: "Whakahohe",
          indexed: "Taurangitia",
          and_more: "& atu..",
          auto_repair: "Whakatika aunoa",
        },
        add_dialog: {
          title: "Whirihorahia te Rohe Toa",
          step1:
            "Ka taea e koe te tautuhi i te tepe IP o te rekoata A mo te rohe DNS me te roheroto mai i nga uara e whai ake nei:",
          step2: "Tāpirihia he rekoata DNS TXT:",
          set_ip_alert:
            "Tautuhia te uara IP o te rekoata A <b>{ip}</b> . CDN",

          input_domain: "URL rohe",
          correct: "He tika nga tautuhinga rohe.",
          set_message: "Tautuhi CNAME o tenei rohe ki {cname}.",
          input_domain_message: "Whakauruhia te ingoa rohe kaore he http me te https.",
          ssl_message:
            "Kaua e manukanuka mo te tiwhikete SSL mo to paetukutuku. Ka tukuna aunoatia tenei tiwhikete mo to rohe e Selldone (beta). Cloudflare CDN",
          active: "Kei te hohe te rohe",
          inactive: "Kua monoa te rohe",
          success_message:
            "Tena koe, kua whakahohea to pae ki {domain}. Ka taea e koe te whakauru i to pakihi me to waitohu ki te ao ma tenei rohe.",

          delete_action: "Mukua rohe?",
        },
        set_home_dialog: {
          title: "Tautuhia te whārangi kāinga rohe",
        },
        notification: {
          add_message: "I tutuki pai te taapiri rohe.",
          update_message: "I pai te whakahou i te rohe.",
          delete_domain_message: "I pai te whakakore i te rohe.",
        },
        alert: {
          title: "Mukua Rohe",
          message:
            "Me muku tenei rohe? Kia mahara ki te tautuhi ano i nga uara rekoata DNS o to rohe kia kore ai etahi atu e tinihanga.",
        },
      },

      /** {@see BDashboardShopOrders} **/
      active_orders: {
        title: "Nga ota",
        title_small:
          "Tikanga-a-tinana, Kōnae, Mariko me nga Whakatau Ratonga",
        pending_orders: "Tarewa ota",
        physical: "Nga ota hua tinana",
        virtual: "Ota hua mariko",
        file: "Kōnae ota hua",
        service: "Nga tono hua ratonga",
        subscription: "Ota ohaurunga",
      },
      /** {@see BDashboardShopProducts} **/
      products: {
        title: "tūemi",
        title_small: "Hua, Ratonga, Kōnae",

        total_products: "Nga momo hua katoa",
      },
      /** {@see BDashboardShopFinance} **/
      finance: {
        title: "Pūtea",
        title_small: "Pūrongo pūtea",
        total_30days_title: "Tapeke moni hoko",
        last_30_day: "30 ra whakamutunga",
        pay_amount_chart_title: "Nga utu",
        sell_amount_chart_title: "Hokonga",
        discount_amount_chart_title: "Te utu",
        currency_convert_to: "Ki te moni",
        active_currencies: "Moni kaha",
        sell_count_chart_title: "Te maha o nga hoko ma te rereke",
        active_gateways: "Te tomokanga utu kaha",
        received_last_24h: "Kua riro mai i nga haora 7 whakamutunga",
        pie: {
          title: "Te tauwehenga hoko o tenei ra",
          no_data: "Karekau he hoko i tenei ra",
        },
        pay_amount: "Te Moni Utu",
        sell_amount: "Te moni hoko",
        discount_amount: "Te moni whakaheke",
        sell_count: "Tatau hoko",
        table: {
          currency: "Moni",
          total_sell_30days: "30 ra katoa nga hoko",
          sell_daily: "Reiti Hoko o ia ra",
          sell: "Hokonga",
          pay: "Utu",
        },
      },
      /** {@see BDashboardShopCustomers} **/
      customers: {
        title: "Nga kaihoko hokohoko",
        title_small: "Pūrongo hurihanga",
        total_customers: "Tapeke",
        new_customers_today: "I tenei ra",
        new_customers_week: "7 ra whakamutunga",
        orders_count: "Ka tatau nga ota",
        days_range: "Awhe Ra",
        new_users: "Nga kaiwhakamahi rehita hou",

        chart: {
          new_customers: "Nga kaihoko hou",
          customers: "Katoa nga kaihoko",
        },
      },
      /** {@see BDashboardShopChannels} **/

      channels: {
        title: "Hongere whiwhi kiritaki",
        subtitle:
          "Kei te tipu haere to pakihi. Anei te purongo o to hongere hoko kaihoko.",
      },

      /** {@see BDashboardShopPerformance} **/
      performance: {
        title: "I tenei ra",
        title_small: "Nga kaiwhakamahi me te tirohanga hokohoko",
        users: "Kaiwhakamahi",
        views: "Tirohanga",
        orders: "Nga ota",
        payments: "Nga utu",
        no_visitor_today: "Karekau he manuhiri i tenei ra.",
      },
    },

    /** {@see BPageShopProductsList} **/
    products: {
      title: "Rarangi hua",
      products_list: "Hoki ki te rarangi",
      products_menu: "Te whakahaere hua",

      view_que_list: "Tirohia te rarangi rarangi",
      menu_products_list: "Rarangi hua",
      menu_export: "Kaweake hua",
      menu_import: "Kawemai nga hua",
      drop_import_message:
        "Whakatakahia te konae rarangi hua (CSV, XLS, XLSX te whakatakotoranga) ki konei kia timata te taapiri hua nui. Me whakarite kei te konae te whakatakotoranga rarangi hua Selldone.",

      menu_advanced_options: "Kōwhiringa matatau",
      menu_advanced_options_description:
        "Connect Google tūemi, Tango tūemi Hua ka Ētahi atu.....",

      products_rss: {
        title: "Hua RSS",
        subtitle: "tūemi Hua tūemi Google, Meta,...",
      },
      products_api: {
        title: "Hua Kī API",
        subtitle: "Te kawemai i nga hua ma te waea API ka taea e te iwi whanui.",
      },
      google_sheet: {
        action: "Rau Hononga",
        tooltip:
          "Ko nga hua o tenei toa e hono ana ki tetahi Rau Google kei Google Drive.",
      },
      show_deletes: {
        false_title: "Huna i ngā mea kua mukua",
        false_description: "huna Kua mukua Hua.",
        true_title: "whakaatu Kua mukua",
        true_description: "whakaatu Kua mukua Hua.",
      },
      show_vendors: {
        false_title: "huna kaihoko",
        false_description: "tūemi whakaatu kaihoko Rangatira.",
        true_title: "Whakaatu kaihoko",
        true_description: "whakaatu katoa kaihoko, ⌘Ctrl+1",
      },
      table_mode: {
        false_title: "Aratau tō me te tuku",
        false_description:
          "Tirohanga Hua tūemi mā Ngā Kāwai ka tūemi.",
        true_title: "Aratau ripanga",
        true_description: "Tirohanga Hua i he tūemi ripanga tūemi.",
      },

      show_notes: {
        false_title: "huna tūemi",
        false_description: "Hunahia nga tuhipoka kau.",
        true_title: "whakaatu tūemi",
        true_description: "whakaatu katoa tūemi, ⌘Ctrl+2",
      },

      rss_dialog: {
        title: "Hua RSS",
        subtitle:
          "Whakamahia tenei whāngai RSS hei pupuri i te rarangi hou o nga hua katoa.",
      },
      api_dialog: {
        title: "Hua Kī API",
        subtitle:
          "Whakamahia tenei whangai API kia mau tonu to rarangi hua. Ka whakaratohia he API mo te iwi whanui ki te tiki hua mai i to toa, he pai mo nga raarangi hua me nga paetukutuku whakataurite.",
      },

      /** {@see BProductWindowProductLarge} **/

      product_widget: {
        product_type: "Momo hua",
        price: "utu",
        value_added: "Komihana",
        discount: "Te utu",
        in_stock: "Kei roto i nga taonga",
        product_unit: "Waeine",
        total_sell: "Tapeke Hokonga",
        total_view: "Katoa nga tirohanga",
        view_unit: "Nga taima",
        review_unit: "Nga arotake",
      },

      /** {@see WidgetFolderCard} **/
      folder_widget: {
        products: "Hua",
        products_unit: "Nga momo",
        sub_categories: "Ngā Kāwai",
        sub_categories_unit: "Nga taonga",
        views: "Tirohanga",
        views_unit: "Nga taima",
      },
    },

    /** {@see BPageShopBlogs}  **/
    blogs: {
      new: "Tuhia hou",
      blogs: "tūemi",

      /** {@see BPageShopBlogsList} **/
      list: {
        title: "Aku Tuhinga",
        subtitle:
          "Tuhia nga tuhinga me tetahi etita raina hou hei whakaatu i a koe me to pakihi. Ko te ihirangi taketake ko te mea tuatahi e hiahia ana koe ki te aro ki te wa e tuhi ana koe i tetahi panui rangitaki-hoahoa SEO-kounga teitei.",
        waiting_message: "Kei te whiwhi korero ... tatari koa",
        published: "Whakaputaina",
        draft: "Hukarere",
        private: "Tūmataiti",
        public: "Tūmatanui",
        table: {
          title: "Taitara",
          author: "Kaituhi",
          claps: "Pakipaki",
          like: "Pērā",
          comments: "Nga korero",
          views: "Tirohanga",
          publish_state: "Tūnga",
          created_at: "Te Ra Hanga",
          updated_at: "Whakahoutanga whakamutunga",
        },
      },
    },

    /** {@see ShopPBPageShopOrdersrocessCenter} **/
    orders: {
      /** {@see BProcessCenterList} **/
      physical: {
        waiting_message: "Kei te whiwhi korero ... tatari koa",
        rejected: "Kua whakakorehia",
        delivery: "Te tuku",
        discount: "Te utu",
        not_paid: "Kaore i utua",

        table: {
          order_number: "Waehere ota",
          order_date: "Te Ra Takutai",
          status: "Te mana whakamutunga",
          price: "Tapeke moni",
          payment_status: "Te mana utu",
        },
      },

      /** {@see BPageShopOrdersVirtual} **/
      virtual: {
        waiting_message: "Kei te whiwhi korero ... tatari koa",

        discount: "Te utu",
        table: {
          order_number: "Waehere ota",
          product: "Hua",
          order_date: "Te ra rehitatanga",
          status: "Te mana whakamutunga",
          price: "Tapeke moni",
          payment_status: "Te mana utu",
        },
      },
    },

    /** {@see BPageShopFinance} **/
    auditing: {
      menu: {
        exchange: "Whakawhitiwhiti",
        gateway: "Te kuwaha",
        invoice: "Te nama",
        report: "Ripoata",
        tax: "Taake",
        drop_shipping: "Nga rīhiti kaihoko",
        reseller_accounts: "Nga kaute kaihoko",
        valuations: "Nga uaratanga",
        customer_wallets: "Pūkoro kiritaki",
      },

      /** {@see BPageShopFinanceBill} **/
      invoice: {
        title: "Pukoro Hononga Hoko",
        subtitle:
          "Ko nga utu, komihana me nga utu toa ka tangohia mai i enei putea putea. Fee",

        title_add: "Honoa he putea putea hou",
        subtitle_add:
          "Me hanga e koe he putea hou mo ia moni ka hono atu ki to toa. Ka tangohia te utu ratonga mai i tenei putea.",
        add_account: "Tāpiri Pūkete",
        charge_account: "Kaute Utu",
        show_transaction: "Whakaatuhia nga whakawhitinga",

        table: {
          currency: "Moni",
          account_number: "Tau Kaute",
          account_name: "Ingoa pūkete",
          balance: "Taurite",
          free_balance: "toenga wātea",
          actions: "Tango",
        },
        add_account_dialog: {
          title: "Te hono i te putea nama ki te rokiroki",
          subtitle:
            "Ina whakatuwhera koe i tetahi putea putea me Selldone, ka hoatu e matou he kaari nama mariko me te kaute pera me etahi atu kaari nama. Ka tangohia nga utu hoko a Selldone mai i tenei kaute, engari ki te kore e ranea to toenga kaute, ka namaa tenei moni. Ka taea e koe te whakamahi i nga ratonga Selldone kia rite ki taau e hiahia ana.",
          account_number: "Tau Kaute",
          account_number_input_message:
            "Ka hono te toa ki tenei kaute. Ka whakamahia tenei kaute mo nga utu me nga utu.",
          balance: "Putea Putea",
          connect_account_to_shop: "Hono Pūkete ki te Toa",
        },
        notifications: {
          connect_account_success: "He pai te taapiri i te kaute.",
          delete_account_success:
            "I pai te tango i te putea mai i te toa.",
        },
        alert: {
          delete_account: "Momotu pūkete",
          delete_account_message:
            "Me tango tenei kaute mai i te rarangi kaute toa?",
          delete_account_action: "Muku Pūkete",
        },
        inform_charge_account: "Utuhia to putea!",
        inform_charge_account_critical:
          "Utuhia to putea hei aukati i te whakatārewatanga o nga ratonga!",
      },

      /** {@see BPageShopFinanceExchange} **/
      exchange: {
        title: "Rokiroki Moni Hohe",
        subtitle:
          "Ka taea e nga kaiwhakamahi te kowhiri i tetahi o nga moni kaha me nga utu o nga hua ka tatauhia ka whakaatuhia. Ko te kaihoko ki te whakauru i te utu whakawhiti.",
        shop_currencies_input: "Moni kaha mo nga kaiwhakamahi",
        shop_currencies_input_error: "Tīpakohia kia kotahi te moni",
        rates: {
          title: "Reiti Whakawhiti Moni",
          sub_title:
            "Ka taea e koe te whakauru i nga utu hua i runga i tetahi o nga moni kei raro iho nei ka tatau aunoa nga utu hua ki nga reiti whakawhiti i waenga i o moni.",
          message:
            "Ma te whakauru i nga reiti whakawhiti i waenga i nga moni, ka tatauhia nga utu o o hua ka whakahoutia.",
          add_new_rate: "Tāpiri reiti hou",
        },
        table: {
          waiting_receive_data: "Kei te whiwhi korero ... tatari koa",
          rate_input: "Te utu whakawhiti",
          from: "Mai",
          to: "Ki",
          exchange_rate: "Te utu whakawhiti",
          edit: "Whakatikatika",
          delete: "Mukua",
          auto: "Reiti Aunoa",
        },
        add_rate_dialog: {
          title: "Tautuhi Reiti Whakawhiti",
          message:
            "Kia tupato ki te whakauru korero. Ko nga utu o o hua ka tatauhia i runga i nga reiti kua whakauruhia ki konei.",
          currency_from: "Mai i te moni",
          currency_to: "Ki te moni",
          exchange_rate: "Te utu whakawhiti",
        },
        alert: {
          title: "Muku Reiti Tahuri",
          message: "Kei te hiahia koe ki te muku i tenei reeti tahuri?",
          action: "Mukua",
        },
        notifications: {
          delete_success: "Kua tangohia te utu whakawhiti.",
          add_success: "Ko te utu whakawhiti kua whakahoutia.",
          update_success: "Rokiroki moni hohe kua whakahoutia.",
        },
      },
    }, // End of auditing

    /** {@see BPageShopLogistic} **/
    logistic: {
      /** {@see BPageShopLogisticWarehouse} **/
      warehouses: {
        title: "Whare putunga",
        subtitle: "Rarangi o aku whare putunga.",
      },
      restriction: {
        title: "Te tuku tūemi",
        subtitle:
          "Ka taea e koe te whakatinana i nga here hei whakarite ko nga kaihoko anake i roto i nga waahi kaipuke kua tohua ka taea te hoko mai i a koe!",
        no_enable_shipping_error:
          "Kaore e taea e tetahi te hoko mai i a koe na te mea karekau koe e taea te tikanga tuku!",
        customer_must_select_a_shipping_msg:
          "Me kowhiria e te Kaihoko tetahi tikanga tuku hei tuku ota.",
        customer_can_order_without_shipping_method_msg:
          "Ka taea e te kaihoko te ota ahakoa karekau he huarahi tuku e tautokohia ana mo te waahi kua tohua.",
        inputs: {
          restriction: {
            false_description:
              "Ka taea tonu e nga kaihoko te tuku ota ahakoa karekau he tikanga tuku motuhake i tohua.",
            false_title: "Whakaae katoa Nga ota",
            true_title: "tūemi Whakaae 🛍️ Ota",
            true_description:
              "Me whiriwhiri nga kaihoko i tetahi waahanga tuku, tiki ranei hei whakaoti i o raatau hoko.",
          },
        },
      },

      warehouse_btn: "Tāpirihia he whare putunga hou",
      warehouse_btn_error:
        "Tautuhia to whare putunga, toa, takenga mai ranei o to pakihi. He mea nui ki te whai i roto i te tatauranga utu kaipuke me te mahi tirotiro.",
      no_multi_warehouse_support_message:
        "Mo te ngawari, kaore tenei waahanga i te waatea i nga toa noa.",
      size_unit: {
        title: "Waeine rahi",
        subtitle: "Ko te waeine e whakamahia ana mo te roa, te whanui me te teitei.",
      },
      mass_unit: {
        title: "Waeine papatipu",
        subtitle: "Ko te waeine i whakamahia mo te taumaha.",
      },
      distance_unit: {
        title: "tūemi Waeine",
        subtitle: "te Waeine Kua whakamahia mō tūemi.",
      },

      menu: {
        return_request: "Kua hoki mai",
        delivery: "Te tuku",
        inventory: "tūemi",
        warehouse: "Whare putunga",
        logistic_apps: "tūemi",
        profiles: "Kōtaha",
        includes: "Kei roto",
        property_sets: "Nga huinga taonga",
        boxes: "tūemi",
      },

      /** {@see BPageShopLogisticInventory} **/
      warehouse: {
        title: "Whare Putunga Toa",
        sub_title: "Ka whakamahia tenei wahitau hei takenga mai o te kaipuke.",
        table: {
          title: "rārangi rārangi",
          sub_title:
            "Rarangi o nga hua toa katoa tae atu ki te tinana, mariko, konae me te ratonga",
        },
      },

      /** {@see BPageShopLogisticShipping} **/
      delivery: {
        title: "Nga tikanga tuku",

        sub_title:
          "Ma te whakarite i nga uara ki tenei waahanga, he maha nga tikanga tuku ka whakahohehia i to toa. Ka taea hoki te whakahohe i te tikanga tuku moni i tenei waahanga.",
        add_transportation: "Tāpiri waka",

        /** {@see DeliveryFormWidget} **/
        delivery_form: {
          payment: "Kōwhiringa utu",
          limits: "Nga rohe",

          currency_input: "Moni tohutoro mo nga tatauranga utu",
          currency_input_message: "Ka tatauhia nga utu ki tenei moni",
          distance_input: "Ko te tawhiti mai i te whare putunga",
          distance_input_message: "0 ranei -1 te tikanga kaore he rohe.",
          max_weight_input: "Te taumaha mōkihi e whakaaetia ana",
          max_weight_input_message: "0 ranei -1 te tikanga kaore he rohe.",
          max_size_input: "Mōrahi Ahu Mōkī",
          width: "Whānui",
          length: "Te roa",
          height: "Teitei",
          days_input: "Nga ra tuku e waatea ana",
          days_input_nodata: "Tohua te ra tuku",
          hours_input: "Nga wa tuku e waatea ana",
          hours_input_nodata: "Tīpakohia he wā wā",
          vacation_input: "Ka tuku koe i nga hararei?",

          pricing: "Tātaihia te utu",
          const_input: "Utu tuturu",
          distance_cof_input: "Utu-ki-tawhiti whakarea",
          distance_cof_input_dim: "tūemi {dim}",
          weight_cof_input: "Tauwehe utu-ki-taimaha",
          weight_cof_input_dim: "tūemi {dim}",
          price_cof_input: "Utu tuku ki te take utu hua",
          price_cof_input_message:
            "Kei roto ko nga utu inihua me etahi atu ratonga penei i nga utu COD.",
          price_cof_input_dim: "tūemi {dim}",
          distance_weight_cof_input: "Whakarea utu ki te whakarea taumaha",
          distance_weight_cof_input_dim: "Mo ia {weight} mo ia {distance}",
          free_shipping_input: "He utu kore utu",
          free_shipping_limit_input: "Utu tono iti mo te tuku kore utu",

          shipping_cost_calculation: "Te tatauranga utu kaipuke",
          sod_true: "Tikina te utu kaipuke mo te haerenga",
          sod_true_message:
            "Me mohio kei te whakaae to ratonga kaipuke ki te utu kaipuke mo te haerenga.",
          sod_false: "Te tatauranga utu kaipuke aunoa",

          cod_input: "Tautoko COD",
          cod_true: "Whakaaehia te moni i runga i te tuku",
          cod_false: "Whakaae ki te utu ipurangi anake",
          cod_true_message: "Kia mohio kei te whakaae to ratonga kaipuke ki te COD.",

          notifications: {
            update_success: "I tutuki pai nga tautuhinga ratonga tuku.",
          },
        },
      },
    }, // End of logistic

    /** {@see BPageShopCrm} **/
    users: {
      menu: {
        customers: "Kaihoko",
        contact_form: "Hoapā",
        faq: "FAQ",
        club: "Karapu",
        report: "Ripoata",
      },

      /** {@see BPageShopCrmReport} **/
      report: {
        title: "Tirohanga Kiritaki",
        sub_title: "Ka kite koe i te mana ora o o kaihoko i konei.",
        report_30days_by_time:
          "Nga mahi a te kaiwhakamahi i runga i te ra me te wa i roto i nga ra 30 kua hipa",
        report_30days_by_location:
          "Nga mahi a nga kaiwhakamahi Geolocation i nga ra 30 kua hipa",
      },
      /** {@see BPageShopCrmClub} **/
      club: {
        title: "Karapu Kiritaki",
        sub_title:
          "Whakauruhia te moni hoko ia marama me ia tau me te moni hei whakahohe i ia taumata. Ka uru atu o kaihoko ki to karapu kaihoko i a raatau e tutuki ana i nga whakaritenga o te hoko ia marama, ia tau ranei.",
        item: {
          members_count: "Te maha o nga mema",
          purchases_count: "Te maha o nga ota",
          monthly: "Ia marama",
          annually: "Ia tau",
          discount_percent: "Te utu utu",
          discount_limit: "Tepe utu",
        },

        edit_dialog: {
          title: "Whakahaere Taumata Kiritaki",
          currency_input: "Moni",
          currency_input_message:
            "Ka tatauhia nga hoko katoa i roto i tenei moni.",
          free_shipping_input: "He utu kore utu",
          monthly_input: "Te utu hoko ia marama",
          annually_input: "Reiti hoko tau",
          discount_percent_input: "Te utu utu mo nga mema o te karapu",
          discount_limit_input: "Tepe utu",
          description_input: "Whakaahuatanga",
          description_input_message: "Whakauruhia he korero poto mo tenei taumata.",
        },
        notifications: {
          update_success: "Ko nga huringa i tukuna ki te karapu kaihoko.",
        },
      },
      /** {@see BPageShopCrmFaq} **/
      faq: {
        title: "Pātai Auau",
        sub_title: "Tāpirihia, whakatika, whakautuhia nga patai a o kaihoko.",
      },
    }, // End of users

    /** {@see BPageShopMarketing} **/
    marketing: {
      menu: {
        campaigns: "Nga Whakatairanga",
        cross_selling: "Ripeka hoko",
        affiliate: "Hononga",
        emails: "Nga imeera",
        funnel: "Kōrere",
      },

      /** {@see BPageShopMarketingCampaign} **/
      campaign: {
        name_input_name: "Ingoa Campaign",
        name_input_placeholder: "Ingoa Campaign",
        create_campaign_action: "Waihanga kaupapa hou",
        table: {
          title: "Taitara",
          state: "Tūnga",
          discount: "Te utu",
          landing: "Whārangi Taunga",
          start: "Tīmata",
          end: "Whakamutunga",
        },
        notifications: {
          add_success_title: "Waihangahia he Pakanga",
          add_success: "I hangaia to kaupapa.",
        },
      },
    }, // End of marketing

    /** {@see BPageShopClassification} **/
    categories: {
      title: "Rārangi Kāwai",
      subtitle:
        "Ko nga waahanga te waahanga nui o to toa ipurangi. Ka akiaki matou ki a koe ki te whakamahi i enei taputapu whakarōpū kaha ki te hanga i tetahi toa hou me te ngawari ki o kaihoko ki te rapu hua tika.",

      menu: {
        add_new: "Tāpiri kāwai hōu",
        categories_list: "Rārangi kāwai",
        export: "Kaweake kāwai",
        import: "Kawemai kāwai (CSV)",
      },
      /** {@see categoriesGraph} **/
      graph: {
        title: "Arataki o nga waahanga",
        subtitle:
          "Ka taea e koe te tino tirohanga mo o waahanga toa me te huarahi whakatere a nga kaiwhakamahi. Ma te whakaritenga tika o nga waahanga, ka taea e o kaihoko te rapu tere i nga hua ka piki ake te tere o te huringa.",
        visits_count: "Haerenga",
        normal: "Maamaa",
        items_count: "Te maha o nga mea",
        home: "Kainga",
      },
    }, // End of categories
    /** {@see AdminShopApps} **/
    apps: {
      menu: {
        app_store: "toa",
        my_apps: "Taupānga",
      },
      /** {@see BPageShopSettingApplications} **/
      my_apps: {
        title: "Rokiroki Taupānga Hohe",
        subtitle:
          "Tirohia te ngawari o te whakahaere i o tono pakihi me Selldone App Store. Ka tukuna e to maatau papa he huinga taputapu matawhānui i hangaia hei arotau i o mahi pakihi. Mai i nga otinga whakahaere auaha ki nga tono whakakaha hua, ka whakawhiwhia e Selldone he wheako pai mo nga pakihi e rapu ana i te pai me te tipu.",
        all: "katoa",
      },
      /** {@see BAppShopStore} **/
      store: {
        title: "Toa taupānga",
        subtitle:
          "Honoa o ratonga tino pai ki to pakihi. Ka taapirihia e matou nga taapiri me nga taupānga i runga i nga tono a o taatau kaihoko.",

        nodata: "Karekau he mono i kitea.",
        all: "katoa",
        more: "atu",
      },
    }, // End of apps
    /** {@see BPageShopPages} **/
    pages: {
      menu: {
        new_page: "Whārangi Hou",
        contact_us: "Whakapā mai",
        privacy: "Tūmataitinga",
        terms: "Nga tikanga",
        about_us: "Mo Tatou",
        landings: "Taunga",
        popup: "Pahūake",
        static: "Pahekeheke",
      },
    }, // End of pages

    /** {@see BPageShopPermission} **/
    permissions: {
      menu: {
        persons: "Kaimahi",
        roles: "Ngā tūranga",
        api: "API",
      },
      /** {@see BPageShopPermissionStaff} **/
      shop_permissions: {
        menu: {
          users: "Rarangi Kaimahi",
          new_user: "Tāpiri Kaimahi Hou",
          new_user_message:
            "Ko nga kaiwhakamahi ka uru ki te papatohu me nga waahanga whakahaere o tenei toa.",
        },
        delete_alert: {
          title: "Tango uru",
          message: "Me muku tenei kaiwhakamahi?",
          action: "Mukua",
        },
        notifications: {
          add_success_title: "Tāpiri Uru",
          add_success: "Kua taapirihia te urunga hou.",
          delete_success_title: "Mukua te uru",
          delete_success: "Kua mukua te uru.",
        },
      },

      /** {@see BPageShopPermissionApi} **/
      shop_api: {
        title: "Rokiroki Nga Kī Uru",
        message:
          "Ka whakamahia enei taviri ki te hanga i te Tohu Whakauru e to kaimau. Kia maumahara he matatapu nga ki, kei runga noa i te tūmau i runga i te huarahi kaore e kitea e etahi atu.",
      },
    }, // End of permissions
    /** {@see BPageShopApplications} **/
    applications: {
      /** {@see BPageShopApplicationsPwa} **/
      pwa: {
        title: "Taupānga Tukutuku",
        sub_title: "Tirohia to taupānga paetukutuku toa i konei",
        message:
          "Tuhipoka: Ka taea e koe te whakatika i to wharangi powhiri paetukutuku i runga i to iPhone me te iPad i roto i te taupānga iOS",
        ios_app: "iOS taupānga",
      },

      /** {@see BPageShopApplicationsIos} **/
      ios: {
        title: "iOS taupānga",
        sub_title: "Kaore e roa ka taea e koe te hanga i tetahi taupānga iOS mo to toa.",
        message:
          "Te rahi o te konae: 512 KB, ka taea e koe te tuku ake i nga konae ki te 1MB te rahi, mena ka taea e to maatau punaha te whakaheke te rahi ki te 512KB. Ko te atahanga kua tuku ake ka ata whakarereketia te rahi.",
      },
      /** {@see BPageShopApplicationsAndroid} **/
      android: {
        title: "Taupānga Android",
        sub_title: "Hangaia taau ake taupānga Android",
        message:
          "Whakakiia tenei puka ka pehi i te matua hanga. Kare he hiahia mo tetahi atu mea. Ko nga kaupae katoa o te tukanga Hanga me te Waitohu o to taupānga ka mahia ki runga i nga tuumau kapua. Ko te taupānga whakamutunga ka tukuna ki nga AppStores katoa.",
        server_un_available:
          "I roto i nga whakamatautau, kaore i te hono te tūmau tono i tenei wa.",
        app_building_progress: "Kei te hangaia to taupānga.",
        app_build_success: "Kua reri to taupānga.",
        download_app_action: "Tikina te taupānga",
        firmware_version: "Putanga Firmware",
        main_info: "Mōkī Tono",
        package_name_input_message: "Me mohio he ahurei te ingoa o te kete.",
        package_name_input_alert:
          "Mena ka huri koe i tenei ingoa, kaore e taea e koe te whakahou i o taupānga o mua ki te AppStore me te whakauru i tetahi mea hou. Kia mahara ka kore e taea e nga kaiwhakamahi taupānga putanga tawhito te takiuru ki o raatau putea.",
        version_input: "Putanga",
        version_input_message:
          "Me nui ake tenei tau i tona uara o mua.",
        version_name_input: "ingoa putanga",
        version_name_input_message:
          "Whakauruhia he ingoa ahurei mo tenei putanga o te taupānga.",
        use_selldone_key: "Selldone mau taviri taunoa",
        use_last_key: "Kī kua tiakina",
        use_new_key: "Kī hou",
        appearance: "Te ahua",
        app_name_input: "Ingoa taupānga",
        app_name_input_message: "Koinei te ingoa matua o to taupānga.",
        app_title_input: "taitara taupānga",
        app_title_input_message: "Ka puta tenei taitara ki runga o te taupānga.",
        app_title_input_placeholder: "Whakauruhia he taitara",
        app_icon_input: "Ata Taupānga",
        app_icon_input_placeholder: "Tīpakohia te kōnae kōtui o ngā ata taupānga.",
        generate_icon_pack_action: "Hanga Icon Pack",
        splash_design: "Splash Screen Desgin",
        splash_logo_input: "Moko Mata Splash",
        splash_logo_input_placeholder: "Tīpakohia he whakaahua mo te Mata Splash.",
        splash_title_input: "Taitara Mata Splash",
        splash_title_input_placeholder: "Whakauruhia he taitara mo Splash Screen",
        splash_title_input_message:
          "Ka whakaatuhia tenei taitara ki te rahi momotuhi.",
        splash_message_input: "Karere Mata Splash",
        splash_message_input_placeholder: "Karere...",
        splash_message_input_message:
          "Ka whakaatuhia tenei karere hei karakōnui i raro nei.",

        dark_theme: "Kaupapa pouri",
        light_theme: "kaupapa marama",
        build: "Hanga",
        building_info:
          "Ko te hanga tono ki runga i te tūmau kapua e 10 hēkona ki te maha meneti. Kia manawanui. Ka puta mai te hononga o te konae tikiake ki konei ina rite ana te tono.",
        update_mode_action: "Whakahou",
        build_mode_action: "Hanga taupānga",
        notifications: {
          icon_pack_not_valid_error: "Kaore i te tika te konae.",
          start_building_title: "Whare...",
          start_building_message:
            "Ko te tono mo te hanga tono Android kua tukuna.",
          build_success_title: "Tena koe",
          build_success:
            "Kua reri to taupānga Android, ka taea e koe te tango inaianei.",
        },
      },
    }, // End of applications

    /** {@see ShopAutomation} **/
    automation: {
      menu: {
        autopilot: "KaiurungiAunoa",
      },
    },

    /** {@see BPageShopSetting} **/
    edit: {
      menu: {
        delete: "Muku Toa",
        activity_time: "Te wa",
        footer: "Tahua",
        info: "Nga korero",
        notification: "Whakamōhiotanga",
        home: "Kainga",
        locations: "Nga waahi",
        license: "Raihana",
        theme: "Kaupapa",
        sms: "SMS Sms",
        email: "Īmēra",
        domain: "Rohe",
        verifications: "Manatokonga",
        flow: "Rere",
        business_model: "Tauira Pakihi",
      },
      delete_dialog: {
        title: "Muku Toa",
        message: "Me muku te Toa <b>{title}</b> ?",
        action: "Muku Toa",
      },

      notifications: {
        delete_success: "Kua mukua te toa.",
      },

      /** {@see BPageShopSettingInfo} **/
      info: {
        title_tab: {
          title: "Ingoa Pakihi me te Reo",
          subtitle: "Tautuhia he ingoa whanui me te reo taunoa.",
        },
        name_tab: {
          title: "Toa ingoa ahurei",
          subtitle: "Ko to ingoa ingoa pakihi.",
        },

        description_tab: {
          title: "Ko to korero pakihi",
          subtitle:
            "Tuhia mo to pakihi me tana kaupapa uara ahurei.",
        },
        contact_tab: {
          title: "Whakapā & Pakihi rohe",
          subtitle:
            "Nga korero whakapā a te iwi me te tatūnga pakihi a Google.",
        },
        image_tab: {
          title: "Moko",
          subtitle: "Tīpakohia he waitohu ahurei mo to waitohu.",
        },
        favicon: {
          title: "tūemi",
          subtitle:
            "Ko nga Favicons he ahua iti, he ahua rongonui e tohu ana i te paetukutuku i roto i nga ripa tirotiro, tohu tohu, me nga atanga taupānga waea.",
        },
      },

      /** {@see ShopEditMenuFooter} **/
      badge: {
        external_title: "Whakahaerehia nga tohu o waho",
        external_message:
          "I konei, ka taea e koe te whakauru i te wahitau o te ahua tohu me te hono ki reira. Me ahu mai nga URL i tetahi paetukutuku kua whakaaetia.",
      },
      /** {@see BPageShopSettingMenus} **/
      edit_menu: {
        title: "Tahua",

        footer: {
          title: "Tahua Hiku",
          sub_title:
            "Ka taea e koe te whirihora i te tahua raro o to wharangi toa i konei. Ko enei tahua he wharangi kaainga, he wharangi hoahoa, he hononga o waho ranei.",
        },

        header: {
          title: "Tahua pane",
          sub_title:
            "Ka whakaatuhia tenei tahua ki runga i nga mata nui ake i te waea pūkoro, kei te tihi o te wharangi kaainga, nga waahanga hua, taunga, me nga wharangi hua.",
        },

        save_menu: "Tiaki Tahua",
        dialog: {
          main_pages: "Whārangi Matua",
          custom_pages: "Nga wharangi taunga",
        },
        table: {
          home: "Kainga",
          shop: "Toa",
          blogs: "tūemi",
          about_us: "Mo Tatou",
          privacy: "Tūmataitinga",
          terms: "Nga Ture Whakamahi",
          faq: "FAQ",
          contact_us: "Whakapā mai",
        },
        notifications: {
          save_success: "Kua pai te whakahou i te tahua.",
        },
      },

      /** {@see ShopSocialField} **/
      social: {
        title: "Pāpori pāpori",
        message:
          "Whakauruhia to hononga pāpāho pāpori whaimana ki tenei waahanga. Whaia nga tohutohu mo ia mea hei whakau i nga wahitau.",
      },

      /** {@see BShopSettingWorkingDays} **/
      time: {
        title: "Nga Ra Mahi",
        sub_title: "Whakaritea nga ra mahi me te wa",
        days_open_input: "Nga ra",
        days_open_input_nodata: "Tīpakohia kia kotahi te ra",
        start_time_input: "Rokiroki te wa tiimata",
        start_time_input_hint: "Nga haora tuwhera o te Toa",
        end_time_input: "Waa Katia te Toa",
        end_time_input_hint: "Rokiroki te wa mutunga mahi",
        customer_support_service: "Te kawenga me te Ratonga Kaihoko",
        response_time_input: "Tūnga Ratonga Kiritaki",
        support_24h: "24 haora 7 ra i te wiki",
        support_normal: "I nga haora tari anake",
        no_service: "kahore",
        notifications: {
          update_success: "Kua whakatikahia te toa.",
        },
      },
    }, // End of edit
  }, // End of admin_shop

  /** {@see BCategoryInput} **/
  category_select: {
    category_input_no_category: "Kore he waahanga",
  },

  /** {@see BProductAddFull} **/
  add_product: {
    title_new: "Hua hou",
    title_new_category: "kāwai hou",
    title_new_drop_shipping: "Taapirihia te tuku tuku",

    title_edit: "Whakatika hua",
    delete: "Mukua te hua",
    completed: "Kua oti",

    menu: {
      type: "Momo hua",
      general_info: "Nga korero whanui",
      rating: "Te rangahau me nga ahuatanga",
      spec: "Whakatakotoranga",
      images: "Pikitia",
      price: "utu",
      inputs: "Whakauru",
      outputs: "putanga",
      extra: "Nga korero taapiri",
      review: "Arotake Hua",
      inventory: "tūemi / Nga momo rereke",
      change_status: "Huri Tūnga",
    },
    menu_extra: {
      description: "Whakaahuatanga",
      form: "puka",
      no_spec: "kahore tūemi",
      has_spec: "He spec",
    },

    select_type: {
      title: "Whiriwhiria to momo hua, momo ratonga ranei",
    },

    /** {@see BProductEditInfo} **/
    edit_info: {
      category_input: "Kāwai",
      category_input_msg: "Putua: Kei te pakiaka o te toa",
      product_name: "Ingoa hua*",
      product_name_placeholder: "Ko to ingoa hua kei konei..",
      product_code: "Waehere hua (Maori)",
      sku: {
        label: "SKU",
        message: "Wae Pupuri Taonga",
      },
      mpn: {
        label: "MPN",
        message: "Tau Waahanga Kaihanga",
      },

      external_link: "Hononga o waho",

      brand_placeholder: "Waitohu Ex. Apple.. (Kōwhiringa)",

      is_original: "He hua taketake tenei?",
      is_original_message:
        "Ki te kore koe e tautuhi i to hua taketake, ka whakaatuhia te tohu rūpahu mo tenei hua.",
      return_warranty:
        "E hia nga ra kei tenei hua te taurangi whakahoki?",
      warehouse_section: "Whare Putunga me te Hangahanga",
      conditions_section: "Nga tikanga hua",
      warranty_section: "Pūtāhui me te Tukunga",

      unit_input: "Waeine hua",
      unit_message: "Whakauruhia te waahanga ki te hiahiatia. Ex: KG, Mita,...",

      unit_section: "Waeine hua",

      unit_float: "Wae maru (ka tata nei)",
      unit_float_hint:
        "Hei tauira, ka taea e te kaihoko te ota kia 3 kg hua, engari he rereke te nui o te moni ka tukuna ki te kaihoko mai i te nui o te whakahau.",
      unit_float_message:
        "I te wa o te tukatuka ota, ka rereke pea te moni kua tukuna mai i te moni kua whakahaua. Ka tātai aunoatia e te punaha tenei rereketanga ka tuhia hei nama ki te kaihoko, ka taea te tono ki nga ota e whai ake nei.",

      marketplace: {
        // Vendor panel > Fixed mode
        vendor_panel_message:
          "Ko <b>{vendor_name}</b> te rangatira o te hua.",
        // Marketplace panel > Subscription products
        subscription_message:
          "Ka taea e nga hua ohaurunga kia kotahi te kaihoko, na reira ka taea e koe te hanga i roto i te roopu kaihoko, te tautapa ranei i tetahi kaihoko ki a raatau i te waahanga hangahanga. Ko te kaihoko kua tohua ko te rangatira o te hua.",
        default_message:
          "Ka taea e koe te tohu mena no te kaihoko kotahi tenei hua, no reira kotahi anake te kaihoko ka taea te hoko, no Marketplace ranei tenei hua ka nui atu i te kotahi te kaihoko ka taea te hoko.",

        single_vendor_title: "Kaihoko kotahi",
        single_vendor_desc:
          "No tetahi kaihoko tenei hua, ka taea e te kaihoko kua tohua te whakahaere.",

        multi_vendors_title: "Nga kaihoko maha",
        multi_vendors_desc:
          "No te kainga hokohoko tenei hua, ka taea anake te whakatika i roto i te roopu maakete.",
      },

      category: {
        subtitle:
          "Whirihorahia te waahanga me te taitara ka whakaatuhia ki to rarangi ingoa.",
      },
      unit: {
        subtitle:
          "Ka taea e koe te whakawhaiaro i te whakaaturanga o te utu me nga huānga karanga-ki-mahi i runga i te wharangi hua.",
      },
      custom_pricing: {
        title: "Ritenga Te utu puka",
        subtitle:
          "Ka taea e koe te hanga, te tautapa ranei i te uara (he puka whakauru utu) ki te hua, ka taea e o kaihoko te kowhiri i waenga i nga momo rerekee, te whakarite ranei i o taonga ma te puka urunga mohio.",
        valuation_need_saved_product_message:
          "Tena koa tiakina te hua i te tuatahi, katahi ka taea e koe te whakatau i tetahi tauira utu.",
        assign_valuation_message:
          "Homai he tauira utu mo tenei hua.",
        edit_pricing_action: "Whakatikatika Te utu puka",
        add_pricing_action: "Waihanga Hou Te utu puka",
      },
      action: {
        title: "Ritenga Hoko pātene",
        message:
          "Ka taea e koe te huri i te ata ma te whai i tenei tauira : {icon name} tapanga...",
        placeholder: "tūemi P0000ātūemi...",
        multi_language_dialog_title: "Ritenga Hoko pātene tūemi",
      },
      external: {
        not_available_message:
          "He mahi taapiri e waatea ana me nga raihana Kamupene me te Hinonga.",
        available_message: "Ka taea e koe te taapiri i tetahi atu waea o waho ki te mahi.",
        input_label: "tūemi P0000ātūemi",
      },
      warehouse: {
        subtitle:
          "Ka whakamahia enei korero e te punaha whakahaere whare putunga me te mahi ki te tautuhi i te hua puta noa i te ao.",
      },
      gtin: {
        hint: "Ko nga uara e tautokohia ana ko UPC (Amerika Te Taitokerau, 12 mati), EAN (Europe, 13 mati), JAN (Hapana, 8, 13 mati ranei), ISBN (pukapuka, 13 mati).",
        label: "Te Tau Tuemi Tauhokohoko Ao o te Hua",
        placeholder: "Tau hokohoko o te ao i konei.. (Kōwhiringa)",
      },
      hsn: {
        hint: "Ko te HSN, ko te Harmonized System of Nomenclature ranei, he punaha whakarōpū paerewa mo te ao mo te whakarōpū i nga taonga me nga hua.",
        placeholder: "6~16 tūemi HSN Waehere.. (tūemi)",
        label: "HSN",
      },
      condition: {
        label: "Hua tūemi i to toa",
        message: "Tena koa whiriwhiria te ahua o tenei hua.",
        subtitle:
          "Whakamōhio atu ki o kaihoko mo te ahuatanga me te waitohu o te hua e tukuna ana e koe.",
      },
      brand: {
        label: "Waitohu",
        placeholder: "Waitohu Ex. Apple.. (Kōwhiringa)",
      },
      warranty: {
        subtitle:
          "Tautuhia te raihana me te mana hua, na te mea ka whakaatuhia enei korero mo te raihana ki te kaihoko me te awhina ki te whakarite i te waa ka taea te whakahoki mai i nga hua i muri i te hokonga.",

        label: "tūemi",
        placeholder: "koura 24 Marama tūemi.. (tūemi)",
      },
      status: {
        title: "Tūnga",
        subtitle:
          "Ko te mana o te hua ka whakatau i tana waatea i runga ipurangi me te toa. Hei hanga tauira hua, huri noa i tona mana ki te 'Inactive'..",
      },
      profiles: {
        title: "Kōtaha",
        subtitle:
          "Ka taea e koe te tautuhi i te raihana, te whakahoki kaupapa here, te tuku kaipuke, me te arahi me te tautapa ki tenei hua. Ka whakaatuhia hei ripa hou ki te wharangi hua.",
      },
      shortcuts: {
        title: "tūemi Hua Ki tūemi Ngā Kāwai",
        subtitle:
          "Ma tenei ahuatanga ka taea e koe te whakarārangi i te hua i roto i nga waahanga maha i te wa e mau tonu ana tana tuunga ki te waahanga tuatahi kua whakaritea i raro i te Hua> Whakatika> Kawai. Ko te tikanga, he rite tonu tana mahi ki te hanga 'pokatata' i roto i nga punaha whakahaere, e taea ai te whakaputa i te hua ki etahi atu waahanga kua tohua.",
      },
    },

    /** {@see BProductEditFeatures} **/
    rating_pros_cons: {
      title: "Rangahau",
      message:
        "Whakauruhia he rarangi o nga whakaritenga hua mo o kaihoko ki te whakatau i to hua. (Ki runga ki te 5 nga mea)",
      rating_input: "Pooti",
      rating_input_message: "Ka taea e koe te taapiri i nga mea e 5 ki te pooti.",
      rating_input_placeholder:
        "Tuhia to patai rangahau ritenga ki konei... ka pehi i te tomo!",

      item_unit: "tūemi",

      pros_title: "AtAKi",
      pros_message: "Whakauruhia nga ahuatanga o te hua. (Ki runga ki te 5 nga mea)",
      pros_input_title: "Taitara Whakaaturanga",
      pros_input_value: "Uara āhuatanga",
      pros_group: "Nga pai o te hua",

      cons_title: "tūemi",
      cons_message: "Whakauruhia nga ngoikoretanga o te hua. (Ki runga ki te 5 nga mea)",
      cons_input_title: "Te taitara ngoikore",
      cons_input_value: "Te uara ngoikore",
      cons_group: "Nga ngoikoretanga o te hua",

      select_from_list: "Tīpakohia mai i te rarangi",

      suggestion_rating: [
        "Te kounga pai",
        "Pai Uara",
        "Ka tūtohu",
        "Nga ahuatanga me nga kaha",
        "Te ngawari ki te whakamahi",
        "He pai te hoahoa",
        "Mahinga Whakapono",
        "Tautoko Kiritaki Whaihua",
        "Wā Tukunga Pai",
        "Whakaahuatanga Whakataurite Hua",
        "He roa me te roa",
        "Utu-Painga",
        "Whakapaipai Ataahua",
        "Hoahoa Hou",
        "Taiao",
        "Rawa Tohutohu/A-ringa",
        "Ka tutuki nga tumanakohanga",
        "Atanga ratarata-kaiwhakamahi",
        "Te Tino Pai",
        "Ratonga Whai-Hoko Nui",
        "Taumahinga Whanui",
        "Utu Whakataetae",
        "Te waatea o nga waahanga mokowhiti / taputapu",
        "Waitohu Whakapono",
        "Haumaru ki te Whakamahia",
        "Ratonga Tere me te Tika",
      ],

      notifications: {
        add_rating_success: "Ko te kōwhiringa {rating_name} whakatau-kaiwhakamahi kua tāpirihia.",
        delete_rating_success: "Kua mukua te kōwhiringa {rating_name} whakatauranga-kaiwhakamahi.",

        add_pros_success: "Kua taapirihia te waahanga hua.",
        delete_pros_success: "Kua tangohia te ahua hua.",

        add_cons_success: "Ko te ngoikore o te hua kua taapirihia.",
        delete_cons_success: "Kua tangohia te ngoikore o te hua.",
      },
    },

    /** {@see BProductEditSpec} **/
    edit_spec: {
      title: "Whakatakotoranga Hangarau",
      subtitle:
        "Whakauruhia nga whakaritenga hua ki konei. Ka whakaatuhia enei korero ki nga kaihoko ka taea hoki te whakamahi i roto i te punaha tātari mohio o te waahanga.",
      manual: {
        title: "tūemi",
        subtitle:
          "Tāpirihia ā-ringa ngā whakaritenga hua mā te tautuhi i ngā rōpū me te takirua matua-uara.",
        add_group_action: "Tāpiri Rōpū",
        add_item_action: "tāpiri tūemi Uara",
      },
      import: {
        subtitle:
          "Ka taea e koe te kawemai i nga whakaritenga hangarau o nga hua rite.",
        action_title: "tūemi",
        action_subtitle: "Kawahia nga korero mai i tetahi atu hua.",
      },
      ai: {
        title: "AI",
        subtitle:
          "Ka taea e koe te hanga aunoa i nga whakaritenga hua ma te tuku i nga raraunga mata o te hua i roto i te ahua tuhinga.",
        action_title: "Hanga Aunoa e AI",
        action_subtitle: "Waihangahia nga whakaritenga hua ma te tere tere.",
      },

      group_input_header: "Tāpiri Rōpū",
      item_title_header: "Tāpiri Tūemi",
      group_input: "Pane roopu",
      group_input_message: "Whakauruhia te taitara.",
      group_add_action: "Tāpiri Rōpū",

      item_title_input: "Taitara",
      item_title_input_message: "Whakauruhia te taitara āhuatanga.",

      item_value_input: "Uara",
      item_value_input_message: "Whakauruhia te uara ahuatanga.",

      add_item_action: "Tāpiri Tūemi",

      dialog: {
        title: "Whakauruhia nga Whakatakotoranga Hangarau",
        code_input: "ID hua",
        code_input_message: "Waehere Hua: P0000...",
        spec_view_title: "Nga korero mo te hua kua tohua",
        spec_view_subtitle:
          "Kua kawemai nga korero mai i te hua kua tohua.",
        set_spec_action: "Whakatakotoranga Hangarau takirua",
      },
      notifications: {
        enter_group_title_error: "Whakauruhia te uara upoko",
        enter_item_title_and_value_error:
          "Whakauruhia te taitara me te uara o te ahuatanga",
        spec_saved_success: "Kua tiakina nga whakaritenga hua.",
      },
      dialog_ai: {
        title: "Hanga Aunoa i nga Whakatakotoranga Hua",
        prompt: {
          title: "Hua Tirohanga",
          subtitle:
            "Tena koa homai he whakamaarama poto me te marama o te hua, me te whakaatu i ona ahuatanga matua me ona taipitopito. Ka whakamahia enei korero ki te whakaputa aunoa i nga whakaritenga hua.",
        },
        generated_spec: {
          title: "AI-tūemi Whakatakotoranga",
          subtitle:
            "Ko enei korero i hangaia e te matauranga horihori.",
        },
        inputs: {
          prompt: {
            label: "Hua Nga korero",
            placeholder: "tūemi te Hua tūemi...",
          },
        },
        actions: {
          run: {
            title: "tūemi Whakatakotoranga",
            subtitle:
              "Paatohia ki te hanga aunoa i nga whakaritenga hua mai i te tirohanga whanui kua whakaratohia.",
          },
          add_generated_spec: "tāpiri tūemi Whakatakotoranga",
        },
      },
    },
    /** {@see BProductEditImages} **/
    edit_images: {
      title: "Whakaahua matua",
      main_image_input_nodata: "Tukua ake te ahua hua.",
      main_image_input_message: "Ka whakaatuhia tenei ahua ki te rarangi hua.",
      preview: "Arokite",
      image_style: "Kāhua whakaahua",
      background_is_white: "Papamuri Ma",
      background_is_white_msg: "He ma te papamuri o te ahua??",

      images: {
        title: "Taiwhanga Whakaahua Hua",
        sub_title:
          "Tukuna nga whakaahua hua, he mea noa ki nga momo hua katoa, i tenei waahanga.",
      },
      video: {
        title: "Ataata",
        sub_title:
          "Tuatahi, tukuna to ataata hua ki runga YouTube, katahi ka tuu te URL ataata ki konei.",
      },
      change_bg_ai: {
        title: "Waihanga Whakaahua tūemi Hou papamuri",
        inputs: {
          prompt: {
            label: "papamuri Whakaahuatanga",
            placeholder: "tūemi te tūemi papamuri...",
          },
        },
        actions: {
          reimagine: {
            title: "tūemi Hou Whakaahua",
            subtitle: "Whakaarohia te ahua katoa me te papamuri hou.",
          },
          replace: {
            subtitle: "tūemi te tūemi papamuri.",
          },
        },
      },
    },
    /** {@see BProductEditPrice} **/
    pricing: {
      edit_price: "Te utu",
      price_input: "utu",
      currency_input: "Moni",
      currency_input_messages: "Ka tatauhia te utu i roto i tenei moni.",
      commission_input: "Komihana",
      commission_message:
        "He pai tenei whiringa mo nga momo kaihokohoko penei i nga kaihoko rarawe. Mēnā ehara koe i te kaihoko rarawe, waiho kia pātea tēnei kōwhiringa. Ko te utu e kitea ana e nga kaihoko ko te utu me te komihana. Mo te taapiri taake pawhiria te Kaute > Taake.",
      add_commission_action: "Tāpiri komihana",

      price_label_input: "Tapanga utu",
      price_label_message:
        "Taapirihia etahi atu korero mo te utu penei ($1.5/Fl Oz). Whakamahia nga keehi: hokohoko, FMCG.",
      add_price_label_action: "Tāpiri tapanga utu",
      discount_input: "Te utu",
      discount_input_message: "Te utu utu",
      has_discount_period_input: "He tika tenei utu mo te wa motuhake?",
      discount_timespan: "Whakauruhia te wa hekenga.",
      start_input: "Tīmata",
      end_input: "Whakamutunga",
      start_input_placeholder: "Tīpakohia te wā tīmata",
      end_input_placeholder: "Tīpakohia te wa mutunga",
      independent_price: "Ko te utu anake mo tenei momo rereke",
      same_price: "Ko te utu mo te hua matua",
      variant_pricing_message:
        "Ma te kowhiri i tenei whiringa, ko te utu o tenei momo rereke ka taea te whakarite motuhake mai i te hua matua.",

      subtitle: {
        marketplace:
          "I roto i te maakete, ko te utu hua tuatahi e whakaatuhia ana ko te utu raarangi, ko te utu tuturu ka whakatauhia i runga i nga rekoata hononga kaihoko-hua.",
        subscription:
          "Mo nga ohaurunga, ko te utu raarangi anake ka whakaatuhia, ko te utu tuturu ka whakatauhia i runga i te mahere ohaurunga kua tohua.",
        service:
          "Mo nga hua e pa ana ki nga ratonga, ko te utu kua whakarārangihia mo nga kaupapa whakaatu anake, me te utu tata mo te ratonga. Ko te utu o te ratonga ka whakatauhia e koe ina oti te tuku ota e te kaihoko (i runga i te tirotiro).",
        file: "E pa ana tenei ki te rarangi me te utu tuturu o te konae, me utu nga kaihoko kia uru atu ai ki nga konae.",
        default:
          "E pa ana tenei ki te utu kua tohua o te hua me te whai whakaaro ano hoki ki te utu o nga momo rereke kaore he utu motuhake.",
      },
      extra_pricings: {
        title: "tāpiri 20% Te utu",
        subtitle: "Whakatinana utu hihiri i runga i te rahinga hoko.",
      },

      vendor: {
        title: "kaihoko Te utu",
        subtitle:
          "Ko tenei utu ka whakaatuhia anake i roto i te raarangi hua. Ka taea e koe te whakarite utu kaihoko ki te Hua> Ripa Kaihoko.",
        manage_vendors: "tūemi kaihoko",
        vendor_product_pricing_link:
          "Ko te utu hua o te kaihoko e hono ana. Ko te whakarereke i te utu raarangi i konei ka pa ki nga hua kaihoko kaore he rereke, he rereke ranei karekau he utu (ma te whakamahi i te utu rite ki te hua matua). Ko tenei utu ka kiia ko te utu o te maakete, ka whakatauhia te utu a te kaihoko i runga i te tauira utu, i te tawhwhnga hua o mua atu ranei.",
        vendor_variant_pricing_link:
          "Ko te whakarereke i te utu rereke i konei ka pa ki te utu o nga hua kaihoko me te rereke ano. Ko tenei utu ka kiia ko te utu o te maakete, ka whakatauhia te utu a te kaihoko ki te tauira utu, ki te tawhwhanga hua o mua atu ranei.",
      },
    },
    /** {@see ProductEditInputs} **/
    inputs_edit: {
      title: "Puka Whakauru mo nga Korero Kaihoko",
      sub_title:
        "Ka taea e koe te hanga puka urunga raraunga mo ia hua, na reira ka tukuna atu e nga kaihoko nga korero ki a koe ina hoko hua. Hei tauira, he ingoa pea enei korero mo te whakairo ki runga kohatu.",
      message_input: "Karere ki te Kaiwhakamahi",
      message_input_message:
        "Ka whakaatuhia tenei karere ki te kaihoko ina hoko hua.",
      inputs_form: "Hanganga korero (whakauru)",
      inputs_hint:
        "Kei a koe te whiringa ki te whakarite i tetahi puka whakawhaiaro hei kohi taipitopito mai i o kaihoko ina reri ana ratou ki te tuku ota.",
    },

    /** {@see BProductEditExtra} **/
    extra_edit: {
      physical: {
        title: "Nga korero tuku",
        sub_title:
          "Ko te utu me te wa whakarite ka tatauhia i runga i nga korero o tenei waahanga. Kia mahara ki te whakauru i nga korero i runga i nga waahanga kua tohua i roto i te toa (taimaha, ine, tawhiti).",
      },

      notifications: {
        save_title: "Kua Whakahoutia te Packaging",
        save_msg: "I pai te whakahou i nga korero mo te kapi hua.",
      },

      order_limit: {
        title: "🛍️ Ota Nga rohe",
        subtitle:
          "Mena he kaihoko rarawe koe, he iti rawa ranei te rohe mo te rahinga o tenei mea mo ia ota, ka taea e koe te tohu i konei.",
        no_limit: "kahore Tepe",
        minimum_purchase_quantity: "tūemi Hokonga Te nui",
        maximum_purchase_quantity: "tūemi Hokonga Te nui",
      },
    },

    variants: {
      not_available:
        "I tenei wa kaore e taea te kowhiri i tetahi momo rereke mo tenei momo hua.",
    },

    delete_product: {
      title: "Tango hua",
      message: "Me muku <b>{product_title}</b> ?",
      tips: "Mena ka whakakorea e koe tetahi hua, ka taea e koe te whakaora ma te whakatere ki to Toa> Hua, ka taea te Whakaatu i nga Tuemi Kua Mukua, ka paato-matau ki te hua, ka kowhiri i te Whakaora.",
    },
  },

  /** {@see BProductExtraPricingAdd} **/
  extra_pricing_add_dialog: {
    title: "20% Te utu mō tūemi {min}",
    subtitle:
      "Ka whakatau aunoa te punaha i te ota me te kaupapa matua mo te tatauranga utu. Whakauruhia koa te rahinga iti e hiahiatia ana kia whai mana ai tenei utu.",
    min_quantity: "Te rahinga iti",
  },
  /** {@see BProductInventoryManagementPhysical} **/
  product_inventory_management_physical: {
    subtitle: {
      dropshipping:
        "Ko nga taonga mo nga hua whakaheke kua whakaritea e nga kaihokohoko, kaore e taea te whakarereke i konei.",
      marketplace:
        "Ko te rarangi hua katoa i te maakete ka whakatauhia ma te taapiri i nga raarangi o nga kaihoko katoa. Kaore e taea e koe te whakarereke i konei.",
      default: "Tauwhāitihia te tatauranga rārangi o te hua ki konei.",
    },
    menu: {
      add_bulk: {
        title: "tūemi tāpiri",
        subtitle: "Tāpirihia nga momo rereke maha ki te mahi kotahi.",
      },
      edit_property_set: {
        title: "Whakaritea te huinga Taonga",
        subtitle: "Hurihia Nga momo rereke tūemi, tūemi ka Nga ota.",
      },
    },
  },

  /**
   * @see USmartMenu
   */
  smart_menu: {
    show_delete_items: "whakaatu Kua mukua Nga taonga",
  },

  /** {@see BProductGoogleCategoryInput} **/
  google_product_category_input: {
    label: "Google Hua Kāwai Waehere",
    hint: "He tohu mo te waahanga o to hua i runga i te taaketanga hua a Google.",
    placeholder: "tūemi Hua te te Kāwai.. (tūemi)",
  },
  /** {@see LogisticProfileType} **/
  LogisticProfileType: {
    WARRANTY: {
      title: "tūemi",
      desc: "Ko te raihana he waahanga o te kirimana ka oati te kaihanga ki te whakatika, ki te whakakapi ranei i tetahi hua i roto i te waa kua tohua. Ka taea e tenei whakapau kaha te toro atu ki nga kaihoko kaore he kirimana tika me te kaiwhakanao.",
    },
    RETURN_POLICY: {
      title: "📦 Hoki mai tūemi",
      desc: "Ko nga kaupapa here whakahoki e whakaatu ana me pehea e taea ai e nga kaihoko te hoki mai, te whakawhiti ranei i nga taonga kore e hiahiatia. Ka tohua e ratou nga mea e tika ana, nga take e whakaaetia ana mo te hokinga mai, me te waa mo te hokinga mai.",
    },
    GUIDE: {
      title: "tūemi",
      desc: "Ka taea e te kaiarahi te whakauru i te tūtohi rahi mo nga kakahu, he aratohu whiriwhiri mo nga taimana. He mea nui kia whai kiko nga kaiarahi me te whai kiko ki nga hua maha.",
    },
    SHIPPING: {
      title: "Te tuku",
      desc: "Ko te kaupapa here kaipuke e whakaatu ana i nga waahanga nui o te tuku mo nga ota ipurangi, tae atu ki nga utu kaipuke, nga tikanga, me nga wa tuku.",
    },
  },
  /** {@see BProductProfileLogistic} **/
  product_logistic_profile: {
    dialog: {
      title: "Tīpakohia te kōtaha logistic",
      add_new_action: "tāpiri Hou kōtaha",
      add_new_disable_msg: "Te kainga hokohoko Rangatira",
    },
  },

  /**
   * @see BPageShopPagesPopups
   */
  b_shop_popups: {
    title: "tūemi whakahaere",
    subtitle:
      "Waihangahia nga matapihi pahū-ake whaiaro, mohio, me te tauwhitiwhiti.",
    add_action: "tāpiri Hou Pahūake",
  },

  /** {@see BProductProfileTax} **/
  product_tax_profile: {
    description: {
      default: "Taunoa",
      dedicated: "tūemi",

      subscription: "Ka taea e taatau te tono taake anake i runga i te waahanga hanga utu.",
      is_disabled: "Kua monoa tenei pūkete taake! (⚠️Whakatupato)",
      shipping: "Te tuku",
      fixed_rate: "Taake",
      location_based_rate: "Taake: Tauwāhi tūemi",
      shipping_location_based_rate: "Te tuku: Tauwāhi tūemi",
      include_in_price: "Kei roto i te utu",
    },
    dialog: {
      header: "tūemi Taake kōtaha",

      title: "Taake kōtaha",
      subtitle:
        "Ka taea e koe te tautapa etahi ture taake ki te hua. Mena karekau he kotaha taake e whiriwhiria mo te hua, ka whakamahia nga ture taake toa taunoa.",

      action_manage: "tūemi Taake",
      action_manage_sub: "Toa > Pūtea > Taake",

      subscription_tips:
        "Ina whiriwhiria e koe he kotaha taake mo te hua ohaurunga, ka whakahōu<code>waehere taake</code> ka tautuhia te aratau whakauru/motuhake ki to kaiwhakarato ratonga utu. Ko te kotaha taake kua tohua ka whakatau mena ka kiia te utu he taake, haunga ranei nga taake. Ko tetahi o <code>inclusive</code> or <code>exclusive</code>. Ina tohua he whakauru, he motuhake ranei, kaore e taea te whakarereke.<br /><br /><b>He mea nui!</b> I muri i te whakarereketanga o te putea taake, me panui koe ki nga mahere utu i runga i te ripa Hua > Inventory ka paato i te paatene Tiaki hei tono huringa.",
    },
  },

  /**
   * @see BProductVendorAbstractView
   */
  product_vendor_profile: {
    vendor: {
      subtitle:
        "Ko te utu me te nui o tenei hua ka whakatauhia e tetahi kaihoko nana nei.",
    },
    vendors: {
      subtitle:
        "Anei te rarangi o nga kaiwhakarato mo te hua. Ko te nui me te utu ka whakatauhia e nga kaiwhakarato.",
    },
  },
  /**
   * @see BVendorAdd
   */
  vendor_add: {
    menu: {
      profile: "kōtaha & Nga korero",
      business: "tūemi & 💳 Utu",
      documents: "tūemi",
      page: "whārangi tauira",
      shipping: "Te tuku",
      access: "Urunga",
      critical_zone: "Rohe Tino",
    },

    profile: {
      title: "kaihoko Nga korero",
      subtitle: "Ka whakaatuhia enei korero ki nga kaihoko ma te whanui.",
      action_sub_caption: "Tūmatanui Hua rārangi whārangi.",
      action_see_listing_page: "tūemi rārangi whārangi",
      send_invitation_email: "Tukua tūemi Īmēra",
      invitation_link: "tūemi Hononga",
    },

    profiles: {
      title: "Kōtaha",
      subtitle: "Ka taea e koe te tohu waahi ki tenei kaihoko.",
    },
    map: {
      no_map_message:
        "Whakaritea he waahi ki te kaihoko mena he kaihoko tauwāhi.",
    },
    page: {
      title: "Ritenga whārangi",
      subtitle:
        "Whakatakotoria he wharangi tauranga ritenga mo te kaihoko, ma te tuku hononga ahurei ki a raatau wharangi kua whakatapua.",
      action_set_page: "tūemi Tūmatanui whārangi",
      action_sub_caption: "Ritenga Taunga whārangi.",
      vendor_has_landing_msg: "kaihoko tūemi he Ritenga Taunga whārangi.",
      no_landing_selected_msg: "kahore Taunga whārangi tūemi.",
      no_landing_page: "kahore Taunga whārangi",
      we_can_create_dedicated_landing_msg:
        "Ka taea e matou te hanga he wharangi taunga motuhake mo koe.",
    },
    contact: {
      title: "Whakapā Nga korero",
      subtitle: "Tena koa homai nga taipitopito whakapā tika.",
    },
    business: {
      title: "tūemi Nga korero",
      subtitle:
        "Tena koa homai nga korero tika mo to pakihi kia tere ai te kohi moni whiwhinga.",
    },
    bank: {
      title: "Nga korero utu",
      subtitle: "Whakauru to Peeke Nga korero tūemi mō tūemi.",
    },
    default_pricing: {
      title: "Taunoa Te utu",
      subtitle:
        "Ka taea e koe te whakatau i tetahi tauira utu taunoa mo te kaihoko. Ina taapirihia e te kaihoko he hua hou, ka whakamahia tenei tauira utu ki te tatau i te tawhwhanga maakete i runga i te utu o te hua. Ka taea e koe te whakatika i te tauira utu mo ia hua takitahi.",
      no_pricing: "kahore Te utu",
    },
    shipping: {
      title: "Te tuku",
      subtitle: "Ka taea e nga kaihoko te whakarite i a raatau ake ratonga kaipuke me nga karere.",
      shipping_services: "Te tuku Ratonga",
      total_number_of_services:
        "Te tapeke o nga ratonga kaipuke kei te kaihoko.",
      couriers: "Karere",
      total_couriers_count: "Te tapeke o nga kaikawe korero kei te kaihoko.",
    },
    configuration: {
      title: "tūemi",
    },
    delete: {
      title: "Tango kaihoko",
      action: "Tango kaihoko",
      verify_description:
        "Kei te mohio ahau ka tangohia nga hononga hua kaihoko katoa!",
      verify_title: "I tūemi Ki Mukua tūemi kaihoko.",
    },

    send_invitation_tips:
      "Ka tukuna e matou he imeera tono ki tenei kaiwhakamahi. Mena ka uru mai te kaiwhakamahi ki to toa hei kaihoko, ka tohatohahia to putea ki a raatau.",
    only_marketplace_owner_can_edit_user:
      "Ko te rangatira kainga hokohoko anake ka taea te whakatika i te kaiwhakamahi. Mena kei te hiahia koe ki te whakarereke i te rangatira o tenei kaihoko, ka taea e koe te tuku tono ki a maatau.",
    vendor_is_disable_msg: "kaihoko to tūemi.",
    email_not_match_with_user_msg:
      "Ko te imeera kua whakauruhia kaore e rite ki te imeera a te kaiwhakamahi (<b>{user_name}:</b>{user_email}). Me tuku imeera koe ki<b>{email}</b>?",

    order_information_email_to_email:
      "Ka tukuna atu e matou nga korero ota ki <b>{email}</b>.",

    set_a_user_for_the_vendor_first_msg: "tūemi he kaiwhakamahi mō te kaihoko tūemi.",

    inputs: {
      slug: {
        label: "tūemi",
        placeholder: "tūemi he Ritenga tūemi...",
        hint: "Hurihia te ara te wharangi taunga hihiri o te kaihoko.",
      },
      email: {
        message: "Ka whiwhi tenei imeera i nga whakamohiotanga me nga whakahoutanga katoa.",
      },
      business: {
        false_description: "Ka mahi ahau hei tangata takitahi.",
        true_description: "Ka mahi ahau hei hinonga pakihi e whakamanahia ana e te ture.",
        false_title: "tūemi",
        true_title: "tūemi",
      },
      bank: {
        hint: "Ko te ingoa o te putea kei reira te kaihoko i a raatau kaute.",
        placeholder: "to Peeke Ingoa..",
      },
      account_name: {
        hint: "Me rite tenei ki te ingoa e hono ana ki te putea putea.",
        placeholder: "to Ingoa..",
      },
      account_number: {
        hint: "Te nama kaute ahurei a te kaihoko.",
        placeholder: "to Peeke Pūkete tūemi..",
      },
      routing_number: {
        hint: "He rereke tenei tau i ia whenua. Ka whakamahia hei tautuhi i te peka putea motuhake e whakamahia ana e te kaihoko.",
        placeholder: "To nama ararere peeke, tohu tohu, tau BSB ranei..",
      },
      iban: {
        hint: "Tau Kaute Peeke o te Ao): Mena kei Uropi te peeke a te kaihoko, etahi atu whenua ranei.",
        placeholder: "to IBAN tūemi.. tūemi. DE89 3704 0044 0000 0000 00",
      },
      swift: {
        hint: "Tau Kaute Peeke o te Ao): Mena kei Uropi te peeke a te kaihoko, etahi atu whenua ranei.",
        placeholder: "to tūemi Waehere/BIC..",
      },
      branch_address: {
        hint: "Ko te wahitau o te peka putea kei reira te kaihoko i a raatau kaute.",
        placeholder: "to Peeke wāhi noho..",
      },
      enable: {
        label: "kaihoko Tūnga",
        hint: "Ka taea e koe te whakahohe, te whakakore ranei i nga kaihoko puta noa i te ao. Kaore e taea e nga kaihoko te hoko mai i nga kaihoko haua.",
        true_description:
          "Ka taea e ratou te hoko i a raatau hua ma to maakete.",
      },
      access: {
        label: "kaihoko tūemi & Urunga",
        true_description:
          "Kei te kaihoko he panui whakatapua ki te whakahou i te rahi me te utu.",
        hint: "Ka taea e koe te whakahohe, te whakakore ranei i te rarangi kaihoko, ka taea e nga kaihoko te whakahou i nga rahinga hua me nga utu me te whiwhi ota waahanga.",
        false_description: "te kaihoko tūemi kahore Urunga Ki tūemi.",
      },

      logo: {
        label: "kaihoko Moko",
      },
      description: {
        placeholder: "tūemi he Tūmatanui Whakaahuatanga...",
      },
    },
  },

  /**
   * @see BVendorDocumentsList
   */
  vendor_documents_list: {
    title: "tūemi",
    action_upload_doc: "tukuake tūemi",
    vendor_subtitle:
      "Tukuna koa te pakihi e tika ana, te IP, me nga tuhinga manatoko wahitau. E hiahia ana matou ki enei korero hei whakaū i to mahi tahi me te whakarato ki a koe te urunga e tika ana.",
    marketplace_subtitle:
      "Ka taea e nga Kaihoko te tuku tuhinga penei i te pakihi, taonga hinengaro, me te manatoko wahitau hei whakahaere i a raatau mahi KYC, ka taea e koe te tuku i te urunga tika.",
    upload_dialog: {
      title: "tukuake tūemi",
      type: {
        title: "tūemi Momo",
        subtitle:
          "Tukuna koa nga tuhinga e tika ana. A ape i te tiri i nga tuhinga kei roto nga korero tairongo. Ka tono matou i nga tuhinga e waatea ana ki te katoa.",
      },
    },
  },

  /**
   * @see BProductBadgesManage
   */
  product_badges: {
    title: "tūemi",
    subtitle: "tāpiri ka Whakatikatika Ritenga Hua tūemi.",
    only_marketplace_owner_msg: "tūemi Te kainga hokohoko tūemi",
    create_new_badge_action: "Waihanga Hou tūemi",
    select_a_badge: "tūemi he tūemi...",
    inputs: {
      custom: {
        false_description: "Ka hanga aunoa nga tohu hua.",
        false_title: "tūemi",
        true_title: "Ritenga Hua tūemi",
        true_description: "tāpiri Ritenga tūemi i te Hua whārangi.",
      },
    },
    add_custom_badge: {
      title: "Ritenga tūemi",
      subtitle:
        "Tāpirihia he tohu ritenga hou ki taku toa. Kia mau nga tohu i raro i te 10. Ka nui ake nga tohu ka whakahekehia to toa.",
      show_advanced_options: "whakaatu Arā atu tūemi",

      inputs: {
        title: {
          placeholder: "Tūmatanui Taitara...",
        },
        image: {
          label: "Whakaahua",
          message: "tūemi Whakaahua tūemi: 128KB",
          placeholder: "tūemi he tūemi Whakaahua",
        },
        pattern: {
          label: "tūemi",
          message:
            "Whakaatu aunoatia tenei tohu mo nga hua kei roto tenei uara rite ki o raatau waahanga. Kua tautokohia te Regex.",
          placeholder: "Ki whakaatu mō katoa Hua tūemi: *.*",
        },
      },
      notifications: {
        badge_added: "Kua angitu te taapiri i te tohu!",
        badge_updated: "Kua pai te whakahou i te tohu!",
        item_not_found_error:
          "Kāore i kitea te tūemi. Tāmatahia te whārangi, whakapā atu rānei ki te tautoko.",
      },
    },
  },
  /**
   * @see BProductTagsInput
   */
  product_tags: {
    title: "Tohutohu",
    tips: [
      "Ka taea e koe te tātari hua i runga i nga tohu i runga i <b>nga wharangi tauranga</b>.",
      "Ka taea e koe me o kaihoko te rapu i enei tohu i te pae<b>search</b>.",
      "Mena kua tautuhia nga tohu, ko nga hua anake me nga <b>nga tohu noa</b> ka whakaatuhia ki te waahanga <b>huatanga</b> kei te wharangi hua; ki te kore, ka whakaatuhia nga hua o te waahanga kotahi.",
    ],
    inputs: {
      tags: {
        placeholder: "Tohu waea ki konei ka pehi i te tomo. ex. kohinga hou",
      },
    },
    notifications: {
      tags_updated: "Kua pai te whakahou i nga tohu.",
    },
  },

  /**
   * @see VPageVendorShippingTransportation
   */
  vendor_shipping_services: {
    title: "Te tuku Ratonga",
    subtitle:
      "Ka tukuna e te maakete nga tikanga tuku hei awhina i nga kaihoko ki te whakatutuki i a raatau ota. Ka taea e koe te taapiri i nga ratonga kaipuke ki konei kia ngawari te tono tapanga kaipuke mai i to wharangi ota. Ma tenei ka ngawari te mahi ma te tukutahi i nga wahitau me nga waehere aroturuki me te kaiwhakarato kaipuke.",

    subtitle_pickup:
      "Ka taea e te maakete te tiki mo nga kaihoko, ka taea e nga kaihoko te kowhiri i te tiki mai i to waahi hei whiringa kaipuke. Ka taea e koe te taapiri i to waahi tiki ki to korero (ka whakamahia to whare putunga hei waahi tiki taunoa).",
    action: "tāpiri te",

    add_dialog: {
      select_service_msg:
        "Ko nga tikanga tuku e whakatauhia ana e te maakete, ka taea e nga kaihoko te whiriwhiri mai i a raatau. Karekau he mana whakahaere a nga Kaihoko mo enei whiringa engari ka taea te whakauru i o raatau roopu me nga ratonga kaipuke ki te hoko tapanga tika. Tīpakohia he tikanga mai i te rarangi i raro nei hei tiro i nga ratonga e waatea ana. Kōwhiria he ratonga kaipuke mai i te rarangi i raro nei. Ko nga ratonga e waatea ana ka taea te taapiri atu ki to toa ka whakaatuhia ki konei.",
      no_available_service: "kahore Kei te waatea te!",
    },
  },
  /**
   * @see VPageVendorShipping
   */
  vendor_shipping: {
    services: "Ratonga",
  },

  /** {@see BProductProfileMap} **/
  product_location_profile: {
    title: "Tauwāhi",
    empty_subtitle:
      "Tautapa he tauwāhi ki te hua mena he taonga i runga i te waahi.",
    dialog: {
      header: "tūemi Tauwāhi kōtaha",
      action_new_location: "Tāpiri tauwāhi hōu",
      title: "Tauwāhi kōtaha",
      subtitle:
        "Ka taea e koe te pine i nga hua ki te waahi mahere ma te tohu tohu waahi ki nga hua.",
      tips: "<b>He mea nui: </b> Whakaritea he waahi ki te hua <b>anake</b> ki te hiahia koe ki te whakaatu i te hua ki te waahi motuhake i runga i te mahere. Ko te hua pinea me whai te waahi hei tohu matua, penei i te <b>nga kaainga reti/nga waahi</b> me nga ratonga a-tangata penei<b>Hairdressing and Spa</b>.",
    },
  },

  /** {@see BTranslationButton} **/
  translation_button: {
    header: "tūemi reo",
    auto_action: "Whakamaori Aunoa",
    premium_user_only: "Mōkī Kaiwhakamahi",
  },

  /** {@see BProductProfileInclude} **/
  product_include_profile: {
    subtitle:
      "Whakauruhia he rarangi o nga taonga ka tae mai me te hua, ratonga ratonga ranei.",
    dialog: {
      header: "Hua tūemi Nga taonga",
      title: "tūemi Nga taonga",
      subtitle: "He kohinga tenei o nga taonga ka tae mai me te hua.",
      action_manage: "tūemi Nga taonga",
      action_disable_msg: "Te kainga hokohoko Rangatira",
      select_items_tips:
        "Waihangahia, kowhiria ranei nga waahanga kete ka tae mai me te hua, ratonga ranei. Whakamahia tenei waahanga hei whakanui i nga mea ka taea e nga kaihoko te tumanako ka riro mai i te hokonga o te taonga. Ka tohutohu matou ki te whakamahi i tenei mahi anake ina he mea nui kia mohio te kaihoko ki nga mea kei roto, penei i te hoko atamai.",
      create_item_tips:
        "I tua atu, ka taea e koe te whakauru i tetahi mea hou. Whakamahia nga whakaahua tapawha, na te mea ka whakarereke aunoa i nga pikitia kua tukuna ki te whakatakotoranga tapawha 256x256. Hei whakarite i te ahua tino pai puta noa i nga tauira katoa, whakamahia nga whakaahua iti me nga papamuri maramara me te pupuri i te tauira rite mo o hua katoa.",
      create_new_item_action: "Waihangahia nga mea hou kua whakauruhia",
    },
  },

  /** {@see BPageProduct} **/
  product_admin: {
    products_list: "Rarangi Hua",
    product: "Hua",

    menu: {
      edit: "Whakatikatika",
      review: "Arotake",
      orders: "Nga ota",
      inventory: "tūemi",
      dashboard: "Papatohu",
      back_to_products: "Hua",
      drop_shipping: "Te tuku tuku",
    },

    /** {@see BPageProductDashboard} **/
    dashboard: {
      /** {@see BProductInfoAbstractView} **/
      info: {
        title: "kōtaha",
        title_small: "Hua",
        category: "Kāwai Hua",
        link: "Hononga Hua",
        type: "Momo",
        inputs: "Nga korero whakauru",
        outputs: "Nga korero whakaputa",
        user_data_form: "Puka Korero Kaihoko",
        staff_messages: "Kaimahi tūemi",
        add_note: "tāpiri Tuhipoka",
        product_tags: "Hua Tohutohu",
        edit_tags: "Whakatikatika Tohutohu",
      },
      /** {@see BProductOrdersAbstractView} **/
      orders: {
        title: "Nga ota",
        title_small: "Karekau i tukatia",
        physical_orders_title: "Kaata Hoko Me tenei Hua",
        send_count_in_period: "Ka tukuna Nga ota tūemi {start} ~ {end}",
        downloads_count_in_period:
          "kōnae tūemi Tatau tūemi {start} ~ {end}",
      },

      /** {@see BProductInventoryAbstractView} **/
      inventory: {
        title: "tūemi",
        title_small: "Whare putunga",
        total_sell: "Tapeke Hokonga",
        total_sell_today: "Nga hoko i tenei ra",
        total_sell_timespan: "Tapeke Hokonga tūemi {start} ~ {end}",
        chart: {
          labels: ["tūemi", "Hokonga"],
        },
        subscribers_title: "Kaiohauru",
        subscribers_title_small: "Tapeke ota ohauru",
      },

      /** {@see BProductFinanceCharts} **/
      finance: {
        title: "Pūtea",
        title_small: "Purongo Hokonga 6 Marama kua hipa {start} {end}",
        select_mode_title: "Whakaatu Na",
        modes: {
          sell: "Hokonga",
          payment: "Utu",
          discount: "Te utu",
        },
        currency_select_title: "Utu ma te moni kua tohua",
        sell_today: "Nga hoko i tenei ra",
        product_price: "Utu Hua",
        currency_unit: "Waeine moni",
        sell_30days: "30 ra hoko",
        sell_daily: "Nga hoko o ia ra",
        sell_yesterday: "Nga hoko inanahi",
      },

      /** {@see BProductRatingCharts} **/
      survey: {
        title: "Whakatauranga Hua",
        title_small: "Pūrongo Pumau Kiritaki",
        total_participation: "Te maha o nga takoha",
        total_participation_tooltip: "Tapeke Tatau o tūemi mō te Hua.",
        today_participation: "I tenei ra nga whakatauranga",
        last7days_participation: "Ko nga takoha 7 ra whakamutunga",
        chart: {
          participate: "Te whai waahi",
          participate_title: "Te maha o nga takoha",
          score: "Tohu",
        },
        users_not_rated_msg: "Kaore ano nga kaiwhakamahi i whakatau i tenei hua.",
      },

      /** {@see BProductMarketingAbstractView} **/
      inform: {
        title: "Nga tumanako a te Kaihoko",
        title_small: "E tatari ana mo nga utu, mo te waatea ranei",
        message:
          "Whakamōhio atu ki nga kaihoko ina hokona ana nga hua ka watea mai ranei",
        waiting_for_auction: "Ko te tatari mo nga hua ka haere ki te hoko",
        waiting_for_available: "E tatari ana kia watea mai nga hua.",
        unit: "(ng) tangata",
      },
    },

    /** {@see BPageProductInventory} **/
    inventory: {
      /** {@see BProductInventoryManagement} **/
      variants: {
        inventory_list: "rārangi rārangi",
        add_variant: "Taapirihia te rereke",

        title: "Rarangi Rarangi Hua / Raarangi Hua",
        sub_title:
          "Mena he nui ake i te kotahi nga momo rereke o tenei hua, whakauruhia ki tenei waahanga. Ka taea e koe te tautuhi motuhake te nui, te utu me te hekenga o ia momo rereke; ki te kore ka whakamahia nga uara taunoa o te hua mo te utu.",
        add_action: "Taapirihia te rereke",
        count_input: "tūemi",
        count_input_action: "Tautuhi tatau",
        management_title: "Te whakahaere i nga taonga me nga momo momo hua",
        add_title: "Tāpirihia te momo hua",
        edit_title: "Whakatikatika momo hua",
        notifications: {
          add_success: "He rereke nga hua kua taapirihia.",
          delete_success: "Te momo hua kua Mukua.",
          save_product_inventory_success: "Rarangi o nga hua kua tiakina.",
        },
      },

      /** {@see BProductInventoryManagementVirtual} **/
      variant_item: {
        price: "utu",
        same_as_product: "He rite ki te hua matua",
        in_stock: "tūemi",
        delete_dialog: {
          title: "Mukua te momo hua",
          message: "Me muku tenei momo rereke?",
        },
      },
      /** {@see BProductVariantAdd} **/
      variant_add_edit: {
        message: "Tau Hua ahurei",
        active_input: "Kei te hohe tenei Whakatau?",
        sku: "SKU",
        sku_tips: "Waehere Whare Putunga Hua.",
        mpn: "MPN",
        mpn_tips: "Ko te waehere hua kua whakauruhia e te kaiwhakanao ki runga i te kete hua",

        has_pricing_input: "He rereke te utu mai i te hua matua?",
        inventory_unit: "Nga taonga",
        images_message:
          "I muri i te tiaki i tenei momo rereke ka taea e koe te taapiri i tetahi ahua ki konei.",

        config: {
          title: "Hua tūemi toa",
          subtitle: "tūemi SKU, MPN, ka tūemi mō Nga momo rereke.",
        },
        variables: {
          title: "Kōwhiringa rerekē",
          subtitle:
            "Whakauruhia he waehere tae hex, hei tauira, <b>#654FFF</b>, e rua ranei nga tae kua wehea e te rītaha, hei tauira, <b>#764FFA/#000000</b>.",
          graphical_asset_tips:
            "Ka taea e koe te tautapa i tetahi uara ki nga taurangi, tae atu ki nga whakaahua ma te whakamahi <b>Nga Rawa Whakairoiro</b>. Tukuna he tauira ka whakamahi i te waehere i hangaia mo te rereke.",
          graphical_asset_action: "Rawa Whakairoiro",
          graphical_asset_action_caption: "Whakaahua mo nga whiringa rereke.",
        },
        smart_unit_price: {
          tips: "Tātaihia te utu wae i runga i te <b>weight</b>, <b>volume</b>, me te <b>te rahi</b>. Hei whakakore i tenei waahanga, whakatere ki te Toa> Tautuhinga> Kaupapa> Wharangi Hua> Utu Waeine Maamaa.",
          smart_unit_price: "Waeine utu tūemi",
        },
        quantity: {
          title: "tūemi",
          subtitle: "Tautuhia nga taumata kararehe mo ia momo rereke.",
        },
        price: {
          title: "Te utu",
          subtitle:
            "Whakatakotoria he utu ritenga mo te momo rereke, whakamahia ranei te utu hua taunoa.",
        },
        package: {
          title: "tūemi & Te tuku",
          subtitle:
            "Tauwhāitihia te taumaha me te rahi mo nga momo rereke hei whakatau i nga whiringa kaipuke.",
          same_as_product_action: "He rite ki te hua",
        },
        image: {
          title: "tūemi Whakaahua",
          subtitle:
            "Tāpirihia nga whakaahua ahurei mo ia momo rereke. Ko te ahua tuatahi ka tukuna ko te ahua tuatahi mo tera momo rereke.",
        },
        inputs: {
          sku: {
            message: "Wae Pupuri Taonga (SKU)",
          },
          mpn: {
            message: "Tau Waahanga Kaihanga (MPN)",
          },
          gtin: {
            label: "Tau Tūemi Tauhokohoko Ao (GTIN)",
            message:
              "Nga whakatakotoranga tautoko: UPC (12 mati), EAN (13 mati), JAN (8, 13 mati ranei), ISBN (13 mati).",
          },
        },
      },
    },

    /** {@see BPageProductArticle} **/
    edit_article: {
      menu: {
        tags: "Tohutohu",
        article: "Tuhinga",
      },
    },
  }, // product_admin

  /**
   * @see TransportationsEligibleView
   */
  transportation_eligible: {
    subtitle:
      "Ko nga ratonga kaipuke e waatea ana ka whakawhirinaki ki te rahi me te taumaha.",
  },

  /** {@see BProductExtraInput} **/
  product_extra_physical: {
    weight: "taumaha mōkihi",
    weight_subtitle:
      "Whakauruhia te taumaha katoa o te kohinga o te hua ki konei. Ka awhina enei korero ki te whiriwhiri i te tikanga tuku tika me te whakarite kia kore te ota e eke ki te rohe tuku.",
    weight_unit: "tūemi",
    size: "Te rahi o te kete",
    size_subtitle:
      "Whakauruhia te rahi o te kohinga o te hua ki konei. Ka whakamahia enei korero ki te whiriwhiri i te tikanga tuku tika me te whakarite kia kaua e neke ake te ota ki te tepe rahi o te peera.",

    width: "Whānui",
    length: "Te roa",
    height: "Teitei",
    lead_time_title: "Te wa whakarite",
    lead_time: "Te wa whakarite",
    lead_time_subtitle:
      "Ka whakatauhia e te wa arahi te roanga e hiahiatia ana hei whakarite i tetahi hua mo te tuku, e awhina ana ki te whakatau tata i te wa tuku.",

    lead_time_dimension: "(ng) hāora",
    bulk_action: "Nga Mahi Nui",
    bulk_action_subtitle:
      "Ka taea e koe te tono i nga korero kapi ki nga momo hua katoa.",

    bulk_action_input: "Whakahoutia nga momo rereke katoa",
    bulk_action_msg:
      "Whakaritea te kohinga hua me te wa whakarite mo nga momo rereke katoa.",
    dynamic_lead_time: "Te wa arahi hihiri",
    instant_delivery: "tūemi Tukunga",
  },

  /**
   * @see SGalleryUploadGrid
   */
  gallery_upload_grid: {
    upload_images: "tukuake Whakaahua",
  },

  /**
   * @see BProductImagesGallery
   */
  product_images_gallery: {
    dialog_alt: {
      title: "Tautuhi Kuputuhi Alt Atahanga",
      placeholder: "Whakauruhia he tuhinga alt whakaahuatanga...",
      success_update_notification: "I momoho te whakahou i te kuputuhi Alt.",
    },
    dialog_bg_remove: {
      title: "Tango Aunoa i te Papamuri",
      action: {
        title: "Tango papamuri",
        subtitle: "Waihanga he mārama papamuri.",
      },
      success_remove_bg_notification: "I pai te tango papamuri!",
    },
  },
  /**
   * @see VariantGraphicalAssetsList
   */
  variant_graphical_assets: {
    title: "Rawa Whakairoiro rereke",
    tips: {
      how_it_works:
        "<b>Pehea te Mahi:</b><br />Tuku ake tauira, whakaahua tuemi ranei ki konei. Hei whakamahi i enei whakaahua hei whiringa rereke, whakaurua <b>{filename}</b> ki te uara rereke. Kia mahara kaore e taea te whakamahi i tenei waehere ki te <b><s>color</s></b> uara.",
      how_set_name:
        "<b>Aki Whakaingoa:</b><br />Ko nga tohu tuatahi 12 o te ingoa kōnae kua tuku ake ka whakamahia hei ingoa rawa. He pai tenei ahuatanga mo te whakaatu i nga ahuatanga whakarite o tetahi hua, penei i te rahi o nga potae mo nga motopaika, tauira papanga ranei. Hei arotau i te whakamahinga o tenei ahuatanga, karohia te tuku whakaahua rereke ki konei mena ka iti ake te maha o nga rawa whakairoiro i te 100, penei i nga momo rakau rereke mo nga taonga.",
    },
  },

  /**
   * @see ExtraPricingLevels
   */
  extra_pricing_levels: {
    extra_pricings: "20% tūemi",
    min_quantity: "tūemi Te nui",
    range_is_out_of_stock: "Ra to tūemi o tūemi!",
  },

  /**
   * @see BProductVariantItem
   */
  product_variant_item: {
    duplicated_variant: "Kua noho kē tenei momo rereke!",
    set_color_warning: "Tīpakohia he tae.",
    set_volume_warning: "Whakawhāitihia koa te rōrahi.",
    set_pack_warning: "Whakawhāitihia te rahi o te mōkihi.",
    set_weight_warning: "Tohua koa te taumaha.",
    set_type_warning: "Tīpakohia koa tētahi momo.",
    set_style_warning: "Tīpakohia he kāhua.",
    out_of_stock: "Kei te pau te taonga i tenei wa",

    image: {
      tooltip: "⚡ Tōia ka whakatakahia he whakaahua ki konei hei tautapa ki te momo rereke.",
    },
    enable: {
      true_tooltip: "Kei te toa ipurangi tenei taonga.",
      false_tooltip: "Kaore tenei taonga i te waatea i te toa ipurangi.",
    },
    restore_dialog: {
      title: "Whakahokia te rereke",
      message: "Kei te tino hiahia koe ki te whakaora i tenei momo rereke kua mukua?",
      action: "Ae, Whakahokia Inaianei",
    },
    notifications: {
      restore_variant_success: "I pai te whakaora i te momo hua.",
    },
  },

  /** {@see BProductImagesGallery} **/
  product_images_list: {
    upload_button: "Ētahi atu Whakaahua Hua",
    upload_an_image: "Tīpakohia he whakaahua.",
    notifications: {
      update_sort_success: "Ko nga whakaahua kua tiakina",
      delete_image_title: "Mukua te ahua hua",
      delete_image_success: "I pai te whakakore i te ahua",
    },
  },

  /** {@see ProductVariantsView} **/
  variants_view: {
    prices: "Utu",
    inventory: "tūemi",
    discount: "Te utu",
  },

  /** {@see BGiftcardTypeAdd} **/
  gift_card_type_edit: {
    title_input: "Taitara",
    color_input: "Tae kaari",
    charge_input: "Te utu tuatahi",
    charge_input_hint: "Te utu tuatahi mo ia kaari",
    currency_input: "Moni",
    currency_input_message: "Mo te hoko me tenei moni anake",
    lifetime_input: "Te mana o te kaari (marama)",
    lifetime_input_message: "Te mana o te kaari",
    card_image_input: "Whakaahua kaari",
    notifications: {
      create_success: "Kari i hanga",
      update_success: "Kua whakahoutia te kaari",
    },
  },

  /** {@see BInventoryWarehouse} **/
  shop_warehouse_edit: {
    title: "Tāpiri Whare Putunga",
    message:
      "Whakauruhia to wahitau whare putunga me to whaarangi ki te tatau i nga utu kaipuke.",
    name_input: "Ingoa Whare Putunga",
    address: "wāhi noho",
    name_input_message: "Whakauruhia he ingoa ingoa mo to whare putunga.",
    person_name: "Ingoa Apiha",
    phone_number: "Tau Whakapā",
    map_title: "Wāhitau Whare Putunga Toa",
    confirm_action: "Whakaaetanga whare putunga",
    address_name: "Whare putunga",
    notifications: {
      success_save: "Kua tiakina pai nga huringa.",
    },
  },

  /** {@see BPageShopLogisticInventoryList} **/
  inventory_list: {
    title: "toa tūemi",
    subtitle: "Rarangi o katoa Hua i toa.",
    cant_set_here: "Kāore e taea te tautuhi ki konei",

    search_input: "Rapu Hua...",
    search_input_title: "Rapua i waenga i nga hua toa",
    apply_filter: "Hoatu tātari",
    inventory: "rārangi rārangi",
    export: "Kaweake pukapuka",
    import: "Kawemai pukapuka",
    bulk_price: "Huringa utu nui",
    bulk_discount: "Huringa hekenga nui",
    category_filter_msg: "⚡ Ka pa ki nga hua katoa me nga waahanga iti!",
    table: {
      waiting: "Kei te whiwhi korero ... tatari koa",
      open_state: "Hohe",
      close_state: "hohekore",
      image: "Whakaahua",
      product: "Ingoa Hua",
      code: "Waehere Hua",
      sku: "SKU",
      mpn: "MPN",
      original: "Taketake",
      status: "Tūnga",
      in_stock: "Kei roto i nga taonga",
      edit: "Hurihia",
    },

    bulk_discount_dialog: {
      title: "Nga mahi nui > utu",
      message:
        "Tuhipoka: Ko te moni e whai ake nei hei paheketanga ki te hekenga o nga hua katoa i to toa. Ano, ko nga uara hekenga kua tatauhia ka ata huri ki te tau porohita tata i runga i te moni kua tohua. Kia tupato ki te whakauru i nga uara.",
      check: "Ka whakaae ahau ki nga huringa utu.",
    },
    notifications: {
      save_success: "Kua tiakina te huringa pukapuka.",
    },
  },

  /**
   * @see BInventoryBulkPrice
   */
  bulk_price_dialog: {
    title: "tūemi Nga Mahi > utu",
    message:
      "Ko te moni e whai ake nei ka uru hei paheketanga ki te utu o nga hua katoa i to toa. Ano, ko nga uara utu tatau ka huri marie ki te tau a tawhio tata i runga i te moni kua tohua. Kia tupato ki te whakauru i nga uara.",
    check: "I tūemi utu tūemi.",
    check_description:
      "Ka hurihia te utu mo te waahanga kua tohua me nga waahanga iti katoa.",
    add_percent_tab: "tāpiri ōrau",
    add_constant_tab: "tāpiri tūemi",
    ending_tab: "Rautaki mutunga",
    marketplace_listing_price: "rārangi utu",

    constant: {
      subtitle:
        "Ko te moni ka taapirihia, ka tangohia mai ranei i te utu o naianei o nga hua.",
    },
    ending: {
      subtitle:
        "Ko te mutunga o te utu o naianei o nga hua ka huri ki te uara kua tohua.",
    },
    marketplace: {
      title: "Te kainga hokohoko rārangi utu",
      subtitle:
        "Ma tenei whiringa ka taea e koe te whakahou aunoa i te utu raarangi o nga hua kei te maakete. I etahi wa, kaore pea te utu o te hua matua e hono ki nga utu a te kaihoko na nga huringa utu a-ringa.",
      strategy: {
        min: {
          title: "Whakatakotoria te utu iti",
          description:
            "Whakaritehia te utu iti rawa o te hua hei utu raarangi.",
        },
        max: {
          title: "Tautuhi Utu Morahi",
          description:
            "Tautuhia te utu morahi o te hua hei utu raarangi.",
        },
      },
    },

    category: {
      subtitle: "Whakawhāitihia te mahi nui ki te kāwai me ngā kāwairoto katoa.",
    },
    vendor: {
      subtitle:
        "Whakaitihia nga mahi nui ki tetahi kaihoko motuhake. Ina tohua he kaihoko, ka tukuna te whakahou utu ki nga hua a te kaihoko, e tohu ana i te utu hoko mo taua kaihoko.",
      listing_price_need_update_msg:
        "Mena kei te hiahia koe ki te whakahou i te utu rarangi (te utu e whakaatuhia ana i te rarangi hua), me tuku ano e koe te whakahou nui me nga whiriwhiringa rite engari me te kore e whiriwhiri i tetahi kaihoko.",
    },
    listing_products_changes: "rārangi Hua Hurihia",
    total_vendor_products: "Tapeke kaihoko Hua",
    total_products: "Tapeke Hua",
    total_variants: "Tapeke Nga momo rereke",
  },

  /** {@see BProductSpecTable} **/
  spec_view: {
    auto_save_input: "Tiaki aunoa",
    auto_save_input_message: "Ka tiakina aunoa nga huringa.",
  },

  /** {@see STimeProgressBar} **/
  time_progress_bar: {
    title_before_start: "E toe ana ki te timatanga",
    title_between_start_end: "E toe ana ki te mutunga",
    title_after_end: "Kua oti",
    title_no_start_end: "Karekau he wa!",
  },

  /** {@see GeoMap} **/
  geo_map: {
    views: "Tirohanga",
    views_unit: "Nga taima",
    product_views: "Nga tirohanga hua",
    buys: "Nga hokonga",
    buys_unit: "Nga ota",
    baskets: "Kaata kaha",
    added_items: "Tāpiri tūemi",
    removed_items: "Tango tūemi",
  },

  /** {@see BPageShopFinanceGateways} **/
  b_page_shop_finance_gateway: {
    title: "💳 Utu tūemi",
    sub_title:
      "Rarangi o nga tikanga utu kaha. Mena kei te hiahia koe ki te taapiri i tetahi tikanga hou ki to toa, pehia te paatene e whai ake nei. Ka tino taunaki matou kia kaua e neke ake i te 10 nga kuaha utu taapiri.",
    add_gateway: "Hou 💳 Utu tūemi",

    add_gateway_dialog: {
      title: "tūemi Moni",
      country_currencies: "Moni rohe",
      crypto_currencies: "tūemi",

      confirm: "Tāpiri tikanga utu",

      online_methods: "Tikanga Ipurangi",
      cod_methods: "COD",
      bank_transfer: "Peeke Whakawhiti",
      cash_methods: "tūemi 💳 Utu (POS)",

      currency_step: {
        title: "tūemi he Moni",
        subtitle:
          "Whakamātauria ki te pupuri i te maha o nga moni i roto i to toa ki te iti rawa—he iti ake i te 3. Ko te nuinga o nga kuaha utu ka taea te whakahaere i nga whakawhitinga i roto i te tata ki nga moni katoa.",
        add_first_currency: {
          subtitle: "Tena koa taapirihia he moni ki to toa i mua i te haere.",
          title: "Tāpiri Moni",
        },
      },
      gateway_step: {
        title: "Hohe 💳 Utu tūemi",
        subtitle:
          "Tena koa whiriwhiri mai i nga tikanga utu e waatea ana. Ina tohua e koe he kuaha, ka taea e koe te whakauru i nga taipitopito hononga e tika ana hei hono tika i to toa ki to tikanga utu e pai ana koe.",
      },

      config_step: {
        import_config: "Ka taea e koe te kawemai i te whirihora mai i etahi atu kuaha rite",
        import_from: "tūemi Mai",
      },

      request_add_new_gateway: {
        title: "Kaore koe e kite i to tikanga utu pai?",
        subtitle:
          "Ki te kore to kaiwhakarato utu rohe i te raarangi, ohauru ki tetahi mahere a-tau (tae atu ki te Whakaoho mo te utu) ka tono kia whakauruhia. Ka oti i roto i nga haora 48 mena ka tukuna he kaute whakamatautau me nga tuhinga.",
      },

      inputs: {
        currency: {
          placeholder: "tūemi he Te kuwaha Ki tūemi...",
        },
        manual: {
          true_title: "tūemi aratau",
          true_description: "Ka taea e koe te whakarite a-ringa te whirihoranga kuaha.",

          false_title: "Hono aunoa",
          false_description:
            "Ka hono aunoa te punaha ki te keeti mena kua whirihorahia e koe he kuaha.",
        },
      },
    },
  },

  /** {@see FaqRequestsList} **/
  faq_list: {
    table: {
      nodata: "Karekau he patai i kitea.",
      more_data_action: "Ētahi atu pātai..",
      user: "kaiwhakamahi",
      tag: "Tūtohu",
      question: "Pātai",
      officer: "Kaimahi",
      answer: "Whakautu",
      created_at: "Rā",
      priority: "Ko te kaupapa matua",
      edit: "Whakatikatika",
    },
    dialog: {
      add_new_action: "Tukuna he patai hou",
      edit_action: "Whakatika FAQ",
      question_input: "Pātai",
      question_input_placeholder: "Uia to patai..",
      link_input: "Hononga",
      link_input_placeholder: "Ka taea e koe te korero ki tetahi hononga hei whakautu.",
      answer_input: "Whakautu",
      answer_input_placeholder: "Whakauruhia he kupu whakautu ..",
      tag_input: "Tūtohu (Taitara Kāwai)",
      tag_input_placeholder: "Whakauruhia he taitara poto",
      priority_input: "Matua: (0 ki te 10)",
    },
    delete_alert: {
      title: "Muku Pātai",
      message: "Me muku tenei patai?",
      action: "Mukua",
    },
    notifications: {
      send_success: "Kua tukuna pai to whakautu.",
      delete_success: "Kua pai te muku i to patai.",
    },
  },
  /** {@see ContactMessagesList} **/
  contact_messages_list: {
    load_more_action: "Ētahi atu karere ..",

    table: {
      nodata: "Karekau he patai i kitea.",
      user: "kaiwhakamahi",
      message: "Te tinana karere",
      category: "Kāwai",
      officer: "Kaimahi",
      answer: "Whakautu",
      created_at: "Rā",
      contact: "Whakapā",
      edit: "Whakatikatika",
    },

    dialog: {
      title: "Whakautu ki te karere",
      category: "Kāwai",
      user_name: "Ingoa",
      email: "Īmēra",
      phone: "Tau Whakapā",
      message: "Kuputuhi karere",
      status: "Tūnga Whakautu",
      status_answered: "Ka whakahoki",
      status_waiting: "Tatari Whakautu",
      answer_input: "Kupu whakautu",
      mail_input: "Tuku mēra",
      mail_input_message:
        "Mena ka taea e koe tenei, ka tukuna to whakautu ki te imeera kaiwhakamahi.",
    },
    delete_alert: {
      title: "Muku Karere",
      message: "Me muku tenei karere?",
      action: "Mukua",
    },
    notifications: {
      send_success: "Kua tutuki pai to tono.",
      delete_success: "Kua mukua te karere.",
    },
  },
  /** {@see BPageCampaign} **/
  campaign: {
    title: "Whakatairanga",
    menu: {
      setting: "Tautuhinga",
      return: "Whakamuri",
      dashboard: "Papatohu",
      links: "Hononga",
      landing: "Taunga",
      ads: "Pānui",
      finance: "Pūtea",
    },
    /** {@see BPageCampaignDashboard} **/
    dashboard: {
      title: "Papatohu",
      status: "Tirohanga mana",
      sessions: "Wātū",
      links_overview: "Tirohanga hononga whakatairanga",
      orders: "Nga ota",
      performance: "Te mahi whakatairanga",
      links_funnel: "Hononga kaupapa Mahinga",
      no_links:
        "Karekau he raraunga e waatea ana! Hangaia he hononga ki to kaupapa whakatairanga.",
      social: "Ka arahina e te hunga pāpāho pāpori",
      conversion_over_time: "Te hurihanga i te waa",
      total_campaign_sell: "Tapeke hoko ma te whakahau",
    },
    /** {@see BPageCampaignFinance} **/
    finance: {
      title: "Pūrongo pūtea",
      campaign_revenue: "Nga moni mo te whakatairanga",
      total_revenue:
        "Tapeke moni ma te kaupapa whakatairanga",
      links_revenue: "Nga hua o nga Hononga Kaupapa",
      payments_over_time: "Nga hokonga i runga i te waa",
      all_links: "Nga hononga katoa",
    },
    /** {@see BPageCampaignLanding} **/
    landing: {
      empty_message: "Tautuhia he wharangi taunga ki nga tautuhinga whakahau.",
      set_landing: "Tautuhia te wharangi taunga",
      create_landing: "Waihangahia te wharangi taunga hou",
    },

    /** {@see BPageCampaignLinks} **/
    links: {
      title: "Hononga whakatairanga",
      url_input_message: "Whakauruhia to url wharangi wharangi",
    },

    /** {@see BPageCampaignSetting} **/
    setting: {
      menu: {
        config: "Whirihora",
        timespan: "Wā wā",
        banner: "kara",
        Notification: "Panuitanga",
      },

      general_setting: {
        title: "Tautuhinga Whakatairanga Ahunui",
        sub_title: "Ingoa, Whārangi Taunga me te Waehere Whakatau",

        name_input: "Ingoa Campaign",
        name_input_hint: "Taapirihia he Ingoa ki to Whakatairanga",
        enable_input: "Kei te kaha te kaupapa whakahau?",
        page_input: "Whārangi Taunga",
        discount_code_input: "Waehere hekenga",
        limit: "Tepe utu",
        max_count: "Tatau morahi",
        max_count_unit: "Nga taonga",
        start: "Tīmata",
        end: "Whakamutunga",
        notification_enable_input: "Kua whakahohea te whakamohiotanga?",

        landing_page: "Whārangi Taunga",
        discount_code: "Waehere hekenga",

        auto: "Whakatairanga taunoa",
        auto_message:
          "Kua whakahohehia te pakanga taunoa i to toa",
      },
      time_setting: {
        title: "Wā wā",
        sub_title:
          "Rā Tīmata & Wā / Rā Whakamutunga & Wā",
        start_input: "Tīmata",
        start_input_placeholder: "Tīpakohia te Wā Tīmata Kaupapa",
        end_input: "Whakamutunga",
        end_input_placeholder: "Tīpakohia te Wa Whakamutunga Kaupapa",
      },
      banner_setting: {
        title: "Panui Toa",
        sub_title:
          "Whakaatu ki runga o te wharangi toa",

        message_input: "Kuputuhi karere",
        message_input_placeholder: "Huihuinga Motuhake ...",
        message_input_hint: "Panui panui panui ki runga o te toa",
        bg_image_input: "URL whakaahua papamuri",
        bg_image_input_hint: "URL whakaahua papamuri katoa",
        repeat_pattern: "Tukurua te tauira",
        simple: "Maamaa",
        dark_input: "Kaupapa pouri?",
        icon_input: "Tohu",
        icon_input_hint:
          "Ingoa Ata Hoahoa Rawa, Fontawsom ranei: fa:fas fa-star Material Design",
        anim_input: "Wāhitau Hākoritanga",
        anim_input_message: "URL kōnae hākoritanga Lottie",
        preview: "Arokite",
        preview_update_action: "Whakahōu arokite",
        bg_color: "Tae papamuri",
        icon_bg: "Tae papamuri ata",
      },
      notifications: {
        title: "Panuitanga",
        sub_title: "Waihangahia he panui ka hono atu ki to kaupapa whakatairanga.",
        update_success: "Kua whakahoutia to kaupapa.",
        action_button: "Pātene mahi",
        background_color: "Tae papamuri",
        text_color: "Tae kupu",
        action_color: "Tae mahi",
        action_text_color: "Tae kuputuhi mahi",
        notification_image: "Whakaahua whakamohiotanga",
        notification_bg: "Papamuri whakamohiotanga",
        show_preview: "Whakaatu arokite",
        persistence: "Te manawanui",
        persistence_message:
          "Kei te pirangi koe ki te whakaatu i nga wa katoa ka toro atu to kaiwhakamahi ki te paetukutuku",
        radius: "Rorohiko",
        show_delay: "Whakaatuhia te roa",
      },
    },
  },

  /** {@see BCategoryAdd} **/
  add_category: {
    title_edit: "Whakatika kāwai",
    title_update: "Tāpiri kāwai hōu",

    config: {
      subtitle:
        "Ka awhina nga waahanga ki te whakamaarama i te whakahaeretanga o nga hua me te ngawari ki o kaihoko ki te rapu hua i to toa.",
    },
    parent: {
      subtitle:
        "Ka puta tenei waahanga i raro i tana waahanga matua. Mena kaore he waahanga matua o tetahi hua, ka whakaatuhia ki te waahanga matua.",
    },
    template: {
      title: "tauira",
      edit_page: "Whakatikatika whārangi",
      Pages_list: "whārangi Rarangi",
      subtitle:
        "Ka taea e koe te whakarei ake i te whakaaturanga o to wharangi waahanga ma te whakakikorua i tetahi wharangi ritenga. Ko te huarahi tino pai ko te hanga wharangi me nga papamuri marama me te hoahoa 1 ki te 2 nga waahanga motuhake mo ia waahanga. Ka taea e koe te hono i tetahi wharangi ki nga waahanga maha, ma te whakamahi i nga ihirangi hihiri ki te whakarite i ia waahanga takitahi.",
    },
    critical_zone: {
      title: "Rohe Tino",
      subtitle:
        "Mena ka tangohia e koe tetahi waahanga, ka nekehia ona waahanga iti me ona hua ki te raarangi matua<b>📁 {parent}</b>. Hei whakahaere i nga waahanga kaore i roto i te rarangi hua, haere ki te Hoko > Nga waahanga ripa.",
      accept_delete: {
        true_description: "I tūemi Ki Tango tūemi Kāwai.",
        true_title: "tūemi Tango Kāwai",
      },
    },

    menu: {
      delete: "Muku kāwai",
      filter: "Tātari",
      settings: "Tautuhinga",
      back: "Whakamuri",
    },
    title_input: "Taitara Kāwai *",
    title_input_error: "Tīpakohia he Taitara Kāwai",
    name_input: "Ingoa kāwai (Ingarihi) *",
    name_input_error: "Tīpakohia he ingoa ahurei mo te kāwai",
    description_input: "Whakaahuatanga waahanga",
    description_input_message: "Whakauruhia he whakaahuatanga poto mo te waahanga..",
    parent_input: "Te waahanga matua",
    parent_input_message: "Ka puta tenei waahanga ki te wharangi kaainga toa.",
    image_input: "Whakaahua kāwai",
    image_input_placeholder: "Tīpakohia he whakaahua mo te kāwai",
    edit_action: "Whakatika kāwai",
    add_action: "Tāpiri Kāwai",
    back_action: "Whakamuri",
    no_category: "Kore he waahanga",
    star: "Whetu",

    filter: {
      title: "Ngā tātari kāwai",
      sub_title: "Whakahaerehia nga whiriwhiringa me nga waahanga e pa ana ki nga hua o tenei waahanga.",
      message:
        "Whakauruhia ka tiakina te taitara ahuatanga hangarau noa mo tenei waahanga ki te pouaka kei raro nei. Whai muri i te penapena i te raarangi, ka whakaatuhia nga waahanga tātari me nga ihirangi o ia waahanga. Ko te tae, te taumaha, te rahi, te waitohu, me etahi atu ka tangohia aunoa mai i nga momo hua.",
    },
    delete_alert: {
      title: "Tango Kāwai",
      message: "Me muku te kāwai {title}?",
      action: "Mukua",
    },
    notifications: {
      add_success: "Kua taapirihia te waahanga hou.",
      edit_success: "Kua whakatikahia te waahanga.",
      delete_success: "Kua mukua {title}.",
    },
    /** {@see BCategoryFilterEditor} **/
    edit_filter: {
      spec_input: "Rarangi Whakaaturanga Hangarau",

      price: "utu",
      min: "tūemi",
      max: "tūemi",
      suggested_list: {
        brand: "Waitohu",
        colors: "Tae",
        styles: "Hoahoa",
        volumes: "Rōrahi",
        weights: "Taumaha",
        packs: "Tau mo ia kete",
        types: "Momo",
      },
      notifications: {
        edit_success: "Kua whakatikahia te waahanga.",
      },
      manually_update_filters_tips:
        "<b>He mea nui!</b> Ka whakahōu aunoatia ngā tātari ina huri ngā hua. Heoi, i muri i te whakarereketanga o nga hua i roto i tetahi waahanga, tena koa hoki mai ki konei ka paato i te paatene tiaki/whakahou hei whakamau i te tātari.",

      inputs: {
        spec: {
          message:
            "Te rarangi o nga ahuatanga e hiahia ana koe kia whakaatuhia ki te waahanga tātari",
          placeholder: "Tīpakohia nga tohu hei whakaatu ki te tātari...",
        },
      },
    },
  },

  /** {@see BAppShopList} **/
  apps_list: {
    nodata: "Karekau he taupānga i whakauruhia ki to toa",
    inactive: "hohekore",
    my_apps: "Aku taupānga",
    app_store: "Tāpiri taupānga hōu",
  },

  /** {@see BShopAdd} **/
  add_shop: {
    menu: {
      shop_title: "taitara toa",
      shop_name: "Ingoa toa",
      description: "Whakaahuatanga",
      contact: "Nga korero Whakapā",
      image: "Whakaahua",
    },
    title_input: "Taitara",
    title_input_tips: "Whakauruhia te ingoa o to toa, waitohu, kamupene ranei.",
    active_message: "Whakaritea kia kaha ki nga tangata katoa ka uru ki to paetukutuku.",
    language_input_message: "Tīpakohia tō reo toa.",
    name_input: "Ingoa",
    name_input_message: "Whakauruhia he ingoa mo to toa.",
    name_input_tips:
      "Kaua e whakamahia nga tohu reo Ingarihi, nga waahi me nga reta, ki te whakamahia, kare pea te ingoa toa e whakaatu tika.",
    description_input: "Whakaahuatanga toa",
    description_input_message: "Tuhia he korero poto mo to pakihi.",
    description_input_tips:
      "Ka puta tenei whakaahuatanga ki te hiku o to toa ka whakamahia ano hei tohu tohu tohu taunoa mo o wharangi toa.",
    address_input: "Wāhitau Toa",
    address_input_message: "Whakauruhia te wahitau waahi toa tika",

    phone_input: "nama waea",
    phone_input_message: "Tau Whakapā Kiritaki",
    phone_input_tips:
      "Whakauruhia te nama whakapā me te waehere whenua. Ka whakamahia hoki tenei tau ki te whakaatu korero i roto i te rapunga a Google.",
    email_input: "Īmēra",
    email_input_message: "Wāhitau imeera mo nga kaihoko",
    locality_input: "Tauwāhi",
    locality_input_message: "Tauwāhi toa",
    region_input: "State, Porowini ranei",
    region_input_message: "Te kawanatanga, te kawanatanga ranei kei hea koe e whakahaere ana",
    country_input: "Whenua",
    country_input_message: "Whenua pakihi",
    postal_code_input: "Waehere Poutāpeta",
    postal_code_input_message: "Waehere Poutāpeta Toa",
    image_input: "Rokiroki Whakaahua",

    map: {
      title: "Wāhitau Toa",
      confirm: "Rēhita wāhi noho",
      address: "wāhi noho",
      receptor: "toa",
    },

    create_shop_action: "Waihanga Toa",
    update_shop_action: "Whakahou",

    notifications: {
      copy_success: "He kape o te wahitau toa.",
      name_available: "Kei te waatea tenei ingoa ({name}).",
      add_success: "Toa {title} hanga.",
      edit_success: "Kua whakatikahia te toa.",
    },
  },

  /** {@see BProductInventoryManagementVirtual} **/
  virtual_items_list: {
    add_virtual_item: "Tāpiri tūemi mariko",
    add_virtual_item_desc:
      "Hei hoko i nga hua mariko, me whakauru e koe ki tenei waahanga. He rite te maha o nga taonga mariko ki te maha o nga taonga kore utu i tenei waahanga. Ina hokona e nga kaihoko te taonga mariko, ka tukuna atu te hono ki a raatau.",

    filter_action: "Hoatu tātari",
    not_processed: "Nga Ota Kaore i Tukatuka",
    title: "Rarangi o nga mea",
    vew_order: "Tirohia te raupapa",

    table: {
      nodata: "Karekau he tuemi",
      product: "Hua",
      data: "Raraunga",
      status: "Tūnga",
      user_message: "Karere kaiwhakamahi",
      updated_at: "Whakahoutanga whakamutunga",
      order: "Ota",
      delete: "Mukua",
    },
    notifications: {
      delete_success: "I tutuki pai te mukunga.",
    },
  },

  /** {@see BProductVirtualItemAdd} **/
  add_virtual_item: {
    title_edit: "Whakatika uara",
    title_add: "Tāpiri tūemi ki te hua",
    sub_title: "Te Waihanga me te Whakatika i nga Hua Mariko",
    message:
      "Ka taea e te kaiwhakahaere toa te whakaatu ki te kaihoko te uara i muri i te utu. Me whakauru enei uara ki te puka kei raro nei. Ko te hanganga o tenei puka kua tautuhia ki te waahanga Whakatika Tuemi> Whakatikaina Nga Korero Putanga.",
    edit_outputs: "Whakatika puka whakaputa",
    notifications: {
      edit_success: "Kua whakatikahia te taonga.",
      add_success: "He mea taapiri ki te rarangi.",
    },
  },

  /** {@see BPageOrderVirtual} **/
  virtual_process_center: {
    title: "Pokapū Tukatuka",
    sub_title: "Tukatuka Tono",
    virtual_product: "hua mariko",
    menu: {
      report: "Ripoata",
      timeline: "Rarangi Wā",
      order: "Ota",
    },

    /** {@see BOrderVirtualItem} **/
    outputs_widget: {
      title: "Puka putanga",
      table: {
        title: "Taitara",
        value: "Uara",
      },
      tips: [
        "I muri i te whakaotinga o te tono a te kaihoko, paatohia te kii whakapumau ota hei whakamohio atu ki te kaihoko o te ota.",
        "Ina whakaaetia te ota, kaore e taea e te kaihoko te whakarereke i nga korero whakauru.",
      ],
      alerts: {
        order_completed_by_admin: "Kua oti te ota.",
        order_completed_by_buyer: "Kua oti te ota e te kaihoko.",
        waiting_for_buyer: "Kei te tatari mo te whakaaetanga a te kaihoko ..",
      },
    },
  },

  /** {@see BPageOrderFile} **/
  file_process_center: {
    title: "Pokapū Tukatuka",
    sub_title: "Tukatuka Tono",
    menu: {
      report: "Ripoata",
      timeline: "Rarangi Wā",
      order: "Ota",
    },
  },

  /** {@see BOrderDashboard} **/
  process_order_page_dashboard: {
    title: "Tukatuka Tono",
    cancel_order_action: "Whakakore ota",
    open_basket_cation:
      "Kei te tuwhera tenei kaata hokohoko, ko te tikanga kei te hoko te kaihoko kaore ano kia utua te ota.",

    reject_dialog: {
      title: "Take ki te whakakore i te Ota",
      message:
        "Tīpakohia te take mo te whakakore i tenei ota mai i te rarangi i raro nei. Me mohio ki te korero ki te kaihoko mo te whakakore me te tukanga whakahoki. Me whakahoki nga moni i roto i nga haora 12.",
      confirm_now_action: "Whakakore 🛍️ Ota tūemi",
      confirm_in48h_action: "Whakakore 🛍️ Ota i 48 tūemi",

      dismiss_reject_action: "Whakakorehia te ota whakakore",

      options: {
        title: "tūemi",
        subtitle:
          "Mena kei te hiahia koe ki te whakakore i to ota tonu, ka taea e koe te whakamahi i tenei waahanga.",
      },

      express_input: {
        true_title: "tūemi | Whakakore 🛍️ Ota tūemi",
        false_title: "Paerewa | Whakakore Ota I muri i nga haora 48",
        true_description:
          "Ma tenei whiringa ka whakakore tonu i to ota ka whakahou tonu i te raarangi.",
        false_description:
          "Ma tenei whiringa ka whakakore i to ota i muri i nga haora 48. Kia tae ra ano, ka rahuitia nga taonga kaore e taea te hoko e etahi atu.",
      },
    },

    notifications: {
      update_status_success: "Kua whakahoutia te mana ota.",
      reject_update_success:
        "Kua whakahoutia te mana whakakore ota.",
      dismiss_reject_success:
        "I hoki mai te ota ki te tikanga noa.",
      set_tracking_success: "Kua tiakina paitia te ID Track ID.",
      shipping_address_update_success:
        "Kua pai te whakahou i te wahitau tuku.",
      vendor_order_status_update_success:
        "Kua whakahōungia te tūnga tutukinga kaihoko.",
    },
  },
  /** {@see BPageOrderPhysical} **/
  /** {@see BPageOrderVirtual} **/
  /** {@see BPageOrderFile} **/
  /** {@see BPageOrderService} **/
  process_center: {
    title: "Pokapū Tukatuka",

    physical_product: "Hua tinana",
    menu: {
      label: "Tapanga mōkī",
      receipt: "nama",
      back: "Whakamuri",
      return: "Hoki mai",
      report: "Ripoata",
      timeline: "Rarangi Wā",
      inputs: "Nga whakaurunga",
      order: "Ota",
    },

    /** {@see OrderPaymentWidget} **/
    payment_widget: {
      title: "Utu",
      checklist: [
        "Tirohia te nui o te ota me te utu a te kaihoko.",
        "Tirohia te wahitau nama nama.",
        "Me whakarite he tika nga korero kua whakauruhia, ina koa ko te Utu Utu i te Tukunga.",
      ],
      action_cod_pay: "Whakaaetia te utu ma te kaihoko",
      action_dir_pay: "Whakaūhia te rihīti utu tukuake",
      payment_receipt_not_uploaded_error:
        "Ko te rihīti utu kaore ano kia tukuake!",

      waiting_for_pay: "Tarewa utu",
      processing_payment: "Tukatuka utu",
      processing_cancel: "Kua whakakorehia te utu",
      require_capture: "Me hopu",
      paid: "Ka utua",
      cod_pay_info:
        "Ina whiwhi koe i te ota mai i te kaihoko ka taea e koe te whakau i te utu me te whakaoti i te ota.",
      dir_pay_info:
        "Ka puta te rīhiti utu ki runga. I mua i to whakaū i te utu, manatokohia te moni putunga me nga korero rihīti ki to peeke.",

      created_date: "Te Ra Hanga",
      pay_date: "Te Ra Utu",
      payment_gateway: "Te kuwaha utu",
      payment_method: "Tikanga utu",
      risk: "Uara morearea",
      billing_details: "Nga korero mo te nama nama",

      add_transaction: "Tāpiri pire hōu",
      add_transactions_message:
        "Me hanga e koe kia kotahi tauwhitinga mo tenei ota. I etahi wa, ka hiahiatia te utu i roto i nga waahanga maha. Ko nga tauwhitinga me utu e te kaiwhakamahi ka whakatauhia e koe, i te wa e mahi ana.",

      add_bill_dialog: {
        title: "Tāpiri nama",
        message:
          "Ka taea e koe te taapiri i tetahi nama hou mo tenei ota. Ka kite te kaihoko i enei nama me te utu ia ia i runga i te ahunga whakamua o te mahi.",
        amount_input: "Te nama nama",
        action: "Tāpiri pire",
      },
      edit_bill_dialog: {
        title: "Whakatika nama",
        message: "Ka taea e koe te whakatika i nga nama tarewa.",
        action: "Whakatika nama",
      },
      bill_cash_payment_dialog: {
        title: "Rēhita utu putea",
        message:
          "Mena kua utua e te kaihoko te nui o tenei nama i roto i te putea, i tetahi huarahi ke atu i te utu ipurangi, i tenei waahanga, ma te whiriwhiri i tetahi waahanga, rēhita me te whakaū i tana utu. I tenei waahanga, ka taea e koe te kite noa i nga keeti utu moni o to toa,Ki te kore koe e kite i tera momo whiringa, me whakahohe e koe he keeti utu moni i roto i nga moni kua tohua i to toa.",
        action: "Whakaū te utu pire",
      },

      table: {
        title: "Taitara",
        amount: "Te nui",
        description: "Whakaahuatanga",
        info: "Nga korero",
      },
      user_payment: "Utu kaihoko",
      cod: "Moni i te Tukunga",
      gateway_wage: "Utu Fee",
      gift_card: "Kaari koha",
      discount_code: {
        title: "Waehere hekenga",
        code: "Waehere",
        percent: "ōrau",
        limit: "Tepe",
      },
      club: {
        title: "Karapu Kiritaki",
        percent: "ōrau",
        limit: "Tepe",
      },

      coupon: {
        title: "tīkitipati",
        percent: "ōrau",
        limit: "Tepe",
        charge: "Te utu",
      },
      delivery: {
        title: "Utu kaipuke",
        not_paid: "Kaore i utua",
      },
      samin: {
        title: "Utu Ratonga Fee",
        description:
          "Kua tangohia tenei moni mai i to putea putea Selldone.",
        tooltip: "Tatauhia ma te tikanga utu Fee",
        percent: "ōrau",
      },
      affiliate: {
        title: "Komihana hononga",
        affiliate_message:
          "Me utu e koe tenei komihana ki to hononga. Ko koe tenei mahi.",
      },
      total_price: "Utu Tapeke Utu",
      total_items_discount: "Hekenga Tuemi",
      total_price_after_offers: "Tapeke i muri i te tono tuku",

      total_order_price_before_tax: "Utu Tapeke i mua i te taake",

      user_payment_amount: "Te utu utu a te kaihoko",
      samin_wage: "Utu Selldone Fee",
      pay_cod_dialog: {
        title: "I whiwhi koe i nga moni ota?",
        message:
          "Whakapumautia tenei whiringa i roto i te moni i runga i te tikanga utu tuku, i muri i te whakarite utu katoa na te kaihoko.",
        confirm_action: "Whakaaetia te utu a te kaihoko",
      },
      pay_dir_dialog: {
        title: "I whiwhi koe i nga moni ota?",
        message:
          "Whakaaetia tenei whiringa i muri i to whakaaetanga i te rihīti utu kua tukuna me to peeke. Kia mohio koe ki te tirotiro i te utu me to putea.",
        confirm_action: "Whakaaetia te utu a te kaihoko",
      },

      notifications: {
        pay_confirm_message:
          "Ka whakamanahia te utu.<br> Tauranga: {gateway_name}<br> Rā: {payment_at}",
        pay_fail_title: "Kaore i utua",
        pay_fail_message: "Kaore e whakaaetia te utu!",
      },
    },
    /** {@see BOrderDashboardCart} **/
    basket_list: {
      checklist: [
        "Tirohia te rarangi pukapuka.",
        "Mena kaore i te waatea te taonga, me whakahoki te moni i utua mo taua mea ki te kaihoko.",
      ],
      message:
        "Mena karekau etahi o nga taonga, kaore ranei e taea te hoko, wetewetehia te pouaka kei te taha ka paato i te paatene <b>Whakapumau Ota</b> .",
      action: "Whakaaetanga Whakatau",
      list_of_items: "Rarangi Kete Hoko",
      apply_change_action: "Hoatu Huringa",
      cant_apply_change: "Kaore e taea te whakarereke",
      lottery_prize_inform:
        "Ka wikitoria to kaihoko 1x o tenei hua. Tuku koha me tenei ota.",

      need_to_refund: {
        title: "Ko te utu e hiahiatia ana ki te kaihoko",
        message:
          "Ka tatauhia e matou te tapeke o nga moni whakahoki e hiahia ana koe ki te mahi a-ringa. I tatauhia i runga i nga taonga kaore i te waatea me te katoa o nga uara kua whakahokia.",
        calculated_approximately:
          "Ka tatauhia te nui. Tena koa tatauhia te moni tika.",
      },
      need_payback: {
        title: "kiritaki Te utu tūemi",
        message:
          "Kua kite matou he nui ake te utu i whakahokia ki te kaihoko i te utu e tika ana i runga i nga taonga kei roto i te kaata. Ka hiahia pea koe ki te utu i to kaihoko ki te whakahoki i te moni taapiri.",
      },
      need_no_refund: {
        title: "Kaore he moni whakahoki",
      },
      total_refund: {
        title: "Tapeke tūemi",
        message:
          "Ko te tapeke o nga moni whakahoki ka mahia aunoatia e to keeti utu. Ko etahi ratonga utu anake e tautoko ana i tenei ahuatanga, no reira me whakahoki a ringa koe.",
      },
    },
    /** {@see BOrderDashboardPreparing} **/
    preparing: {
      title: "Whakaritenga",
      packing_weight: "Taumaha Tapeke",
      delivery_type: "Tikanga tuku",
      packing_size: "Te Ahu Puka",
      distance_warning: "Ka hipa atu i te rohe tawhiti e whakaaetia ana.",
      weight_warning: "He nui ake i te taumaha e whakaaetia ana.",
      volume_warning: "He nui ake i te rahi o te kete e whakaaetia ana.",
      items_weight_calculation:
        "tūemi Taumaha o katoa Nga taonga to {weight} {unit}",
      not_available_message:
        "Kaore i te waatea i te wa e haua ana koe ki te tono mo te wahitau kaipuke me te kore hoki i whakakiia e te kaiwhakamahi te wahitau i muri i te hoko!",

      message:
        "<b>Patohia</b> te paatene Whakataa E whai ake nei mena kua reri te ota ki te tuku.",
      action: "Kua reri te ota ki te tuku",
      checklist: [
        "Te whai i nga ture paerewa i roto i te kohinga.",
        "Tāngia he rārangi tūemi ka hoatu ki roto i te mōkihi.",
        "Tāngia ka whakapirihia he tapanga ki te pouaka.",
      ],
    },

    basket_items: "Rarangi ota taonga",

    delivered_dialog: {
      title: "Kua riro mai tenei ota e te kaihoko?",
      message: "Whakapumautia tenei whiringa ina mohio koe kua riro i te kaihoko te ota",
      confirm_action: "Ka whakapumautia e au kua riro i te kaihoko te kete",
    },
    return_delivery_dialog: {
      title: "I raru te ota i te wa e tukuna ana, i whakahokia mai ranei?",
      message:
        "Whakapumautia te whiringa ki te whakahoki mai i te tuku mena ka whakahokia mai te ota i muri i te tuku mo etahi take me te hiahia kia tukuna ano te tuku.",
      confirm_action: "Whakapumautia Kua whakahokia mai te tuku",
    },

    notifications: {
      update_status_success: "Kua whakahoutia te mana ota.",
      reject_update_success: "Kua whakahoutia te mana whakakore ota.",
      dismiss_reject_success: "I whakakorehia te ota.",
      pay_cod_success: "Ka whakamanahia te utu mo te tuku.",
      pay_dir_success:
        "I whakamanahia te utu ma te utu kaari-ki-kaari, whakawhiti moni ranei.",
      set_tracking_success: "Kua tiakina paitia te ID Track ID.",
      new_bill_success: "I angitu te taapiri pire hou.",
      edit_bill_success: "I angitu te whakahou i te Pire.",
      update_bill_status_success: "I angitu te whakahou i te mana pire.",
      set_payment_in_cash_success: "Ko te utu pire kua whakamanahia ma te ringa.",
    },

    delete_bill_dialog: {
      title: "Mukua te pire",
      message: "Me muku koe i tenei pire?",
      action: "Ae, Muku Inaianei",
    },
  },

  /** {@see AdminShopGatewayManagementPage} **/
  shop_gateway: {
    title: "Te kuwaha",
    menu: {
      dashboard: "Papatohu",
      edit: "Whakatikatika",
      transactions: "Nga tauwhitinga",
      back: "Nga kuwaha",
    },

    /** {@see BPageGatewayDashboard} **/
    dashboard: {
      title: "Te Whakahaere o te Gateway",

      /** {@see BGatewayPaymentsChart} **/
      gateway_data_widget: {
        title: "Ripoata Utu",
        subtitle:
          "I hangaia nga utu me nga tauwhitinga i mahia i roto i te waa kua tohua.",
        chart: {
          times: "Tatau",
          buys: "Hoko tatau",
          pays: "Utu angitu",
          amount_pay: "Ka utua",
          amount_wage: "Utu Fee",
        },
      },
    },

    /** {@see BPageGatewayEdit} **/
    edit: {
      title: "Whakatika Waahi",
      menu: {
        delete: "Tango Gateway",
      },
      edit_action: "Whakatika Waahi",
    },

    /** {@see BPageGatewayTransactions} **/
    transactions: {
      title: "Nga tauwhitinga",
      table: {
        amount: "Te nui",
        currency: "Moni",
        livemode: "Momo",
        status: "Tūnga",
        target_id: "Ota",
        risk: "Morearea",
        card: "Kaari",
        issued_at: "Te Ra Putanga",
        payment_at: "Te Ra Utu",
      },
      live_mode: "Ora",
      test_mode: "Pouaka kirikiri",
    },
  },

  /** {@see BGatewayOverview} **/
  b_gateway_overview: {
    title: "Nga korero",
    subtitle: "Tirohanga o 💳 Utu Te kuwaha tūemi ka Tūnga.",
    status: "Tūnga",
    cod_transportation: "tūemi Te tuku",
    cod_transportations_error:
      "Whakahohehia te moni mo te utu tuku mo te iti rawa kia kotahi tikanga tuku!",
    payment_type: "💳 Utu Momo",
    cod: "COD",
    online: "tūemi 💳 Utu",
    wage_percent: "Fee ōrau",
    min_wage: "tūemi Fee",
    max_wage: "tūemi Fee",
    min_amount: "tūemi Tauwhitinga",
    max_amount: "tūemi Tauwhitinga",
    timeout: "💳 Utu wā",
    timeout_unit: "tūemi",
    link: "Te kuwaha Hononga",
    your_gateway_status: "to Te kuwaha Tūnga",
    oauth2_button: "tūemi Connect Ki {gateway}",
    fee_payment_message:
      "Ko te moni katoa ka utua e te kaihoko ka whakawhitia ki to putea. Ko te utu ratonga ka tangohia mai i to putea i Seldone.",

    auto_config: "tūemi Whirihora",

    keys: {
      title: "Tūmataiti tūemi",
      subtitle:
        "Rārangi o ngā kī tūmataiti mō te utu. He mea nui enei korero mo te hanga me te whakahaere utu.",
    },
    extra_config: {
      title: "💳 Utu te tūemi",
      subtitle: "Nga korero whirihoranga taapiri mo te kuaha utu.",
    },

    recommended_actions: {
      title: "tūemi Nga Mahi",
      subtitle:
        "Ma te tango i enei mahi, ka timata koe ki te whiwhi utu tuturu mai i o kaihoko.",
      action_enable: "tūemi Ki Whakahohe tūemi",
      make_live_enable: "tūemi Ki Ora tūemi",
    },
  },

  /** {@see BGatewayAdd} **/
  edit_gateway: {
    public_keys: "Ngā Kī Tūmatanui",
    private_keys: "Kī tūmataiti (matatapu)",
    link: "paetukutuku",
    status_title: "💳 Utu te Tūnga",
    status_message:
      "Whirihorahia te tikanga utu hei Hohe, Hohekore ranei. Ko nga tikanga kua whakaritea hei hohe ka whakaatuhia ki nga kaihoko.",
    status_true_description: "Ka taea e o kaihoko te utu ma tenei tikanga utu.",

    manual_title: "Utu a-ringa / hopu aunoa",
    manual_message:
      "Ina hanga utu koe, ka taea e koe te pupuri i tetahi tikanga utu tika hei rahui i nga moni ka taea e koe te hopu i muri mai. Hei tauira, he maha nga wa ka whakamanahia e nga hotera he utu katoa i mua i te taenga mai o te manuhiri, katahi ka hopu i te moni ka tirohia e te manuhiri.",

    gateway_status: "Tūnga",
    wallet: "Putea Poraka",
    developer_title: "Aratau kaiwhakawhanake",
    developer_message:
      "Ka taea e koe te whakahaere i nga whakawhitinga rūpahu mena ka tautokohia e te kuaha te aratau patuiro.",
    live_mode_message: "Ko nga whakawhitinga katoa ka noho ki te aratau ora.",

    notifications: {
      edit_success: "Gateway {gateway_name} kua whakatika.",
      success_delete: "Kua mukua te tikanga utu!",
    },
    debug: {
      title: "Whakawhanake & tūemi",
      subtitle: "Ka taea e koe te whakaahei i te aratau patuiro hei whakamatautau i te kuaha utu.",
    },

    limit: {
      title: "🛍️ Ota Tepe",
      subtitle: "Tautuhia te rahinga ota iti mo tenei kuaha utu.",
    },
    limit_input: {
      title: "tūemi 🛍️ Ota Te nui",
      msg: "Kore te tikanga kaore he rohe.",
    },
    currency_input: {
      message: "Ko te moni e tautokohia ana e tenei kuaha utu.",
    },
    gateway_code: "Te kuwaha Waehere",

    hold_only_for: "tūemi 💳 Utu tūemi Kei te waatea mō",
    manual_input: {
      false_description:
        "Ka huri aunoatia te mana utu mai i te tarewa ki te whakamana i roto i te punaha kaiwhakarato utu.",
      false_title: "tūemi 💳 Utu tūemi",
      true_title: "Purihia nga utu me te whakamana a-ringa",
      true_description:
        "Purihia he utu ka hiahia koe ki te manatoko i nga utu mo tetahi ota hei whakarereke i o raatau mana mai i te tatari ki te whakamana i roto i te punaha kaiwhakarato utu.",
    },
    delete_verify_input: {
      true_title: "E hiahia ana ahau ki te tango i tenei tikanga utu.",
      true_description:
        "Ka tangohia tenei tikanga utu mai i te toa ka kore e waatea ki nga kaihoko.",
    },
    delete_alert: {
      title: "Mukua 💳 Utu tūemi",
      message: "Me tino muku koe i tenei tikanga utu mai i to toa?",
      action: "Mukua 💳 Utu tūemi",
    },
  },

  /** {@see BPageGiftcard} **/
  gift_card: {
    title: "Kaari Koha",
    menu: {
      edit: "Whakatikatika",
      new_card: "Waihanga kāri",
      list: "Kaari",
      orders: "Nga ota",
      dashboard: "Papatohu",
      back: "Kaari Koha",
    },

    /** {@see AdminGiftCardDashboardPage} **/
    dashboard: {
      title: "Papatohu Whakahaere Kaari Koha",

      /** {@see BGiftcardStatisticsPayment} **/
      amount_widget: {
        title: "Utu Kaari",
        title_small: "30 ra",
        chart_amount_pay: {
          y: "Huihuinga",
          label: "Utu",
        },
        chart_buys: {
          y: "Ka hoko",
          label: "Ka hoko",
        },
        total_buy: "Tapeke moni hoko",
        total_pay_30days: "30 ra te utu katoa",
        total_pay_today: "Te utu o tenei ra",
      },

      /** {@see BGiftcardStatisticsCount} **/
      count_widget: {
        title: "Te maha o nga kaari kua tukuna",
        title_small: "30 ra",
        total_cards: "Te maha o nga kaari",
        used_cards_30days: "30 ra te whakamahi",
        used_today: "Te Whakamahinga o tenei ra",
        chart: {
          new_cards: "Kaari hou",
          used_cards: "Ko nga kaari kua whakamahia",
          new_card_name: "Kaari hou",
          used_card_name: "Kua whakamahia",
        },
      },

      /** {@see BGiftcardExport} **/
      export: {
        title: "Tikina te kōnae Huaputa",
        cards_list: "Tikina he rarangi kaari",
        get_report: "Tikina Ripoata",
      },
    }, // End of dashboard

    /** {@see AdminGiftCardManagementPage} **/
    cards: {
      title: "Rarangi o nga kaari kua tukuna",
      subtitle:
        "Tirohia me te whakahaere i nga kaari katoa kua tukuna. Ka taea e koe te tuku kaari ki nga kaiwhakamahi me te aukati ranei ina hiahiatia.",

      loading_message: "Kei te whiwhi korero ... tatari koa",
      issue_to_user: "Putanga ki te kaiwhakamahi",
      table: {
        card_number: "Tau Kaari",
        balance: "Taurite",
        paid: "Ka utua",
        edit: "Whakatikatika",
        delete: "Mukua",
        expire: "Ka pau",
        user: "kaiwhakamahi",
      },
      delete_alert: {
        title: "Tangohia te kaari",
        message: "Kei te tino hiahia koe ki te muku i tenei kaari?",
        action: "Mukua",
      },
      notifications: {
        delete_success: "Kua mukua te kaari koha.",
        edit_success: "Kua whakarereketia.",
        issue_success: "Kua tukuna pai te kaari koha ki te kaiwhakamahi.",
      },
      issue_dialog: {
        title: "Tukuna he kaari koha",
        action: "Whakaū ahau. Tukuna te kaari inaianei",
      },
    }, // End of cards

    /** {@see BGiftcardAdd} **/
    gift_card_edit: {
      title: "Waihanga Kaari Koha",
      validity: "Te whaimana",
      validity_unit: "Marama",
      currency: "Moni",
      initial_balance: "toenga kāri tuatahi",
      count: "Tatau",
      count_unit: "(ng) tūemi",
      enable_input_message: "Te mana o nga kaari i hangaia",
      notifications: {
        create_success_title: "Tiaki",
        create_success: "{count} nga kaari hou i hangaia",
      },
    },

    /** {@see AdminGiftCardEditPage} **/
    gift_card_edit_page: {
      title: "Whakatika Kaari Koha",
    },
  },

  /** {@see BPageDiscountCode} **/
  shop_discount: {
    title: "Waehere hekenga",
    up_to: "ki runga ki",
    menu: {
      dashboard: "Papatohu",
      orders: "Nga ota",
      back: "Waehere hekenga",
    },

    /** {@see BPageDiscountCodeDashboard} **/
    dashboard: {
      title: "Papatohu Whakahaere Waehere hekenga",
      chart_amount_buy: {
        y: "Te moni hoko {currency}",
        label: "Hoko",
      },
      chart_amount_discount: {
        y: "Heke {currency}",
        label: "Te utu",
      },
      chart_used: {
        y: "Te nui o nga whakamahinga",
        label: "Nga taima",
      },
    },
  },

  /** {@see BPageCoupon} **/
  shop_coupon: {
    title: "Tikitipati",
    up_to: "ki runga ki",
    menu: {
      dashboard: "Papatohu",
      back: "Tikitipati",
      orders: "Nga ota",
    },

    /** {@see BPageCouponDashboard} **/
    dashboard: {
      title: "Papatohu Whakahaere Tiikiti",
      chart_amount_buy: {
        y: "Te moni hoko {currency}",
        label: "Hoko",
      },
      chart_amount_discount: {
        y: "Tikitipati {currency}",
        label: "tīkitipati",
      },
      chart_used: {
        y: "Te nui o nga whakamahinga",
        label: "Nga taima",
      },
    },
  },

  /** {@see BPageOffer} **/
  shop_offer: {
    title: "Nga tuku",
    up_to: "ki runga ki",
    menu: {
      dashboard: "Papatohu",
      orders: "Nga ota",
      back: "Nga tuku",
    },

    /** {@see BPageOfferDashboard} **/
    dashboard: {
      title: "Papatohu Whakahaere Tuku",
      chart_amount_buy: {
        y: "Te moni hoko {currency}",
        label: "Hoko",
      },
      chart_amount_discount: {
        y: "Tuku {currency}",
        label: "tuku",
      },
      chart_used: {
        y: "Te nui o nga whakamahinga",
        label: "Nga taima",
      },
    },
  },

  /**
   * @see BOrderConnectsList
   */
  order_connect_list: {
    subtitle:
      "Rarangi o nga ota tukutahi-a-waho me whakatutuki e nga kaiwhakarato tuatoru.",
  },
  /** {@see BAccountCard} **/
  account_card: {
    pay_create_receipt: "Utu utu whakahohenga",
    balance: "Taurite",
    locked: "Kua maukati",
    transfer: "Whakawhiti",
    charge: "Putunga",
  },

  /** {@see TimelineStatus} **/
  timeline: {
    note: "{message}",
    info: "{message}",
    error: "{message}",

    buy: "Takitaki",
    payment: "Utu",

    confirm:
      "Whakapumautanga ota, <b>{success}</b> whakau, a <b>{fail}</b> i paopao.",
    prepare: "Te whakarite ota.",
    send: "Ka tukuna te ota ma te <b>{delivery}</b> .",
    delivered: "Kua tukuna te ota ki te kaihoko.",
    "fill-inputs": "Ko te puka korero whakauru kua oti i te kaihoko.",
    "return-add": "Tukuna he tono whakahoki hua, {product}.",
    "return-accept": "Kua whakamanahia te tono whakahoki {return_code}.",
    "return-reject": "Ko te tono {return_code} i paopao.",

    "reject-order": "Whakakahoretia te ota. Waehere: {reject_code}.",
    "reject-order-clear": "Whakakorehia te whakakore ota. Waehere: {reject_code}.",

    "delivery-returned":
      "Ko te ota kaore i riro mai i te kaihoko ka whakahokia mai. Me tuku ano.",
    "delivery-canceled": "Kua whakakorehia te tuku. Me tuku ano.",
    "edit-receiver-info": "Ko te wahitau tuku ota i whakatikahia e te kaihoko.",
    "edit-billing-info": "Ko te wahitau nama nama kua whakatikahia e te kaihoko.",
    "payment-cod-confirm": "I whakamanahia te utu COD.",
    "payment-cash-confirm":
      "Ko te utu o te {amount} {currency} i roto i te putea mo te pire i whakamanahia.",
    "payment-dir-confirm": "I whakaūngia te rihīti utu tika.",

    vendor: "I hurihia te ahua ota kaihoko mai <b>i}</b> ki <b>te</b> . {from} {to}",

    "connect-sync": "Tono tukutahia e Tūhono OS. Connect OS",
    "connect-shipped": "Kua tukuna te ota hono ma te {shipping}.",
    "connect-returned": "Ko te ota hono kua whakahokia mai e {shipping}.",
    "connect-failed": "I rahua te whakatutuki ota e te hoa. {reason}",
    "connect-canceled": "I whakakorehia te ota e te hoa. {reason}",
    "connect-action-cancel": "Tono ki te whakakore ota. {reason}",
    "connect-put-hold": "Ka mau te ota. {reason}",
    "connect-remove-hold": "I tangohia te ota mai i te pupuri. {reason}",
    "connect-confirmed": "I whakapumautia te ota.",

    "vendor-payout": "💸 Utu ki te kaihoko.",
    "vendor-wallet-transaction": "tūemi Pukoro Tauwhitinga.",

    "vendor-sms":
      "🔔 🛍️ Ota tūemi SMS tūemi te Tukuna Ki <b>{vendor}</b> ╏ {phone}.",

    // ━━━━━━━━━━━━━━━━━━━━━━ Email Packs ━━━━━━━━━━━━━━━━━━━━━━

    "email-buy": "Tukuna he imeera whakaū hoko ki te kaihoko.",
    "email-payment": "Tukuna he imeera whakaū utu ki te kaihoko.",
    "email-update": "Tukua 🛍️ Ota Whakahou Īmēra Ki kiritaki.",

    "email-vendor":
      "🔔 Kua tukuna he imeera whakamohio ki a <b>{vendor}</b> ╏ {email}.",

    // ━━━━━━━━━━━━━━━━━━━━━━ SMS Packs ━━━━━━━━━━━━━━━━━━━━━━
    "sms-payment":
      "Tukua he 💳 Utu tūemi SMS Ki te kiritaki tūemi {phone}.",
    "sms-ready-to-ship":
      "Tukuna he SMS ki te kaihoko e whakaatu ana kua reri ana ota kia tukuna ma {phone}.",
    "sms-ready-to-pickup":
      "Tukuna he SMS ki te kaihoko e whakaatu ana kua reri tana ota mo te tiki ma {phone}.",
    "sms-order-confirmation":
      "Tukua he 🛍️ Ota tūemi SMS Ki te kiritaki tūemi {phone}.",

    "count-adjustment":
      "te Te nui o te Hua \"{product}\" tūemi te tūemi Mai {from} Ki {to}.",
  },

  /** {@see FinanceInlineChart} **/
  inline_chart: {
    today: "I tenei ra",
    last_days: "{days} 30 ra tūemi",
  },
  /** {@see BProductsWindow} **/
  products_select: {
    move_category: "Nuku kāwai <b>{category}</b> ki tētahi atu kāwai",
    move_product: "Nuku hua <b>{product}</b> ki te kāwai",
    load_more_products:
      "He maha atu nga hua kei roto i tenei waahanga. Paatohia kia utaina etahi atu mea.",
    product_menu: {
      dashboard: "Papatohu hua",
      inventory: "tūemi ka Nga momo rereke",
    },
    notifications: {
      copy_success: "Te kape o te hua.",
      change_category_success: "Kua whakatikahia te waahanga hua.",
    },

    filter_box: {
      no_root_filter_message: "He hua kei te putake engari kaore he tātari.",
      has_root_filter_message: "Ka tautuhia e koe nga whiriwhiringa mo te waahanga pakiaka.",
      set_filter_message: "Ka taea e koe te whakarite whiriwhiringa.",
      edit_action: "Whakatikatika tūemi Tātari",
      clear_action: "Ūkui Tātari Pūtake",
    },
    ai: {
      title: "AI Hua tūemi",
    },
    menu: {
      select_all_products: "tūemi katoa Hua",
      unselect: "tūemi",
      sort_categories: "tūemi / tūemi Ngā Kāwai",
      subscription_vendor_not_support_message:
        "Kaore e taea te tohu kaihoko mo nga hua ohaurunga! Ko nga hua ohaurunga me whai kaihoko tonu hei rangatira!",
      vendor_owner_not_assignable_message:
        "Kaore e taea te tautapa i tetahi kaihoko mo nga hua me te kaihoko ko tona rangatira!",
      category_assign_profile: "tūemi kōtaha Ki Hua i te Kāwai",
      category_assign_profile_subtitle: "Taake, Te tuku, tūemi, tūemi,...",
      bulk_discount: "tūemi Te utu",
      bulk_discount_subtitle: "tūemi Te utu tūemi katoa Hua.",
    },

    engine: {
      title: "20% Hua tūemi",
      subtitle:
        "He maha atu nga hua ka utaina mai i nga waahanga me nga tohu kua tohua.",
      load_in_tips: "tūemi Hua Mai Ngā Kāwai i te {category}.",
      action: "Whakatikatika {category} tūemi",
    },
  },
  /** {@see BProductsPanel} **/
  product_panel: {
    excel_import: {
      title: "Whakatakahia te konae Excel / CSV ki konei.",
      select_file: "tūemi kōnae",
      max_file: "Rahi kōnae mōrahi: 20mb",
      download_sample: "tikiake tūemi",
    },
    ai: {
      title: "AI Hua tūemi",
    },
  },

  /** {@see BOrderDashboardVendor} **/
  order_vendor: {
    has_panel: "He panui",
    no_panel: "kahore tūemi",
    manual_action: {
      title: "tūemi Nga Mahi",
      message: "Ka taea e koe te whakahou i te mana ota kaihoko.",
      guides: [
        "Ka taea e nga Kaihoko whai urunga a te roopu te whakahou i o raatau whakatutukitanga (nga ota-iti) ki o raatau papatohu kua whakatapua.",
        "Mena karekau he urunga a te kaihoko, ka huri aunoatia te mana ota kaihoko ma te huri i te mana kete.",
        "Ka taea e koe te whakahou i nga ota kaihoko.",
      ],
    },
    payouts: {
      title: "tūemi",
      message: "tūemi te kaihoko tūemi ka Nga tauwhitinga.",

      charge_tooltip: "Te utu kaihoko Pukoro.",
      withdraw_tooltip: "tūemi Mai kaihoko Pukoro.",
      payout_tooltip: "tūemi Ki te kaihoko.",
    },
    order_canceled: "🛍️ Ota tūemi",
    order_rejected: "🛍️ Ota Kua whakakorehia mā kaihoko",
  },

  /**
   * @see VPageVendorDashboard
   */
  vendor_dashboard: {
    info: {
      subtitle: "Te kainga hokohoko tūemi.",
    },
    performance: {
      subtitle: "tūemi Hokonga Mahi.",
    },
    products: {
      title: "Hua",
      subtitle:
        "I konei ka taea e koe te tiro i te tatauranga o o hua kua raarangi i runga i te maakete. Ko nga hua a nga kaihoko maha kaore i te whakauruhia.",
    },
    wallets: {
      title: "tūemi Pukoro",
      subtitle:
        "Kaore koe e hiahia ki te taapiri i nga putea ma te ringa! Ka hanga aunoa nga putea kaihoko.",
    },

    open_marketplace_page: "tūemi Te kainga hokohoko whārangi",
    open_my_store_page: "Tuwhera taku wharangi toa",
    open_my_listing_page: "Whakatuwherahia taku wharangi raarangi",
    add_products_permission: "tāpiri Hua tūemi",
    add_categories_permission: "tāpiri Ngā Kāwai tūemi",
  },

  /**
   * @see VPageVendorInventory
   */
  vendor_inventory: {
    title: "Rarangi hua",
    subtitle:
      "He rarangi tenei o o hua kei te maakete. Whakahouhia nga utu me nga rahinga mo o hua kua tohua i roto i te raarangi. Hei kaihoko, ka kite koe i nga hua kaihoko kotahi kei a koe i te rarangi hua. Ko etahi atu hua kua tohua ka taea te whakarereke i konei.",
  },

  /**
   * @see VPageVendorAccess
   */
  vendor_member: {
    title: "Nga mema o te kapa",
    subtitle:
      "Koinei te rarangi o nga kaiwhakamahi e uru ana ki to roopu kaihoko. Ka taea e koe te taapiri i nga mema hou me te whakarite i o raatau whakaaetanga ki konei.",
    add_action: "tāpiri Hou tūemi",
    pending: {
      title: "Tarewa ana tūemi",
      message: "Tirohia te rarangi o nga powhiri e tatari ana kia whakaaetia.",
    },
  },

  /**
   * @see VMemberAddDialog
   */
  vendor_member_add: {
    title_edit: "Whakatikaina Mema o te Kapa",
    title_new: "Tāpirihia he mema o te roopu hou",
    member: {
      title: "Nga Taipitopito Mema o te Kapa",
      subtitle:
        "Tāpirihia he mema hōu o te tīma ka whakatakoto i ō rātou whakaaetanga. Patohia ia mea hei whakatika i te urunga panui/tuhi. He rite ia rarangi ki te taumata whakaaetanga mo te papatohu toa matua.",
    },
    access: {
      view_data: "Tirohanga Raraunga",
      apply_changes: "Tiaki tūemi",
    },
    inputs: {
      email: {
        label: "Īmēra wāhi noho",
        placeholder: "tūemi te tūemi Īmēra wāhi noho...",
      },
    },
    notifications: {
      add_success: {
        title: "Mema kua tapirihia",
        message: "Kua tutuki pai te whakaurunga o te mema o te kapa.",
      },
      update_success: {
        title: "Mema kua whakahoutia",
        message: "Kua pai te whakahou i nga korero a te mema o te roopu.",
      },
    },
    actions: {
      add: "tāpiri tūemi",
    },
  },

  /**
   * @see VPageVendorPayments
   */
  vendor_payments: {
    list: {
      title: "tūemi Hītori",
      subtitle:
        "Ka taea e koe te tiro i to hitori utu i konei. Ko enei utu ka taea te tukatuka a-ringa ma te whakawhiti putea, ma te whakamahi aunoa ranei i nga ratonga utu e tuku ana i nga kaha utu.",
    },
    connect: {
      title: "Connect Peeke",
      subtitle:
        "Ka tukuna e matou nga waahanga utu wehewehe mo nga huarahi utu e whai ake nei. Ki te whiwhi utu marie i muri i ia hokonga, honoa to putea putea ma te whakamahi i enei whiringa.",
    },
    verify_dialog: {
      title: "tūemi 💳 Utu",
      option: {
        title: "kaihoko tūemi",
        subtitle:
          "Kei a koe te whiringa ki te whakaae, ki te whakakore ranei i tenei whakawhitinga. Ka noho tenei mahi hei rekoata hitori anake. Me whakamana noa mena kua whiwhi koe i nga moni; ki te kore, kaore he take ki te mahi.",
      },
      inputs: {
        verify: {
          label: "Kua whakapumautia e koe tenei utu?",
          false_description:
            "Kao, kaore ano kia riro mai tenei utu i roto i taku putea putea.",
          true_description: "āe, I tūemi Kua riro ka Manatoko tūemi 💳 Utu.",
        },
      },
    },
  },

  /**
   * @see BVendorOrderRefund
   */
  vendor_order_refund: {
    title: "tūemi 🛍️ Ota",
    subtitle:
      "<b>He mea nui! </b>Ma te mahi i tenei mahi ka puta he tauwhitinga kino i roto i te putea a te kaihoko ka tangohia te moni kua tohua mai i tera. Ko te moni kaore e nui ake i te toenga o nga whakawhitinga mo te ota kua tohua. I tua atu, ki te mea kua whakawhitia nga moni ki nga putea putea hono (penei i te Stripe Connect me te whakawhiti waea a-ringa ranei), me hanga e koe he <b>whakawhitinga whakawhiti</b>.",
    inputs: {
      note: {
        message: "Ka kitea e te kaihoko.",
        placeholder: "Ka taea e koe te taapiri i tetahi tuhipoka ki konei...",
      },
      amount: {
        error_message: "Kua hipa te toenga moni.",
      },
    },
  },

  /** {@see BOrderVendorPaymentManagement} **/
  order_vendor_payment: {
    message:
      "He tirohanga whanui tenei mo nga whakawhitinga i roto i te putea mariko a te kaihoko i runga i te kainga hokohoko. I nga wa katoa ka tukuna he ota, ka taapirihia te wahanga o te kaihoko ki o raatau putea.",
    vendor_wallets: "kaihoko Pukoro",
    my_wallets: "Taku Pukoro",
    refund_order_action: "tūemi 🛍️ Ota",
    automatic_payout_list: {
      subtitle:
        "He rarangi utu aunoa tenei ki to putea putea hono. Kia mahara ko nga utu aunoa anake ka whakaatuhia ki konei, kaore e whakaatuhia nga utu a-ringa.",
    },
    reversal_transfer_action: "tūemi Whakawhiti",
  },

  /** {@see BPageOrderPhysicalTrack} **/
  physical_order_track: {
    title: "Nga Ripoata morearea me nga Aromatawai",
  },
  /** {@see BPageOrderVirtualTrack} **/
  virtual_order_track: {
    title: "Nga Ripoata morearea me nga Aromatawai",
  },
  /** {@see BPageOrderVirtualTimeline} **/
  virtual_order_timeline: {
    title: "Rarangi Wā",
  },
  /** {@see BPageOrderPhysicalTimeline} **/
  physical_order_timeline: {
    title: "Rarangi Wā",
  },
  /** {@see BPageOrderPhysicalInput} **/
  physical_order_inputs: {},
  /** {@see BOrderTimeline} **/
  order_timeline: {
    message:
      "I tenei waahanga, ka taea e koe te tiro i te raarangi o nga huihuinga e pa ana ki te ota, whakatutukitanga, utu, me o raatau ahuatanga. Hei taapiri, ka taea e koe te tirotiro i nga ota kua tukuna ki nga kaihoko me nga kaihoko.",
    add_note_subtitle: "Tuhia he karere ka whakahua i o hoa mahi.",

    message_input: "Tuhia he karere ki konei...",
    pin_action: "Pin ki runga",
    unpin_action: "Wewete titi",
    copy_html: "Tārua Html",
    notifications: {
      add_note_success: "Note tapiri.",
      edit_note_success: "Kua whakatikahia te tuhipoka.",
      delete_note_success: "Kua mukua te tuhipoka.",
    },
    delete_dialog: {
      title: "Mukua te tuhipoka",
      message: "Me muku tenei tuhipoka?",
      action: "Mukua te tuhipoka",
    },
    resend_dialog: {
      title: "Tukua ano te imeera",
      message: "Me tuku ano tenei imeera ki to kaihoko?",
      action: "Ae, tukuna ano",
      success: "Ka tukuna e te imeera ki to kaihoko.",
    },
  },

  /** {@see BPageShuttleWalletAccounts} **/
  accounts: {
    title: "Taku Pukoro",
    message:
      "Me hanga e koe kia kotahi ake te kaute me te moni e whakamahia ana e koe i Selldone. Me utu e koe tenei kaute hei utu i te utu ratonga.",
    add_new: "Waihanga pūkete hōu",
    new_account: "Tāpiri pūkete hōu",
    new_account_dialog: {
      title: "Pūkete Hou",
      account_name_input: "Ingoa pūkete",
      currency_input: "Waeine moni",
      currency_input_message:
        "Ko te kaute kua hangaia ka pupuri noa i nga uara o tenei moni.",
      new_account_wage: "Utu hanga kaute hou: Kore utu mo te kaute tuatahi",
      create_account_action: "Waihanga Pūkete",
    },
  },

  /** {@see UserGifts} **/
  user_gifts: {
    menu: {
      send_gift: "Tuku Koha",
      sent: "Tukua",
      received: "Kua riro",
    },

    /** {@see BPageShuttleWalletGiftsSend} **/
    send_gifts: {
      title: "Nga Taonga Tukua",
      sub_title:
        "Ka taea e koe te tuku taonga ki o hoa, kaihoko ranei. Ka whakaae pea te kaiwhiwhi ki to koha. Ka maukatihia te moni i roto i to putea ka tangohia mena ka whakaae te kaiwhiwhi. Ki te kore, ka whakawhitia tenei moni ki to putea.",
      table: {
        user: "kaiwhakamahi",
        amount: "Te nui",
        currency: "Moni",
        account_name: "Mai",
        status: "Tūnga",
        message: "Karere",
        created_at: "Te Ra Hanga",
      },
    },

    /** {@see BPageShuttleWalletGiftsReceived} **/
    received_gifts: {
      title: "Nga Koha i whiwhi",
      sub_title:
        "I whiwhi koe i nga koha mai i a Selldone, etahi atu ranei",

      table: {
        amount: "Te nui",
        currency: "Moni",
        account_from: "Mai",
        status: "Tūnga",
        message: "Karere",
        created_at: "Te Ra Hanga",
        receive: "Whiwhi",
      },
    },
  },

  /** {@see BAccountGiftRedeem} **/
  get_gift_dialog: {
    title: "Whiwhi i tetahi Koha",
    more: "Ētahi atu ..",
    amount: "Te nui",
    message: "Karere",
    account_to_input: "pūkete ūnga",
    account_to_input_message: "Ka tukuna nga moni koha ki tenei kaute.",
    get_action: "Whakaaetia te Koha",
    notifications: {
      received_success_title: "Tena koe",
      received_success:
        "Ko te nui o te koha {amount} {currency} kua tukuna ki to putea.",
    },
  },
  /** {@see BAccountGiftSend} **/
  send_gift_dialog: {
    title: "Tuku Koha",
    account_from_input: "Kōwhiria he pūkete",
    account_from_input_message: "Ka tangohia nga moni koha mai i tenei kaute.",
    receiver_input: "Kaiwhiwhi",
    receiver_input_message: "Ka taea e koe te whakauru i te wahitau imeera me te nama waea.",
    message_input: "Karere",
    message_input_message: "Ka taea e koe te waiho he karere ki te kaiwhiwhi.",
    amount_input: "Te nui",
    currency_input: "Waeine moni",
    currency_input_message: "Ka tatauhia te utu i roto i tenei moni.",
    send_action: "Tuku Koha",
    notifications: {
      send_success: "I tukuna he koha {amount} ki a {currency}. {receiver}",
    },
  },

  /** {@see BAccountInput} **/
  account_list: {
    account_input: "Tau Kaute",
    account_input_message:
      "Ko nga moni koha mo tenei moni ka tukuna ki tenei kaute.",
    free_balance: "Taurite kore utu",
    account_name: "Ingoa pūkete",
  },
  /** {@see OPageMonetizeAccounts} **/
  user_monetize_accounts: {
    title: "Pūkete Putunga",
    sub_title:
      "Ka tukuna aunoa nga moni koha ki enei kaute.",
    message:
      "Ka tuku moni a Selldone mo o mahi paapori pāpori, te whai waahi ki te whakawhanaketanga o Selldone, te whakauru i nga hoa me etahi atu. Ma te kowhiri i te putea putea mo ia moni, ka tukuna nga moni koha ki roto i taua kaute i to rohe. Ko enei moni ka taea te nama ki a Selldone. Ko nga tangohanga ka taea anake mo nga hoa pakihi.",
    add_account_action: "Tāpiri Pūkete",
    add_dialog: {
      title: "Honoa he kaute ki te whiwhi taonga",
      account_input: "Tau Kaute",
      account_input_message:
        "Ko nga moni koha mo tenei moni ka tukuna ki tenei kaute.",
      set_as_default_action: "Tīpakohia hei pūkete taunoa",
    },
    table: {
      currency: "Moni",
      account_number: "Tau Kaute",
      account_name: "Ingoa",
      payment: "Te moni putunga",
      reset_at: "Tautuhi anō ki",
      delete: "Mukua",
    },
    notifications: {
      set_success: "He pai te taapiri i te kaute.",
      delete_success:
        "I angitu te tango i te kaute mai i te rarangi o nga kaute taunoa mo te whiwhi taonga.",
    },
    delete_alert: {
      title: "Muku Pūkete",
      message:
        "Kei te pirangi koe ki te tango i tenei kaute mai i te rarangi Pūkete Koha?",
      action: "Mukua",
    },
  },
  /** {@see UserMonetize} **/
  user_monetize: {
    title: "Hanga Moni",
    menu: {
      plans: "Mahere",
      accounts: "Nga kaute",
      participant: "Kaiuru",
    },
  },
  /** {@see OPageMonetizeReferral} **/
  user_monetize_plans: {
    title: "Kaupapa Mahi tahi me te Moni Moni",
    sub_title:
      "Ka whiwhi koe i te nama kaute mo nga koha tuku. Ka utua enei moni ki to putea ka taea te tango ki nga hoa pakihi a Selldone anake (Kaore i te Katoa). Ka taea e koe te whakapau moni mo te utu ratonga Selldone. IP",

    discount: "Te utu",
    your_income: "To moni whiwhi",
    level_2: "Mai i te Taumata Tuarua",
    level_3: "Mai i te Taumata Tuatoru",
    link_message: "Mema: {registered}, accepted: {accepted}",
    link_tip:
      "Ka taea e koe te taapiri i te rerenga korero ?ref={link_code} ki te mutunga o ia wahitau ka tohatoha koe.",
    table: {
      currency: "Moni",
      amount: "Taurite",
      total: "Utu katoa",
      payment_date: "Te ra utu whakamutunga",
      withdraw: "Tangohanga",
    },

    notifications: {
      get_credit_success:
        "Kua tukuna te moni i tonoa ki to putea.",
    },
  },
  /** {@see BPageShuttleIdentity} **/
  basic_information: {
    title: "Toku Tuakiri me taku Kotaha",
    email: "Wāhitau īmēra",
    tel: "Tau waea pūkoro",
    address: "wāhi noho",
    payment: "Nga tauwhitinga whaimana",
    personal_information: "Nga Korero Tuakiri",
    main_profile: "Kōtaha Matua",
    main_profile_subtitle:
      "E waatea ana enei korero ki te katoa.",
    kyc_waiting_info:
      "Kei te tirotirohia e matou te tika o nga korero ka whakauruhia e koe. Ko tenei tukanga he whakahau i runga i nga ture KYC ka whakaitihia te tupono o nga whakawhitinga putea. Ka taea e koe te whakamahi i nga painga o naianei me nga mea kei te heke mai o tenei ratonga.",

    personal_info_dialog: {
      title: "Nga Korero Tuakiri",
      name_input: "Ingoa tuatahi",
      family_name_input: "Ingoa whakamutunga",
      number_input: "Tau Uruwhenua (Waehere Motu ranei)",
      birthday_input: "Ra whanau",
      birthday_input_placeholder: "Whakauruhia to ra whanau",
      address_input: "wāhi noho",
      postcode_input: "Waehere Poutāpeta",
      tel_input: "Tau Whakapā",
      passport_image_input: "Tīpakohia te whakaahua uruwhenua",
      passport_image_input_message:
        "Tukuna he karapa o to ahua uruwhenua.",
      personal_image_input: "Kōwhiria tō ake whakaahua ID",
      personal_image_input_message:
        "Tukuna he whakaahua karapa whakamuri, whakamuri i runga i to kaari motu ki tetahi ahua. ID",
    },
    profile_dialog: {
      title: "Kōtaha Matua",
      subtitle:
        "Whakatikahia o korero korero mo te iwi whanui i konei. Ka kitea pea enei korero ki o kaihoko me etahi atu kaiwhakamahi.",
      full_name_input: "Ingoa Katoa",
      tel_input: "Tau Whakapā",
      email_input: "Īmēra",
      website_input: "paetukutuku",
      address_input: "wāhi noho",
      about_input: "Mo au",
      about_long_input: "He korero roa mo au",
      image_input: "Tīpakohia te pikitia kōtaha",
      image_input_message:
        "Tukuna ake to ahua. Ka whakamahia tenei ahua hei pikitia mo to whakaahua.",
      notifications: {
        update_profile_success: "Kua whakahōutia tō kōtaha.",
        update_personal_info_success:
          "I rehita pai to korero",
      },
    },
    charge_account_approve_message:
      "Utuhia to putea ki te whakaae i to tauwhitinga putea.",
    payment_approval: "Whakaaetanga utu",
    residential_address_checking:
      "Ko to waahi noho noho kei te tirotiro i te rarangi.",
    add_residential_address:
      "Taapirihia to waahi me te tohu o nga tuhinga waahi noho.",
  },
  /** {@see BUserPhoneVerify} **/
  phone_verify: {
    title: "Tau waea pūkoro",
    step1_title:
      "Whakauruhia to nama waea. SMS",
    phone_number_input: "Tau waea pūkoro",
    next: "Te taahiraa o muri",
    sms_code_input: "Waehere manatoko",
    sms_code_message: "6-mati tohu tohu i tukuna ki {phone_number}.",

    notifications: {
      duplicated_number_error: "Ko tenei nama kua whakaaetia e koe.",
      send_activation_code_success_title: "Waehere manatoko",
      send_activation_code_success:
        "I tukuna te waehere whakaū ki {phone_number}.",
      verify_success: "Kua manatokohia to nama waea",
    },
  },
  /** {@see BPageShuttleCompanies} **/
  companies: {
    title: "Aku Pukete Pakihi",
    message:
      "Ka taea e nga pakihi, nga kaipakihi, me nga roopu ngaio te hanga hua me nga tono i runga i te Selldone. He whakaritenga ki te hanga i tetahi korero pakihi.",
    new_company: "Kamupene hou",
  },
  /** {@see BCompanyAdd} **/
  add_company: {
    name_input: "Ingoa kamupene",
    website_input: "paetukutuku",
    email_input: "Īmēra",
    address_input: "wāhi noho",
    tel_input: "Waea",
    mobile_input: "Pukoro",
    edit_action: "Whakatika kamupene",
    create_action: "Waihanga kamupene",
    notifications: {
      add_success: "I hangaia he putea pakihi {name}.",
      edit_success: "Kua whakatikahia te putea pakihi.",
    },
  },
  /** {@see UserAccessControl} **/
  user_access_control: {
    menu: {
      personal_key: "Whaiaro",
      shop_key: "Kiritaki",
      apps: "Taupānga",
    },
    connected_apps_title: "Taupānga hono",
    connected_apps_message:
      "Ka kite koe i te rarangi o nga toa me nga taupānga katoa e hono ana ki to putea rohe. Ka taea e koe te whakakore i te uru ki tetahi o ratou.",
    clients_title: "Kiritaki",
    clients_message:
      "Ka whakamahia enei taviri ki te hanga i te Tohu Whakauru e to kaimau. Kia maumahara he muna te mau taviri, ka tuu noa ki runga i te tūmau kia kore ai e kitea e etahi atu.",
    personal_keys_title: "Nga Kī Whakauru Whaiaro",
    personal_keys_message:
      "Ka taea e koe te hanga i nga ki o to putea i konei. Kia mahara ka taea e enei ki te uru ki to putea. Waihangahia me te whakamahi i enei taviri mena ka mohio koe ki taau e mahi ana.",
  },

  /** {@see BPageShuttleSecurity} **/
  user_security_control: {
    title: "Haumarutanga Pūkete",
    message: "Ka taea e koe te whakatika i to tautuhinga haumarutanga putea i konei.",

    delete: {
      title: "Muku Pūkete",
      subtitle:
        "I mua i te whakakore i to putea, me mohio kua mukua e koe nga toa katoa.",
      action: "E mohio ana ahau, mukua taku kaute",
      agreement:
        "Ka whakaae ahau ka mukua aku korero katoa mai i to tuumau. Ka whakakorehia o korero i runga i te raarangi kua whakatauhia e matou hei aukati i te tinihanga, te whakamahi kino ranei.",
    },

    list: {
      email: {
        title: "Īmēra Tuatahi",
        sub_title:
          "Kei a koe te whiringa ki te whakahou i te imeera matua e hono ana ki tenei kaute.",
        action_title: "Hurihia te Īmēra Tuatahi",
        action: "Tuku hono manatoko",
      },
      password: {
        title: "Kupuhipa",
        sub_title: "Kōwhiria he kupuhipa kaha mo tō pūkete.",
        action_title: "Kupuhipa uru kaute",
        not_exist: "Kaore i uru",
        action: "Huri Kupuhipa",
      },
      phone: {
        title: "Tau waea pūkoro",
        sub_title:
          "Whakaūhia to nama waea. Ka taea e koe te takiuru ma tenei nama.",
        action_title: "Te nama waea tuatahi o te kaute",
        action: "Manatoko Tau",
      },

      two_step: {
        title: "E rua nga Waahi Whakauru",
        sub_title:
          "Whakaūhia to nama waea. Ka taea e koe te takiuru ma tenei nama.",
        action_title: "Whakanuia te haumaru kaute",
        activated_action: "Whakahohe",
        active_action: "Hohe inaianei",
      },
    },
    two_step_auth: {
      step_1: {
        title: "Tāutahia te taupānga Google Authenticator",
      },
      step_2: {
        title: "Whakauruhia te waehere i hangaia e te taupānga",
        message:
          "Ko te waehere 6-mati ka hangaia e te algorithm i to waea pūkoro. Ka taea e koe te tango i te taupānga mai i te hononga e whai ake nei. Pēhia te kī whakaputa waehere hei waihanga i tētahi waehere QR kei roto te kupuhipa.",
        qr_code_info:
          "Mena kaore e taea te karapa, ka taea e koe te kape me te whakapiri i te waehere ki roto i te taupānga:",
        create_new_code: "Waihanga waehere hou",
        qr_code_scanning:
          "Matawai i te waehere QR i hangaia e Google Authenticator Mobile App me te whakauru i te waehere i hangaia hei whakamana i roto i te pouaka o tenei wharangi.",
        qr_code_regenerate_info:
          "Tuhipoka: Ma te pehi i te taviri i runga ake nei ka hangaia he taviri hou, ka kore e taea te whakamahi te taviri o mua. Whakauruhia te taviri hou ki to taupānga waea.",
      },
      step_3: {
        title: "Whakauruhia te Waehere Hangaia",
        message:
          "Whakauruhia te waehere uru i hangaia i roto i te tono i te mara i raro nei.",
        activation_key_input: "Waehere Uru",
      },
      activated_alert: "2-taahiraa takiuru kua whakahohea.",
      activated_message:
        "E rua-taahiraa takiuru kua whakahohea mo to putea. Mai i tenei wa ka hiahia koe ki te whakauru i tetahi waehere tuarua ka huri ia 30 hēkona ki te uru atu ki to putea. Hei whakakore i te takiuru-rua-taahiraa, paato-rua noa Patohia tetahi taahiraa ka haere i roto i nga hikoinga.",
      cancel_two_step_login_action: "Whakakorea te takiuru-rua-taahiraa",
      cancel_two_step_login_info:
        "Mena kei te hiahia koe ki te whakakore i te manatoko 2-taahiraa, whakauruhia to kupuhipa ka paato i te rua-taahiraa matua takiuru.",
      password_input: "Kupuhipa",
      cancel_2fa_action: "Whakakorea te takiuru-rua-taahiraa",
    },

    change_password_dialog: {
      title: "Huri Kupuhipa",
      message: "Whakamahia he kupuhipa uaua me te maumahara.",
      password_input: "Kupuhipa o naianei",
      new_password_input: "Kupuhipa hou",
      re_new_password_input: "Whakahokia te kupuhipa hou",
      tips: {
        length: "Me 8 te iti rawa o te kupuhipa",
        chars: "Kia kotahi te reta matua me te nama kotahi.",
        match: "Kupuhipa i taurite.",
      },
      change_password_action: "Huri Kupuhipa",
      notifications: {
        change_password_title: "Huri Kupuhipa",
        change_password: "I momoho te huri kupuhipa.",
      },
    },
  },
  /** {@see AuthorizedClients} **/
  authorized_clients: {
    title: "Rarangi o nga toa / taupānga hono",
    table: {
      name: "Ingoa",
      app: "Taupānga",
      scopes: "Te whānuitanga",
      shop: "Toa Rahui",
    },
    remove_action: "Tango uru",
    delete_alert: {
      title: "Mukua",
      message:
        "Kei te pirangi koe ki te whakakore i tenei uru? Kia mahara ka kore tenei taupānga e uru atu ki to putea.",
      action: "Whakakahoretia te uru",
    },
  },
  /** {@see Clients} **/
  clients: {
    title: "Rarangi Kiritaki",
    create_new_action: "Waihanga kiritaki hou",
    table: {
      client_id: "ID Kiritaki",
      app: "Taupānga",
      name: "ingoa ingoa",
      secret_key: "Kī ngaro",
    },
    create_dialog: {
      title: "Kiritaki hou",
      error_msg: "<strong>Hapa!</strong> Aroha mai, kua puta he hapa!",
      name_input: "Ingoa",
      name_input_message: "He mea e mohio ana, e whakawhirinaki ana hoki o kaiwhakamahi.",
      redirect_input_message: "Whakahokia te wahitau ki to pae.",
    },
    edit_dialog: {
      title: "Whakatika urunga",
    },
    delete_alert: {
      title: "Mukua",
      message: "Me muku tenei kiritaki?",
      action: "Mukua",
    },
  },
  /** {@see PersonalAccessTokens} **/
  personal_access_tokens: {
    title: "Rarangi o nga Tohu Whakauru Whaiaro",
    create_action: "Waihangahia he tohu hou",
    table: {
      name: "Ingoa",
      scopes: "Nga urunga",
      expire_at: "Te ra paunga",
    },
    create_dialog: {
      title: "Whare Tohu",
      error_message: "<strong>Hapa!</strong> Aroha mai, kua puta he hapa!",
      name_input: "Alias Ex. toku ingoa tohu",
      name_input_message: "Ingoa tohu tino pai.",
      scopes_list: "Rarangi Whakauru Tohu",
    },
    access_token_dialog: {
      title: "Kī Uru Whaiaro",
      message:
        "Anei te tohu o to tohu urunga whaiaro. Koinei anake te wa ka whakaatuhia tenei tohu, na kaua e ngaro! Ka taea e koe te whakamahi i tenei tohu ki te tono API.",
    },
    delete_alert: {
      title: "Mukua",
      message: "Me muku tenei tohu uru?",
      action: "Mukua",
    },
  },

  /** {@see Scopes} **/
  scopes: {
    //--------------------------- Profile ---------------------------
    profile: {
      name: "kōtaha",
      description:
        "Panuitia nga korero moohio tae atu ki te ingoa, te wahitau imeera, te ahua me te mana motuhēhēnga.",
    },
    phone: {
      name: "nama waea",
      description: "uru ki te nama waea.",
    },
    address: {
      name: "Wāhitau",
      description: "Pānuihia te wāhitau kua tiakina mai i te pukapuka wāhitau.",
    },
    "user:profile:write": {
      name: "kōtaha whakahaere",
      description: "Whakatikatika ka tūemi kaiwhakamahi kōtaha tūemi.",
    },

    //--------------------------- Storefront ---------------------------
    "my-gift-cards": {
      name: "Kaari Koha",
      description: "Te uru ki aku Kaari.",
    },
    "order-history": {
      name: "Nga Ota History",
      description: "Pānuihia nga ota History.",
    },
    buy: {
      name: "Hokonga",
      description: "Whakaritehia he ota me te hoko mai i te toa.",
    },

    //--------------------------- Backoffice ---------------------------
    articles: {
      name: "Whakatika Ihirangi",
      description: "Whakatika Tuhinga.",
    },
    "backoffice:notifications": {
      name: "Whakamōhiotanga whakahaere",
      description: "Tukua ka Whiwhi Whakamōhiotanga.",
    },
    "backoffice:order:write": {
      name: "🛍️ Ota whakahaere",
      description: "Whakatikatika ka tūemi toa Nga ota.",
    },
    "backoffice:order:read": {
      name: "🛍️ Ota Urunga",
      description: "tūemi toa Nga ota.",
    },
    "backoffice:support-tickets": {
      name: "Tautoko tūemi",
      description: "tūemi Tautoko tūemi.",
    },
    "backoffice:shop:read": {
      name: "Toa tūemi",
      description: "tūemi Toa Nga korero ka Tauanga.",
    },
    "backoffice:shop:write": {
      name: "Toa whakahaere",
      description: "Waihanga ka tūemi Toa Nga korero.",
    },
    "backoffice:shop:add": {
      name: "tāpiri Toa",
      description: "tāpiri he Hou Toa Ki te tūemi.",
    },
    "backoffice:shop:delete": {
      name: "Mukua Toa",
      description: "Mukua he Toa Mai te tūemi.",
    },
    "backoffice:giftcard:write": {
      name: "Whakahaere Kaari Koha",
      description: "Waihanga me te whakahaere i nga kaari koha.",
    },
    "backoffice:giftcard:read": {
      name: "Te Uru Kaari Koha",
      description: "Pānuihia nga korero mo te kaari koha me nga tatauranga.",
    },
    "backoffice:faq:write": {
      name: "FAQ whakahaere",
      description: "Waihanga me te whakahaere i nga patai auau.",
    },
    "backoffice:faq:read": {
      name: "FAQ Urunga",
      description: "tūemi FAQ Nga korero ka Tauanga.",
    },
    "backoffice:category:write": {
      name: "Kāwai whakahaere",
      description: "Waihanga ka tūemi Hua Ngā Kāwai.",
    },
    "backoffice:category:read": {
      name: "Kāwai Urunga",
      description: "tūemi Kāwai Nga korero ka Tauanga.",
    },
    "backoffice:product:write": {
      name: "Hua whakahaere",
      description: "tāpiri ka tūemi Hua.",
    },
    "backoffice:product:read": {
      name: "Hua Urunga",
      description: "tūemi Hua Nga korero ka Tauanga.",
    },
    "backoffice:report:read": {
      name: "Ripoata Urunga",
      description: "tūemi toa Ripoata ka tūemi.",
    },
    "backoffice:finance:write": {
      name: "Pūtea Pūkete whakahaere",
      description: "tūemi Pūtea Nga kaute tūemi Ki te toa.",
    },
    "backoffice:finance:read": {
      name: "Pūtea Pūkete Urunga",
      description: "tūemi Pūtea Pūkete Nga korero ka Tauanga.",
    },
    "backoffice:page:write": {
      name: "whārangi whakahaere",
      description: "Whakatikatika Toa Nga wharangi.",
    },
    "backoffice:page:read": {
      name: "whārangi Urunga",
      description: "tūemi Toa whārangi Nga korero ka Tauanga.",
    },
    "backoffice:logistic:write": {
      name: "tūemi whakahaere",
      description: "Whakahaerehia nga korero arorau.",
    },
    "backoffice:logistic:read": {
      name: "tūemi Urunga",
      description: "Pānuihia nga korero mo te logistics me nga tatauranga.",
    },
    "backoffice:staff:write": {
      name: "Kaimahi whakahaere",
      description: "Whakatikatika ka tūemi Kaimahi Nga korero.",
    },
    "backoffice:staff:read": {
      name: "Kaimahi Urunga",
      description: "tūemi Kaimahi tūemi ka Tauanga.",
    },
    "backoffice:business-profile:write": {
      name: "tūemi kōtaha whakahaere",
      description: "Whakahaerehia nga korero pakihi.",
    },
    "backoffice:business-profile:read": {
      name: "tūemi kōtaha Urunga",
      description: "Pānuihia nga korero mo te pakihi pakihi.",
    },
    "backoffice:discount-code:write": {
      name: "Whakahaere Waehere hekenga",
      description: "Whakahaerehia nga waehere whakahekenga toa.",
    },
    "backoffice:discount-code:read": {
      name: "Utu Waehere Uru",
      description: "Pānuihia nga taipitopito waehere utu me nga tatauranga.",
    },
    "backoffice:wallet:write": {
      name: "Pukoro whakahaere",
      description: "tūemi kaiwhakamahi Pukoro.",
    },
    "backoffice:wallet:read": {
      name: "Pukoro Urunga",
      description: "tūemi Pukoro tūemi ka Tauanga.",
    },
    "backoffice:coupon:write": {
      name: "tīkitipati whakahaere",
      description: "tūemi toa Tikitipati.",
    },
    "backoffice:coupon:read": {
      name: "tīkitipati Urunga",
      description: "tūemi tīkitipati Nga korero ka Tauanga.",
    },
    "backoffice:offer:write": {
      name: "tuku whakahaere",
      description: "tūemi toa Nga tuku.",
    },
    "backoffice:offer:read": {
      name: "tuku Urunga",
      description: "tūemi tuku tūemi ka Tauanga.",
    },
    "backoffice:cashback:write": {
      name: "tūemi whakahaere",
      description: "tūemi toa tūemi Nga tuku.",
    },
    "backoffice:cashback:read": {
      name: "tūemi Urunga",
      description: "Panuihia nga korero mo te whakahoki moni me nga tatauranga.",
    },
    "backoffice:lottery:write": {
      name: "Rota whakahaere",
      description: "tūemi toa tūemi.",
    },
    "backoffice:lottery:read": {
      name: "Rota Urunga",
      description: "tūemi Rota Nga korero ka Tauanga.",
    },
    "backoffice:community:write": {
      name: "Hapori whakahaere",
      description: "tūemi Hapori tūemi.",
    },
    "backoffice:community:read": {
      name: "Hapori Urunga",
      description: "tūemi Hapori Nga korero ka Tauanga.",
    },
    "backoffice:customer:write": {
      name: "kiritaki whakahaere",
      description: "tūemi kiritaki Nga korero.",
    },
    "backoffice:customer:read": {
      name: "kiritaki Urunga",
      description: "tūemi kiritaki tūemi ka Tauanga.",
    },
    "backoffice:ribbon:write": {
      name: "tūemi whakahaere",
      description: "tūemi toa tūemi.",
    },
    "backoffice:ribbon:read": {
      name: "tūemi Urunga",
      description: "Pānuihia nga taipitopito riipene me nga tatauranga.",
    },
    "backoffice:ai:write": {
      name: "AI whakahaere",
      description: "tūemi AI tūemi.",
    },
    "backoffice:ai:read": {
      name: "AI Urunga",
      description: "tūemi AI tūemi ka Tauanga.",
    },
    "backoffice:vendor-payment:write": {
      name: "kaihoko 💳 Utu whakahaere",
      description: "tūemi kaihoko Nga utu.",
    },
    "backoffice:vendor-payment:read": {
      name: "kaihoko 💳 Utu Urunga",
      description: "tūemi kaihoko 💳 Utu Nga korero ka Tauanga.",
    },
    "backoffice:company:read": {
      name: "Kamupene Urunga",
      description: "tūemi Kamupene Nga korero ka Tauanga.",
    },
    "backoffice:company:write": {
      name: "Kamupene whakahaere",
      description: "tūemi Kamupene tūemi.",
    },
    "backoffice:note:read": {
      name: "Tuhipoka Urunga",
      description: "Pānuihia nga korero me nga korero e pa ana.",
    },
    "backoffice:note:write": {
      name: "Tuhipoka whakahaere",
      description: "Whakahaere tuhipoka.",
    },
    "backoffice:affiliate:write": {
      name: "Hononga whakahaere",
      description: "tūemi Hononga tūemi.",
    },
    "backoffice:affiliate:read": {
      name: "Hononga Urunga",
      description: "tūemi Hononga Nga korero ka Tauanga.",
    },
    "backoffice:print:write": {
      name: "tūemi whakahaere",
      description: "Whakahaere kōwhiringa tā.",
    },
    "backoffice:print:read": {
      name: "tūemi Urunga",
      description: "Pānuihia nga korero tuhi.",
    },
    "selldone:token:read": {
      name: "(ng) Urunga",
      description: "tūemi (ng) Nga korero ka Tauanga.",
    },
    "selldone:token:write": {
      name: "(ng) whakahaere",
      description: "Whakahaere tohu.",
    },

    //--------------------------- Selldone ---------------------------
    "selldone:monetization:read": {
      name: "tūemi Urunga",
      description: "Pānuihia nga taipitopito monetization me nga tatauranga.",
    },
    "selldone:monetization:write": {
      name: "tūemi whakahaere",
      description: "Whakahaerehia te moni, tae atu ki nga kaupapa tuku.",
    },
    "selldone:identification": {
      name: "tūemi Urunga",
      description: "Urunga kaiwhakamahi tūemi Nga korero.",
    },
    "selldone:developer:read": {
      name: "Kaihanga Urunga",
      description: "tūemi Kaihanga Nga korero ka Tauanga.",
    },
    "selldone:developer:write": {
      name: "Kaihanga whakahaere",
      description: "tūemi Kaihanga tūemi.",
    },
    "selldone:security:write": {
      name: "Haumarutanga whakahaere",
      description: "tūemi Haumarutanga tūemi.",
    },

    //--------------------------- Connect ---------------------------
    "connect:provider:write": {
      name: "tūemi whakahaere",
      description: "tūemi Kua hono te Kaituku.",
    },
    "connect:provider:read": {
      name: "tūemi Urunga",
      description: "Pānuihia nga taipitopito me nga tatauranga a te kaiwhakarato.",
    },

    //--------------------------- Agency ---------------------------
    "agency:read": {
      name: "tūemi Urunga",
      description: "Pānuihia nga taipitopito me nga tatauranga mo nga umanga.",
    },
    "agency:write": {
      name: "tūemi whakahaere",
      description: "Whakahaerehia nga korero umanga.",
    },

    //--------------------------- Vendor ---------------------------

    "vendor-read": {
      name: "kaihoko Urunga",
      description: "tūemi kaihoko Nga korero ka Tauanga.",
    },
    "vendor-write": {
      name: "kaihoko whakahaere",
      description: "tūemi kaihoko tūemi.",
    },
  },

  scope_group: {
    backoffice: {
      title: "tūemi",
      description: "Urunga Ki tūemi ka toa whakahaere tūemi.",
    },
    storefront: {
      title: "tūemi",
      description: "Urunga Ki kiritaki ka toa tūemi.",
    },
    profile: {
      title: "kōtaha",
      description: "Urunga kaiwhakamahi kōtaha tūemi.",
    },
    selldone: {
      title: "Selldone",
      description: "Urunga Selldone Ratonga ka tūemi.",
    },
    others: {
      title: "tūemi",
      description: "He maha nga waahanga kaore i tautuhia i roto i te punaha.",
    },
    vendor: {
      title: "kaihoko",
      description: "Urunga kaihoko tūemi ka whakahaere tūemi.",
    },
    connect: {
      title: "Connect OS",
      description: "Urunga Kua hono Ratonga ka Kaituku.",
    },
    agency: {
      title: "tūemi",
      description: "Te uru atu ki nga korero mo nga umanga me nga whakahaere.",
    },
  },

  /** {@see AdminCompanyPage} **/
  company_page: {
    title: "Kamupene",
    create_app: "Waihangahia he tono hou",
    menu: {
      edit: "Whakatikatika",
      apps: "Taupānga",
      dashboard: "Papatohu",
      companies: "Kamupene",
    },
    /** {@see AdminCompanyPage_Dashboard} **/
    dashboard: {
      /** {@see CompanyInfoWidget} **/
      info_widget: {
        title: "Nga korero rehita kamupene",
        address: "wāhi noho",
        website: "paetukutuku",
        email: "Īmēra",
        phone: "nama waea",
        register: "Rehita",
        developer: "Kaihanga",
        verified: "Manatoko",
        golden: "koura",
      },
    },
    /** {@see AdminCompanyPage_Develop} **/
    developer: {
      title: "Kaihanga",
      menu: {
        new: "Hou",
        apps: "Taupānga",
      },
    },
    /** {@see AdminCompanyPage_Edit} **/
    edit: {
      title: "Whakatikatika",
    },
  },
  /** {@see BAppAdd} **/
  add_app: {
    category_input: "Kāwai",
    code_input: "Waehere",
    name_input: "Ingoa",
    description_input: "Whakaahuatanga",
    video_input: "URL ataata",
    public_form: "Hanganga Korero Whanui",
    private_form: "Hanganga Korero Whaiaro",
    icon_input: "Ata Taupānga",
    logo_input: "Moko App",
    enable_input: "Kei te kaha te tono?",
    edit_action: "Whakatika Taupānga",
    create_action: "Waihanga Taupānga",
    notifications: {
      add_success: "I hangaia he putea pakihi {name}.",
      delete_success: "Kua whakatikahia te putea pakihi.",
    },
  },

  /** {@see AppPage} **/
  app_page: {
    title: "Taupānga",

    menu: {
      edit: "Whakatikatika",
      publish: "Whakaputa",
      history: "Putanga",
      dashboard: "Papatohu",
      company: "Kamupene",
      api: "API",
      images: "Whakaahua",
    },
    /** {@see AppPage_Dashboard} **/
    dashboard: {
      /** {@see AppInfoWidget} **/
      app_info_widget: {
        title: "Taupānga",
        app_code: "Waehere taupānga",
        category: "Kāwai",
        name: "Ingoa",
        installs: "Tāuta taupānga",
        uninstalls: "Wetetāuta taupānga",
        actives: "Nga taupānga kaha",
        description: "Whakaahuatanga",
        published: "Whakaputaina",
        waiting_for_review: "E tatari ana te whakaaetanga",
        app_mode: "Aratau tono",
      },
    },
  },
  /** {@see ShopPermissions} **/
  shop_permissions: {
    ADMIN: { text: "Rangatira", description: "Te uru katoa ki nga toa toa katoa." },
    PRODUCT: {
      text: "Kaiwhakahaere Hua",
      description: "Tāpiri, whakatika me te whakahaere i nga hua toa.",
    },
    AUDITING: {
      text: "Kaiwhakahaere Pūtea",
      description: "Tirohia nga korero putea me nga utu.",
    },
    CONTENT: {
      text: "Whakahaere Ihirangi",
      description: "Whakautua nga paatai, tuhia nga rangitaki, me nga wharangi hoahoa.",
    },
    OFFICER: {
      text: "Kaiwhakahaere Matua",
      description: "Tirohia me te aro turuki i nga mahi a nga kaimahi.",
    },
    EMPLOYEE: {
      text: "Kaimahi",
      description: "Tirohia nga ota me te whakatutuki i nga ota.",
    },
    MARKETING: {
      text: "Te hokohoko",
      description:
        "Te uru ki nga tautuhinga me te hanga kaupapa whakatairanga, te hokohoko imeera me etahi atu waahanga e pa ana ki te hokohoko.",
    },
    VIEWER: {
      text: "Kaitirotiro",
      description:
        "Te kaha ki te tiro i nga korero toa me te kore e taea te whakarereke, te whakarereke ranei.",
    },
  },

  /** {@see ShopRoles} **/
  shop_roles: {
    PhysicalOrdersChecking: {
      text: "Te Tirohanga Tinana",
      description: "Tirohia nga ota kua riro mai.",
    },
    PhysicalOrdersPackaging: {
      text: "Whakapai tinana",
      description: "Te whare putunga me te mahi takai.",
    },
    PhysicalOrdersDelivery: {
      text: "Tukunga tinana",
      description: "Ota kaipuke.",
    },
    PhysicalOrdersSupport: {
      text: "Tautoko Tinana",
      description: "I muri i te tautoko hoko.",
    },
  },

  /** {@see AccountStatus} **/
  account_status: {
    Checking: "Takitaki",
    Approved: "Kua whakapumautia",
    Rejected: "Kua whakakorehia",
    Deleted: "Kua mukua",
    Banned: "Kua rāhuitia",
    Payment: "Utu",
    SelldoneSprite: "Kaute putea Selldone",
    SelldoneStorage: "Te Rokiroki a Selldone Storage",
  },
  /** {@see BPageAccount} **/
  account_page: {
    title: "Pūkete",
    menu: {
      setting: "Tautuhinga",
      charge: "Te utu",
      transfer: "Whakawhiti",
      transactions: "Nga tauwhitinga",
      history: "Hītori",
      cards: "Kaari",
    },

    /** {@see BPageAccountTransactions} **/
    transactions: {
      title: "Nga Tauwhitinga Kaute",
      detail: {
        title: "Tauwhitinga",
        amount: "Te nui",
        description: "Whakaahuatanga",
        from: "Mai",
        to: "Ki",
        created_at: "Rā",
        receipt: "Rihiti",
        receipt_note: "Tuhipoka",
      },
      table: {
        type: "Momo",
        from: "Mai i te kaute",
        to: "Ki te kaute",
        amount: "Te nui",
        receipt: "Rihiti",
        created_at: "Rā",
        description: "Whakaahuatanga",
      },
    },

    /** {@see BPageAccountTransfer} **/
    transfer: {
      title: "Nga Utu Whakawhiti",
      form: {
        title: "Puka Tono Whakawhiti",
        sub_title:
          "Pupuri ki o ake putea, ki etahi atu ranei Selldone",
        message:
          "Kia tupato ki te whakawhiti moni, kare e taea e koe te hoki mai mena ka oti te whakawhiti.",
      },
    },
    /** {@see BPageAccountDeposit} **/
    deposit: {
      form: {
        title: "Kaute Kaute",
        sub_title:
          "Me utu e koe te toenga o to putea ki te hoko raihana, whakahou ake i to putea me te utu i nga riiriti punaha.",
        amount_input: "Te nui",
        charge_action: "Kaute Utu",
      },
      notifications: {
        qr_title: "Utu",
        qr_message: "Wāhitau utu {currency} hanga.",
        gateway_title: "Utu",
        gateway_message: "Te hono atu ki te kuaha utu.",
      },
    },

    /** {@see BPageAccountEdit} **/
    setting: {
      title: "Tautuhinga Pūkete",
      account_name_input: "Ingoa pūkete",
      notifications: {
        updated_success: "I pai te whakahou i to putea.",
      },
    },
  },

  /** {@see BAccountReceiptPaymentDialog} **/
  receipt_payment_dialog: {
    title: "Utu Rihiti",
    /*  message: 'If the following information is correct, hit the transaction confirmation key to make the transaction.',
      to: 'Destination account',
      amount: 'Amount',
      receipt: 'Receipt',
      description: 'Description',
      status_paid: 'This receipt has already been paid.',
      status_canceled: 'This receipt has been canceled.',
      pay_action: 'Pay via port',
      pay_internal_action: 'Payment from account',*/
  },
  /** {@see BAccountTransactionForm} **/
  transaction_form: {
    title: "Puka Whakawhitiwhiti",
    from_input: "Mai i te nama kaute",
    to_input: "Ki te nama kaute",
    from_input_message: "Ka tangohia te moni mai i tenei kaute.",
    account_name: "Ingoa pūkete",
    currency: "Moni",
    free_balance: "toenga wātea",
    to: "pūkete ūnga",
    amount: "Te nui",
    receipt_title: "Te utu mo te whiwhinga",
    receipt_input: "Nama rihīti",
    paid_status: "Kua utua kētia tēnei kirihiti.",
    paid_canceled: "Kua whakakorehia tenei rihīti.",
    description: "Whakamaramatanga",
    created_at: "Rā",
    duration: "Roanga",
    unlimited: "He mutunga kore",
    add_note_action: "Tāpiri Whakaahuatanga",
    note: "Tuhipoka",
    transfer_action: "Whakawhiti",
    confirmation: {
      title: "Tuku Moni",
      subtitle: "Whakaaetanga Whakawhitiwhiti",
      message:
        "Mena kei te tika nga korero e whai ake nei, paatohia te kii whakapumau tauwhitinga kia mahia ai te tauwhitinga.",
      from: "Mai i te kaute",
      to: "Ki te kaute",
      amount: "Te nui",
      fee: "Utu tauwhitinga",
      total_pay: "Te moni i tangohia mai i to putea",
      receipt: "Rihiti",
      description: "Whakaahuatanga",
      accept_action: "Whakaaetanga Whakawhitiwhiti",
    },
    enter_account_number: "Whakauruhia te nama kaute.",
    currency_not_match:
      "Ko te moni kaute e kore e rite ki te puna",
    account_number_not_valid: "He tika te nama kaute.",
    account_number_is_valid: "He tika te nama kaute.",
    receipt_not_valid: "Kei te hē te nama nama.",
    receipt_is_valid: "He tika te nama rihīti.",

    notifications: {
      confirm_success: "I tutuki pai te tauwhitinga.",
    },
  },

  /** {@see OPlanCalculator} **/
  price_calculator: {
    title: "Tatau ╏ Whakauru ki to pakihi",
    description:
      "Tatauhia me te whiriwhiri i te mahere pai i runga i o hokonga marama:",
    sale_input: "Te moni hoko ia marama",
    hypernova_plan: "Tonoa mo Hypernova %0!",
    show_normal_mode: "Whakaatuhia te aratau noa",
    show_hypernova_mode: "Neke atu i te 1M$ taku tauwhitinga!",
  },

  /** {@see OPlanFeaturesCard} **/
  price_card: {
    billed_monthly: "Kua namahia {amount} ia marama.",
    billed_yearly: "Ko te nama nama ia tau. {amount}",
    buy_license_action: "Hokona inaianei",
    select_your_store: "Tohua to toa",
  },

  /** {@see BSessionsMap} **/
  shop_session_widget: {
    total_sessions: "Hui katoa",
  },
  /** {@see BDashboardShopCustomersActivityMap} **/
  user_activities_keys: {
    add_baskets: "Tāpiri ki te Kete",
    remove_baskets: "Mukua mai i te Kete",
    buys: "Hoko",
    products_views: "Tirohia te Hua",
  },
  /** {@see BDashboardShopAcquisition} **/
  shop_visitors_widget: {
    title: "Tātaritanga Manuhiri",
    new_visitors: "Nga manuhiri hou",
    amp: "Nga tirohanga AMP",
    returning_visitors: "Ka hoki mai nga manuhiri",
    page_views: "Nga tirohanga wharangi",
    other_pages: "Ētahi atu whārangi",
    from_previous_7_days: "mai i nga ra 7 kua hipa",
  },

  /** {@see BUserAppLevel}   {@see AppLevel}  **/
  app_levels: {
    title: "Taumata Taupānga",
    Newbie: "Tauhou",
    Beginner: "Kaimatamata",
    Novice: "Tauhou",
    Intermediate: "Takawaenga",
    Advanced: "Arā atu",
  },

  /** {@see Login} **/
  login: {
    register_title: "Ko te huarahi o nga pakihi,<br> He wheako ngahau.",
    register_subtitle:
      "Waitohu i tenei ra mo te kore utu. E arohaina ana a Selldone e nga tiimata, nga umanga, nga kaihoko me nga kaihoko.",

    password_recovery_title: "Kupuhipa whakaora",
    password_recovery_message:
      "Mena kua wareware koe ki to kupuhipa, whakauruhia te wahitau imeera o to putea ka pehi i te kii whakau. Ka tukuna atu he imeera ki a koe me te hono ki te tautuhi i to kupuhipa.",
    email: "Tō wāhitau īmēra",
    password_recovery_action: "Whiwhi īmēra whakaora",

    register_google: "Haere tonu me Google",
    register_apple: "Haere tonu ki a Apple",
    continue_with: "Haere tonu",
    register_linkedin: "Haere tonu me LinkedIn",
    register_facebook: "Haere tonu me Pukamata Facebook",
    register_github: "Haere tonu me GitHub",
    create_your_account: "Waihangahia to putea",
    password_8_chars: "Me 8 te iti rawa o te kupuhipa.",
    password_upercase_number: "Kia kotahi te reta matua me te nama kotahi.",
    password_confirm: "Kua taurite te kupuhipa me te whakau.",
    privacy_agreement:
      "Ma te hainatanga, ka whakaae koe ki nga <a href='/terms' target='_blank'>Ture Whakamahi</a> me te <a href='/privacy' target='_blank'>Kaupapahere Whaiaro</a> a Selldone.",
    register_action: "Haere tatou, he kore utu!",
    login_action: "takiuru",
    already_signup: "Kua hainatia?",
    login_your_account: "Takiuru ki tō pūkete",
    login_google: "Haere tonu me Google",
    login_linkedin: "Haere tonu me LinkedIn",
    login_facebook: "Haere tonu me Pukamata Facebook",
    login_github: "Haere tonu me GitHub",
    remember_me: "Kia mahara ki ahau.",
    login_button: "takiuru",
    two_step_problem: "He raru koe ki te kupuhipa e rua-taahiraa?",
    two_step_problem_action: "E hiahia ana ahau ki te whakakore i te takiuru-rua-taahiraa.",
    two_step_confirm: "Whakaūhia te waehere uru",
    forgot_password: "Kua wareware te kupuhipa?",
    signup_free: "Waitohu mo te kore utu",
    checking_robot_message: "Te tirotiro mena he karetao koe, he tangata ranei..",
    notifications: {
      step2_login_success: "Takiuru-aunoa ki te kaute",
    },
    signup_email: "Waitohu ma te imeera",

    name_placeholder: "Ingoa",
    email_placeholder: "Īmēra",
    email_or_username_placeholder: "Īmēra, Ingoa Kaiwhakamahi, tau Waea rānei",
    password_placeholder: "Kupuhipa",
    re_password_placeholder: "Whakaū Kupuhipa",
    new_in_samin: "Hou i Selldone?",
    auto_create_shop: "Te hanga aunoa i te toa",
  },
  /** {@see Layout} **/
  layout: {
    logout: "Takiputa",
    menu: {
      my_public_profile_subtitle: "tūemi Tūmatanui kōtaha {name}.",

      personal_information: "Nga Korero Whaiaro me te Profile",
      personal_information_subtitle: "Whakatikatika tūemi kōtaha, wāhi noho ka KYC.",
      wallet: "Pukoro me nga kaute",
      wallet_subtitle: "tūemi Nga kaute ka Kua hono 💳 Utu Kaari.",

      companies: "Nga Kamupene me nga Kaute Pakihi",
      companies_subtitle: "Rēhitahia aku pūkete pakihi me te taake.",

      access: "Te uru ki taku Kaute API",
      access_subtitle:
        "OAuth Rokiroki Nga Kī Uru, Whaiaro Urunga tūemi ka Kua hono Taupānga.",

      security: "Haumarutanga",
      security_subtitle: "Whakaritehia nga take e rua me te whakahaere i nga whiringa takiuru hapori.",

      preferences: "Manakohanga Kaiwhakamahi",
      preferences_subtitle:
        "Hurihia Kaupapa, tūemi, reo ka Taunoa Moni.",
      my_subscriptions: "Taku Ohaurunga",
      my_subscriptions_subtitle:
        "Te uru ki te utu AI me nga waahanga whakaritenga.",
    },
  },

  /** {@see PageBuilder} **/
  page_builder: {
    title: "Hoahoa Paetukutuku",

    menu: {
      user_view: "Tirohanga",
      design: "Hoahoa",
      behavior: "Whanonga",
      seo: "SEO",
      setting: "Tautuhinga",
      embed: "Whakamau",
    },
    waiting_fetch: "Kei te whiwhi korero ..",

    /** {@see SettingCustomPage} **/
    setting: {
      title_input: "Taitara Whārangi",
      description_input: "Whakaahuatanga poto",
      name_input: "URL Whārangi (Reta me Tau)",
      bg_color_input: "Tae papamuri",
      bg_style_input: "Whārangi Kāhua Papamuri",
      page_label_input: "Tae tapanga",
      page_dir_input: "Te aronga wharangi",
      ltr: "Mauī ki Matau",
      rtl: "Matau ki Mauī",
      designer_note: "Tuhipoka Kaihoahoa",
    },

    /** {@see LPageEditor} **/
    design: {
      themes: {
        title: "Kaiāwhina Hoahoa Whaiaro",
        message: "Hoahoa nga wharangi pai rawa atu mo to pakihi me te kore tohu.",
        page_title_input: "Whakauruhia he taitara mo to wharangi",
      },
      tools: {
        rearrange: "Ota",
        tools: "Utauta",
        history: "Hītori",
        style: "Kāhua",
        typography: "Kuputuhi",
        hierarchy: "tūemi",
      },
      no_category: "Kore he waahanga",
    },

    /** {@see LPageEditorSeo} **/
    seo: {
      message:
        "Ko nga tautuhinga SEO ka whakatika aunoa e te Selldone SEO engine.",
    },
  },

  /** {@see VerifyEmailCaution} **/
  verify_email_caution: {
    title: "Whakaūhia to Wāhitau Īmēra",
    message:
      "Kua tukuna te hononga whakahohe hou ki to wahitau imeera {email}. Tena koa tirohia to imeera i mua i te mahi.",
    resend_message:
      "Mena kare ano koe i whiwhi i te hono whakahohenga i to imeera, panuihia te paatene i raro nei hei tuku ano.",
    resend_action: "Tukua ano te hono whakahohe",
    resend_notice: "Kua tukuna he hononga whakahohe.",
  },

  /** {@see ShopAutomation_Autopilot} **/
  autopilot: {
    start: "Te ra timata",
    end: "Te ra mutunga",
  },

  /** {@see Help} **/
  help: {
    menu: "Tahua",
    more_helps: "Pānuitia atu",
  },

  /** {@see WidgetCompanyCard} **/
  company_card: {
    pending_state: "E tatari ana mo te arotake",
    register: "Kua rehitatia",
    developer: "Kaihanga",
    verified: "Manatoko",
    premier: "Pirimia",
  },

  /** {@see ShopSubscriptionList} **/
  shop_license: {
    add: "Tāpiri ohaurunga",
    only_last: "Ko te ohaurunga whakamutunga anake",
    vouchers: "Nga putea",
    buy: "Hokona mahere ohaurunga",
    monthly: "Ia marama",
    yearly: "Ia tau",
    start_date: "Te ra timata",
    end_date: "Te ra mutunga",
    price: "utu",
    verify: {
      true_title: "tūemi Nga tikanga",
      true_description: "Ka panui ahau me te whakaae ki nga tikanga whakamahi a Seldone",
    },
    need_charge: "Me utu",
    buy_now: "Hokona inaianei",
    use_voucher_now: "Whakamahia te putea inaianei",
    plan_placeholder: "Tīpakohia to mahere ohaurunga",
    plan: "Mahere raihana",
    use_date: "Te ra whakahohenga",
    validity_duration: "Te roanga mana",
    validity_duration_hint:
      "Ma te hoko raihana 12-marama, ka whiwhi koe i te 30% hekenga.",

    active_license: {
      title: "Raihana Hohe",
      subtitle:
        "To korero raihana kaha.",
      subtitle_partner:
        "To korero raihana {name} kaha. Ko tenei toa he mahi hoa mahi i tenei wa. Mena kei te pirangi koe ki te whakakore i te mahi me te whakapai ake ki tetahi mahere taumata teitei, tena koa whakapā atu ki ta matou roopu tautoko mo te awhina.",
    },
    licenses_table: {
      title: "Hītori i hokona",
      subtitle:
        "He rarangi tenei o nga raihana kaha me nga raihana e tatari ana. Ka taea e koe te hoko utu i muri i to putea putea. Hei taapiri, kia mahara ka whakakorehia e koe he ohaurunga, ka whakahokia te toenga ohaurunga ki to putea mariko.",
    },
    license_detail: {
      title: "Nga korero",
      subtitle:
        "Ko nga here enei o to mahere o naianei. Mo etahi atu korero, tirohia te wharangi utu. Ka taea e koe te uru atu ki nga ahuatanga katoa puta noa i nga mahere katoa, tae atu ki te mahere kore utu, ki te timata i to pakihi me te tipu. I a koe e whakanui ana i to pakihi, whakaarohia te whakapai ake ki tetahi mahere taumata teitei hei whakaheke i o utu ratonga.",
    },

    table: {
      plan: "Mahere",
      start_date: "Te Ra Timata / Whakamutunga",
      renewal: "Whakahou Aunoa",
      linked_account: "Kaute Hononga / Putea",
      payment: "Utu",
      cancel: "Whakakore",
    },
    notifications: {
      subscribe_success: "I pai te ohauru.",
      cancel_success: "Kua whakakorea te ohaurunga.",
    },
    cancel_dialog: {
      title: "Whakakore te haamauraa ohaurunga",
      message: "Me whakakore tenei ohaurunga?",
      action: "Whakakore ohaurunga",
    },
    add_dialog: {
      wallet: {
        title: "tūemi he Pukoro",
        subtitle:
          "Tīpakohia he putea mariko hei whakamahi mo to ohaurunga. Mena kaore ano koe i a koe, ka taea e koe te hanga i roto i te waahanga putea. Ka whakamahia tenei putea mo nga utu katoa.",
      },
      auto_renewal: {
        true_title: "Whakahou Aunoa",
        true_description:
          "Ka whakahou aunoa to ohaurunga mena he nui te toenga o to putea, mena kei a koe he putea whaimana.",
      },
      options: {
        title: "Kōwhiringa taapiri",
      },
      verify: {
        title: "tūemi",
      },
    },
  },

  /** {@see BLayoutBackofficeShop} **/

  admin_shop_layout: {
    download_txt: "Tikiake kuputuhi raraunga",
    download_excel: "Tikiake raraunga Excel",
  },
  /** {@see BUserPreferences} **/
  user_preferences: {
    access: "Urunga",
    template: "tauira",
    feedback: "Urupare Fee",
    date_time: "Rā & Te wa",

    company: "Kamupene",
    company_message: "Mena kei a koe he kamupene me te hiahia ki te whakaputa taupānga.",
    access_keys: "Pātuhi Uru",
    access_keys_message: "Whakaatuhia nga kaihoko me nga taviri uru.",
    rating: "Me pehea e whakatau ai koe i to wheako me Selldone?",
    large_font: "Nga momotuhi nunui",
    large_font_message: "Whakaatuhia te kuputuhi nui ake i te tikanga",
    samples: "Whakaatuhia nga toa tauira",
    samples_message:
      "Ki te whakahohea, ka whakaatuhia etahi toa tauira ki to panui.",

    automation: "Aunoatanga",
    automation_message:
      "Whakaatuhia te ripa aunoa ki te papatohu toa. (Webhooks, Hotaka Ataata, ...)",
  },

  /** {@see BPageShopSettingLocations} **/
  shop_locations: {
    title: "Nga whenua e tukuna ana he ratonga",
    sub_title:
      "Tīpakohia nga whenua e hiahia ana koe ki te hoko i to hua mai i te rarangi i raro nei.",

    delete_restriction: "Mukua te here",
    set_restriction: "whakatakoto here",
    notifications: {
      locations_update: "I pai te whakahou i te waahi.",
    },
    table: {
      country: "Whenua",
      currency: "Moni",
      available: "Kei te waatea",
    },
  },

  /** @see ProductCard **/
  product_card: {
    sold_out: "Kua hokona atu",
    review_unit: "Arotake",
    not_original: "Ehara i te taketake",
  },

  /** @see BProductAddStudio **/
  product_studio: {
    add_mode: "Tāpirihia etahi atu korero",
    auto_category: "Tautuhi-aunoa kāwai",
    add_in_current_category: "Tāpirihia ki te kāwai o nāianei",
    category_mode_message:
      "Mena ka taea, ka taapirihia te hua ki te waahanga taunoa, ki te waahanga iti ranei.",
    sku_name_input: "SKU / Ingoa",
    info: {
      subtitle:
        "Hei whakaputa i te hua, ka taea e koe te whakauru i nga korero tino nui e tika ana mo tona hanganga.",
    },
    price: {
      subtitle:
        "Whakauruhia te utu me te utu mo tenei hua. Ina taapirihia te hua, ka whai waahi koe ki etahi atu whiringa.",
    },
    by_sku: {
      title: "tāpiri mā SKU",
      subtitle: "te tūemi i to Whenua to tūemi Kei te waatea.",
    },
    drop_shipping: {
      subtitle:
        "Kimihia nga hua ki te maakete rarawe ka taapiri atu ki to toa.",
    },
    by_connect: {
      subtitle:
        "Maamaa te taapiri i nga hua mai i te POD, i nga kaiwhakarato whakaheke ranei me te Selldone Connect OS — hono noa to toa me te pai ki te whakauru hua aunoa, kaore he taputapu e hiahiatia ana.",
    },
    your_license_is_not_eligible: "Ko to raihana kaore e tika.",
    sku_dialog: {
      title: "tāpiri Hua mā SKU",
      subtitle:
        "Kei a koe te kaha ki te kimi hua ma o raatau SKU i roto i ta maatau papaa raraunga me te taapiri ma te paato noa.",
    },
    dropshipping_dialog: {
      title: "tāpiri Te tuku tuku Hua",
    },
  },

  /**
   * @see BProductAddDropshippingShops
   */
  product_add_dropshipping_shops: {
    subtitle:
      "I konei, ka kitea e koe nga kaihoko rarawe e whakararangi ana i a raatau hua ki runga i a Selldone, e waatea ana mo etahi atu kaihokohoko. Ko te papaa tuku tuku-a-roto a Selldone he whakangawari i te mahi, he maamaa, he maamaa hoki mo nga kaihoko rarawe me nga kaihokohoko.",
  },

  /** @see Affiliates **/
  my_affiliate: {
    title: "Taku Kirimana Hononga",
    message:
      "Rarangi o nga kirimana hononga me etahi atu toa.",
  },

  /** @see BPageShopMarketingAffiliate **/
  affiliates: {
    title: "Nga hoa hono",
    subtitle:
      "Waihangahia nga hononga, whai i a raatau mahi i roto i te waa, ka mahi i runga i nga korero tika i te waahi kotahi. Ka awhina matou i a koe me o hoa ki te hanga hononga kaha.",

    affiliate_code: "Waehere hononga",
    crate_date: "Waihangahia te ra",
    last_payment_date: "Te ra utu whakamutunga",
    for_all_products: "Hoatu ki nga hua katoa",
    balance: "Taurite",
    fix_commission: "Komihana pumau",
    percent_commission: "Komihana i runga i te ōrau",
    total_payment: "Nga utu katoa",
    new_affiliate: "He hoa hono hou",
    new_affiliate_message: "Tāpiri hoa hono hou",

    link_factory: {
      title: "Kaihanga hononga hoa hono",
      link_input: "To hononga",
      link_output: "Hononga hanga",
    },
  },

  /**
   * @see BAffiliateEdit
   */
  affiliate_edit: {
    affiliate_commission: "Hononga Komihana",

    config: {
      new: "Hou Hononga tūemi",
      edit: "Whakatikatika Hononga tūemi",
      subtitle:
        "Whirihorahia nga tautuhinga whanui me te ingoa o te hononga ki konei.",
    },

    payment: {
      title: "💳 Utu tūemi",
      subtitle:
        "Whakaritehia te hanganga komiti mo te hononga. Ka taea e koe te whakakotahi i nga komihana pumau me te paheketanga-a-raupapa, te whakarite ranei i nga komihana motuhake mo ia hua.",
    },

    contact: {
      title: "Nga korero Whakapā",
      subtitle: "Whakaratohia nga taipitopito whakapā o te hoa hononga ki konei.",
    },

    restriction: {
      title: "Komihana tūemi",
      subtitle:
        "Tautuhia ko nga hua e tika ana mo te komihana. Ka taea e koe te aukati i nga komihana ki etahi hua.",
    },

    link_domain: {
      title: "tūemi Rohe",
      subtitle:
        "Honoa tetahi o o rohe ki tenei hoa hono. Ko nga manuhiri ka hoko mai i tenei rohe ka kiia ki tenei hononga.",

      domain_not_approved_msg: "Ko te rohe kaore ano kia whakaaetia!",
      domain_is_not_enable_msg: "Kaore i te whakahohea te rohe!",
      domain_is_linked_to_other_affiliate_msg:
        "Kua hono kē tenei rohe ki tetahi atu hononga!",
    },

    set_users_first_message: "Whakatakotoria te kaiwhakamahi i te tuatahi!",

    cluster: {
      title: "tūemi",
      subtitle:
        "Whakauruhia tenei hononga ki tetahi roopu hei whakahaere i te taha o etahi atu rauemi i te waahi kotahi.",
      manage_action: "tūemi",
    },

    notifications: {
      create_success: "I tutuki pai te hono hou.",
      update_success: "I momoho te whakahou i te hononga.",
    },

    inputs: {
      name: {
        label: "Hononga tūemi Ingoa",
        message: "hei tauira, John Doe, Alex Company, X Marketers, etc.",
      },
      commission: {
        label: "Komihana tūemi",
        false_description:
          "Tatauhia ma te whakamahi i te huinga o nga komihana tuturu me te ōrau.",
        true_description:
          "Tātaihia mā te whakamahi i ngā kōmihana hua-tauwhāiti kua whakaritea ki te ripa utu.",
        false_title: "tūemi + tūemi Komihana",
        true_title: "Hua-tūemi Komihana",
      },
      user: {
        label: "kaiwhakamahi (tūemi)",
        message: "Kōwhiringa, hei tauira, ingoa hononga i Seldone",
      },
      web: {
        placeholder: "Kōwhiringa, hei tauira, hononga-web.com",
      },
      address: {
        placeholder: "Kōwhiringa, hei tauira, 1200 POX, NY, USA",
      },
      tel: {
        placeholder: "Kōwhiringa, hei tauira, 001-808-210354",
      },
      bank: {
        placeholder: "Kōwhiringa, hei tauira, Arstra Bank, Nama: 10002-325-800-845213",
      },
      domain: {
        placeholder: "tūemi he Rohe... (tūemi)",
      },
      pos: {
        true_description:
          "Ka taea e nga hononga te tuku ota tika mo o raatau kaihoko ma o raatau roopu.",
        true_title: "POS Urunga",
      },
      enable: {
        true_description:
          "Kei te kaha tenei hononga i tenei wa? Ki te hohekore, kare to raatau kaupapa hono e pa ki nga ota i mahia ma o raatau hononga.",
      },
      products: {
        label: "Hua tūemi mō Komihana",
        select_product: "tūemi Hua",
      },
    },
    actions: {
      update: "Whakahou Hononga",
      add: "tāpiri Hononga tūemi",
    },
    /**
     * @see BPageAffiliateEdit
     */
    delete: {
      subtitle:
        "Ka taea e koe te whakatau i nga nama taapiri me te hononga i mua i te tango mai i to toa. Ko nga korero hononga ka noho waatea mo te wa iti i muri i te whakakorenga.",
    },
  },

  /**
   * @see BPageShopFinanceValuations
   */
  shop_valuations: {
    title: "Ritenga Te utu ka Nga uaratanga",
    subtitle:
      "Whakaritea he uara (he puka whakauru utu) ki tetahi hua, ka taea e nga kaihoko te kowhiri i nga momo rereke, te whakawhaiaro ranei i o taonga. Ka tatauhia e te punaha te utu i runga i nga whakaurunga ritenga.",
    add_valuation_action: "Waihanga Hou tūemi puka",
    empty_message:
      "Engari ki te hanga i nga momo rereke maha, whakaarohia te whakawhanake i tetahi rautaki whakatau ka taea te whakamahi ki tetahi hua ngawari.",
    delete_dialog: {
      title: "tūemi Tango",
      message:
        "Kei te tino hiahia koe ki te muku i tenei puka utu? Tena koa kia tangohia tenei rautaki utu ritenga mai i nga hua katoa i te tuatahi.",
      action: "Ae, Muku Inaianei",
    },
    notifications: {
      delete_success: "Kua pai te whakakore i te uara.",
    },
  },

  /**
   * @see BValuationInput
   */
  valuation_input: {
    label: "Te Utu me te Tauira Whakatau",
  },

  /**
   * @see BValuationAdd
   */
  valuation_add: {
    title_edit: "Whakatikatika tūemi puka",
    title_add: "Waihanga Hou tūemi puka",
    config: {
      subtitle:
        "Whakaturia he punaha uara hei hanga utu ngawari ma te whakamahi i nga tauira me nga ripanga tohutoro mo o hua. Me timata ma te hoatu ingoa ahurei tenei uaratanga.",
    },
    valuation_preview: "Arokite tūemi puka",
    load_sample: "Uta Tauira",
    conditions: {
      title: "Nga tikanga",
      subtitle:
        "Tautuhia te arorau mo nga rereke ritenga. Ko tenei whirihoranga ka ngawari te whakahaere o te waatea rereke.",
    },
    structure: {
      title: "tūemi",
      subtitle:
        "Hangaia to puka whakauru utu ki konei ma te whakauru i nga tauira me nga ripanga tohutoro.",
      expand_action: "tūemi",
      collapse_action: "tūemi",
    },
    inputs: {
      title: {
        message: "Kaore tenei taitara e kitea e nga kaihoko.",
        placeholder: "Whakauruhia he ingoa mo te utu, hei tauira, 'Te Utu Putea'",
      },
    },
    samples: {
      title: "tūemi Nga uaratanga",
    },
  },

  /** @see AdminGiftcardPage_Orders **/
  gift_card_orders: {
    title: "Nga ota me te Giftcard",
    table: {
      order: "Ota",
      order_price: "Te nui ota",
      paid_by_card: "Ka utua e te kaari",
      payment_status: "Te mana utu",
      date: "Rā",
    },
  },

  /** @see BPageOfferOrders **/
  offer_orders: {
    title: "Whakatakoto Ota",
    table: {
      order: "Ota",
      order_price: "Te nui ota",
      offer_amount: "Te rahinga tuku",
      payment_status: "Te mana utu",
      product: "Hua",
      date: "Rā",
    },
  },

  /** @see BPageCouponOrders **/
  coupon_orders: {
    title: "Nga Ota Tiikiti",
    table: {
      order: "Ota",
      order_price: "Te nui ota",
      coupon_amount: "Te nui o te coupon",
      payment_status: "Te mana utu",
      date: "Rā",
    },
  },

  /** @see BPageDiscountCodeOrders **/
  discount_code_orders: {
    title: "Nga Ota me te Waehere Whakatau",
    table: {
      order: "Ota",
      order_price: "Te nui ota",
      discount_amount: "Te moni whakaheke",
      payment_status: "Te mana utu",
      date: "Rā",
    },
  },

  /** @see BPageShopIncentives **/
  incentives: {
    discount_code: "Te utu",
    coupon: "tīkitipati",
    offer: "tuku",
    gift_card: "Kaari koha",
    lottery: "Rota",
    cashback: "tūemi",
  },

  /** {@see BPageShopIncentivesDiscountCode} **/
  discount_codes: {
    title: "Waehere hekenga",
    sub_title:
      "I tenei wahanga ka taea e koe te hoahoa i nga tiitieti utu utu kia rite ki o hiahia. He wehe kee enei utu mai i nga utu hua katoa ka pa ki te rahinga whakamutunga o te ota kaiwhakamahi.",
    add_new: "Tāpirihia te waehere utu utu hou",
    card: {
      created: "Waihanga",
      start: "Tīmata",
      end: "Whakamutunga",
      currency: "Moni",
      discount_percentage: "Te utu utu",
      discount_limit: "Tepe utu",
      state: "Tūnga o nāianei",
      enabled: "Inaianei kua whakahohea",
      disabled: "Kua monokia inaianei",
      used: "Kua pau",
      count: "Tatau",
      unit: "(ng) tūemi",
    },
    /** {@see BDiscountCodeAdd} **/
    dialog: {
      title_edit: "Whakatika waehere utu",
      title_add: "Tāpirihia te waehere utu utu hou",
      discount_code_input: "Waehere hekenga",
      discount_percent_input: "Te utu utu",
      discount_percent_input_hint: "Te utu utu mo te nui o te hoko",
      count_input: "Tatau",
      count_input_hint: "Te whakamahi morahi o tenei waehere",
      limit_input: "Tepe utu",
      limit_input_hint: "Te hekenga teitei i runga i te moni",
      currency_input_message: "Ka waatea nga utu mo tenei moni.",
      limit_input_message:
        "Ko te uara o te 0 mo te tepe hekenga ko te tikanga karekau he rohe mo te utu utu.",
      title_input: "Taitara",
      title_input_hint: "Ka taea e koe te whakauru i tetahi taitara mo tenei waehere utu",
      description_input: "Whakaahuatanga",
      description_input_hint:
        "Ka taea e koe te whakauru whakaahuatanga mo tenei waehere utu",
      date_start_input: "Tīmata",
      date_start_input_placeholder: "Tīpakohia te wa ka timata te hekenga",
      date_end_input: "Whakamutunga",
      date_end_input_placeholder: "Tīpakohia ka mutu te utu",
      notifications: {
        add: "I angitu te taapiri i te waehere utu.",
        edit: "I momoho te whakatika i te waehere utu.",
      },
      config: {
        title: "tūemi Whirihora",
        subtitle:
          "Ko te tātai o te tātaitanga waehere hekenga ko: te teitei (LIMIT, PERCENT * utu kaata). Kia mahara ki te hanga i nga waehere kia whai take me te ngawari ki te mahara.",
      },
      limit: {
        title: "tūemi",
        subtitle:
          "Ka taea e koe te tautuhi i te tepe whakamahi morahi mo ia waehere utu, me te nui o te utu utu, i tenei waahanga.",
      },
      duration: {
        title: "Roanga Tepe",
        subtitle:
          "Mena kei te pirangi koe ki te whakahohe i tetahi waehere utu utu mo tetahi waahi motuhake, ka taea e koe te whakarite i taua roanga ki konei.",
      },
      design: {
        subtitle:
          "Ka taea e koe te whakarite taitara me te whakaahuatanga mo ia waehere utu. Ka whakaatuhia enei korero ki te kaiwhakamahi.",
      },
      cluster: {
        subtitle:
          "Ma te hono i tenei waehere hekenga ki te tautau, ka taea e koe te whakahaere i te taha o etahi atu rauemi i te waahi kotahi.",
      },
    },
    delete_alert: {
      title: "Tango Waehere hekenga",
      message: "Me muku tenei waehere hekenga?",
      action: "Tango hekenga",
    },
    notifications: {
      delete_success: "I pai te whakakore i to korero",
    },
  },

  /** {@see BPageShopIncentivesGiftcard} **/
  gift_cards: {
    title: "Kaari koha",
    sub_title:
      "Waihangahia he kaari koha ka mahia kia rite ki taau e hiahia ana. He utu kei runga i tenei kaari, ka taea anake te whakamahi mo nga hoko i to toa.",

    create_new: "Tāpirihia he kaari koha hou",
    card: {
      created: "Waihanga",
      lifetime: "Te ra paunga",
      lifetime_unit: "Marama mai i te ra i hangaia ai ia kaari",
      currency: "Moni",
      initial_balance: "Te utu tuatahi",
      count: "Tatau",
      count_unit: "tūemi",
      used: "Kua whakamahia",
    },
    dialog: {
      title: "Hoahoa Kaari Koha",
    },
    alert: {
      title: "Tango Waehere hekenga",
      message:
        "Me muku tenei waehere hekenga?",
      action: "Mukua",
    },
    notifications: {
      delete_title: "Mukua te kaari koha",
      delete_message: "Kua pai te muku i to korero.",
    },
  },

  /** {@see BPageShopIncentivesCoupon} **/
  coupons: {
    title: "Tikitipati",
    sub_title:
      "Mena kei te hiahia koe ki te tuku utu utu ki o kaihoko ka taea te whakamahi me nga tikanga motuhake, ka taea e koe te hanga tiitieti. Ka taea e nga tīkitipati te whakauru i te hekenga pahekeheke me te tepe, te moni tuturu, te huinga ranei o nga mea e rua.",
    create_new: "Tāpirihia he coupon hou",
    qualified_products: "Hua e tika ana",
    min_purchase: "Te hokonga iti",
    only_first_order: "Ko te hoko tuatahi anake",

    one_time_use: "Whakamahinga kotahi",
    multiple_use: "Te whakamahi maha",
    amount_usage: "Te nui kua whakamahia",

    delete_alert: {
      title: "Mukua te coupon",
      message: "Me muku koe i tenei coupon?",
      action: "Ae, mukua te coupon",
    },
    notifications: {
      delete_success: "Kua pai te whakakore i te Tikitipati.",
    },
  },
  /** {@see BCouponAdd} **/
  coupon_edit: {
    title_edit: "Whakatika coupon",
    title_add: "coupon hou",
    discount_charge: "Te utu me te utu",
    charge: "Te utu",
    charge_message: "Te nui o te utu tuatahi",
    percent: "hekenga ōrau",
    percent_message: "Te nui o te hekenga ōrau",
    limit_input: "Tepe",
    limit_input_hint: "Te rahinga tepe utu",
    currency_input_message: "E waatea ana te tiiti mo tenei moni.",

    count_input: "Tatau",
    count_input_hint: "Te whakamahi morahi o tenei waehere",
    limit_input_message:
      "Ko te uara o te 0 mo te tepe hekenga ko te tikanga karekau he rohe mo te utu utu.",
    date_start_input: "Tīmata",
    date_start_input_placeholder: "Tīpakohia te wa ka timata te coupon",
    date_end_input: "Whakamutunga",
    date_end_input_placeholder: "Tīpakohia ka mutu te utu",
    title_input: "Taitara",
    title_input_hint: "Ka taea e koe te whakauru i tetahi taitara mo tenei waehere utu",
    description_input: "Whakaahuatanga",
    description_input_hint:
      "Ka taea e koe te whakauru whakaahuatanga mo tenei waehere utu",
    coupon_code: "Waehere coupon",
    need_code: "Kei te hiahia waehere?",

    coupon_color: "Tae coupon",
    min_purchase_amount: "Te rahinga hoko iti",
    min_purchase_amount_hint:
      "Ko te nui o te hoko me nui ake, kia rite ranei ki tenei uara.",
    should_exist_product: "Kia kotahi o enei hua kei roto i te kaata",
    apply_for_first_order: "Mo te hoko tuatahi anake",
    apply_for_first_order_on:
      "Ka taea anake te whakamahi i tenei coupon mo te hoko tuatahi a te kaihoko!",
    apply_for_first_order_off:
      "Ko te maha o nga whakamahi coupon mo ia kaihoko ka taea te whakatika i raro nei.",

    has_qualify_constraints: "Kei te hiahia koe ki te tautuhi i etahi atu tepe?",
    has_qualify_constraints_message:
      "Whakaritea etahi atu tikanga hei whakamana i te kaihoko ki te whakamahi i tenei coupon.",
    one_time_use_message: "Ka taea e te kaihoko te whakamahi i tenei coupon mo te wa kotahi.",
    daily_limit: "Te rohe o ia ra",
    no_limit: "Kaore he rohe",
    daily_limit_message: "Te nui o te whakamahi i ia ra e te kaihoko",

    monthly_limit: "Te rohe o ia marama",
    monthly_limit_message: "Te nui o te whakamahi i ia marama e te kaihoko",

    yearly_limit: "Te rohe o ia tau",
    yearly_limit_message: "Te nui o te whakamahi i ia tau ma te kaihoko",

    notifications: {
      add: "I momoho te waihanga coupon hou.",
      edit: "Kua whakahoutia te coupon.",
    },

    config: {
      subtitle:
        "Ko te tātai o te tātai hekenga coupon ko: CHARGE + max (LIMIT, PERCENT * utu kaata).",
    },
    limit: {
      subtitle:
        "I roto i tenei waahanga, ka taea e koe te whakarite i te maha morahi o nga whakamahinga mo ia coupon me te utu utu nui ka taea e ia coupon te whakarato.",
      zero_message: "tūemi: tūemi to kahore Tepe!",
    },
    duration: {
      subtitle:
        "Mena kei te pirangi koe ki te whakarite i te waa mo te coupon kia kaha, ka taea e koe te whirihora i te roanga o tenei waahanga.",
    },
    design: {
      subtitle:
        "Ka taea e ia coupon te taapiri i te taitara me te whakaahuatanga. Ka kitea enei korero ki te kaihoko.",
    },
    constraints: {
      subtitle:
        "I konei, kei a koe te kaha ki te whakarite i etahi taapiri taapiri mo te coupon.",
      no_limit: "kahore 20% Te herenga",
      has_limit: "20% tūemi",
      has_code_message: "Me whakauru te kaiwhakamahi i te waehere hei taapiri coupon.",
    },
    club: {
      subtitle:
        "Ko te whakamahi o tenei coupon ka taea anake ki nga mema o nga karapu kaihoko kua tohua.",
    },
    cluster: {
      subtitle:
        "Ma te hono i tenei coupon ki te tautau, ka taea e koe te whakahaere i te taha o etahi atu rauemi i te waahi kotahi.",
    },
    preview: {
      subtitle: "tūemi te Tūmatanui Arokite o te tīkitipati.",
    },
  },

  /** {@see BPageShopIncentivesOffer} **/
  offers: {
    title: "Nga tuku",
    sub_title:
      "Ka taea e koe te hanga tuku motuhake mo o kaihoko na, i muri i te hokonga o tetahi mea, ka whiwhi ratou i tetahi atu mea mo te utu utu, ahakoa kore utu.",
    create_new: "Tāpirihia he tuku hōu",
    qualified_products: "Hua e tika ana",
    min_quantity: "Te rahinga iti",
    min_purchase: "Te hokonga iti",
    amount_usage: "Te nui kua whakamahia",

    delete_alert: {
      title: "Mukua te tuku",
      message: "Me muku koe i tenei tuku?",
      action: "Ae, mukua te tuku",
    },
    notifications: {
      delete_success: "Kua tutuki pai te tuku.",
    },
  },

  /** {@see BOfferAdd} **/
  offer_edit: {
    title_edit: "Whakatika tuku",
    title_add: "He tuku hou",

    design_style: "Hoahoa me te ahua",
    eligible_constraints: "Nga rohe e tika ana mo nga kaihoko",

    title_input: "Taitara",
    title_input_hint: "Ka taea e koe te whakauru i tetahi taitara mo tenei waehere utu",
    description_input: "Whakaahuatanga",
    description_input_hint: "Tuhia he whakaahuatanga mo tenei waehere utu",

    currency_input_message: "Kei te waatea te tuku mo tenei moni.",
    qualified_products: "Hua e tika ana",
    qualified_products_message:
      "Kia whai waahi nga hua, ko te katoa o nga taonga kei roto i te kaata kia rite, kia nui ake ranei i te rahinga iti.",
    min_items: "Te rahinga iti o nga taonga",
    min_items_message: "Te maha o nga taonga me hoko e te kaihoko",
    offered_products: "Hua tuku",
    offered_products_message: "Kia kotahi o enei hua kei roto i te kaata.",
    discount_percent: "Te ōrau hekenga",
    discount_percent_message: "Te ōrau hekenga mō ēnei tūemi.",
    discount_percent_hint: "Te nui o te ōrau hekenga",

    per_order: "Te maha o nga whakamahinga mo ia ota",
    per_order_message:
      "Ko te nuinga o nga wa ka taea te tuku te tuku ki tetahi ota",

    count_input: "Te nui",
    count_input_hint: "Te whakamahi morahi o tenei tuku",

    min_purchase: "Te rahinga hoko iti",
    min_purchase_hint:
      "Ko te utu hoko me nui ake, kia rite ranei ki tenei uara",
    limitation: "Te herenga",

    date_start_input: "Tīmata",
    date_start_input_placeholder: "Tīpakohia te wa ka timata te coupon",
    date_end_input: "Whakamutunga",
    date_end_input_placeholder: "Tīpakohia ka mutu te utu",
    max_actives_is_3: "E toru noa nga tuku kaha i te wa kotahi.",
    enable: "Hoatu aunoa",

    notifications: {
      add: "I momoho te hanga tuku hou.",
      edit: "I pai te whakahou i te tuku.",
    },
    config: {
      subtitle:
        "Me timata ki te hanga i tetahi mahi mohio ma te whakarite i tana taitara, whakaahuatanga, me te mana o te waatea.",
    },
    limit: {
      subtitle: "Tautuhia te tepe whakamahi morahi mo tenei tuku i tenei waahanga.",
    },
    duration: {
      subtitle:
        "Mena kei te pirangi koe ki te whakarite i te waa mo te tuku, ka taea e koe te whirihora i te roanga i konei.",
    },
    constraints: {
      subtitle:
        "Whakaritehia nga paearu whakauru me nga tikanga mo te tuku ki konei. Ka pa te tuku ki nga taonga kaata e tutuki ana i enei tikanga.",
    },
    discounted_products: {
      title: "tūemi Hua",
      subtitle:
        "Ka taea e koe te tohu i nga hua ka tukuna he utu whakahekenga.",
    },
    cluster: {
      subtitle:
        "Ma te hono i tenei tuku ki te tautau, ka taea e koe te whakahaere i te taha o etahi atu rauemi i te waahi kotahi.",
    },
  },

  /** {@see BPageCampaignAds} **/
  campaign_ads: {
    menu: {
      banner: "kara",
      reportage: "Pūrongo",
      social: "Pāpori pāpori",
      offline: "Tuimotu",
    },
  },

  /** {@see BPageShopMarketingEmail} **/
  emails: {
    title: "Īmēra Marketing",
    subtitle:
      "Waihangahia nga imeera ma te toia me te whakataka, te whakarite me te whakarewa i nga kaupapa hei whakanui ake i te pupuri, me te kite i nga korero pono me te tika i te waahi kotahi.",
    cation_message:
      "⚠ Kua whakawhäitihia mo nga kaihokohoko kaore i whakamanahia. (Beta)",

    subject: "Kaupapa",
    from: "Mai",
    in_que: "E tatari ana",
    schedule_at: "Hōtaka i",
    sent: "Tukuna",
    sends: "Ka tuku",
    delivers: "Kua tukuna",
    views: "Tirohanga",
    clicks: "Pāwhiri",
    buys: "Nga hokonga",
    new_email: "Tāpirihia he imeera hou",
    new_email_message: "Waihanga me te whakarite i nga imeera hokohoko hou",
  },

  /** {@see BPageEmailMarketingDashboard} **/
  email_dashboard: {
    review: "Arotake",
    waiting_for_approve: "E tatari ana mo te whakau na {name}",
    approved_by: "I manatokohia te imeera e {name} ka tukuna.",
    not_submitted_send_request:
      "Kaore ano koe kia tukuna he tono kia tukuna mai tenei imeera.",
    reviewed_by: "I arotakehia e {name}",
    status: "Tūnga īmēra",
    conversion: "Reiti hurihanga",

    sell_performance: "Mahi hoko",
    total_buys: "Te maha o nga hokonga",
    amount_buy: "Tapeke moni hoko",
    average_sell: "Nga hoko toharite",
    sessions: "Wātū",
    overview: "Tirohanga",
    purchase_amount: "Te moni hoko",
    actions: "Mahi Kaiwhakamahi",
  },

  /** {@see BPageEmailMarketing} **/
  email_page: {
    emails: "Nga imeera",
    edit: "Whakatikatika",
  },
  /** {@see BPageAffiliator} **/
  affiliate_page: {
    affiliates: "Nga hoa hono",
    orders: "Nga ota",
    payments: "Nga utu",
  },

  /** {@see BPageAffiliateDashboard**/
  affiliate_dashboard: {
    link: {
      title: "Hononga Hononga",
      subtitle:
        "Hangaia he hononga ahurei mo to hoa hono ki te whai i nga hoko me nga komihana. Tāpirihia nga waehere tohutoro '{code}' ki tetahi hononga, tautapa ranei he rohe motuhake mo to hononga. Ka aroturukihia e te punaha mo te 48h.",
    },
    fix_commission: "Komihana pumau",
    percent_commission: "Komihana i runga i te ōrau",
    last_payment: "Utu whakamutunga",
    balance: "Taurite",
    total_payment: "Nga utu katoa",
    status: "Tūnga hoa hononga me nga here",
    available_for: "Wātea mō",
    orders_count: "Te nui o nga ota",
    orders_accepted: "Nga Ota Whakaae",
    finance: "Pūrongo pūtea",

    amount_accepted: "Te moni kua whakaaetia",
    amount_paid: "Te Moni Utu",
    sessions: "Wātū",
    affiliate_payment: "Utu Hoa Hononga",
    amount_payment: "Te Moni Utu",
    amount_payment_hint: "Te nui o te tauwhitinga",
    i_paid: "I utua e ahau te {amount} {currency} ki a {name}",
    pay_now: "Utu inaianei",
  },

  /** {@see BPageShopPermissionRoles**/
  permission_roles: {
    new_role: "Tāpiri tūranga hou",
    new_role_message: "Kia ngawari ake te mahi a te roopu.",
    new_role_action: "Tāpiri tūranga hou",
  },
  /** {@see Affiliate**/
  user_affiliates: {},
  /** {@see BPageShopIncentivesLottery**/
  shop_lotteries: {
    title: "Rota",
    sub_title:
      "Ka taea e o kaihoko te takaro keemu me te wikitoria. Me kai nga kaihoko o to toa kia 10 maramara mo ia keemu. Hei tauira, ki te hiahia koe kia whai waahi o kaihoko ki te wikitoria 1000 USD, me whakarite e koe te nui o ia utu maramara ki te 100 USD. Ko te uara o te 0 mo ia maramara ko te tikanga ka whiwhi te kaihoko i tetahi maramara mo ia hokonga.",
    manually_send_chip: "Kei te pirangi koe ki te tuku maramara ki nga kaiwhakamahi?",
    manual_send_token_action: "Tohu a-ringa",
    lottery_enable: "Kua whakahohea te rota?",
    chip_price_input: "Utu maramara",
    zero_chip_message: "Homai kia 1x maramara mo ia ota",
    chip_has_price_message: "Homai kia 1 maramara mo ia {amount} {currency} o te ota.",
    chip_disabled_message: "Kua monoa te maramara!",
    create_new: "Tāpiri taonga hou",
    delete_alert: {
      title: "Mukua te Tohu",
      message: "Me muku tonu tenei taonga?",
      action: "Ae, Muku Inaianei",
    },
    notifications: {
      delete_success: "I pai te whakakore i te taonga.",
    },

    config: {
      subtitle:
        "Whakanuia nga hoko a nga kaihoko me te wheako whakaari i to toa. Whakaritehia nga taonga me nga whiringa toa mo nga kaihoko ka whiwhi tohu mai i nga hoko. He 10 nga tohu mo ia taakaro keemu, ka taea e koe te tuku taonga motuhake mo nga kaihoko tuatahi. Ko te keemu taunoa ko Wheel of Fortune, engari ka taea e koe te whakarite kia uru ki te tahora o to toa.",
    },
    empty_prize_list: "Waihanga me te whakahaere taonga i konei...",
  },

  /** {@see BLotteryAdd**/
  lottery_edit: {
    title_edit: "Whakatikaina mea rota",
    title_add: "Tuemi rota hou",
    general_config: "whirihoranga whānui",
    design_style: "Te ahua me te ahua",
    enable_message: "Ko te nui rawa o nga mea kei roto i te wira he 14.",
    title_input: "Taitara",
    title_input_hint: "Tuhia he taitara poto.",
    description_input: "Whakaahuatanga",
    description_input_hint: "Tuhia he whakaahuatanga mo te taonga.",
    quantity: "Te nui",
    quantity_message: "Katoa nga taonga e waatea ana",
    image: "Whakaahua",
    image_placeholder: "Tīpakohia he whakaahua mo te taonga",
    chance: "Faingamālie",
    chance_message: "Wāhanga Toa i roto i te ōrau",
    free_for_first: "He kore utu mo nga kaihoko hou?",
    free_for_first_message: "Ka taea e nga kaihoko hou te wikitoria i tenei taonga.",
    not_free_for_first_message: "Kaore tenei taonga e waatea mo te takaro koreutu tuatahi.",

    amount: "Te moni utu",
    amount_hint: "Ka wikitoria e nga kaiwhakamahi tenei moni utu mo o raatau ota.",
    currency_message:
      "Moni mo tenei mea. Ko etahi atu moni ka hurihia ma te utu utu.",
    discount: "Te utu",
    discount_message: "Hekenga hekenga mo te ota a nga kaihoko",
    discount_limit: "Tepe utu",
    discount_limit_hint: "Te utu morahi mo te kaihoko.",
    color: "Tae",
    condition: "Nga tikanga",

    notifications: {
      add: "I momoho te hanga o nga taonga rota hou.",
      edit: "I momoho te whakarereketanga o nga taonga rota.",
    },
    config: {
      subtitle:
        "I konei, ka taea e koe te hanga taonga rota o to toa. Whakaritehia te taitara, te whakaahuatanga, te tae me te ahua mo ia taonga.",
    },
    design: {
      subtitle:
        "I konei, kei a koe te whiringa ki te kowhiri i tetahi ahua me te tae mo te taonga.",
    },
    constraints: {
      subtitle: "Tauwhāitihia ngā paearu me ngā herenga mō te toa i tēnei taonga.",
    },
    prize: {
      title: "tūemi",
      subtitle:
        "Whakaritehia nga taipitopito taonga i konei. Ko nga whiringa ko nga waehere utu, nga kaari koha, nga whiwhinga, me etahi atu utu hei utu rota.",
    },
    product_input: {
      label: "Hua tūemi",
      message: "Tīpakohia he hua mo te utu (he kore utu)",
    },
  },

  /** {@see BGiftcardTypeInput**/
  gift_card_types_list: {
    label: "Momo kaari koha",
    message: "Tīpakohia he momo kāri koha mai i te rārangi.",
  },

  /** {@see BEmailMarketingEditor**/
  email_builder: {
    service_unavailable: "Kāore te tūmau mēra i te hohe i tō pūkete.",
    design_section: "To pereti hoahoa imeera",
    schedule_switch: "Whakaritea te tuku imeera?",
    schedule_placeholder: "Tīpakohia te wā ki te tuku īmēra",
  },

  /** {@see BCustomersFunnel**/
  user_funnel: {
    REGISTER_DATE: {
      title: "Te ra rehitatanga",
      description: "Tātarihia mā te rā rēhitatanga o te kaiwhakamahi.",
    },
    LAST_BUY_DATE: {
      title: "Te hoko whakamutunga",
      description: "Te ra hoko whakamutunga.",
    },
    LOGIN_DATE: {
      title: "Te ra takiuru",
      description: "Te ra takiuru whakamutunga.",
    },
    LEVEL: {
      title: "karapu Kiritaki",
      description: "Nga mema o te karapu Kiritaki.",
    },
    SUBSCRIBED: {
      title: "Kua ohauru",
      description: "Kia whai ohaurunga mo te whiwhi imeera.",
    },
    NOT_PURCHASED: {
      title: "Karekau he Hokonga",
      description: "Ko nga kaihoko kaore ano kia hoko mai i tenei wa.",
    },
    SEX: {
      title: "Ira tangata",
      description: "Ira tangata.",
    },
    LOCATION: {
      title: "Tauwāhi",
      description: "Tauwāhi Kaiwhakamahi.",
    },

    LIMIT: {
      title: "Tepe",
      description: "Whakatakotoria he rohe mo te maha o nga kaiwhakamahi.",
    },

    SEGMENTS: {
      title: "Nga waahanga",
      description: "Whakaitihia nga kaihoko i roto i nga waahanga motuhake.",
    },
  },

  /** {@see InlineHelp**/
  inline_help: {
    helps_title: "Aratohu Kaiwhakamahi Ratonga",
    blogs_title: "Ihirangi e pa ana ki a Selldone Blog",
  },

  /** {@see BPageShopFinanceTax**/
  shop_tax: {
    region: "Rohe",
    label: "Tapanga",
    country_tax: "Taake Whenua",
    customer_tax: "Kaihoko VAT",
    business_tax: "Pakihi VAT",
    shipping: "Te tuku",
    override: "Whakakore",
    country: "Whenua",
    vat_range: "Awhe VAT",
    custom: "Ritenga",
    regions: "Rohe",
    rest_world: "Te toenga o te ao",
    overrides: "Whakakore",
    dialog_set_title: "Tautuhinga taake",

    tax_setting: "Tautuhinga taake",
    vat: "VAT",

    calculate_tax_input: "Tātai aunoa tāke",
    calculate_tax_message:
      "Ma te whakahohe i tenei whiringa, ka tatauhia te VAT ka tono mo o ota.",

    tax_number: "Te nama taake",
    tax_number_message:
      "Whakauruhia to nama taake. Ka tuhia tenei nama rehitatanga ki te nama a te kaihoko.",

    tax_included_in_price_input: "Taake kei roto i te utu o nga hua",
    tax_included_in_price_message:
      "Tīpakohia tēnei kōwhiringa mēnā ka whakauruhia e koe te moni tāke ki te utu o tō hua. Ka tatauhia te nui o te taake me te tauira e whai ake nei:",
    tax_included_in_price_formula:
      "Taake = (utu * VAT) / (1 + VAT), hei tauira, ko te nui o te taake mo tetahi hua i te utu o te 100$ me te uara taapiri o te 10% ka 9.09$.",

    tax_exclude_title: "Kua wehea ╏ Ka taapirihia te taake i te wa o te utu",
    tax_excluded_message:
      "Ko nga taake ka tatauhia i mua i nga utu i te wa o te tirotiro ka taapiri atu ki te utu taake kua whakakorehia i runga i nga taunakitanga a to kaihoko.",

    tax_on_shipping_input: "Tonoa nga taake ki nga kaipuke",
    tax_on_shipping_message:
      "Ma te kowhiri i tenei whiringa ka tukuna te taake ki nga kaipuke.",

    fixed_tax_rate: "Reiti taake pumau",
    dynamic_tax_rate: "Taake i runga i nga rohe",

    locations_tip:
      "Ko te rarangi o runga ake e whakaatu ana i nga whenua kua tohua e koe i roto i nga Tautuhinga Toa> Nga waahi hei whenua e hokona ana e koe.",
    locations_action: "Rokiroki Tautuhinga Tautuhinga",

    save_alert: "Kaore i te tiakina nga tautuhinga!",

    pos_tax_setting: "Tautuhinga Taake POS",
  },

  /** {@see BPageTransportation**/
  transportation_page: {
    menu: {
      dashboard: "Papatohu",
      transportations: "Nga tikanga tuku",
      orders: "Nga ota",
      setting: "Tautuhinga",
      delivery_services: "Ratonga",
      delivery_persons: "Karere",
    },

    go_to_courier_list: "Tirohia te rarangi karere",
    go_to_service_list: "Tirohia te rarangi ratonga kaipuke",
  },

  /** {@see BPageTransportationCouriers**/
  transportation_persons: {
    title: "Karere",
    notifications: {
      success_add: "I tapirihia e te kaikawe.",
      success_reset: "Tautuhi tatauranga.",
      success_remove: "I momoho te tango a te kaikawe.",
      success_update: "I angitu te whakahou i nga korero mo nga karere.",
    },
    reset: {
      title: "Tautuhihia nga tatauranga",
      message: "Me tautuhi ano koe i nga tatauranga o tenei karere?",
      action: "Ae, tautuhi ano inaianei",
    },
    remove: {
      title: "Tango Karere",
      message: "Me muku tonu tenei karere?",
      action: "Ae, Tango inaianei",
    },
    add_dialog: {
      title: "Tāpirihia he karere hou",
      action: "Tāpiri karere",
    },
  },
  /** {@see BPageTransportationOrders**/
  transportation_orders: {
    title: "Nga Ota mo te Tukunga",
    notifications: {
      success_update: "I pai te whakahou i te mana tuku.",
      success_payment: "I tutuki pai te utu COD.",
    },
  },
  /** {@see BPageCourier**/
  delivery_person: {
    actions_title: "Tautuhinga Karere",
    action_delivered: "Ka tukuna",
    action_paid: "Ka utua te COD",
    action_returned: "Kua hoki mai",
    cod_confirm_action: "Whakaaetia te utu COD",
    pickup_dialog: {
      title: "Ko koe te kaikawe mo tenei kete?",
      message:
        "Mena kei a koe te kawenga mo te kawe i tenei peera, tohua te Ae, mena ka tohua te Kao.",
    },
    status_dialog: {
      title: "Hurihia te tūnga ki te {status}",
      message: "Me huri ke te mana tuku ki te {status}?",
      action: "Ae, ka whakapumautia e ahau!",
    },
    cod_dialog: {
      title: "Utu COD",
      message:
        "Kei te whakaū koe kua riro mai i a koe te moni ota mai i te kaihoko?",
      action: "Ae, ka whakamanahia e au te utu!",
    },
  },

  /** {@see BPageTransportationServices**/
  transportation_services: {
    title: "Ratonga tuku",
    notifications: {
      success_add: "Kua taapirihia te ratonga tuku.",
      success_reset: "Tautuhi tatauranga.",
      success_remove: "I momoho te tango ratonga.",
      success_update: "I momoho te whakahou i te ratonga.",
    },
    reset: {
      title: "Tautuhihia nga tatauranga",
      message: "Me tautuhi ano koe i nga tatauranga o tenei ratonga?",
      action: "Ae, tautuhi ano inaianei",
    },
    remove: {
      title: "Tango ratonga tuku",
      message: "Me muku tonu tenei ratonga?",
      action: "Ae, Tango inaianei",
    },
    add_dialog: {
      title: "Tāpiri ratonga tuku hou",
      action: "Tāpiri ratonga",
    },
  },
  /** {@see BPageTransportationPickups**/
  transportation_pickups: {
    addresses: {
      title: "Wāhitau tiki",
      subtitle:
        "Tāpirihia, whakatikahia ngā wāhitau tiki ki konei. Ka taea e nga kaiwhakamahi te kowhiri i te wahitau tiki i te wharangi utu.",
      add_action: "Tāpiri tauwāhi hōu",
    },
  },

  /** {@see DeliveryReceiptWidget**/
  delivery_receipt: {
    title: "Rihiti tuku",
    has_return: "Te hokinga mai o te tuku",
    cashed: "Utu moni",
  },
  /** {@see BPageTransportationServiceDashboard**/
  delivery_dashboard: {
    select_destinations: "Tāpiri ota ki te rārangi",
    price_calculation: "Whakatauhia nga utu kaipuke",
    price_calculation_action: "Tātaihia te utu",
    add_request: "Tono Ratonga",
    add_request_action: "Tuku Tono Ratonga",
    refresh_action: "Whakahoutia nga korero",
  },

  /** {@see DeliveryOrderWidget**/
  delivery_order: {
    title: "Ota tuku",
    has_return: "Te hokinga mai o te tuku",
    cashed: "Utu moni",
  },

  /** {@see BPagePos**/
  cash_register: {
    menu: {
      panel: "Rehita moni",
      orders: "Nga ota",
      customers: "Kaihoko",
      devices: "Pūrere",
      inventory: "tūemi",
    },
    payment_confirm: "Whakapumautanga utu",
    payment_confirm_msg: "Ka utua e te kaihoko {basket_id}.",
  },

  /** {@see BPosDeviceBarcodeScanner**/
  barcode_scanner: {
    wifi_pass: "To Kupuhipa Wi-Fi",
    message: "Whakauruhia to kupuhipa pouara WiFi ka matawai i te waehere QR.",
    wifi_ssid: "Wi-Fi SSID",
    ssid_message: "Whakauruhia to ingoa whatunga Wi-Fi (SSID).",
  },

  /** {@see PosDeviceTypes**/
  pos_devices: {
    Scanner: "Matawai Waeherehere",
    Scanner_desc:
      "Hei whakamahi i te matawai paewaehere, me whakauru tika koe i te waehere SKU hua ki nga tautuhinga hua.",
    Printer: "Pūreretā",
    Printer_desc:
      "Tāngia te rīhiti o ō kiritaki mā te tūhono ki tētahi pūreretā rīhiti.",
    CustomerView: "Tirohanga Kiritaki",
    CustomerView_desc:
      "Ma tenei wharangi ka taea e koe te whakaatu i nga korero ota ki o kaihoko.",
    CardReader: "Kaipānui Kāri",
    CardReader_desc: "He taputapu panui kaari mo te utu COD na o kaihoko.",
    ChipReader: "Kaipanui maramara",
    ChipReader_desc: "Kaipānui tohu NFC hei taapiri i nga taonga ki te kaata ma te whakamahi tohu.",
  },
  /** {@see BPagePosDevices**/
  pos_register_devices: {
    delete_dialog: {
      title: "Tango taputapu",
      message: "Me tango e koe tenei taputapu mai i to kōtaha POS?",
      action: "Ae, Tango inaianei",
    },
  },

  /** {@see BPageOrderPos**/
  pos_process_center: {
    title: "Pokapū tukatuka tono POS",
    menu: {
      label: "Tapanga mōkī",
      receipt: "Rihiti",
      back: "Whakamuri",
      timeline: "Rarangi Wā",
      order: "Ota",
    },
  },
  /** {@see BPagePosPanel**/
  pos: {
    name: "POS",
    message:
      "Me whai i to waahi hokonga-kore-kore, whakahaere i runga i tetahi waea atamai, papa, PC, hanga ota-a-ringa ranei.",
    add_customer: "Tāpirihia he Kiritaki",
    set_delivery: "Tautuhi Wāhitau Tukunga",
    add_giftcards: "Tāpiri Kaari Koha",
    add_discount_code: "Tāpiri Waehere hekenga",
    add_coupon: "Tāpiri Tiikiti",
    set_campaign: "Tautuhi Kaupapa",
    select_buyer: "Tohua te kaihoko!",
    send_to_pos: "Tukuna ki te miihini panui kaari",
    print_receipt: "Tāngia te rīhiti",
    confirm_cash_payment: "Whakaaetia te utu putea",
    payment_cod: {
      title: "Utu utu mo te tuku",
      message:
        "Ko te nui o tenei ota ka utua i runga i te tukunga. I muri i te whiwhinga o te moni mai i te kaihoko, me whakau koe i te utu hei whakaoti i te ota i roto i te punaha.",
    },
    payment_online: {
      title: "Tīpakohia te tikanga utu ipurangi",
      message:
        "Ma te kowhiri i nga whiringa e whai ake nei, ka hangaia he hononga utu ka tukuna ki te kaihoko. Ma te utu i tenei hononga, ka huri te mana o te ota ki te aratau utu. Ka hangaia he hononga utu ma te matawai i te waehere QR i runga i te mata o te kaihoko, SMS, imeera ranei. Hei whakahohe i tetahi hongere ki te tuku hono, pawhiria taua whiringa ka pehi i te paatene hei tuku rihīti ki te kaihoko.",
    },
    notifications: {
      success_pay: "Kua oti te utu ota.",
      success_order:
        "I pai te hanga ota me te tatari mo te kaihoko ki te utu.",
    },
    payment_dialog: {
      check_pay_now: "Takina ā-ringa te utu",
      close_action: "Katia te korero. Kaore au e pai ki te tatari mo te utu a nga kaihoko",
      go_to_order: "Haere ki te wharangi ota",
      paid_by_gift_card_msg:
        "Ko te tapeke ota i tangohia mai i nga kaari koha! Kaore te kaihoko e utu nui ake.",
      cod_msg: "Ka utua e te kaihoko ina whiwhi ota.",
      online_msg: "Tonoa te kaihoko ki te matawai i te waehere QR i runga i te mata o te kaihoko.",
    },
  },

  /** {@see BPageShopProductsImporter**/
  exporter: {
    title: "Kaweake raraunga",
    subtitle:
      "Nau o raraunga! Ka taea e koe te tango i o korero pakihi i roto i te whakatakotoranga paerewa i nga wa katoa. Heoi, whakaute i te mana motuhake o o kaihoko, kaua e paopao ki a raatau, kaua rawa e tohatoha o raatau korero ki tetahi atu tuatoru.",

    excel:
      "Ka whiwhi koe i te rarangi o nga {type} katoa i roto i te toa me nga korero katoa i te whakatakotoranga Excel.",
    csv: "Ka whiwhi koe i te rārangi o ngā {type} katoa i te toa me ngā mōhiohio katoa i te hōputu CSV. E hāngai ana tēnei hōputu ki te hōputu paerewa a Selldone mō te kawemai {type} ki te toa.",
  },
  /** {@see BPageShopProductsImporter**/
  importer: {
    category: {
      title: "Kawemai ngā kāwai",
      subtitle:
        "Tukuna ake te konae CSV o nga waahanga ki konei. Me whai koe i te whakatakotoranga paerewa.",
    },
    product: {
      title: "Kawemai nga hua",
      subtitle:
        "Tukuna ake te konae CSV o nga hua ki konei. Me whai koe i te whakatakotoranga paerewa.",
    },
    product_images: {
      title: "Kawemai whakaahua",
      subtitle:
        "Ko te tango i nga korero whakaahua ka pau te waa. Kia mahara ki te whakaute i te mana pupuri i te wa e kawemai ana i nga whakaahua.",
    },
    inventory: {
      title: "Rarangi whakahou nui",
      subtitle:
        "Tukuna ake te konae CSV o te raarangi ki konei. Me whai koe i te whakatakotoranga paerewa ma te kaweake i te konae CSV i te tuatahi, ka whakatika i taua konae.",
    },

    back_to_products: "Hoki ki te rarangi hua",
    back_to_categories: "Hoki ki te rarangi kāwai",
    back_to_inventory: "Hoki ki te rārangi rārangi",

    step_select_file: "Tīpakohia te Kōnae CSV",
    step_send_to_server: "Tukuna ki te tūmau",
    step_view_result: "Tirohia te Hua",
    step_view_que: "Tirohia te Rarangi Tatari",
    send_to_server_action: "Tukuna ki te tūmau",

    shop_license: "Raihana toa",
    max_items_limit: "Tepe mōrahi tūemi",
    total_items: "Nga mea katoa",

    /** {@see BShopCustomerImporter**/
    customer: {
      title: "Kaihoko kawemai",
      subtitle:
        "Tukuna nga konae CSV o nga kaihoko ki konei. Me whai koe i te whakatakotoranga paerewa, a, me whai ano koe i nga ture tūmataiti.",
    },

    /**
     * @see BVendorsImporter
     */
    vendor: {
      title: "tūemi kaihoko",
      subtitle:
        "Tukuna nga konae CSV kaihoko ki konei. Me whakarite he tika te whakatakotoranga me te u ki nga ture tūmataiti.",
      checklist: {
        title: "Rarangi Arowhai Hiranga",
        subtitle: "Kia kaha ki te whai i nga aratohu.",
        sample_files: "Tauira Kōnae",
        valid_vendor_name_needed:
          "Me homai e koe he <b>ingoa whaimana</b>. Ka whakamahia te <b>name</b> ki te whakatau mehemea ka <i>Waihanga</i> he kaihoko hou, ka <i>Whakahou</i> ranei tetahi o naianei.",
        assign_user_after_import:
          "I muri i te kawemai i nga kaihoko, ka taea e koe te tautapa a-ringa i tetahi kaiwhakamahi kia uru atu ki te paewhiri kaihoko.",
      },
      need_kyc_alert:
        "Me manatoko koe i o korero whaiaro i mua i te kawemai i nga kaihoko.",
      back_to_vendors_list: "Whakamuri Ki kaihoko Rarangi",
    },
  },

  /**
   * @see BPageMarketplaceProducts
   */
  marketplace_products: {
    title: "kaihoko Hua",
    subtitle:
      "Kei roto i tenei raarangi nga hua katoa e tukuna ana e nga kaihoko. I te maakete, ka taea te hono tetahi hua ki tetahi, neke atu ranei nga kaihoko. Ko ia kaihoko e hono ana ki tetahi hua he urunga ahurei kei roto ko nga raarangi me nga taipitopito utu mo taua kaihoko.",
    filter_vendor: {
      placeholder: "Tātari mā kaihoko...",
    },
    filter_status: {
      placeholder: "Tātari mā Tūnga...",
    },
  },

  /**
   * @see BPageMarketplaceRequests
   */
  marketplace_requests: {
    open_vendors_panel: "tūemi kaihoko tūemi",
    title: "kaihoko tūemi Rarangi",
    subtitle:
      "He rarangi tono mai i nga tangata kua tono kia noho hei kaihoko i to maakete.",
  },
  /**
   * @see BPageMarketplaceSetting
   */
  marketplace_setting: {
    marketplace: {
      title: "Te kainga hokohoko",
      subtitle:
        "Ma te whakakore i te maakete ka whakaweto etahi mahi. Mena kei te hiahia koe ki te kati i to maakete katoa, me whakakore e koe to toa i roto i nga tautuhinga.",
    },
    distribution_model: {
      title: "Tauira Tohaina",
      subtitle:
        "Ka taea e nga maakete te mahi i raro i nga tauira tohatoha rereke. Ahakoa e pai ana koe ki nga kaihoko ki te tuku tika atu ki nga kaihoko, ki te whakakotahi ranei i nga ota ki to whare putunga mo te tuku, ka taea e koe te whiriwhiri i te tauira e pai ana ki o hiahia i konei.",
    },
    access: {
      title: "kaihoko Urunga",
      subtitle:
        "Hei kaiwhakahaere, ka whakahaere koe i te taumata o nga kaihoko uru ki te papatohu hua. Whakaritehia nga tautuhinga urunga o te ao kia taea ai e nga kaihoko te taapiri i a raatau ake hua me o raatau waahanga, te aukati ranei i tenei uru ki nga kaiwhakahaere anake.",
    },
    need_enable_shipping_for_vendors_tips:
      "Whakahohehia te tuku kaipuke mo nga kaihoko i roto i te Hoko> ripa Logistic mo ia tikanga takitahi.",
    transportation_available_tooltip: "Kei te waatea tenei whiringa mo nga kaihoko.",
    transportation_not_available_tooltip:
      "Kaore i te waatea mo nga kaihoko! Ka taea e koe te whakahohe i roto i nga tautuhinga kawe.",
    panel: {
      title: "kaihoko tūemi",
      subtitle: "tūemi to kaihoko tūemi to Ritenga Rohe.",
    },
    documents: {
      title: "tūemi",
      subtitle:
        "Ka taea e koe te tono ki o kaihoko ki te tuku tuhinga me te haina i etahi kirimana. Tauwhāitihia ngā tuhinga e hiahiatia ana ki konei.",
      add_document_action: "Tāpiri Tono Tuhinga",
    },
    inputs: {
      enable: {
        false_description:
          "Ka weto te kainga hokohoko, ka whakakore i nga mahi katoa o te maakete.",
        true_description:
          "te Te kainga hokohoko to Hohe, tūemi katoa Te kainga hokohoko tūemi.",
      },
      product: {
        label: "tāpiri Hou Hua",
        true_title: "kaihoko tūemi tāpiri Hua",
        false_description:
          "Ka taapirihia e koe nga hua me te whakatau i nga kaihoko. Ka taea e nga kaihoko te whakahaere i nga utu me nga taonga.",
        true_description:
          "Ka taea e nga kaihoko te taapiri me te whakahaere tika i a raatau ake hua ki o raatau roopu kaihoko.",
      },
      need_verify: {
        label: "Hua tūemi Rere",
        false_description: "Hou kaihoko Hua tūemi Ora tūemi.",
        false_title: "Karekau he manatoko e hiahiatia ana",
        true_description:
          "Ko nga hua hou a nga kaihoko me whai whakaaetanga i mua i te haere ki te kainga hokohoko.",
        true_title: "Me Manatoko",
      },
      category: {
        label: "tāpiri Hou Kāwai",
        true_title: "kaihoko tūemi tāpiri Ngā Kāwai",
        false_description:
          "Ka taapirihia e koe nga waahanga. Ka taea e nga kaihoko te taapiri i nga hua ki nga waahanga o naianei.",
        true_description:
          "Ka taea e nga Kaihoko te taapiri me te whakahaere tika i nga waahanga ki o raatau roopu kaihoko.",
      },
      shipping: {
        label: "Te tuku tūemi",
        true_title: "kaihoko tūemi Te tuku tūemi",
        false_description:
          "Kaore e taea e nga kaihoko te taapiri, te whakarite ranei i a raatau tikanga tuku.",
        true_description:
          "Ka taea e nga kaihoko te whakarite i a raatau ake tikanga tuku, te taapiri i nga karere, me te whakarite i nga kaipuke mo o raatau hiahia.",
      },
      hidden_customer: {
        label: "whakaatu tūemi Whakapā tūemi",
        true_description: "Ko nga korero whakapiri a te kaihoko ka huna mai i nga kaihoko.",
        false_description:
          "Ko nga korero whakapiri a te kaihoko, penei i te waea me te imeera, ka kitea e nga kaihoko.",
      },
      multi: {
        false_description:
          "Ka taea e te kaiwhakamahi kia kotahi anake te kaute kaihoko. Ka tukuna atu nga kaiwhakamahi ki o raatau roopu i / kaihoko.",
        true_description:
          "Ka taea e te kaiwhakamahi te maha o nga kaute kaihoko. He ahua beta tenei; tena koa whakapiri mai ki a maatau i mua i te whakamahi.",
        true_title: "tūemi-kaihoko aratau",
        false_title: "tūemi kaihoko aratau (Taunoa)",
      },

      listing: {
        label: "rārangi mō kaihoko",
        true_description: "Kua whakahohea te raarangi mo nga kaihoko. Ka taea e nga kaihoko te whakahaere rarangi taonga me nga kamupene.",
        false_description: "Kua monoa te raarangi mo nga kaihoko. Ka taea e te rangatira o te toa anake te whakahaere i nga rarangi taonga me nga kamupene.",
        requires_listing_active_text:
          "Hei tuku i nga kaihoko ki te whakamahi i te Rarangi (tuemi + kamupene), tuatahi whakaahei i te waahanga Rarangi toa i te Hoera → Rarangi. Na ka hoki mai ki konei kia uru mai nga kaihoko.",
        requires_listing_active_button: "tūemi Hongere rārangi Tautuhinga",
      },



    },
    notifications: {
      update_success: "I pai te whakahou i nga tautuhinga kaihoko.",
    },
    add_document_dialog: {
      title: "Nga Whakaritenga Tuhinga",
      type: {
        title: "tūemi Momo",
        subtitle:
          "Tāpirihia he tūemi tono tuhinga ki konei. Ka puta ki te papa o te kaihoko, ka akiaki ia ratou ki te tuku i te tuhinga e hiahiatia ana.",
      },
      guide: {
        title: "tūemi",
        subtitle:
          "Homai he aratohu poto hei awhina i nga kaihoko ki te mohio ko wai nga tuhinga ka tukuna me pehea te mahi tika.",
      },
      code: {
        title: "Whakamau Waehere",
        subtitle:
          "Whakapirihia te waehere whakauru mo tetahi tuhinga, puka, kirimana ranei me haina e te kaihoko.",
      },
      link: {
        title: "tūemi Hononga",
        subtitle:
          "Whakaratohia he hononga o waho ki tetahi tuhinga, kirimana ranei me haina e te kaihoko.",
      },
      inputs: {
        title: {
          label: "Taitara",
          placeholder: "Whakauruhia he taitara mo te tono tuhinga.",
        },
        guide: {
          label: "tūemi (tūemi)",
          placeholder:
            "Ka taea, hoatu he aratohu poto, tohutohu ranei, tae atu ki nga hononga ki nga kirimana me etahi atu tuhinga.",
        },
        code: {
          label: "Whakamau Waehere",
          placeholder: "tūemi te Whakamau Waehere tūemi...",
          message: "Ka taea te whakauru i te waehere HTML.",
        },
        url: {
          label: "URL",
          message: "Whakauruhia te URL mo te tuhinga, kirimana ranei ki konei...",
        },
      },
    },
  },

  /**
   * @see ShopMarketplaceModes
   */
  ShopMarketplaceModes: {
    Collective: {
      title: "tūemi aratau",
      description:
        "I tenei aratau, ka tukuna nga ota katoa ki to whare putunga i te tuatahi ka tukuna ki nga kaihoko mai i te waahi pokapū. Ka tukuna e nga kaihoko a raatau taonga ki to whare putunga, no reira ko te wahitau tuku mo nga kaihoko ko to waahi whare putunga.",
    },
    Direct: {
      title: "tūemi Te tuku Mai kaihoko",
      description:
        "I tenei aratau, ka tukuna tika nga kaihoko ki nga kaihoko. Ko te wahitau tuku mo nga ota kaihoko ko te wahitau o te kaihoko.",
    },
  },
  /**
   * @see VendorDocumentType
   */
  VendorDocumentType: {
    Identification: {
      title: "Tuhinga Tohu Kaipupuri",
      description:
        "Tukuna he TT i tukuna e te kawanatanga hei manatoko i te tuakiri o te rangatira pakihi, kaiwhakahaere matua ranei.",
    },
    Business: {
      title: "Tuhinga Pakihi",
      description:
        "Whakaratohia nga tuhinga pakihi nui penei i nga raihana me nga tiwhikete rehitatanga hei whakaū i te mana ture o to pakihi.",
    },
    Address: {
      title: "wāhi noho tūemi",
      description:
        "Tukuna he tuhinga hei manatoko i to wahitau pakihi, penei i te pire whaipainga, te nama taake, te kirimana riihi ranei.",
    },
    Contract: {
      title: "tūemi & tūemi",
      description:
        "Tukuna mai nga kirimana me nga kirimana e whakaatu ana i nga tikanga me nga tikanga o o hononga pakihi.",
    },
    Copyright: {
      title: "Tuhinga Mana pupuri",
      description:
        "Whakaratohia nga tuhinga rehitatanga mana pupuri hei whakau i o motika taonga hinengaro.",
    },
    Privacy: {
      title: "Tuhinga Kaupapahere Tūmataiti",
      description:
        "Whakauruhia o tuhinga kaupapa here tūmataiti hei whakaatu i to pono ki te tiaki i nga korero a nga kaiwhakamahi me nga kaihoko.",
    },
    Embed: {
      title: "tūemi puka",
      description:
        "Whakaotia te puka whakauru hei whakarato i nga korero e hiahiatia ana.",
    },
    Link: {
      title: "tūemi Hononga",
      description:
        "A pee i te hononga o waho ki te whakarato i nga korero e tika ana.",
    },
  },

  /**
   * @see BPageMarketplaceWallets
   */
  marketplace_wallets: {
    title: "kaihoko Pukoro",
    subtitle:
      "Kaore he take ki te taapiri a-ringa i nga putea! Ka hanga aunoa nga putea kaihoko. Mena ka taea e koe he tikanga utu e tautoko ana i nga utu wehewehe (penei i te Stripe Connect), ka tohatoha aunoa te punaha i nga utu ki nga kaute a nga kaihoko hono. Ki te kore e taea tenei ahuatanga, me utu a ringa koe ki nga kaihoko, ka taea e koe te tirotiro i nga moni nama ki a ratou i te rarangi putea.",
    filter_vendor: {
      placeholder: "Tātari mā kaihoko...",
    },
  },

  /**
   * @see BVendorAccountTransactionsList
   */
  vendor_account_transactions: {
    subtitle:
      "Ka whakaratohia e tenei waahanga he rarangi taipitopito o nga whakawhitinga katoa mo te putea kua tohua. Ka tuhia he tauwhitinga utu ina utu pai te kaihoko mo tetahi ota. Mena ka whakakorehia tetahi ota, ka whakahokia mai ranei tetahi waahanga, ka tuhia ano te tauwhitinga moni e rite ana ki konei.",
    vendor_bank: "kaihoko Peeke",
    order_fee: "🛍️ Ota Fee",
    order_refund: "🛍️ Ota tūemi",
    reverse_fund: "Whakahokinga Putea",
    payout: "tūemi",
  },

  /**
   * @see BPageMarketplacePayouts
   */
  marketplace_payouts: {
    title: "tūemi Hītori",
    subtitle:
      "Ko tenei ahuatanga ka whakamaarama i te kaute mo koe me o kaihoko ma te pupuri i nga tuhinga taipitopito o nga whakawhitinga katoa. Ka taea te whakahaere a-ringa i nga utu ma te whakawhiti peeke, etahi atu tikanga ranei, ka whakahaere aunoa ranei e nga kaiwhakarato utu e tautoko ana i nga utu wehewehe, penei i a Stripe.",
    top_up_vendor_action: "tūemi ma kaihoko Pūkete",
  },

  /**
   * @see BPageMarketplacePricings
   */
  marketplace_pricings: {
    title: "Te utu Mahere",
    subtitle:
      "Whakaritehia nga tauira utu mo to maakete, penei i te utu tawhē 5% mo nga hua mamati. Ma tenei ka tere ake te whakahaere utu me te pai ake.",
    add_pricing_action: "tāpiri Te utu Mahere",
  },

  /**
   * @see BVendorPricingAdd
   */
  vendor_pricing_add: {
    title: "Te kainga hokohoko Te utu tūemi",
    subtitle:
      "Ina whakatauhia e koe he tauira utu ki nga hua kaihoko, ka tatau aunoa te utu hei Utu Kaihoko * (1 + Komihana%).",
    delete: {
      subtitle:
        "Ko te whakakore i tetahi tauira utu ka pa ki nga hua kaihoko katoa e hono ana ki a ia. I mua i te tango i tetahi tauira utu, kia mohio koe ki te whakahou i nga hua kaihoko katoa ma te whakamahi me te whakarite i tetahi tauira utu rereke.",
      remove_pricing_action: "Tango Te utu tūemi",
    },
    inputs: {
      title: {
        placeholder: "hei tauira, te karaehe Hua Mamati...",
      },
      description: {
        placeholder: "Ka taea e koe te taapiri i tetahi tuhipoka ki konei...",
      },
      accept_delete: {
        true_description: "E hiahia ana ahau ki te muku i tenei tauira utu.",
        true_title: "Whakapumautia te Tango Tauira Utu",
      },
    },
  },

  /**
   * @see BVendorPayoutAdd
   */
  vendor_payout_add: {
    vendor: {
      title: "kaihoko",
      subtitle:
        "Tīpakohia te kaihoko e hiahia ana koe ki te whakawhiti moni. Ina tohua, ka whakaatuhia nga whiringa utu e waatea ana. Mena kua whakahohea e koe nga kuaha utu utu, ka puta ano enei whiringa ki konei.",
    },
    vendor_wallet: "kaihoko Pukoro",
    vendor_bank_account: "kaihoko Peeke Pūkete",
    with_balance_tooltip:
      "Ina hono koe i nga ratonga utu pera i a Stripe Connect, ka tikina e matou, ka whakaatu i te toenga e waatea ana ki to putea Stripe hei tohutoro.",
    with_balance: "tūemi Taurite",
    without_balance: "tūemi Taurite - tūemi aratau",
    bank_transfer: {
      title: "Peeke Whakawhiti",
      subtitle:
        "He rekoata tenei mo te hitori utu. Kaore he moni tuturu e tukuna ki te kaihoko e te punaha; me utu a ringa koe ki o kaihoko.",
    },
    vendor_bank: "kaihoko Peeke",
    gateway_in_debug_mode_warning:
      "I te mea ko te keti utu kei roto i te aratau patuiro i tenei wa, karekau nga moni e tukuna ki tetahi kaute tuturu.",
    payment: {
      title: "Utu",
      subtitle:
        "Kōwhirihia te moni me te whakauru i te moni e hiahia ana koe ki te whakawhiti ki te kaihoko. Mena ka whakauruhia e koe he moni kino, ka tangohia nga moni mai i te putea a te kaihoko. Mo nga kaute hono me nga whiringa utu, ka tangohia nga moni mai i te putea hono, penei i te putea Stripe a te kaihoko.",
    },
    history: {
      title: "Hītori",
      subtitle:
        "Ko tetahi waahanga o tenei utu kua whakahokia mai ma te whakawhiti whakamuri, ko te tikanga kua tangohia te toenga mai i te kaute a te kaihoko ka whakahokia ki to putea.",
    },
    refund: {
      title: "tūemi",
      subtitle:
        "Ko te moni whakahoki ka tangohia mai i te toenga o te kaihoko. Ko nga utu katoa (me nga moni e toe ana) kei roto i te kaute a te kaihoko (penei i te Stripe Connect) ka whakahokia mai ma te whakawhiti whakamuri.",
    },
    inputs: {
      vendor: {
        placeholder: "tūemi he kaihoko...",
      },
      note: {
        placeholder:
          "Ka taea e koe te taapiri i tetahi tuhipoka ki konei... Kaore tenei tuhipoka e kitea e te kaihoko.",
      },
      accept_refund: {
        true_description: "I tūemi Ki Whakakore tūemi 💳 Utu.",
        true_title: "Whakakore & tūemi 💳 Utu",
      },
    },
    notifications: {
      reverse_fund_success: {
        title: "Putea Whakamuri",
        message:
          "Kua oti pai te whakahoki moni a te kaihoko, me te whakawhiti moni mai i te peeke ki te putea me te taapiri atu ki te toenga putea.",
      },
      payout_success: {
        title: "tūemi",
        message:
          "Ko te utu a te kaihoko kua oti te tukatuka me te taapiri.",
      },
    },
  },

  /**
   * @see BShopQuotaImporter
   */
  quota_importer: {
    quota: "tūemi",
    max_batch_msg: "Te maha o nga taonga ka taea e koe te kawemai ki te puranga kotahi.",
    max_daily_limit_msg: "Te maha o nga taonga ka taea e koe te kawemai i te ra.",
    extra_daily_limit_msg: "Ko nga taonga taapiri ka whakaaetia ki tua atu i te tepe o ia ra.",
    add_items_today_msg: "Nga taonga tūemi I tenei ra.",
    file_statistic_msg: "Tauanga mō tūemi kōnae.",
    used_quota: "Kua whakamahia tūemi",
  },

  /** {@see BPageShopMarketingCampaign**/
  campaigns: {
    title: "Nga Whakatairanga",
    subtitle:
      "Waihangahia nga kaupapa whakatairanga, hononga hono, ka kite i nga korero pono me te tika i te waahi kotahi. Ka awhina matou ki a koe ki te whakarite whakatau pai ake i runga i te raraunga.",
    add_action: "Tāpiri Kaupapa Hou",
    empty_message: "Waihangahia to kaupapa tuatahi inaianei!",
    delete: {
      title: "Mukua te kaupapa whakahau",
      message: "Me muku koe i tenei pakanga me nga raraunga katoa?",
      action: "Ae, Muku Inaianei",
    },
  },

  landing_categories: {
    raw: "Nga tauira mata",
    all: "Nga tauira katoa",

    marketing: "Marketing & Pānui",
    product: "Whakataki hua",
    health: "Hauora me te oranga tinana",
    food: "Kai me nga hua",
    house: "Nga whare me te Whare",
    device: "Miihini me te taputapu",
    car: "Waka",
    software: "Pūmanawa",
    clothing: "Kākahu & Hu",
    jewellery: "Te whakapaipai",
  },

  /** {@see BPageShopSettingTheme**/
  shop_theme: {
    theme_colors: "Tae kaupapa",
    theme_colors_description:
      "Ka taea e koe te whakawhaiaro i te papatae tae tuatahi o to toa i konei. Ka taea e ia tauira te whakamahi i enei tae ki te whakaputa aunoa i te awhe tae katoa e hono ana ki to waitohu.",
    products_view_mode: "Tirohanga rarangi hokohoko",
    products_view_mode_desc:
      "Ka taea e koe te whakatika i te ahua whakaatu o nga hua i roto i to toa i runga i te rahi o te mata. Ka whakaarohia tenei aratau ma te taunoa ka kore e whakamahia mena ka huri te kaiwhakamahi i te aratau whakaatu.",

    tabs: {
      vendor_page: "kaihoko whārangi",
    },

    color_light: "Te tae tuatahi",
    color_dark: "Te tae pouri tuatahi",
    color_deep_dark: "Te tae pouri hohonu",
    color_info: "Hononga",
    light_checkout: "Kaupapa utu marama",
    light_header: "Pane marama",
    light_footer: "Hiku marama",
    light_filter: "Kaupapa tātari kāwai",
    checkout_color: "Takitaki Tae",

    typography: "Kuputuhi",
    header: "Pane",
    footer: "tūemi",
    custom_code: "Waehere ritenga",
    custom_css: "CSS ritenga",
    custom_variants: "Nga momo rereke",

    wrap_product_title: {
      title: "tūemi Taitara",
      subtitle: "Ko te teitei o te taitara ka whakatika hihiri.",
    },

    animation: {
      title: "tūemi",
      static: {
        false_title: "Me nga Animations",
        false_description:
          "He hākoritanga tauwhitiwhiti ngā kāwai me ngā hua hei whakaatu i te mahi.",
        true_title: "Pahekeheke",
        true_description: "Ngā Kāwai ka Hua tūemi kahore tūemi.",
      },
    },

    quick_actions: {
      title: "tūemi Nga Mahi",
      hover: {
        false_description:
          "Aratau ngawari me te iti kaore he mahi taapiri i runga i nga kaari hua.",
        true_description:
          "Whakaatuhia te hoko Tere me te taapiri hei whakataurite i nga paatene i runga i te kaari hua e whakaparonga ana.",
        true_title: "Whakaatu Hohenga ki runga Hover",
      },
    },
    filter_products: {
      title: "Tātari Hua",
      only_available: {
        false_description: "whakaatu katoa Hua i te Toa Rarangi tūemi Taunoa.",
        true_description:
          "whakaatu tūemi Kei te waatea Hua i te Toa Rarangi tūemi Taunoa.",
        true_title: "Kei te waatea Hua",
        false_title: "katoa Hua",
      },
    },
    filter_categories: {
      title: "Tātari Ngā Kāwai",
      hide_empty_folders: {
        false_description: "whakaatu katoa Ngā Kāwai i te Toa Rarangi.",
        false_title: "whakaatu katoa Ngā Kāwai",
        true_title: "huna tūemi Ngā Kāwai",
        true_description:
          "whakaatu tūemi Ngā Kāwai tūemi Hua i te Toa Rarangi.",
      },
    },

    product_page: {
      title: "Hua whārangi",
      subtitle:
        "Whakaritehia to wharangi hua ki konei. Ka taea e koe te whakatika i te whakaaturanga o nga utu me te whakarereke i te whakaritenga o nga waahanga kia rite ki o hiahia.",
      buy_button_color: "Hoko pātene Tae",
      smart_price: {
        title: "tūemi Waeine utu",
        true_description:
          "Te tatau i te utu wae ma te whai whakaaro ki te taumaha, te rōrahi, me nga taonga rereke o te kete. Ex. Mena he rereke to 2L me te $5 hei utu, ka kite te kaihoko i te $2.5/L i te wharangi hua.",
      },
      reverse_currency: {
        title: "Moni tūemi Turanga",
        false_title: "Aratau noa",
        true_title: "tūemi aratau",
        false_description:
          "Ka tuhia te tohu moni i te timatanga o nga nama. Hei tauira: $95 , €30",
        true_description:
          "Ka tuhia te tohu moni ki te mutunga o nga nama. Ex: 95$, 30€",
      },
      variants_selector: {
        title: "Nga momo rereke tūemi",
        subtitle:
          "Ka taea e koe te kowhiri i nga huarahi rereke hei whakaatu rereke ki te wharangi hua.",
      },

      tabs_order: {
        title: "tūemi 🛍️ Ota",
        reset_to_default: "tūemi Ki Taunoa",
      },
    }, // end: product_page
  },

  /**
   * @see BShopThemeVendor
   */
  b_shop_theme_vendor: {
    onboarding: {
      title: "tūemi & takiuru",
      subtitle:
        "Whakawhaiarohia te kaihoko ki runga i te paparangi me te wheako takiuru me tetahi ataata ritenga, ahua, karere ranei.",
    },
    inputs: {
      page_vendor_login: {
        label: "kaihoko takiuru whārangi",
        message:
          "Tīpakohia he whārangi taunga ritenga mo te takiuru kaihoko. Whakauruhia he ataata, whakaahua, tuhinga ranei mo tetahi hoahoa ahurei. Ka whakaatuhia tenei wharangi ki nga kaihoko kaore ano kia takiuru.",
      },
      page_vendor_registered: {
        label: "kaihoko Kua rehitatia whārangi",
        message:
          "Tīpakohia he whārangi ritenga mo nga kaihoko kua takiuru. Ka whakaatuhia tenei wharangi ki te tautoko to maakete i nga kaihoko maha mo ia kaiwhakamahi.",
      },
    },
  },

  /** @see BShopOptionsVariants **/
  b_shop_options_variants: {
    custom_variants: {
      title: "Ritenga Nga momo rereke",
      subtitle:
        "He rereke nga hua o te tinana. Ka taea e koe te huri i te ingoa taunoa o nga momo rereke. Ko ta matou whakaaro me waiho enei mea hei taunoa me te noho kau kia whakamaori aunoa a seldone mo o kaihoko ki o raatau reo. Ko te roa rawa ko te 24.",
    },
  },
  /** @see BShopThemeTypography **/
  b_shop_theme_typography: {
    main_font: {
      title: "tūemi paetukutuku tūemi",
      subtitle: "Kōwhirihia to momotuhi tino pai mai i nga Momotuhi Google.",
    },
    inputs: {
      font_family: {
        label: "Whanau Momotuhi",
        message: "Whakauruhia te uara momotuhi-whanau.",
      },
      font_res: {
        label: "Rauemi Momotuhi",
        message: "Tauwhāitihia te rauemi momotuhi.",
      },
    },
  },

  /** @see BShopThemeHeader **/
  b_shop_theme_header: {
    logo: {
      title: "Pane Moko",
      subtitle:
        "Ma te taunoa, ka whakaatuhia te moko o to toa ki te pane, engari ka taea e koe te tuku ake i te waitohu ritenga. Mena karekau he owehenga tapawha o to waitohu, ka hiahia koe ki te whakatika a-ringa i te teitei me te whanui mo te whakaaturanga tino pai.",
    },
    header_title: {
      title: "Pane Taitara",
      subtitle:
        "Ka taea e koe te whakawhaiaro i te taitara e whakaatuhia ana i te pane o o wharangi konei. Ki te hiahia koe ki te tango katoa, pawhiria te paatene 'Tautuhi taitara kau'.",
    },

    inputs: {
      logo: {
        label: "Waitohu Moko",
      },
      height: {
        label: "Moko Teitei",
      },
      width: {
        label: "Moko Whānui",
      },
      title: {
        label: "Pane Taitara",
        set_empty: "Tautuhia te taitara kau",
      },
    },
  },

  /** @see ShopThemeVariantsMode**/
  shop_theme_variants_mode: {
    smart: {
      title: "tūemi aratau",
      description:
        "Ko nga momo rereke i roto i nga whiringa motuhake ka taea te kowhiri, he pai mo te nuinga o nga keehi.",
    },
    select: {
      title: "tūemi aratau",
      description:
        "Ka whakaatuhia nga momo rereke katoa i roto i te takaiho, he pai mo te maha o nga momo rereke.",
    },
  },

  /** @see BPropertySetAdd**/
  property_set_add: {
    edit_title: "Whakatika Tautuhinga Taonga",
    add_title: "Tāpiri Tautuhinga Taonga",
    config: {
      subtitle:
        "Tautuhia te taitara me te whakaahuatanga mo to huinga rawa. Ka kitea e nga kaimahi anake.",
    },
    variants: {
      title: "Nga momo rereke",
      subtitle:
        "Ka taea e koe te whakarite i te ingoa rereke, tohu, me te ota i konei. Mena kei a koe nga momo momo hua me nga momo rerekee kaore e uru ki nga momo taunoa 6, ka taea e koe te whakarite i nga momo rereke mo ia hua i konei.",
    },

    inputs: {
      title: {
        placeholder: "Whakauruhia he taitara mo te huinga rawa...",
      },
      description: {
        placeholder: "Tāuruhia he whakaahuatanga poto... (kōwhiringa)",
      },
      custom_variants: {
        true_title: "Ritenga Nga momo rereke",
        false_title: "Taunoa Nga momo rereke",
        true_description: "tūemi to Hua Nga momo rereke.",
        false_description: "tūemi Taunoa tūemi Nga momo rereke.",
      },
    },
  },

  /** @see BPropertySetVariantRow **/
  property_set_variant_row: {
    inputs: {
      values: {
        placeholder: "Tautuhi uara kua whakaritea... (he kōwhiringa)",
        messages:
          "Ka taea e koe te tuku whakaaro mo nga uara me te whakarite ota.",
      },
    },
  },

  /** {@see AppTemplates**/
  app_templates: {
    default: "Taunoa",
    blue: "Te moana kahurangi",
    amber: "Amipere",
    green: "tūemi",
    red: "tūemi",
    magenta: "Puawai",
    master: "Kaiwhakaako",
    black: "Te taha pouri",
  },

  /** {@see BPageAffiliateOrders**/
  affiliate_orders: {
    title: "Nga Ota Hoa Hononga",
    table: {
      order_price: "Te nui ota",
      order_payment: "Mana utu tono",
      order_link: "Tono hononga",
      commission_status: "Te mana o te Komihana",
      commission_amount: "Te nui o te Komihana",
      items: "Nga taonga",
      actions: "Nga Mahi",
      create_date: "Waihangahia te ra",
    },
    notifications: {
      update_success: "Kua whakahoutia te mana hononga.",
    },
  },

  /** {@see BPageAffiliatePayments**/
  affiliate_payments: {
    title: "Utu Hoa Hononga",
    table: {
      staff: "Kaimahi",
      amount: "Te nui",
      note: "Tuhipoka",
      date: "Rā",
    },
  },
  /** {@see BPageShuttleRoles**/
  my_roles: {
    title: "Ko aku mahi",
    message:
      "Ko nga mahi he momo urunga ki nga kaimahi o te toa. Hei tauira, ko nga kaimahi te kawenga ki te tuku, ki te tapaki ota ranei.",
  },
  /** {@see WidgetMyRoleCard**/
  role_card: {
    complete_tasks: "Kua oti nga mahi",
    reject_tasks: "Nga mahi kua whakakorehia",
    last_check: "Takitaki tata nei",
  },

  /** {@see BAppConfigDialog**/
  app_config: {
    reviews: "Nga arotake",
    rate_now: "Whakatauhia inaianei",
    category: "Kāwai",
    uninstall: "Wetetāuta",
    get_app: "Tikina te taupānga",
    public_keys: "Ngā Kī Tūmatanui",
    private_keys: "Kī tūmataiti (Haumaru)",
    config: "whirihoranga taupānga",
    app_status: "Te mana tono",
    app_info: "Nga korero tono",
    update_at: "Whakahoutanga tata",
    visit_website: "Haere ki te paetukutuku",
    privacy: "Kaupapahere Tūmataiti",
    installs: "Tāuta",
    basic_profile_permission: "Nga korero moohio taketake",
    developer: "Kaihanga",
    permissions: "Whakaaetanga",
    total_comments: "Katoa nga korero",
    delete_dialog: {
      title: "Wetetāuta taupānga",
      message: "Me tango tenei taupānga mai i to toa?",
      action: "Ae, Mukua te taupānga",
    },
    notifications: {
      uninstall_success: "Kua oti te tango i te tono.",
    },
  },

  /** {@see BPageShopChannelBots**/
  shop_bots: {
    not_set: "Kaore i Tautuhi!",
    tokens: "(ng) Tohu",
    webhook: "Tukutuku Webhook",
    calls: "Waea",
    dialog: {
      title: "{name} Whirihoranga Bot",
      message:
        "He anga karetao matawhānui mo te hanga wheako hokohoko-akomanga hinonga.",
    },
  },

  /** {@see BPageShopCrmCustomers**/
  shop_customers: {
    title: "Whakahaerenga Kiritaki",
    subtitle:
      "Ka taea te taapiri i nga kaihoko ki to toa i nga huarahi maha; ā-ringa, kawemai nui Excel, ma te POS, ma to toa ipurangi ranei. I nga wa katoa ka uru mai tetahi kaiwhakamahi ki to toa, ka rehita aunoatia hei kaihoko.",
    add_customer_action: "tāpiri Hou kiritaki",

    all_customers: "Tangohia nga kaihoko katoa",
    filtered_customers: "Kua tātarihia nga kaihoko",
    dialog_chips: {
      title: "Tautuhia te maha o nga maramara kaihoko",
    },
    dialog_club: {
      title: "Whakaritehia te taumata karapu kaihoko",
    },
  },

  /** {@see BCustomerAdd**/
  customer_add: {
    edit_title: "Whakatikatika kiritaki",
    add_title: "tāpiri Hou kiritaki",
    subtitle:
      "Hei taapiri i tetahi kaihoko ki to toa, tena koa homai he wahitau imeera, he nama waea ranei.",
    more: {
      title: "Tāpirihia etahi atu korero",
      subtitle: "tūemi Moni, kiritaki Karapu, Nga waahanga, wāhi noho ka Ētahi atu...",
    },
    detail: {
      subtitle:
        "Kia mau ki te rekoata o nga korero whaiaro mo to kaihoko. Whakahonoretia te noho muna a to kaihoko.",
    },
    preferences: {
      subtitle: "tūemi Taunoa kaiwhakamahi Moni, kiritaki Karapu, ka Nga waahanga tūemi.",
    },
    address: {
      subtitle:
        "Ka taea te whakamahi i tenei wahitau ki te POS. Kaore e taea e te kaiwhakamahi te kite i enei korero.",
    },
  },

  /** {@see AppPageDashboard**/
  app_dashboard: {
    status: "Whakaputa i te mana",
    statistics: "Tauanga",
  },
  /** {@see AppPage_Publish**/
  app_publish: {
    production_version: "Putanga whakaputa",
    beta_version: "Putanga Beta",
    alpha_version: "Putanga Arepa",
  },
  /** {@see AppPublishWidget**/
  app_publish_widget: {
    header: "Pane Paetukutuku Toa",
    header_code: "Waehere pane",
    admin_page: "Whārangi Kaiwhakahaere",
    name: "Ingoa",
    icon: "Tohu",
    url: "Taupānga – Whakaputa – tūemi",
  },

  /** {@see AppPage_API**/
  app_api: {
    title: "Kī Urunga API",
    revoke_api: "Whakakorehia te API",
    api_key: "Kī API",
    secret: "Kī ngaro",
    client: "Kiritaki",
    redirect_url: "Tukuna url",
    client_name: "Ingoa Kiritaki",
  },

  /** {@see BPageApplication**/
  app_loader_page: {
    no_ui_title: "Ko tenei tono karekau he UI!",
    no_ui_desc:
      "Ko etahi o nga tono karekau he atanga kaiwhakamahi ki o raatau rohe whakahaere. Ka taea e tenei tono te taapiri waehere ki te waahanga pane o to paetukutuku toa.",
    header: "Waehere pane",
  },
  /** {@see BPageShopFinanceReport**/
  finance_report: {
    export_title: "Kaweake i nga raraunga putea",
    export_sub_title:
      "Tikiakehia nga korero putea ki te konae kotahi.",
    start_input_placeholder: "Tīpakohia te rā tīmatanga o te pūrongo",
    end_input_placeholder: "Tīpakohia te rā mutunga o te pūrongo",
    download_action: "Tikiake rārangi pūtea",
  },

  /** {@see BPageShopSettingNotifications**/
  shop_notifications: {
    title: "Nga panui hokohoko",
    message: "Whakatikaina o tautuhinga whakamohiotanga toa me o hiahia",
    channel: "Hongere",
    status: "Tūnga",
    events: "Nga huihuinga",
    info: "Nga korero",
    actions: "Nga Mahi",
    connected: "Kua hono",
    not_connected: "Kaore i hono!",
    connect_now: "Hono atu inaianei",
    send_test: "Tukuna nga raraunga whakamatautau",
    disconnect: "Momotu",

    dialog: {
      title: "Whirihoranga Bot",
      message:
        "He anga whakamohiotanga matawhānui mo te hanga i nga wheako e-hokohoko-ahua hinonga.",
      action: "Pāwhiri ki konei!",
      set_config: "Tautuhi whirihora",
    },
  },
  notification_events: {
    new_order: "🛍️ Ota",
    payment: "💳 Utu",
    return: "📦 Hoki mai",
    weekly_report: "📈 Nga Purongo o ia wiki",
  },

  /** {@see BOrderDashboardDelivery} **/
  order_delivery: {
    title: "Tukunga",
    add_to_delivery_que_action: "Taapirihia ki te rarangi tuku",
    in_que: "I roto i te rarangi kaipuke",
    message:
      "Whai muri i te tuku i te kete ki te kaikawe kaipuke, pawhiria te paatene Whakaaetia te <b>Tukunga</b> .",
    message_add_to_service:
      "Patohia te paatene e whai ake nei ki te hiahia koe ki te tuku i tenei kete ma te <b>ratonga tuku</b> .",
    courier_action: "Tukunga tūemi",
    courier_action_subtitle: "Ma taku kaikawe e tiki te peera.",

    pickup_ready_action: "Kua reri te tiki",
    pickup_ready_action_subtitle: "te 🛍️ Ota to tūemi mō kiritaki tūemi.",

    service_action: "Whakapumautanga Mahi",

    checklist: [
      "Tirohia te kaari tuakiri a te kaikawe.",
      "Ko te pai o te whakakao hua me te tuku i te waa ka piki ake te reeti pupuri a o kaihoko.",
    ],
    manual: {
      title: "E tatari ana mo te whakapumautanga o te tuku mai a te kaihoko.",
      action_received: "Te Whakaaetanga Kaihoko",
      action_returned: "Whakahoki Tukunga",

      message:
        "Tuhipoka: Mena ka whiwhi te kaihoko i te ota, ka taea e koe te whakamahi i nga whiringa e whai ake nei hei whakaoti i te whakatutukitanga. Ko etahi o nga ratonga kaipuke i roto i te punaha ka whakaū aunoa i te tuku.",
    },
    auto_complete_note:
      "I muri i tetahi wa, ka huri te mana ota ki te tuku. He rereke tenei waa i runga i te momo ota ka whakatauhia e te punaha.",
    received: {
      title: "Ka tukuna ki te Kaihoko.",
      delivered_date: "Te Ra Tukunga",
    },
    receiver: "Kaiwhiwhi",
    bill: "Rihiti",

    tracking: {
      title: "Tāpirihia te waehere aroturuki / Url",
      code: "Waehere aroturuki",
      url: "URL aroturuki",
      message:
        "Mena kua tukuna e koe ma te ratonga e whakarato ana i te waehere aroturuki mo tenei ota, ka taea e koe te whakauru i te waehere me te hononga aroturuki e tika ana i tenei waahanga. Ka whakaatuhia enei korero ki te kaihoko.",
    },
    preferred_delivery_time: "Wā Tukunga Manakohia",
    manage_couriers: "Whakahaerehia aku karere",
    manage_service: "Whakahaere ratonga",

    option_deliver_by_courier: "Kōwhiringa 1: Tukuna mā ō karere.",
    option_add_to_que:
      "Kōwhiringa 2: Tāpirihia ki te tūtira ki te tukatuka i te wharangi ratonga kaipuke.",
    option_instant_shipping: "Kōwhiringa 3: Te tuku tere.",

    verify_delivery_input: {
      true_title: "tūemi Tukunga tūemi",
      true_description:
        "Ka whakapumautia e ahau kua tukuna te ota ki te kaihoko.",
    },
  },

  /** {@see BShopHomeSelect} **/
  shop_home_edit: {
    title: "Tautuhinga whārangi kāinga",
    sub_title:
      "Tautuhia to wharangi taunoa paetukutuku",

    default_home: "Whārangi kāinga taunoa",
    default_home_placeholder: "Ko te wharangi toa te wharangi kaainga paetukutuku",
  },
  /**
   * @see OReferralBank
   */
  referral_bank: {
    title: "tūemi Papatohu",
    subtitle:
      "I runga i to tono, ka utua te utu komihana ki te putea putea kua tohua e koe. Tena koa homai to korero putea ki konei.",
    no_bank_info: "kahore Peeke Nga korero!",
    actions: {
      edit_my_bank_info: "Whakatikatika tūemi Peeke Nga korero",
    },
    bank_dialog: {
      title: "tūemi Peeke Nga korero",
      info: {
        title: "Peeke",
        subtitle:
          "Whakauruhia koa o korero putea ki konei. Ka whakamahia e matou enei korero ki te whakawhiti moni. Me whakarite kei roto i to ingoa te putea putea kei raro ranei i to ingoa pakihi, ina tika ana.",
      },
      inputs: {
        name: {
          label: "Peeke Ingoa",
          message: "Te ingoa katoa o te peeke kei reira te kaute.",
        },
        holder_name: {
          label: "Pūkete tūemi Ingoa",
          message:
            "Te ingoa katoa o te tangata, te hinonga ranei e pupuri ana i te kaute.",
        },
        account_number: {
          label: "Tau Kaute",
          message:
            "Ko te tau ahurei e hono ana ki te putea putea motuhake.",
        },
        routing_number: {
          label: "Tau Ararere (USA), Waehere Kōmaka ranei (UK)",
          message:
            "He nama e tohu ana i te peka putea motuhake (he rereke nga kupu e whakamahia ana i nga whenua rereke).",
        },
        swift: {
          label: "tūemi/BIC Waehere",
          message:
            "He waehere ao e whakamahia ana hei tautuhi i nga peeke o te ao, ina koa mo nga whakawhitinga o te ao.",
        },
        iban: {
          label: "IBAN (tūemi Peeke Pūkete tūemi)",
          message:
            "I whakamahia i Uropi, ko te IBAN he punaha kua whakaaetia i te ao mo te tautuhi i nga putea putea puta noa i nga rohe o te motu.",
        },
        contact: {
          label: "Nga korero Whakapā",
          message: "Te nama waea, te wahitau imeera ranei e hono ana ki te kaute.",
        },
        note: {
          label: "Tuhipoka",
          message: "20% Tuhipoka",
        },
      },
    },
    notifications: {
      save: {
        message: "I pai te whakahou i to korero putea.",
      },
    },
  },

  /**
   * @see OReferralTier
   */
  referral_tier: {
    message:
      "Mai i nga utu ratonga katoa, ohaurunga me etahi atu utu i mahia e to hunga i tuku.",
  },

  /**
   * @see ReferralTiers
   */
  ReferralTiers: {
    Bronze: {
      title: "Parahi taumata",
      description:
        "Kei roto koe i te taumata parahi. Kua whiwhi koe i te 2% komihana mai i o tohutoro.",
    },
    Silver: {
      title: "Tier Hiriwa",
      description:
        "Kei roto koe i te taumata Hiriwa. Kua whiwhi koe i te 4% komihana mai i o tohutoro.",
    },
    Gold: {
      title: "Tier Gold",
      description:
        "Kei roto koe i te taumata koura. Kua whiwhi koe i te 6% komihana mai i o tohutoro.",
    },
    Platinum: {
      title: "Tier Platinum",
      description:
        "Kei roto koe i te taumata Platinum. Kua whiwhi koe i te 8% komihana mai i o tohutoro.",
    },
    Diamond: {
      title: "Taimana taumata",
      description:
        "Kei roto koe i te Taimana Tier. Kua whiwhi koe i te 10% komihana mai i o tohutoro.",
    },
  },

  /**
   * @see OPageMonetize
   */
  page_monetize: {
    title: "Whakamoni",
    subtitle: "Horahia te uru ki nga huarahi pakihi.",
  },

  /** {@see OPageMonetizeReferral} **/
  selldone_referral_page: {
    referral_link: "Hononga tohutoro",
    registered: "Kua rehitatia",
    accepted: "Whakaaetia",
    bank: "Peeke",
    bank_title: "Nga korero putea mo te tangohanga",
    new_users: "Kaiwhakamahi hou",
    performance: "Mahi",
    credit_mode: "Aratau nama",
    get_stickers: "Tikina te waehere Html whakapiri hei kape me te mua",
    click_copy_sticker_code: "Patohia te tohu hei kape i te waehere html.",
  },
  /** {@see OReferralTransactions} **/
  selldone_referral_payment: {
    title: "Utu Tohunga",
    subtitle:
      "Ko te rarangi o nga whakawhitinga tangohanga. Ina whakawhiti koe i te putea mai i te putea tuku ki to putea putea, ki to putea Seldone ranei, ka tuhia ki konei.",

    officer: "Apiha",
    amount: "Te moni hokohoko",
    note: "Tuhipoka",
    date: "Rā",
  },

  /** {@see BGatewayCard} **/
  widget_shop_gateway: {
    link_account_caution:
      "Honoa he kaute ({currency}) ki to toa! Pāwhiri ki konei..",
  },

  /** {@see BProductOrderSalesPerformance} **/
  product_sell_widget: {
    title: "Te mahi hoko hua",
  },

  /** {@see BOrderDashboardDropshippingFulfillment} **/
  fulfilment_order_widget: {
    title: "Te tutukinga tuku",
    check_action: "Whakaaehia te tuku tuku",
    uncheck_action: "Whakapumautia te ota",
    pay_action: "Utu utu nga taonga..",
    pay_completed: "Whakapumautanga utu",
    list_of_items: "Te Hoko Tuemi",

    checklist: [
      "Utu te nui o nga hua ka hokona atu e koe ki te kaihoko taketake.",
      "Ka taea e koe te utu me te whakatau i nga utu o nga hua ki te kaiwhakarato i roto i te waahanga kaute> Dropshipping o te toa.",
      "I muri i te utu, ka whiwhi te kaiwhakarato i to ota ka tukuna atu ki te kaihoko i muri i te whakarite.",
    ],
    message:
      "I muri i te utu i te utu ota ki te kaiwhakarato, tirohia te ahunga whakamua o te tuku ota i tenei waahanga.",
  },

  /** {@see BPageWholesaler} **/
  drop_shipping: {
    menu: {
      dashboard: "Papatohu",
      orders: "Nga ota",
      inventory: "tūemi",
      shops: "Kaihoko",
      requests: "Nga tono",
    },
    drop_shipping_charge: "Te utu whakaheke",
  },

  /** {@see DropshipRequestStatus} **/
  dropship_request_status: {
    PENDING: "Tarewa ana",
    ACCEPT: "Whakaae",
    REJECT: "Whakakahore",
  },

  /** {@see BPageWholesalerShop} **/
  dropshipping_reseller_page: {
    menu: {
      dashboard: "Papatohu",
      orders: "Nga ota",
      wallet: "Pukoro",
    },
  },

  /** {@see BPageWholesalerShopDashboard} **/
  dropshipping_reseller_dashboard: {
    title: "Papatohu kaihoko",
  },

  /** {@see BPageWholesalerShopOrders} **/
  dropshipping_reseller_orders: {
    title: "Nga ota kaihoko",
  },

  /** {@see BPageWholesalerShopWallet} **/
  dropshipping_reseller_wallet: {
    title: "He putea kaihoko",
  },

  /** @see MasterPaymentDialog **/
  master_payment: {
    notifications: {
      payment_success_title: "Whakapumautanga utu",
      payment_success: "Kua oti te utu.",
      buy_title: "Hoko",
      pay_by_giftcards: "I utua to ota ma te kaari koha.",
      free_order: "He kore utu to ota.",
      pay_by_cod: "Kua rehitatia to ota hei putea mo te tuku.",
      pay_by_dir:
        "Kua rehitatia to ota hei utu tika, tena koa whakaotia te utu me te tuku i te rihīti ki to wharangi ota.",

      pay_title: "Utu",
      qr_code_payment: "Utu QR waehere i hanga.",
      connecting_to_online_gateway: "Te hono ki te kuaha utu ipurangi.",
    },
  },

  /** @see DropShippingAccountCharges **/
  dropship_charges: {
    notifications: {
      pay_confirm_message: "Ka whakamanahia te utu.<br> Rā: {payment_at}",
      pay_fail_title: "Kaore i utua",
      pay_fail_message: "Ko te utu kaore e taea te manatoko!",
    },
  },

  /** @see BPageShopFinanceResellerFulfillment **/
  dropship_checkout: {
    pay_dialog: {
      title: "Ka tono utu nga kaihoko",
      balance: "To toenga kaute",
      total_cost: "Tapeke utu ota",
      need_charge: "Me utu",
      no_account_cation: "Karekau he kaute! Karangahia te kaihoko rarawe.",
      orders: "Nga ota",
    },
    notifications: {
      success_pay:
        "Ko to utu mo enei ota i whakapumau ka oti pai.",
    },
  },
  /** @see BPageShopFinanceResellerWallets **/
  dropship_accounts: {
    charge_account: "Kaute Utu",
    charge_dialog: {
      title: "Utuhia te putea whakaheke",
      amount_input: "Utu kaute",
      charge_now: "Utu inaianei",
    },
  },

  /** @see BPageWholesalerDashboard **/
  dropship_admin_dashboard: {
    title: "Nga tono kaihoko",
    today_order_count: "Ka tatau nga ota i tenei ra",
    yesterday_order_count: "Ka tatau nga ota inanahi",
    total_7_days: "Tapeke ota i roto i nga ra 7 whakamutunga",
  },

  /** @see BProductAddDropshipping **/
  dropshipping_products: {
    no_shops:
      "Aroha mai, kaore he toa i kitea. Ka nui ake nga toa a muri ake nei ka taea e koe te whakauru i a raatau hua ki to toa.",
    show_all_shops: "Whakaatuhia nga toa katoa",
    show_eligible_shops_message: "Whakaatuhia nga toa e tika ana",
    show_all_message: "Ka kite koe i nga toa katoa",
    current_shop: "Toa o naianei",
    request_account: "Tono kaute",
    request_accepted: "Ka whakaaetia te tono",
    request_rejected: "Kua whakakorehia te tono",
    request_pending: "Tono tārewa ana..",
    notifications: {
      add_product: "Hua kua taapiri atu ki to toa.",
      request_access:
        "I tukuna to tono urunga ki te kaihoko rarawe.",
    },
  },

  /** @see BProductDropshippingAbstractView **/
  product_dropshipping: {
    need_re_enable:
      "Kua hurihia tenei hua e te kaihoko taketake, na enei huringa utu i whakakore i te hua i roto i to toa. Tirohia nga huringa me te whakaae koe ki te whakahohe ano i te hua, pehia te paatene i raro nei.",
    need_re_enable_action: "Whakaae, tono huringa me te whakahohe",
    changed_value:
      "Ko tenei hua kua whakatikahia e te kaihoko taketake. Tirohia nga huringa a ki te whakaae koe ki te tono i nga huringa ki to hua, paatohia te paatene kei raro nei.",
    changed_value_action: "Whakaae, whakarereketia",
    parent_deleted: "Kua tangohia tenei taonga e te kaihoko taketake.",
    parent_closed:
      "Kua whakakorehia tenei hua e te kaihoko taketake! Kaore e taea e koe te hoko i tenei hua i roto i to toa, engari ki te whakahohehia e te kaihoko taketake te hua ka taea e koe te whakahohe i tenei hua i roto i to toa.",
    notifications: {
      re_enable_success: "Kua whakahohehia tenei hua.",
      update_success:
        "Ko enei korero hua i whakahoutia me nga korero hua taketake.",
    },
  },

  /** @see BPageProductDropshipping **/
  product_dropshipping_admin: {
    title: "Te tuku tuku",
    save_alert:
      "Kua rereke nga tautuhinga whakahekenga o tenei hua. Kei te pirangi koe ki te tiaki i nga huringa?",
    reselling_message:
      "Ma te whakahohe i tenei whiringa, ka taea e etahi atu kaihoko te taapiri i to hua ki o raatau toa.",
    reselling: "Whakahohehia te Dropshipping",
    price_merchants: "Utu mo nga kaihokohoko",
    price_commission: "Komihana Kaihoko (Whakatika)",
  },

  /** @see SShopBuyButton **/

  buy_button: {
    waiting_for_availability: "E tatari ana mo te waatea",
    inform_me_when_available: "Whakamōhio mai ina watea ana!",
    pay_buy: "Utu & Hokona",
    quantity: "Tatau",
    quantity_in_basket: "Nga taonga kei te kaata",
    count_unit: "tūemi",
    remove: "Tango",
    add_to_basket: "Tāpiri ki te Kete",
    book_now: "Tuhia inaianei",
    reserve_now: "Rahui inaianei",
    buy_now: "Hokona inaianei",

    buy: "Hoko",
    notifications: {
      inform_add_success: "Ka whakamohio atu koe ina watea ana.",
      inform_remove_success: "Kua tangohia koe i te rarangi tatari.",
    },
  },
  /** @see SProductOverview **/
  product_info: {
    fake: "rūpahu",
    action: "tūemi",
    compare_limit: "10 nga mea teiti",
    compare_add: "Tāpirihia ki te Whakataurite",
    compare_be_in_list: "Hei whakataurite rarangi",

    type: "Momo",
    brand: "Waitohu",
    category: "Kāwai",
    discount: "Te utu",
    track_price: "Utu ara",
    waiting_for_auction: "Kei te tatari mo te hokohoko?",
    inform_auction: "Whakamōhio mai i te wā hokohoko",
    return_in_days: "{days} Nga Ra Whakahoki Whakahoki",
    support24h7: "Tautoko 24 haora 7 ra",
    support_normal: "Tautoko haora mahi",
    original_guarantee: "Taurangi taketake",
    cod_payment: "Te putea mo te tautoko tuku",

    pros: "AtAKi",
    cons: "tūemi",

    external_link: "Pānuitia atu",
    notifications: {
      congratulation: "Tena koe",
      waiting_list_add_success:
        "Ka whakamohio atu matou ki a koe ina peekehia tenei hua.",
      waiting_list_delete_success: "I puta koe i te rarangi tatari hokohoko.",
    },

    offer_message: "Hokona {min_quantity} Whiwhi {percent}% mo enei mea",
    up_to: "Tae atu ki te {per_order}x mo ia ota",
    get_free: "HAERE KOREUTU",
    get_off: "TIKI {percent}% OFF",
  },

  /** @see SStyler **/
  styler: {
    size_class: "Rahi me te karaehe",
    delete_bg: "Mukua te ahua papamuri",
    bg_color: "Tae papamuri",
    section_style: "Whakaritea te ahua o te waahanga",
    link: "Tautuhi hononga",
    show_products: "Nga Hua / Nga waahanga whakahaere rarangi",
    show_product: "Tīpakohia te hua",
    delete_section: "Mukua te waahanga",
    text_color: "Tae kupu",
    tex_align: "Tirohanga Kuputuhi",
    text_style: "Kāhua kupu",
    bg_image: "Whakaahua papamuri",

    product: {
      title: "Tīpakohia te hua",
    },

    /** @see SPageProductsFilter **/
    products: {
      title: "Te whakahaere rarangi hua me nga waahanga",
      sort_options: "Tīpakohia ngā tautuhinga Kōmaka me te Tirohanga",
      item_types: "Whakaatuhia nga hua / waahanga anake",
      product_only: "Nga hua anake",
      category_only: "Ngā kāwai anake",
      no_category: "Karekau he waahanga kei roto i to toa.",
      limit: "Te rohe o nga hua",
      limit_message: "Whakauruhia nga tatauranga hua",
      select_categories: "Tīpakohia ngā kāwai e wātea ana",
      categories_limit: "Te rohe o nga waahanga",
      categories_limit_msg:
        "Whakauruhia te tau mōrahi o ngā kāwai. Tautuhia te kore mo te kore tepe.",
    },
  },

  /** @see BPageShopCrmChat **/
  shop_contacts: {
    popup: "Whakahohehia te tautoko pakūake",
    popup_message:
      "Ma te whakahohe i tenei whiringa, ka kitea te kii tautoko i te taha matau o raro o to toa.",
  },

  /** @see BPagePopup **/
  popup_page: {
    preview: "Tirohia pakūake ora",

    menu: {
      design: "Hoahoa",
      appearance: "Te ahua",
      filter: "Tātari",
      setting: "Tautuhinga",
    },
    notifications: {
      save: "I ora pai te pahūake.",
      edit: "I pai te whakahōu pahūake.",
    },
  },
  /** @see LMenuLeftPopupAppearance **/
  popup_appearance: {
    position: "Turanga",
    position_desc:
      "Tauwhāitihia te waahi whakaatu o tenei panui ki te wharangi.",
    width: "Whānui",
    width_desc: "Ka taea e koe te whakarite te whanui o te wharangi.",
    height: "Teitei",
    height_desc: "Ka taea e koe te whakarite i te teitei teitei o te whaarangi.",
    radius: "Putoro kokonga",
    radius_desc:
      "I tenei waahanga ka taea e koe te tautuhi i te radius o te pihinga o nga kokonga o te panui.",
    shadow: "Atarangi",
    shadow_desc: "Me whakamarumaru te matapihi pakū-ake?",
    delay: "Whakaatuhia te roa",
    delay_desc:
      "Kia uru mai te kaiwhakamahi ki to toa, i muri i te paunga o tenei wa, ka whakaatuhia he panui ki te kaiwhakamahi.",
    hide: "Huna whakaroa",
    hide_desc:
      "Ka taea e koe te whakarite ka kati aunoa te panui i etahi hēkona i muri i te whakaaturanga. Ko te uara o te 0 te tikanga ka kore te panui e kati aunoa.",
    transition: "hākoritanga pahūake",
  },

  /** @see BPagePopupFilter **/
  popup_filter: {
    registered: "Me whakauru me te whakaurunga",
    registered_desc:
      "Ka taea e koe te kowhiri ki te whakaatu i tenei panui ki te penapena i nga kaiwhakamahi, manuhiri ranei.",
    only_guests: "Ko nga manuhiri anake",
    only_users: "Kua takiuru noa nga kaiwhakamahi",
    purchased: "Hītori hoko",
    purchased_desc:
      "Ka taea e koe te kowhiri ki te whakaatu i tenei whakamohiotanga ki nga kaiwhakamahi kua hoko kee, ki te hunga kaore ano kia hoko.",
    no_purchase: "Kaore ano kia hokona",
    yes_purchase: "I hokona i mua",
    sex: "Ira tangata",
    sex_desc:
      "Kei te pirangi koe kia whakaatuhia tenei panui ki nga kaiwhakamahi o te ira tangata motuhake?",
    age: "Te tau iti",
    age_desc:
      "Mena ka whakauruhia he uara nui ake i te 6, me whakauru te kaiwhakamahi me te whakauru i tana ra whanau.",
    levels: "Taumata Karapu Kiritaki",
    levels_desc:
      "Mena kei te kaiwhakamahi tetahi o nga taumata kua tohua i raro nei, ka whakaatuhia tenei panui ki a ia.",
    countries: "Nga here waahi",
    countries_desc:
      "Kei te pirangi koe kia whakaatuhia tenei panui ki nga kaiwhakamahi mai i tetahi whenua motuhake?",
  },

  /** @see BPagePopupSetting **/
  popup_setting: {
    published: "Te mana kua whakaputaina",
    repeat: "Whakahokia",
    repeat_msg:
      "Kei te pirangi koe kia nui ake te whakaatu i tenei panui ki te kaiwhakamahi?",
    intervals: "Te roanga o te whakaaturanga pakūake",
    intervals_msg:
      "Ko te wa iti rawa i waenga i te whakaatu whakamohiotanga ki tetahi kaiwhakamahi kua pau.",
    designer_zone: "Rohe Kaihoahoa",
    title: "taitara pahūake",
    note: "Tuhipoka Kaihoahoa",
    image: "Whakaahua hipoki",
    delete: {
      title: "Mukua",
      sub_title:
        "Ina mukua e koe he pakūake, karekau he hokinga mai. Tena koa kia mohio koe.",
      action: "Mukua tenei Pahūake",
    },
    notifications: {
      delete: "Kua mukua te pahūake.",
    },
  },

  /**
   *  {@see BPageShopSettingSms}
   *  {@see BShopSmsTemplateList}
   *  {@see BShopSmsTemplateEditor}
   *
   **/
  shop_sms: {
    tabs: {
      templates: "tūemi",
      provider: "tūemi",
    },

    manage: {
      title: "tūemi SMS",
      subtitle:
        "Ka taea e koe te kite i nga ihirangi o nga karere kuputuhi i tukuna mai e to toa i konei. Na te iti o te rehita i nga tauira whakamana i roto i te ratonga karere, kaore e taea te whakawhaiaro i nga karere kuputuhi i tenei wa.",
    },

    template: {
      title: "tūemi",
      valid_message: "Ka taea e koe te hanga karere SMS ritenga.",
      provider_not_enable_message: "Ko te kaiwhakarato SMS kaore i te whakahohea!",
      need_custom_provider_message:
        "Ka mahi noa nga karere SMS kua whakaritea mena ka whakaritea e koe he kaiwhakarato ritenga.",
      add_new: "tāpiri Hou",
      disable_reason: {
        set_provider: "Whakaritehia to kaiwhakarato tuatahi!",
        provider_is_disabled: "tūemi to tūemi!",
        otp: "OTP - Karekau he mea whakarite!",
        select_another_provider: "Tīpakohia tētahi atu kaiwhakarato.",
      },
      enable_only_filter: "Whakaatu anake kua whakahohea",

      text_template_not_supported_msg:
        "Kaore to kaiwhakarato e tautoko i nga karere kuputuhi noa.",
      structure_template_not_supported_msg:
        "Kaore to kaiwhakarato e tautoko i nga karere hanganga.",

      structured_data: "tūemi Raraunga",

      menu: {
        reset_error: "Tautuhi hapa",
        send_test: "Tukua tūemi Karere",
      },
    },
    template_edit: {
      title_add: "tāpiri Hou Karere tauira",
      title_edit: "Whakatikatika Karere tauira",
      config: {
        title: "Karere Whirihora",
        subtitle:
          "Ka taea e koe te hanga tauira mo te karere SMS i runga i te paetukutuku a to kaiwhakarato SMS, katahi ka tohua tana waehere me te hanganga raraunga ki konei. Ka tukuna atu e matou enei korero ki to kaiwhakarato, kei reira ka puta te karere SMS.",
        code: "Waehere",
        language_msg: "Waiho kia putua hei tautuhi hei taunoa.",
        enable_msg: "Ka whakamahia tenei tauira hei hanga karere.",
      },
      text: {
        title: "Kuputuhi noa",
        subtitle:
          "Whakauruhia he karere SMS whaiaro ki konei. Ka whakakapia nga uara hihiko ki o raatau uara tuuturu i mua i te tuku ki te kaiwhakamahi.",
        message: "Karere",
        reset_to_default: "tūemi Ki Taunoa",

        auto_fill: {
          title: "Tawhā whakakī aunoa",
          subtitle:
            "Ka taea e koe te whakamahi ia o enei tawhā i roto i te tuhinga ka whakakapia e Selldone ki te uara e rite ana.",
        },

        sample: {
          title: "tūemi Karere",
        },
      },
      template: {
        title: "tūemi tauira",
        subtitle:
          "Ka taea e koe te tautuhi i te matua tauira me te uara ki konei. Ko te waehere tauira kia rite ki taau i whakatakoto ki to kaiwhakarato ratonga SMS, a ko te <code>key</code> te mea e hiahia ana to kaiwhakarato ki te tuku i te waehere. Mena kei te putua te mara <code>value</code> o te tauira, ka kiia ko <code>{code}</code> hei waehere tauira.",
        add_new_parameter: "tāpiri Hou tūemi",

        auto_fill: {
          title: "Tawhā whakakī aunoa",
          subtitle:
            "Ka taea e koe te whakarite ia o enei tawhā hei uara, ka whakakapia e Selldone ki te uara e rite ana. He pai ake te tautuhi i nga uara taunoa hei tauira i to kaiwhakarato SMS mo te ngawari. I tenei keehi, kaore he take ki te tautuhi i nga uara tawhā i konei.",
        },
        request: {
          title: "Tauira utu",
        },
        sample: {
          title: "tūemi tauira Karere",
          subtitle: "Ka taea e koe te kape me te tuku i tenei tauira ki to kaiwhakarato SMS.",
        },
        add_message_action: "tāpiri Karere tauira",
      },
    },
  },
  /** @see SmsTemplateMode **/
  sms_template_mode: {
    text: {
      title: "Kuputuhi Maama",
      desc: "Ka hangaia he karere mai i te tuhinga me nga tawhā whakakapi aunoa me te tuku.",
    },
    template: {
      title: "tūemi tauira",
      desc: "Ka hangaia he ahanoa mai i nga tawhā ka tukuna ki te kaiwhakarato.",
    },
  },

  /** @see BShopSmsProvider **/
  shop_sms_provider: {
    title: "Ritenga SMS tūemi",
    reset_error: "Tautuhi hapa",
    subtitle:
      "Whakaritehia he kaiwhakarato ratonga SMS ritenga me te whakarite i to motuhēhēnga SMS me to ingoa pakihi. Ka tukuna noa e te kaiwhakarato taunoa he SMS motuhēhēnga me te whakamohio i nga karere ki etahi whenua kua raarangi ma.",
    plain_text: "Kuputuhi noa",
    structural_template: "tūemi tauira",
    errors_limit_msg:
      "Mena ka nui ake to ratonga i te 100 hapa, ka whakakorehia aunoatia! Ka taea e koe te tautuhi i nga hapa ki te whakahohe ano.",

    enable_input: {
      false_description:
        "Ka tukuna nga karere OTP ma Seldone i nga whenua kua tohua, ka tukuna ano etahi atu SMS panui ota.",
      true_description:
        "Ka tukuna nga karere SMS ma te whakamahi i to kaiwhakarato ratonga.",
    },
    test_input: {
      true_title: "Whakamana Whirihoranga",
      true_description:
        "Ka tukuna he karere whakamatautau ki to nama waea 📞 <b>{phone}</b>, ka taea e koe te utu, engari ka taea e koe te whakarite kia pai nga mea katoa.",
    },
    sync_action: {
      title: "Nga Mahi",
      subtitle:
        "Ko etahi ratonga SMS e whakarato ana i etahi atu ratonga penei i nga kaupapa whakatairanga, hokohoko, me etahi atu waahanga. Ka taea e koe te tukutahi i o hoapaki me etahi atu korero kia tere ake te whakamahi i o raatau ahuatanga.",
      action: "tūemi Hoapā",
    },
    tokens: {
      title: "tūemi",
      subtitle:
        "Tautuhia nga tawhā e whai ake nei hei hono i to kaiwhakarato ratonga SMS me to toa.",
    },
  },
  sms_messages_options: {
    all: {
      title: "tūemi + 🛍️ Ota Whakamōhiotanga",
      description:
        "Ka tukuna nga karere motuhēhēnga me te whakamohiotanga ota.",
    },
    otp: {
      title: "OTP Anake",
      description: "Ka tukuna nga karere motuhēhēnga.",
    },
  },

  /** @see BPageShopSettingEmail **/
  shop_emails: {
    title: "Whakahaerehia nga imeera",
    sub_title:
      "Ka kite koe i te rarangi o nga imeera i tukuna e te toa ki nga kaihoko me nga kaiwhakahaere i tenei waahanga. Ko nga imeera ka hangaia me te hanga i runga i nga korero kua whakauruhia e koe mo to toa. Ano hoki, ko te tupono ki te whakawhaiaro i nga karere me nga whakaahua ka waatea ki te marea a meake nei.",

    tabs: {
      preferences: "tūemi",
      templates: "tūemi",
      provider: "tūemi",
    },

    preferences: {
      title: "Panuitanga Nga imeera",
    },
  },

  /**
   * @see BShopEmailProvider
   */
  shop_email_provider: {
    title: "Ritenga mēra tūemi",
    subtitle:
      "Connect to mēra te tūemi Ki Tukua Nga imeera tūemi to Ritenga Rohe.",
    provider: "tūemi",
    enable_input: {
      false_description: "Ka tukuna o imeera ma to rohe myseldone.",
      true_description: "Ka tukuna o imeera ma to rohe mēra ritenga.",
    },
    config: {
      title: "tūemi",
      subtitle:
        "Hei whirihora i to ratonga mēra ritenga mo to toa, whakauruhia to taviri API ratonga me etahi atu tautuhinga e tika ana.",
    },
    sender: {
      title: "Mai",
      subtitle:
        "Ka taea e koe te whirihora i te ingoa o te kaituku taunoa me te wahitau imeera, ka whakamahia hei tuku imeera ki o kaihoko.",
    },

    inputs: {
      from: {
        label: "Mai Īmēra",
      },
      from_name: {
        label: "Mai Ingoa",
      },
    },

    actions: {
      send_test_email: "Tukua he tūemi Īmēra",
    },

    messages: {
      provider_error_limit:
        "Mena ka nui ake to ratonga i te 100 hapa, ka whakakorehia aunoatia! Ka taea e koe te tautuhi i nga hapa ki te whakahohe ano.",

      encryption_ports_guide:
        "Ka taea e nga kaiwhakarato imeera te tuku SSL, TLS, hononga kore whakamuna ranei i runga i nga tauranga e waatea ana. Mena ka puta he hapa i te hono, ma te whakarereke i te aratau whakamunatanga ka whakatau pea te take. Paerewa <b>TLS</b> whakamahi Tauranga : <b>587</b> me <b>SSL</b> whakamahi Tauranga : <b>465</b>.",

      enable_receive_test:
        "Whai muri i te tiaki i nga huringa, ka whiwhi koe i te mēra whakamatautau i runga <b>{email}</b>. Tiakina nga huringa i mua i te tuku imeera whakamatautau me te whirihoranga hou.",
    },
  },

  /** @see InstagramViewMedias **/
  instagram: {
    title: "Hoko i runga i Instagram",

    add_product: "Tāpiri hua hou",
    create_product: "Waihanga hua",
    link_box_msg:
      "Whakauruhia tenei hononga ki to koiora i runga i te Instagram, Telegram me etahi atu paapori paapori kia tere ake ai te hoko mai a to hunga whakarongo, ngawari ake, kia nui ake.",
    remove_account: "Huri / Tango hononga Instagram",
  },
  /** @see BPageShopChannelInstagram **/
  shop_channel_instagram: {
    title: "Instagram Hokonga Hongere",
    subtitle:
      "Waihangahia he waahanga motuhake mo o hua, whakakiia me nga whakaahua me nga ataata, ka tohatoha ki to paapori pāpori. He rite ki a Instagram me TikTok, engari ma te mana whakahaere, ka taea e koe te hoko tika i o hua.",
  },

  /** @see ServiceBillingItem **/
  service_bill: {
    task: {
      title: "Rarangi mahi",
      message:
        "Tuhia nga taitara o te mahi e hiahia ana koe ki te mahi ka whakamohio ki te kaihoko mai te tawhiti o te ahunga whakamua.",
    },
    booking: {
      title: "Rahui / Rahui",
      message:
        "Ka tukatukahia tenei ota i te wa kua tohua, kua tohua e te kaihoko te waa e hiahiatia ana, ki te hiahia koe ki te whakarereke i te waa kua tohua, kia mohio te kaihoko.",

      selected_checkin: "Te Ra Takiuru Kiritaki kua tohua",
      selected_checkout: "Kua tohua te ra putanga o nga kaihoko",
      change_days_question:
        "Me huri koe i nga ra kua tohua e te kaihoko?",
      show_calendar: "Whakaatu Maramataka o Ota",
    },
    pricing: {
      title: "Te utu",
      message: "E hia nga utu mo tenei ratonga?",
    },
    subscription: {
      title: "Ohaurunga",
      message:
        "Ka taea e koe te whiriwhiri i te wa ohaurunga ma te hoko i tenei ratonga i ia ra. Kia mohio te kaihoko ki te hiahia koe ki te whakarereke i nga uara taunoa kua whakauruhia e koe.",
      duration: "Te wa ohaurunga",
    },
    charge: {
      title: "Te utu",
      message:
        "Kua tohua e koe he utu mo tenei mea. Ka taea e koe te whakarereke i te utu utu mo te taonga i hokona e te kaihoko.",
      charge: "Te utu",
    },

    save_item: "Manatokohia ka tautuhi",
    start_service: "Tīmatahia te ratonga",
    end_service: "Whakaoti",
  },
  /** @see BOrderDashboardServiceTasks **/
  service_tasks: {
    notification_finish: {
      title: "Ka mutu te mahi",
      message:
        "Mena kua oti tenei, pehia te taviri whakau hei whakaoti me te kati i te ota.",
      action: "Whakauhia ka whakaoti i te ota",
    },
  },

  /** {@see BProductEditOutputs} **/
  product_outputs: {
    outputs_form: "Hanganga korero o te Whakaatu (putanga)",

    virtual: {
      title: "Puka Korero Putanga Hua",
      sub_title:
        "Ko nga korero e hiahia ana koe ki te whakaatu ki te kaiwhakamahi i muri i te hoko. He tino tika tenei puka mo te hoko hua mariko penei i te kaari koha.",
    },
    service: {
      title: "Hoahoa Ratonga",
      sub_title:
        "Tuatahi tohua to momo ratonga ka uru ki nga korero e hiahiatia ana. Hei tauira, ka taea e koe te tautuhi i te rarangi todo, te patai ranei ki te kaihoko mo te wa tono, he wa whakarite ranei.",
    },
  },

  /** {@see BProductServiceManage} **/
  service_design: {
    type: "Momo ratonga",
    type_message:
      "Tīpakohia te momo ratonga ka whakaratohia e koe mo tenei hua. Ma te kowhiri i ia mea, ka whakaatuhia te puka e tika ana. I runga i te momo ratonga ka tohua e koe, ka nui ake nga korero ka riro mai i te kaihoko ina ota.",
  },

  /** {@see AvocadoHomePage} **/
  avocado: {
    name: "Awhekātō Avocado",
    title: "Te hoko tere, tika me te ngahau i runga i nga paapori pāpori katoa",
    message:
      "Kotahi te moni me te taitara kotahi te mea e hiahia ana koe ki te tiki moni mai i o kaihoko! Ka tukuna tika nga moni ki to putea ka whai wheako hokohoko ahurei o kaihoko.",
    new_order: "pire hou",
    create_by_seller_title: "Waihanga nama",
    create_by_seller_message:
      "Mena kei te pirangi koe ki te hanga nama me etahi moni ka tukuna atu ki tetahi, ka taea e koe te patene i te paatene i raro nei hei whakaatu i te puka nama. Homai he hononga hono ki te kaihoko ki te utu me te whai i tana ota ma roto.",

    create_by_buyer_title: "Te ahua o te avocado Avocado",
    create_by_buyer_message:
      "Ma te whakahohe i tenei waahanga, ka whakaatuhia he hononga ki a koe, hoatu tenei hononga ki o kaihoko kia taea ai e ratou te ota.",

    orders_list: "Rarangi ota",
    active_mode: "Hohe Receipt Order",
    inactive_mode: "hohekore",
    active_message:
      "Ma te whakahohe i tenei puna, ka taea e o kaihoko te tuku ota mo te hua ma te puka tono ka kitea e koe i raro nei.",
    avocado_form_link_message:
      "Whakauruhia tenei hononga ki to koiora i runga i te Instagram, Telegram me etahi atu paapori paapori kia tere ake ai te hoko mai a to hunga whakarongo, ngawari ake, ka mutu ake.",
    need_address: "Tikina Wāhitau",
    need_address_msg:
      "Mena ka hiahia koe ki te tiki i te waahi me te wahitau o te kaihoko, tohua tenei whiringa.",
    avocado_link_message:
      "He nui ki te whakarato i te hononga i runga ake nei ki te kaihoko ki te utu i te ota. Ka taea e te kaihoko te whai i te ota ma tenei hononga.",
    edit_order: "Whakatika Whakatau",
    progress: "Reeti Te ahunga whakamua",
    buyer_link: "Hononga Kaihoko",
    buyer_link_message:
      "Homai tenei hononga ki to kaihoko. Ka taea e te kaihoko te utu me te whai i tana ota ma tenei hononga. He tohu tenei hononga, mena ka tukuna te hono ki tetahi, ka taea e ia te utu me te kite i nga korero. Ka noho kaha tenei hononga mo nga ra 60. Paatohia te hono i raro nei hei kape.",
    OrderConfirm_done: "Whakapumautia te ota.",
    OrderConfirm_message:
      "Mena ka whakamanahia e koe te utu ka tika te ota, panuihia te paatene i raro nei.",
    OrderConfirm_action: "Whakaaetanga Whakatau",
    PreparingOrder_message:
      "Mena kua reri te ota a te kaihoko ki te tuku, paatohia te paatene OK i raro nei.",
    PreparingOrder_action: "Kua reri te ota ki te tuku",
    finish_message:
      "Mena kua oti tenei ota, paatohia te paatene mutunga hei kati i tenei ota.",
    finish_action: "Whakaotia te tukatuka ota",
    finish_dialog: {
      title: "Whakaotia te ota",
      message: "Kua oti te ota me te tino hiahia koe ki te kati?",
      action: "Ae, whakaū i te otinga o te ota",
    },
    configuration: "Tautuhinga Avokado Avocado",
    delivery_price: "Utu kaipuke",
    edit_avocado_action: "Whakatikahia nga korero",
    submit_pricing_avocado_action: "Tukuna te utu o te ota",
    add_avocado_action: "Waihanga Hononga Nama",
    avocados_list: "Rarangi o Avocados",
    not_pricing: "Kaore tenei taonga i te utu!",

    show_all: "Whakaatu Katoa",
    show_accepted: "Whakaatuhia nga mea kua Whakapumautia",

    notifications: {
      update_success: "I momoho te whakahou i te avocado. Avocado",
      add_success: "I momoho te hanga awhecado. Avocado",
    },
  },

  /** {@see BDashboardShopOrdersAvocado} **/
  avocado_orders: {
    title: "Awhekātō Avocado",
    title_small: "Hoko Paapori",
    pending_orders: "Nga whakahau kaha",
    avocado: "Ota Awhekātō Avocado",
  },
  /** {@see BAvocadoInvoice} **/
  avocado_process: {
    step1: "Te utu",
    step1_msg:
      "Whakauruhia te utu o ia mea ka tirohia te pouaka matomato. Mena kaore e taea te hoko i tetahi mea, hurihia tona mana ki te whakakore.",
    step2: "Whakaū nama",
    step2_msg:
      "Waihangahia he nama mo tenei ota. Ka taea e koe te whakauru i te taitara me te whakaahuatanga mo te nama. Ko te moni ota ka tatauhia i runga i te utu katoa o nga taonga.",
    title_input: "Taitara Nama *",
    show_to_customer: "Ka whakaatuhia enei korero ki te kaihoko.",
    description_msg:
      "Me tuhi tuhipoka mo tenei ota? Ka whakaatuhia enei korero ki te kaihoko.",
    price_input: "Te rahinga ota *",
    tax_none: "Taake-kore",
    tax_included: "Kei roto i te utu",
    tax_add: "Tāpiri taake",
    tax_message: "Ka taea e koe te whiriwhiri mena kei te whakauruhia te taake ki te utu, kaore ranei.",
    tax_included_messages: "Ko te taake kei roto i te utu.",
    tax_add_messages: "Me tapiri te taake ki te utu whakamutunga.",
    invoice: {
      subtitle:
        "Ma te whakamahi i tenei puka, ka taea e koe te hanga hononga utu mo tetahi moni motuhake. He mea ngawari hoki te whai i te nama me te whakau i te whakaotinga utu.",
    },
  },

  /** {@see BPageProductHyper} **/
  /** {@see BPageShopChannelHyper} **/
  hyper: {
    name: "Hiper Hyper",
    print_label:
      "Taa te tapanga e whai ake nei ka tuu ki mua o te hua",
    hyper_form_link_product_message:
      "Ka taea e o kaihoko te hoko tere {product} ma tenei hononga. Ka uru ano ratou ki te hononga kotahi ma te karapa i te waehere QR.",
    title: "Hoko Tere Hyper",
    message:
      "Ma te awhina o te hongere hoko Hyper Selldone, ka taea e o kaihoko te hoko hua ma te matawai i te waehere QR kaore he mahi e hiahiatia ana mai i to taha. Whakamahia tenei huarahi ki te hoko tere i etahi taonga i nga hakari, i nga whakaaturanga, i nga huihuinga nui ranei. I muri i te whakahohe i te Hyper i tenei waahanga, ka taea e koe te hanga i tetahi waehere QR hyper mo nga hua tinana i roto i te waahanga Whakahaere Hua.",
    orders_list: "Rarangi ota Hyper",
    active_mode: "Kua Whakahohehia te Hongere Hokohoko Hyper",
    inactive_mode: "hohekore",
    active_message:
      "Mena ka taea e koe tenei whiringa, ka whakaatuhia te waahi ki te hanga i nga Waehere-Hoko Tere i roto i te waahanga whakahaere o ia hua.",
    hyper_form_link_message: "Hononga wharangi nui o to toa",
    configuration: "Tautuhinga Hyper",
  },

  /** {@see BDashboardShopOrdersHyper} **/
  hyper_orders: {
    title: "Hiper Hyper",
    title_small: "Ripoata Hokohoko",
    hyper: "Kua whiwhi ota",
  },

  /** {@see BProductInventoryManagementFile} **/
  product_file: {
    no_file: "Karekau he kōnae i tukuake!",
    delete_dialog: {
      title: "Mukua te kōnae",
      message: "Me muku tonu tenei kōnae?",
      action: "Ae, Muku Inaianei",
    },
    notifications: {
      delete_success: "I pai te muku i te konae.",
    },
    drag_sort_msg: "Ka taea e koe te toia me te whakariterite i nga konae.",
    paid_mode_msg: "Ka taea e nga kaiwhakamahi te tango i muri i te hokonga o te hua.",
    free_mode_msg:
      "Ka taea e nga kaiwhakamahi te tango mo te kore utu. Ko nga tauira tauira me iti iho i te 50 MB.",
    has_gust_shopping_msg:
      "Ka taea e nga kaiwhakamahi kua rehita me nga manuhiri te tango i nga konae mai i te mea ka taea te hoko manuhiri ki nga tautuhinga o te toa> rere.",
    only_registered_shopping_msg:
      "Ko nga kaiwhakamahi kua rehita anake ka taea te tango i nga konae tauira.",
    file_count_limit: "tūemi Tatau Tepe",
  },

  /**
   * @see BProductLocationRestrictionsList
   */
  product_location_restrictions: {
    title: "Kei te waatea Nga waahi",
    subtitle:
      "Mena kei te waatea noa to hua, ratonga ranei i roto i nga rohe motuhake, kua tautuhia e te whenua me te tohu tohu, tohu pine ranei, ka taea e koe te tohu i aua waahi ki konei. Ka whakamanahia enei here waahi ki te taumata hua. I te wharangi hua, ka akiakihia nga kaihoko ki te kowhiri i o raatau waahi. Mena kei te waatea te hua i roto i ta raatau waahi whiriwhiri, ka taea e raatau te haere tonu ki te hoko.",

    add_location_action: "tāpiri Whenua",
    no_restriction: "kahore tūemi",
    import: {
      title: "tūemi kōtaha",
      subtitle: "tūemi he tūemi Nga waahi kōtaha.",
    },
    export: {
      title: "Tiaki kōtaha",
      subtitle: "Rokiroki i nga waahi hei whakamahi a meake nei.",
    },
    has_restriction_input: {
      title: "tūemi Tauwāhi tūemi",
      description:
        "Ka taea e nga kaihoko anake te hoko i tenei hua i roto i te whenua kua tohua me te waehere zip, na te mea kua whakatauhia te waahi waahi.",
    },
    zip_pin_code: "Waehere Zip / Pin",
    no_country_selected_error:
      "Whakaurua koa kia kotahi te whenua me te rarangi o nga waehere zip/pin. Ki te kore te reira, e kore e wātea tenei hua.",
    tips: "Patohia te paatene <b>+ Tāpiri Whenua</b> hei timata ki te hanga i tetahi huinga tauwāhi hou. Kia oti koe, ka taea e koe te tiaki ma te paatene i te taha matau-runga <b>⋮ Tahua</b> me te whiriwhiri <b>Tiaki Profile</b>.",
    need_save_message:
      "Kua hurihia nga tautuhinga here waahi mo te hua. Hei pupuri i enei huringa, paatohia te paatene 'Tiaki' i raro nei.",

    add_dialog: {
      title: "tāpiri Whenua",
      message:
        "Kōwhiria he whenua ka pāwhiri 'Tāpiri'. Ma tenei ka taapiri te whenua ki te ripanga, ka taea e koe te tautapa i nga Waeherehere, Waehere Pin, Ingoa Taone, ingoa Rohe ranei.",
    },
    save_profile_dialog: {
      title: "tāpiri Nga waahi kōtaha",
      message:
        "Whakaurua koa he ingoa hei tiaki i tenei huinga waahi. Ko tenei huinga waahi kua tiakina ka taea te tono ki etahi atu hua, ka taea te uta tere i te waahi ma te paato noa. Mēnā kei te noho kē he kōtaha me te ingoa kua tohua, ka whakahōuhia ki ēnei uara hōu.",
      title_input: {
        title: "kōtaha Taitara",
        placeholder: "he Kāwai Ingoa, te toa Ingoa,...",
      },
    },
    load_profile_dialog: {
      title: "tūemi Nga waahi kōtaha",
      message:
        "Ka taea e koe te uta i nga waahi ma te kowhiri i tetahi waahi kua tiakina i mua ki konei.",

      select_input: {
        title: "Tauwāhi kōtaha",
        placeholder: "tūemi he kōtaha...",
      },
    },
  },

  /** {@see NotificationTopBar} **/
  notification_top_bar: {
    free_charge: "🎊 Utuhia te {amount} ka whiwhi i te nama {amount_total}.",
    charge_action: "Te kereme nama kore utu",
    claim_title: "Tonoa to nama kore utu",
    claim_charge_caution:
      "Tuhipoka: Kaore e taea te huri i nga moni i waenga i o putea putea. No reira, tohua he kaute e tika ana mo te moni i whakawhitia i to toa.",
    account_input: "Pukete putea",
    account_msg: "Ka tukuna he utu ki tenei kaute",
  },

  /** {@see BDashboardShopApplications} **/
  selldone_applications: {
    title: "Kaua e ngaro o ota!",
    msg: "Whakahaerehia to pakihi ki hea me nga wa katoa.",
    scan_qr_code: "Matawai ahau ki to waea!",

    notification: {
      title: "Panuitanga",
      subtitle: "tūemi Hou Nga ota Panuitanga.",
    },
    order: {
      title: "Nga ota",
      subtitle: "Tirohia o ota me te whakahaere.",
    },
    product: {
      title: "Hua",
      subtitle: "tūemi whakahaere ka Hua tūemi.",
    },
  },

  /** {@see BPageShopSettingSeo**/
  shop_seo: {
    title: "Whirihoranga SEO",
    msg: "He tere whakamua ki te hokohoko tuihono ma te whakamana i nga hongere hoko a nga kaihoko.",
    auto: {
      title: "Miihini SEO Aunoa",
      msg: "Whakaratohia he wheako waea pūkoro mo te hunga whakarongo me nga miihini rapu i runga i to paetukutuku.",
    },

    amp: {
      title: "AMP",
      msg: "Ko nga wharangi pūkoro tere te whakaputa aunoa.",
    },
  },

  /** {@see BPageShopSettingEmail} **/
  shop_preferences: {
    order: {
      title: "Nga ota",
      desc: "Mena ka utua e te kaihoko te ota, ka tukuna he imeera kei roto nga korero ota ki tenei wahitau.",
    },
    return: {
      title: "Hoki mai",
      desc: "Mena ka whakahokia mai te hua e te kaihoko, ka tukuna tenei imeera. Ko te tuku i tenei imeera kei runga i to mahere i hokona.",
    },
    pos: {
      title: "POS",
      desc: "Ka tukuna te ripoata hoko POS ki tenei imeera.",
    },
    avocado: {
      title: "Awhekātō Avocado",
      desc: "Tukuna he purongo o ia ra mo nga ota Avocado.",
    },
    accounting: {
      title: "Kaute",
      desc: "Whakaorangia nga purongo putea me nga whakahou.",
    },
    subscription: {
      title: "Ohaurunga",
      desc: "Ko te mana o te mahere kua hokona tata nei me nga panui ohaurunga tiimata me te mutunga.",
    },
    technical: {
      title: "Hangarau",
      desc: "Nga whakamohiotanga hangarau mo te rohe me te mana ratonga.",
    },
    report: {
      title: "Ripoata",
      desc: "Ka tukuna nga purongo ia wiki me ia marama ki tenei imeera",
    },
    review: {
      title: "Arotake",
      desc: "Mena ka hiahia koe ki te whakaū i tetahi tukanga i roto i te toa, ka tukuna te imeera e tika ana ki tenei wahitau. Hei tauira ki te whakaū i te tuku i te imeera hokohoko ki te penapena i nga kaiwhakamahi.",
    },
    bulk: {
      title: "tūemi 🛍️ Ota Ripoata",
      desc: "Whiwhi i tetahi imeera ia ra me nga korero mo nga ota mo taua ra.",
    },
  },

  /** {@see BPageShopSettingLanguages} **/
  shop_languages: {
    title: "Rokiroki reo",
    sub_title: "Tāuta me te whakahohe i ngā pōkai whakamāori-aunoa toa.",
    message:
      "Ka taea e koe te whakauru ki te 10 nga putea reo ki to toa. Whakapaa mai ki te hiahia koe ki te whakauru i etahi atu kete.",
  },

  /** {@see SInviteFriends} **/
  invite_friends: {
    title: "Korerohia to pakihi ki te ao!",
    msg: "Whakauruhia a Selldone ki o hoa. Ka whiwhi ratou i te $99 i roto i te putea kamupene ina haina ratou, a ka whiwhi koe i te $15 i roto i te putea whakaoho ina oti ana ta raatau tauwhitinga tuatahi.",
    invite: {
      title: "Hono atu ki te OS pakihi a Selldone kia whiwhi moni utu $99.",
      description:
        "Ka tono ahau ki a koe ki a Selldone ki te timata i to pakihi ipurangi me tetahi taputapu tino pai.",
      hashtags: "tauhokohoko,Kaihanga paetukutuku",
      quot: "I hanga e au he toa ipurangi na Selldone, ka taea ano e koe to pakihi inaianei.",
    },
  },

  /** {@see BPageShuttleShops} **/
  shops_list: {
    title: "Aku Toa",
    message:
      "Rarangi o aku toa, kua whakaaetia me nga toa tauira.",

    sample: {
      title: "tūemi Toa",
      subtitle:
        "I konei, ka kitea e koe etahi momo toa tauira. Ka whakaaetia koe ki te tirotiro i nga korero kei roto i enei toa. Heoi, kaua e uru ki roto, ki te tuku ota ma te whakamahi i nga korero whaiaro, na te mea ka waatea enei korero ki te marea. Mena ka uru pohehe koe i o korero ki konei, tena koa puta atu i te toa me te whakapiri mai ki a maatau mo etahi atu awhina.",
    },

    add_new_store: "tāpiri Hou toa",
    i_have_a_deal: "I tūemi he tūemi",
    access: {
      title: "Urunga tūemi",
    },
  },

  /** {@see BTransportationServiceLabels} **/
  shipping: {
    warehouse_error:
      "Whakaritehia to whare putunga tuatahi! Ka whakamahia tenei wahitau hei takenga mai o te tuku.",
    errors: {
      set_origin: "Tautuhia to mahere waahi.",
      set_country: "Tautuhia to whenua.",
      set_address: "Tautuhia to wahitau.",
      set_zip: "Tautuhia to waehere poutāpeta.",
    },
    set_warehouse: "Tautuhi whare putunga",
    pickup_location: "Tauwāhi tiki",
    refresh_rates: "Reiti tāmata",
    no_rate: "Kaore he reeti...",
    view_messages: "Tirohia nga karere...",
    not_set_yet: "Kaore ano kia whakaritea!",
    drop_orders_here: "Whakataka ota ki konei...",
  },

  /** {@see BPageShopBlogsCategories} **/
  blog_category: {
    title: "Ngā Kāwai Blog",
    subtitle:
      "Ka taea e koe te hanga i te maha o nga waahanga mo o whakairinga rangitaki kia pai ake ai te whakahaere. Ma te whai rohenga waahanga ka pai ake te whakahaere i to paetukutuku. Ko te mea nui, ahakoa ko nga tari purongo nui kei te pupuri i te maha o nga waahanga, he iti ake i te 100, ki te whakamaarama i nga whakahaere me te pupuri i te tirohanga whanui.",

    add_action: "Tāpiri kāwai hōu",
    add_dialog: {
      title: "Hōu / Whakahōu Kāwai",
      force_edit: "Whakahauhia te ara whakatika",
      force_edit_msg: "Hurihia te ingoa kāwai me te whakahou i nga tuhinga katoa.",
      star_msg: "Ko nga waahanga kua wheturangitia he kaupapa matua ki te whakaatu ki te blog.",
      star: "Ka titiro",
      delete_action: "Muku kāwai",
      add_action: "Tāpiri Kāwai",
      edit_action: "Whakatika kāwai",
    },
  },
  /** {@see BPageShopBlogsTimeline} **/
  blog_timeline: {
    title: "Apiti Tuku Ihirangi",
    subtitle:
      "Anei te rarangi o nga tuhinga kua whakaritea mo te whakaputanga a meake nei. Kei a koe te kaha ki te whakarite i nga panui rangitaki tae atu ki nga whakaahuatanga hua hei whakaputa i nga waa motuhake.",
  },

  /** {@see BPageShopPagesAboutUs} **/
  about_us: {
    title: "Mo matou wharangi",
    subtitle:
      "Tuhia he tuhinga mo to pakihi hei whakaatu i runga i te wharangi About Us. Ma tenei whaarangi ka awhina i to hunga whakarongo ki te mohio ki to tohu me to kaupapa.",
  },
  /** {@see BPageShopPagesTerms} **/
  terms: {
    title: "Whārangi o te ratonga",
    subtitle:
      "Whakamaramahia nga tikanga mo te hoko me te whakamahi i o ratonga mo o kaihoko.",
  },
  /** {@see BPageShopPagesPrivacy} **/
  privacy: {
    title: "Whārangi kaupapa here tūmataiti",
    subtitle:
      "Ko te kaupapa here tūmataiti he korero, he tuhinga ture ranei e whakaatu ana i etahi, i te katoa ranei o nga huarahi e kohikohi ana, e whakamahi ana, e whakaatu ana, e whakahaere ana i nga raraunga a tetahi kaihoko, kiritaki ranei.",
  },
  /** {@see BPageShopPagesContactUs} **/
  contact_us: {
    title: "Whakapaa mai ki a maatau wharangi",
    subtitle:
      "Tuhia to korero whakapā, wāhi noho, me te nama waea hei whakaatu ki o kaihoko ki te wharangi whakapā mai.",
  },

  /** {@see BPageShopApplicationsMetaverse} **/
  metaverse: {
    title: "Piriti Metaverse",
    subtitle:
      "Ka whakarato a Selldone i te %100 nga ratonga a-API i te waahi hokohoko. Ka taea e te keemu, poraka, me nga kaiwhakawhanake metaverse te whakamahi i enei API kaha ki te hono i nga pakihi, nga kaihoko, me nga ratonga kei te heke mai o te ao mahi, oranga me te ao hokohoko.",
    message: "Te Uru Moata Mo Nga Kaihanga Keemu",
  },

  /** {@see BProcessCenterBulkExport} **/
  orders_bulk_export: {
    title: "Kaiāwhina Whakatau",
    subtitle:
      "Kaweake me te tango i nga tapanga mo te tukatuka ota nui - he pai mo te whakahaere neke atu i te 100 ota ia ra.",

    timespan: {
      title: "Wā wā",
      subtitle: "Whiriwhiria nga ra tiimata me te mutunga mo te whakatakoto ota.",
    },
    tasks: {
      title: "Putanga Mahi",
      subtitle:
        "Tikiake i te rarangi o nga mahi mo te tukatuka ota i te nuinga. Ko te hua ko nga ota utu me te COD anake.",
      export_title: "Te rarangi o nga mahi",
    },
    labels: {
      title: "Ngā Tapanga mōkī",
      subtitle:
        "Tikiake i te rarangi o nga tapanga ota katoa i roto i te waa me te mana kua tohua. Ko te hua ko nga ota utu me te COD anake.",
      export_title: "Nga tapanga pouaka",
    },
  },

  /** {@see BPageShopMarketingFunnel} **/
  customers_funnel: {
    title: "Taupori Kiritaki",
    subtitle:
      "Tātarihia me te kaweake i nga raraunga o o kaihoko kia pai ake ai te whakatau ki te whakaputa wariu ake me te iti ake o te hokohoko! Tena koa whakaute i te muna a o kaihoko, kaua rawa e tukuna he imeera nui ki a raatau! Kaua rawa e tiritiri i nga korero a o kaihoko ki nga ratonga tuatoru me te kore whakaaetanga mai i a raatau katoa.",
  },

  /** {@see BPageAffiliatorDashboard} **/
  affiliate_pos: {
    title: "Hononga POS",
    open_pos_action: "Te tono tere POS",
    description:
      "He pai tenei whiringa mo nga kaiwhakahaere o nga wharangi Instagram me etahi atu whatunga hapori. Whakamahia te Hononga POS ki te tango me te taapiri i nga ota mai i nga kaihoko. Ka taea e o hoa hononga (te kaporeihana ranei) te taapiri i nga ota hou ki te POS Hononga me te tuku i te pepa putea.",
  },

  /** {@see BPageShopSettingFlow} **/
  shop_configuration: {
    /** {@see BShopOptionsLoginMethods} **/
    login: {
      title: "Nga tikanga takiuru",
      subtitle: "Whakaritea te huarahi whakauru a nga kaiwhakamahi ki to toa.",

      tips: "<b>He mea nui.</b> Ina rehita ana tetahi kaiwhakamahi ki to toa, ka hangaia he kaiwhakamahi Selldone e rite ana, mena kaore ano i te noho. Ka whai waahi tenei kaiwhakamahi ki tana pukapuka wahitau puta noa i nga toa katoa ka taea e ia te tiro i o raatau hitori ota, ahakoa kua kati tetahi toa. Kaore matou e whakamahi i enei korero mo nga kaupapa hokohoko engari me tono kia aukati i te tinihanga i nga taha e rua. Kia noho marama, ka tukuna he imeera ki te kaiwhakamahi e whakaatu ana mo to rehitatanga.",
    },

    /** {@see BShopOptionsCheckout} **/
    checkout: {
      title: "Rere utu",
      subtitle:
        "Whakaritehia to haerenga kaihoko me te rere hoko i roto i to ecommerce.",
      map: {
        title: "Mapi",
        subtitle:
          "Me whiriwhiri nga kaiwhakamahi i te waahi tuku i runga i te mapi (penei i a Uber).",
        subtitle_no_map:
          "Ko te tawhiti i roto i te tatauranga kaipuke ka whakaaro kore ki te kore te waahi i te waahi i tukuna e te kaihoko.",
      },
      mode: {
        title: "Waahi takiuru a nga kaihoko",
        default: {
          title: "ASAP (Taunoa)",
          msg: "Ka uru wawe nga kaiwhakamahi. (He tahuri ke atu)",
        },
        on_checkout: {
          title: "I runga i te utu",
          msg: "Whakahauhia nga kaiwhakamahi ki te takiuru i te taahiraa taki.",
        },
        login_free: {
          title: "Takiuru kōwhiringa",
          msg: "Takiuru whiriwhiri i muri i te utu ota..",
        },
      },

      express: {
        title: "Anga atu i muri i te taapiri ki te kata",

        false: {
          title: "Aratau noa",
          msg: "Me noho tonu ki te whaarangi ka haere tonu ki te hokohoko i muri i te paato i te taapiri ki te kata.",
        },
        true: {
          title: "Aratau whakapuaki",
          msg: "Whakahokia te kaihoko ki te wharangi utu i muri i te panui i te taapiri ki te kata.",
        },
      },

      shipping_address: {
        title: "Tonoa mo te wahitau kaipuke",
      },
    },

    /** {@see BShopOptionsCheckoutForm} **/
    checkout_form: {
      title: "Takitaki puka",
      subtitle:
        "Whakaritea nga mara puka utu ki te kohikohi i nga korero e hiahia ana koe mai i o kaihoko.",
    },
  },
  /** {@see CheckoutOptions} **/
  checkout_options: {
    default: {
      title: "Tinana me te Ratonga (Taunoa)",
      desc: "Tonoa te kaihoko ki te whakauru i tetahi wahitau i roto i nga tirotirohanga tinana me te ratonga.",
    },
    physical: {
      title: "Tinana anake",
      desc: "Tonoa te kaihoko ki te whakauru i tetahi wahitau i roto noa i te tirotiro tinana.",
    },
    off: {
      title: "Kaua rawa",
      desc: "Kaua rawa e patai mo te wahitau kaipuke. ⚠ Whakamahia tenei kōwhiringa ki te whakamahere koe ki te whai ratonga me nga momo rereke ma te whiriwhiri i te momo tinana mo nga hua hei utu mo te momo ratonga.",
    },
  },

  /** {@see BPageShopSettingBusinessModel} **/
  business_model: {
    title: "Whakaritenga tauira pakihi",
    subtitle:
      "Whakaritehia to tauira pakihi, ka whirihorahia e matou to OS i muri i te tirohanga.",
  },
  business_type: {
    title: "Nga momo hokohoko",
    subtitle:
      "Tautuhia te momo hua e hiahia ana koe ki te hoko ki to toa. Ko nga momo hua kei Selldone he nui ake i te momo hua noa; he mea nui ake mo te tukanga whakatutuki.",

    send_activation_request_to_support:
      "Tukuna he tono whakahohe i roto i te tikiti tautoko.",
    require_kyc: "Ko te rangatira pakihi ko KYC.",
    needs_enterprise_license: "Me whai raihana hinonga koe.",
    marketplace_caution_message:
      "Whakatakotoria te Maakete hei tauira e pa ana ki to whirihoranga OS pakihi. Hei tauira, me tautuhi koe kia kotahi te kaihoko mo ia hua me te utu motuhake me te raarangi. Kaore e taea e nga kaihoko te hoko hua kaore he kaihoko.",
  },

  /** {@see BusinessModelView} **/
  business_model_view: {
    normal: {
      title: "Toa Pūnoa",
      desc: "I te nuinga o te wa he SME, he pakihi kaaanga, he kaihopu takahuri ranei - e hoko tika ana i a raatau hua ki nga kaihoko, e hoko ana ranei i nga hua a nga kaihoko rarawe hei kaihoko.",
    },

    dropshipping: {
      title: "Te tuku tuku",
      desc: "Kaua e pupuri i nga hua me te kore he rarangi ingoa - ka utua e koe ki tetahi atu tuatoru, he kaihoko rarawe (penei i te Printify, Wholesale2B, ...) he kaihanga ranei, a ka whakatutukihia e ratou nga ota kei muri i te whakaaturanga.",
    },

    wholesaler: {
      title: "Kaiwhakarato Dropshipping",
      desc: "He kaihoko rarawe, he kaihanga ranei - ka tuku ota tika ki te kaihoko mutunga. Ka taea e etahi atu kaihokohoko i runga i a Selldone te tuku i o hua ki o raatau toa.",
    },
    marketplace: {
      title: "Te kainga hokohoko",
      desc: "Ko nga maakete a-ipurangi e hono ana i nga kaihoko me nga kaihoko i runga i te turanga rangatira me te kaupapa matua.",
    },

    franchise: {
      title: "Franchise / Hononga",
      desc: "Ko te kaporeihana he tikanga mo te tohatoha hua, ratonga ranei e uru ana ki tetahi kaipakihi e whakatuu ana i to waitohu (Franchising) ka hoko ranei i o hua ma o raatau tohu (Affiliate).",
    },
  },

  /** {@see BPageMarketplace} **/
  marketplace: {
    vendor_wallets: "Pukoro",
    pricing_models: "Te utu",
    add_vendor: "Tāpiri kaihoko",
    requests: "Nga tono",
  },

  price_input_type: {
    title: "Momo whakauru utu",
    default: {
      title: "Taunoa",
      desc: "Tīpakohia he uara hei rahinga kei roto i te kata.",
    },
    area: {
      title: "Rohe",
      desc: "Whakauruhia te whanui me te roa hei whakaurunga hei tatau i te rahinga.",
    },
    volume: {
      title: "rōrahi",
      desc: "Whakauruhia te whanui, te roa, me te teitei hei whakaurunga hei tatau i te rahinga.",
    },
    custom: {
      title: "Ritenga (Ma te puka whakauru)",
      desc: "Kaore he hiahia mo nga rau rereke! Waihangahia he puka tatauranga utu ritenga mo nga hua tino whakarite.",
    },
  },

  /** {@see BPageShuttleVendors} **/
  vendors_list: {
    title: "Aku Kaihoko",
    message:
      "Rarangi o aku ake kaihoko. Selldone",
  },

  vendor_page: {},

  /** {@see BDomainsList} **/
  domains: {
    message:
      "Honoa nga rohe maha ki to OS pakihi ka whakahaere katoa i te waahi kotahi. Ka taea e koe te whakamahi i nga waahanga maha hei hanga i nga rohe motuhake mo ia waahanga o to paetukutuku, penei i te rangitaki me te hapori, ka whakamahi ranei i nga waahi rereke mo o hononga.",
  },

  /** {@see BNoteButton}   {@see BNoteDialog} **/
  notes: {
    add_action: "Tāpirihia he tuhipoka mo to roopu",
    notifications: {
      success_add: "Kua tutuki pai te taapiri.",
      success_delete: "Kua pai te whakakore i te tuhipoka.",
    },
  },

  /** {@see BDashboardShopExternalServices} **/
  external_services: {
    title: "Nga ratonga o waho",
    subtitle:
      "Aroturuki i te mana whakamutunga o nga ratonga o waho penei i te SMS me nga imeera i konei.",
  },

  /** {@see BPageShopSettingQuota**/
  shop_quota: {
    title: "Motunga whakamahi",
    sub_title:
      "Koinei te rarangi motunga mo to toa. Ka taea e koe te whakanui ake i enei tepe ma te whakahou i te raihana o to toa. Mena ka nui ake koe i nga tepe, ka taea e koe te tono taapiri mo te utu mo ia ra mai i a maatau. Ka tautuhia nga whakamahinga katoa ia ra.",
  },

  /** {@see WidgetAppCard} **/
  app_card: {
    category: "Kāwai",
    installs: "Tāuta",
    actives: "Hohe",
    uninstalls: "Wetetāutatanga",
    app_code: "Waehere taupānga",
    app_mode: "Aratau tono",
  },

  product_status: {
    open: {
      name: "Hohe",
      title: "Hohe ╏ Hokonga Tuihono & Tuimotu",
      description: "Ka waatea tenei hua mo nga ota ipurangi.",
    },
    close: {
      name: "hohekore",
      title: "Hohekore ╏ Hoko Tuimotu anake",
      description: "Kaore tenei hua e waatea mo nga ota ipurangi.",
    },
    pending: {
      name: "Tarewa ana",
      title: "Tarewa ana",
      description: "Me arotake te maakete i mua i te haere ora.",
    },
    rejected: {
      name: "Kua whakakorehia",
      title: "Kua whakakorehia",
      description: "I paopaohia tenei hua e te maakete.",
    },
    unlisted: {
      name: "tūemi",
      title: "tūemi",
      description:
        "Ko tenei hua kaore i te raarangi i roto i te toa, engari ka uru ma te hono tika.",
    },
  },

  /**
   * BDashboardShopFeedback
   */
  direct_feedback: {
    title: "Tuku Tono Tika",
    message:
      "Tautuhia to pakihi 3 runga rawa atu ki te tipu me te whai hua i runga i a Seldone. Ka tohatohahia enei korero ki nga kaihanga me nga kaiwhakahaere, ka ata whakaarohia. Ka taea hoki e koe te whakahou i o hiahia i muri i te tukunga.",
    expand_action: "tūemi te tūemi...",
    input_label: "to tūemi",
    to: {
      ceo: "Tukua Ki tūemi",
      team: "Tukua Ki tūemi",
    },
    placeholder: {
      ceo: "Kare rawa e tika kia pai, he pai ki ahau te whakarongo ki to patai tika, tika-ki-te-tohu.\ne.g. Me whai...",
      team: "Tena koa ngana ki te pai ki a raatau.\ne.g. Me whai...",
    },
  },

  /**
   * BInventoryFilter
   */
  inventory_filter: {
    placeholder: "tūemi Tātari *.*",
  },
  /**
   * BVendorInput
   */
  vendor_input: {
    placeholder: "Tātari kaihoko...",
  },

  /**
   * BShopProductsImportProcessing
   */
  product_import_processing: {
    title: "Tukatuka hua kawemai",
    message:
      "Kei te mahi matou ki te kawe mai i nga hua me nga whakaahua, ka roa pea te whakaoti.",
  },

  /**
   * BOrderInput
   */
  order_input: {
    message:
      "Ko nga korero e whakaatuhia ana i konei kua tukuna e te kaihoko, i runga i te hanganga puka whakauru hua.",
  },

  /**
   * BPageConnectDashboard
   */
  shop_connect: {
    test: {
      title: "toa",
      subtitle: "tūemi te Ripoata",
      action: "Whakamatau hononga",
      last_update: "Whakahoutanga whakamutunga na",
    },
    service_status: {
      title: "te Tūnga",
      syncing_message: "I te tukanga tukutahi...",
      action: "tūemi katoa",
      connect_status: "Connect Tūnga",
      service_status: "te Tūnga",
      auto_confirm_enable_msg: "Ka taea te whakamana-aunoa mo nga ota.",
      auto_confirm_disable_msg:
        "Ko te whakaū aunoa o nga ota kaore e taea.",
      shipping_enable_msg:
        "Ko te utu kaipuke ka tatauhia i runga i nga korero i tukuna i te wa e tirotirohia ana.",
    },
    detail: {
      title: "Connect tūemi",
      auto_confirm_order: "Whakaū aunoa ota",
      test_mode: "tūemi aratau",
      status: {
        subtitle: "Nga korero whanui mo te mana tukutahi.",
      },
      sync: {
        title: "Taipitopito tukutahi",
        subtitle:
          "Nga taipitopito tukutahi o roto. Ka tiakina enei korero i muri i te mutunga pai o te wa tukutahi.",
        no_data: "Karekau he korero tukutahi!",
      },
    },

    products: {
      title: "Hua",
      subtitle:
        "Rarangi o nga hua taapiri mai i {name}. Me mohio he ingoa ahurei to hua me te SKU. Ka whakamahia etahi ratonga Hono mo te heke, na mena he hua kei a koe he <b>SKUs</b> me <b>Names</b>, ka hurihia aua hua!",
      action: "tūemi Hua",
      syncing_message: "I te tukanga tukutahi...",
    },

    logs: {
      title: "tūemi",
      subtitle:
        "Te rarangi o nga raarangi katoa i nga ra e 7 kua hipa. Ka tautokohia te waahanga takiuru ki nga kaiwhakarato ratonga o waho.",
    },

    edit: {
      title: "tāpiri Hou Connect",

      service: {
        title: "te",
        subtitle:
          "Tīpakohia te papaa wharangi, te kaiwhakarato whakaheke, te maakete ranei hei hono atu.",
      },
      test: {
        false_title: "tūemi katoa",
        false_desc: "tūemi katoa Kei te waatea Hua, Ngā Kāwai, Kaihoko.",
        true_title: "Wā (tūemi mō tūemi)",
        true_desc:
          "Tikina kia 5 nga hua, nga waahanga, me nga kaihoko. Ka taea e koe te huri ki te aratau katoa i roto i te Hononga> Tautuhinga i nga wa katoa.",
      },
      overwrite: {
        false_title: "Kaua e huri mena kei te noho",
        false_desc: "Kia mau tonu nga taipitopito hua me nga waahanga o naianei.",
        true_title: "tūemi",
        true_desc:
          "Whakahoutia nga korero hua, momo rereke me nga momo waahanga.",
      },
      migration_tips:
        "<b>TTips: </b>Mena kei te pirangi koe ki te whakamatau i te mahi, tohua\n" +
          "            <b>Limited</b> kōwhiringa. Ma te kowhiri i te 'Tukutahi katoa', ka whiwhi matou i nga raraunga katoa mai i o ratonga hono me te tango whakaahua e pa ana, na reira ka whakaitihia to rokiroki koreutu. Ano, he uaua te tango i nga hua maha, nga waahanga, me nga kaihoko!",
      add_action: "Hono aunoa ki",
      order: {
        title: "tūemi Tūnga",
        subtitle:
          "Whirihorahia nga tautuhinga whakahaere ota a te ratonga hono mo to toa. Kei roto i tenei ko te whakarite i te whakaaetanga ota, te whakamana i te whakau ota aunoa (mehemea ka tautokohia), me te whirihora i nga tatauranga utu kaipuke.",
      },
      enable: {
        label: "Connect Tūnga",
        true_desc: "Ka tukutahia nga rarangi me nga ota.",
      },
      confirm: {
        label: "Whakaū aunoa ota",
        false_desc: "Me whakaū ā-ringa koe i te ota.",
        true_desc:
          "Ko nga ota me te utu ora ka whakaaetia aunoa. Ko nga ota me te utu patuiro e kore e whakaae aunoa.",
        tips: "Ko nga ota ka utua e te kaari koha, he COD ranei, he kuaha utu ranei i roto i te aratau patuiro he mea kee ka kore e whakaū aunoa.",
      },

      shipping: {
        label: "Te tuku",
        tips: "Ka tukuna e tenei ratonga he tatauranga kaipuke, ka taea te whakahohe, te mono ranei i runga i to hiahia. Ki te whakahohea, ka hiahiatia nga API tatauranga utu me nga ratonga tuatoru, tera pea ka roa te neke atu i te 300ms mo te whakahou i te utu i te wharangi utu.",
        false_title: "tūemi",
        false_desc:
          "Whakamahia taku tatauranga utu kaipuke, i roto i te Toa > Logistic > Te kawe waka mo nga hua katoa, tae atu ki nga mea kawemai.",
        true_title: "tūemi Te tuku",
        true_desc:
          "Whakamahia te API tatauranga reiti kaipuke e tukuna ana e tenei ratonga.",
      },

      remove: {
        verify: "Manatoko ahau ki te tango i tenei hononga.",
        action: "Tango tūemi",
      },
    },

    /**
     * BPageShopChannelConnect
     */
    list: {
      title: "Connect OS",
      subtitle:
        "Honoa to pakihi ki nga ratonga o waho me te whakahaere i nga hongere katoa i te waahi kotahi.",
      action: "Connect Hou Ratonga",
      empty_message:
        "Wareware i nga taupānga me nga taapiri mo te whakaheke! Ko te Hononga te heke mai o te whakahaere ota-whakawhitiwhiti.",
      auto_confirm: "Whakaū Aunoa",
      shipping: "Te tuku",
      syncing_now: "Tukutahi inaianei!",
      sync_not_started: "Kaore ano kia timata te tukutahi!",
    },
  },

  /**
   * ConnectMode
   */
  connect_mode: {
    Migration: {
      title: "tūemi",
      desc: "Tikina nga hua, nga waahanga, me nga kaihoko. I muri i te whakakore a te kaiwhakamahi i tenei ratonga, kaore nga hua kua riro, nga waahanga, me nga kaihoko e kore e mukua.",
    },
    Dropshipping: {
      title: "Te tuku tuku",
      desc: "Tikina nga hua, nga waahanga me nga ota. Ka tangohia e te punaha nga hua i muri i te tango a te kaiwhakamahi i te ratonga.",
    },
    Marketplace: {
      title: "Te kainga hokohoko",
      desc: "Tukutahi noa i nga raarangi ka whiwhi ota (Ka honoa nga hua e SKU).",
    },
    Accounting: {
      title: "Kaute",
      desc: "Tukua Nga ota ka Pūtea tūemi.",
    },
    Other: {
      title: "tūemi",
      desc: "tūemi Ratonga.",
    },
  },

  /**
   * BPageShopPagesLandings
   */
  shop_landings: {
    title: "Nga wharangi taunga",
    add_new_action: "tāpiri Hou whārangi",
    subtitle:
      "Waihangahia nga wharangi ritenga motuhake, aro mai mo to waitohu ma te whakamahi i te kaiwhakatika raina-a-AI, ka whakaputa aunoa i nga ihirangi hihiri ma te whangai i nga raraunga.",
    empty_message: "Hoahoa wharangi ritenga ma te toia me te taka...",
  },

  /**
   * BPageShopPermissionStaff
   */
  shop_staff: {
    list: {
      title: "Kaimahi whakahaere",
      subtitle:
        "Ko nga kaiwhakamahi ka uru ki te papatohu me nga waahanga whakahaere o tenei toa.",
    },
    pending: {
      title: "Tarewa ana",
      message: "whakaatu Tarewa ana tūemi.",
    },
  },

  /**
   * BPageShopCommunityComments
   */
  community_comments: {
    title: "Nga korero",
    subtitle: "tūemi to he Rarangi o te tūemi Nga korero.",
    empty_msg: "Karekau he korero i hangaia i tenei wa...",
  },

  /**
   * BPageShopCommunityCategories
   */
  community_categories: {
    title: "Ngā Kāwai",
    subtitle: "tūemi to he Rarangi o te tūemi Ngā Kāwai.",
    new_category_action: "Hou Kāwai",
    empty_msg: "Karekau he waahanga i hangaia i tenei wa...",
    filter: {
      delete: {
        title: "Kua mukua",
        description: "whakaatu Kua mukua Ngā Kāwai.",
      },
    },
  },
  /**
   * BPageShopCommunityPosts
   */
  community_posts: {
    title: "tūemi",
    subtitle: "He rarangi tenei o nga panui a te hapori.",
    filter: {
      deleted: {
        title: "Kua mukua",
        description: "whakaatu Kua mukua tūemi.",
      },
      reported: {
        title: "tūemi",
        description: "Whakaatuhia nga panui kua panuitia.",
      },
    },
    empty_msg: "Karekau he panui i hanga i tenei wa...",
    menu: {
      open_post: {
        title: "Whakatuwheratia te Panui",
        subtitle: "Tirohanga ka Whakatikatika tūemi i te Hapori.",
      },
      spam: {
        title: "Ripoata & Tango tūemi",
        subtitle: "Tangohia nga pou me te aukati i te kaiwhakamahi.",
      },
      delete: {
        title: "Mukua tūemi",
        subtitle: "Tangohia tenei whakairinga.",
      },
    },
  },

  /**
   * {@see BGoogleTagManager}
   */
  channel_google: {
    gtag: {
      title: "Tūtohu tūemi",
      subtitle:
        "Ka taea e Google Analytics te ine i te ROI panui i te wa e whai ana i te Flash, ataata, me nga papanga paapori me nga taupānga. Whakahaerehia nga tohu paetukutuku ma te ngawari me nga otinga koreutu, ngawari me te pono a te Kaiwhakahaere Tūtohu a Google—kaore e hiahiatia he whakatika waehere.",
      tag_id: {
        label: "Google Tūtohu tūemi ID",
      },
    },
    shopping: {
      title: "Google tūemi",
      subtitle:
        "Ko nga Raraunga Raraunga e tohu ana i te takenga mai o to korero korero i runga i a Google. Whaaia tenei hono ki te taapiri aunoa i nga hua me nga waahanga katoa ki nga taonga hokohoko a Google.",
      tips: "Ka taea e koe te whakamahi i tenei taputapu ki te whakahaere aunoa i nga panui a Google.",
      link_message: "Google Hua tūemi URL Fee",
    },
    console: {
      title: "rapu tūemi",
      subtitle: "Taapirihia he tohu meta ki to wharangi kaainga mo te manatokonga a Google Search Console.",
      code: {
        label: "tūtohu meta manatoko",
        placeholder: "Mai i google, e.g. viQg9mjdBj...",
      },
    },
    serp: {
      title: "Google tūemi & tūemi ERP",
      subtitle:
        "Ka whiwhi matou i nga raraunga arotake SEO tino tika mai i a Google me te whakaatu atu ki a koe i runga i nga tikanga ngawari. Me whai noa i nga kaupae e whakaratohia ana.",
    },
    serp_content: {
      title: "Connect Ki Google rapu tūemi",
      subtitle:
        "Honoa to toa ki te papatohu rapu ka tiimata ki te tirotiro i nga kupumatua, rarangi, me etahi atu taputapu SEO koreutu i runga i a Selldone.",
      tips: {
        title:
          "I mua i te hono atu ki a Selldone me Google Search Console, me whakarite enei e whai ake nei:",
        add_in_selldone: "Tāpiri, manatoko, ka tautuhi hei tuatahi mo to Rohe Ritenga.",
        add_in_google: "tāpiri ka tūemi <b>{domain}</b> i to<a href=\"https://search.google.com/search-console\" target=\"_blank\"> <b>Google rapu tūemi</b></a>.",
      },
      action_custom_domain: "Ritenga Rohe",
      action_connect_now: "Hono atu inaianei",
      action_remove_connection: "Tango tūemi",
    },
  },

  /**
   * {@see BPageShopIncentivesCoupon}
   */
  cashback: {
    add_new: "tāpiri Hou tūemi",
    title: "Kaupapa Whakahoki Moni",
    boosted: "tūemi (2tūemi)",
    min_purchase: "tūemi Hokonga",
    only_first_order: "Ota Tuatahi anake",
    total_cashback: "Tapeke tūemi",
    delete_alert: {
      title: "Mukua te Kaupapa Whakahoki Utu",
      message: "Kei te mohio koe ki te whakakore i tenei kaupapa utu moni?",
      action: "Ae, Muku Inaianei",
    },
    notifications: {
      delete_success: "I tutuki pai te whakakore i te kaupapa Cashback.",
    },
  },
  /**
   * {@see BPageCashback}
   */
  shop_cashback: {
    title: "Kaupapa Whakahoki Moni",
    up_to: "ki runga ki",
    menu: {
      back: "Whakamuri tūemi",
      dashboard: "Papatohu",
      orders: "Nga ota",
      edit: "Whakatikatika",
    },
    /**
     * {@see BPageCashbackDashboard}
     */
    dashboard: {
      chart_amount_buy: {
        label: "Hoko",
        y: "Te moni hoko {currency}",
      },
      chart_amount_cashback: {
        label: "tūemi",
        y: "tūemi Te nui {currency}",
      },
      chart_used: {
        label: "Kua whakamahia",
        y: "Nga taima",
      },
    },
    /**
     * {@see BPageCashbackOrders}
     */
    orders: {
      title: "tūemi Nga ota",
      table: {
        amount_cashback: "tūemi Te nui",
        payment_status: "Te mana utu",
        date: "Rā",
      },
    },
  },

  /**
   * {@see BCashbackAdd}
   */
  cashback_edit: {
    config: {
      subtitle: "Tautuhia te ōrau chasback, te rohe me tona mana ki konei.",
    },
    percent_input: {
      title: "Te Ōrau ukauka",
      message: "Whakauruhia te paheketanga cashback mai i te 1% ki te 20%.",
    },
    currency_input: {
      message: "Ka watea te kaupapa whakahoki moni mo tenei moni.",
    },
    limit_input: {
      title: "Tepe",
      hint: "Whakauruhia te nui rawa o te hokinga moni.",
      zero_message: "tūemi 0 mō He mutunga kore tūemi.",
    },
    duration: {
      title: "Roanga Tepe",
      subtitle: "Whakaritea te roanga o te kaupapa whakahoki moni. He mea whiriwhiri.",
    },
    start_input: {
      title: "Tīmata Rā",
      placeholder: "tūemi Tīmata Rā...",
    },
    end_input: {
      title: "Whakamutunga Rā",
      placeholder: "tūemi Whakamutunga Rā...",
    },
    design: {
      subtitle:
        "Whakaritehia te taitara me te whakaahuatanga poto mo te kaupapa whakahoki moni. Ka whakaatuhia enei korero ki nga kaihoko.",
    },
    title_input: {
      title: "Taitara",
    },
    description_input: {
      title: "Whakaahuatanga",
    },
    constraints: {
      subtitle: "Whakaritehia nga here mo te kaupapa whakahoki moni.",
    },
    min_purchase_input: {
      hint: "Whakauruhia te moni hoko iti mo te kaupapa whakahoki moni.",
      title: "tūemi Hokonga",
    },
    only_first_buy_input: {
      false_desc: "Ka watea te kaupapa whakahoki moni mo nga ota katoa.",
      true_desc:
        "Ka watea te hotaka whakahoki moni mo te ota tuatahi.",
      true_title: "Ota Tuatahi anake",
      false_title: "katoa Nga ota",
    },
    qualify_input: {
      false_title: "katoa Kaihoko",
      true_title: "tūemi Kaihoko",
      true_msg:
        "Ka watea te kaupapa whakahoki moni mo nga kaihoko noa e whai tohu ana.",
    },
    club: {
      subtitle:
        "Whakaritehia te karapu mo te kaupapa whakahoki moni. Ka watea te kaupapa utu mo nga kaihoko kei roto i tenei karapu.",
    },
    cluster: {
      subtitle:
        "Ma te hono i te roopu ki te kaupapa putea, ka taea e koe te whakahaere pai ake.",
    },
    notifications: {
      add: "He pai te whakaurunga o te kaupapa Cashback.",
      edit: "I angitu te whakahou i te kaupapa Cashback.",
    },
    enable_input: {
      true_msg: "Kei te kaha te kaupapa whakahoki moni ka taea e nga kaihoko te whakamahi.",
      false_msg:
        "He hohekore te kaupapa whakahoki moni kaore e taea e nga kaihoko te whakamahi.",
    },
    boosted_input: {
      false_title: "Pūnoa",
      true_title: "Whakanuia (2x Whakahoki Moni)",
      true_msg:
        "Kei te aratau whakanui te kaupapa whakahoki moni ka whiwhi nga kaihoko ki te 2x cashback.",
      false_msg:
        "Ko te kaupapa whakahoki moni kei roto i te aratau noa, ka whiwhi nga kaihoko i te moni whakahoki.",
    },
  },

  /**
   * @see BOrderPaymentRowPayment
   */
  row_payment: {
    actions: {
      refund: {
        title: "tūemi 💳 Utu",
        subtitle: "Ka tautokohia te moni whakahoki, utu katoa ranei.",
      },
      delivery: {
        title: "Whakaū I te Tukunga",
        subtitle: "Ka taea e koe te whakaū i te tukunga mo te kaiwhakarato utu.",
      },
    },
  },

  /**
   * @see BOrderPaymentActionsRefundDialog
   */
  payment_refund_dialog: {
    title: "tūemi 💳 Utu",
    message:
      "Mo nga take haumarutanga, ko nga ota kua utua i roto i nga ra e 7 kua hipa ka tika mo nga moni whakahoki.",
    payment_amount: "Te Moni Utu",
    total_refund_amount: "Tapeke tūemi Te nui",
    can_refund: "Ka taea te whakahoki",
    refund_amount: "tūemi Te nui",
    verify: {
      title: "Manatokohia te Whakahoki",
      description:
        "Ka whakaū ahau he tika te moni whakahoki ka hiahia au ki te haere tonu.",
    },
    action: "Whakahokia Inaianei",
  },

  /**
   * @see BOrderPaymentActionsDeliveryDialog
   */
  payment_delivery_dialog: {
    title: "kiritaki Tukunga tūemi",
    message:
      "Ko etahi ratonga utu e hiahia ana kia whakamanahia kua tukuna te ota ki te kaihoko. I te nuinga o te waa ka tukuna atu e matou tenei whakapumautanga i te wa o te whakaotinga o te ota ka whiwhi matou i te whakapumautanga tuku a te kaihoko. Heoi, ki te hiahia koe ki te mahi a-ringa i tenei mahi, ka taea e koe i konei.",
    verify: {
      title: "tūemi Tukunga",
      description:
        "Ka whakapumautia e ahau kua tukuna te ota ki te kaihoko.",
    },
    action: "Whakaū Inaianei",
  },

  /**
   * @see BPageProductCrossSelling
   */
  product_cross_selling: {
    title: "Ripeka Hoko",
    add_new_actions: "tāpiri Hou Hua",
    subtitle:
      "I tenei waahanga, ka taea e koe te taapiri me te whakahaere i nga hua hoko-whiti mo tenei wharangi hua. Ka whakaatuhia enei hua hei whakaaro mo nga kaihoko ki te hoko i te taha o te taonga matua. Ka taea hoki e koe te whakarite utu hei akiaki me te whakatenatena i etahi atu hoko. Ka whakaatuhia e te punaha te nuinga o nga mea e 3 hei whakaaro ma te kaihoko.",
    dialog: {
      edit_title: "Whakatikatika hua hoko ripeka",
      add_title: "Tāpirihia nga hua hoko ripeka hou",
      target: {
        title: "tūemi Hua",
        subtitle:
          "Whiriwhiria nga hua e hiahia ana koe ki te whakatairanga hei whiringa hoko-whiti i te taha o tenei hua.",
        select_products: "tūemi Hua(tūemi)",
      },
      list: {
        title: "tūemi Hua",
        subtitle: "Kua tohua tenei hua mo te whakatairanga whakawhiti-hoko.",
        manage_product: "tūemi Hua",
      },
      discount: {
        title: "Te utu & Karere",
        subtitle:
          "Kei a koe te whiringa ki te whakauru i tetahi panui me te tuku utu utu hei akiaki i nga kaihoko ki te hoko i nga taonga taapiri kua tohua. Ka taea e enei whakatenatena te awhina i nga hoko me te whakarei ake i te wheako hokohoko katoa.",
        amount_input: {
          message:
            "Ko te paheketanga hekenga ka tukuna ki te utu hua i muri i te whai whakaaro ki nga utu whakahekenga katoa.",
        },
        mismatch_type_warning:
          "Kaore e taea te utu utu ki etahi atu momo i tua atu i nga momo hua matua, na te mea karekau te puna me nga mea i whaaia e tuu ki roto i te kete kotahi.",
        not_support_subscription_type_warning:
          "Kaore e taea te whakarite utu mo nga hua ohaurunga whakawhiti, na te mea kaore e taea te whakarereke i nga mahere utu.",
        message_input: {
          message:
            "Hangaia he taitara whakahihiri me te whakapohehe hei hopu i te aro o nga kaihoko.",
          placeholder:
            "Paihere me te Tiaki: Whakaotia to Tirohanga me ta maatau Whiriwhiringa kua tohua!",
        },
      },
      action: {
        title: "tūemi",
        subtitle:
          "Whakarerekēhia te mahi hokohoko o te hua i konei. Mena ka hiahia to hua ki te whakauru i nga korero ritenga, penei i te Puka Aromatawai, kaua e kowhiria te \"Taapiri ki te Kete\" hei mahi, na te mea ka puta he he.",
      },
    },
  },
  /**
   * @see BProductVariantsBulkAdd
   */
  product_variants_bulk_add: {
    title: "tāpiri Nga momo rereke i tūemi",
    variants: {
      title: "Kōwhiringa rerekē",
      subtitle:
        "Tīpakohia kia rua nga whiringa rereke. I muri i te kowhiringa, ka taea e koe te kowhiri i nga momo rereke ka hangaia aunoa.",
    },
    values: {
      title: "Nga uara rereke",
      subtitle: "Whakauruhia nga uara ka taea mo ia whiringa rereke ki konei.",
      prevent_duplicates_tips:
        "<b>Tuhipoka:</b> Ka tirohia aunoatia e matou nga momo rereke kia kore ai nga taarua.",
    },
    inventory: {
      title: "Tuhinga o mua",
      subtitle: "Tautuhia te tatauranga pukapuka timatanga mo nga rereke hou.",
    },
    add_variants_action: "Waihanga Nga momo rereke",
  },

  /**
   * @see CrossSellActionType
   */
  CrossSellActionType: {
    AddToCart: {
      title: "Tāpiri ki te Kete",
      description:
        "Whakaatu tika i te paatene Tāpiri ki te Kete. Kia mahara kaore tenei e tika mo nga hua ohaurunga.",
    },
    ViewProduct: {
      title: "Tirohia te Hua",
      description:
        "Whakaatuhia he paatene Tiro atu e arahi ana i nga kaihoko ki te wharangi hua ma te whakatuwhera i tetahi matapihi hou.",
    },
  },

  /**
   * @see BProductFlow
   */

  product_flow: {
    title: "Hokonga Rere",
    subtitle: "Koinei te rerenga hoko me te tirotiro hauora o to hua.",

    /**
     * @see BProductFlowRowConnect
     */
    connect: {
      subtitle: "Kua taapirihia tenei hua ma <b>{name}</b>.",
      enable_msg: "Kua whakahohea tenei piriti ki te ratonga o waho.",
      disable_msg: "Kua monoa tenei piriti ki te ratonga o waho.",
      has_shipping_msg:
        "Kei te tautoko a {name} i te tuku. Nō reira ka taea e koe te whakahohe i te tātaitanga tuku aunoa mā {name} i te whārangi utu.",
    },
    /**
     * @see BProductFlowRowHealth
     */
    health: {
      title: "Hua tūemi",
      available_in_stock_msg: "Hua to Kei te waatea i tūemi.",
      out_of_stock_msg: "Hua to tūemi o tūemi.",
      file_count_msg:
        "{files_count} nga konae me te {files_size} katoa i tukuna mo tenei hua.",
      no_file_uploaded_msg: "Kare ano he kōnae i tukuake.",
      no_image_uploaded_msg:
        "Tukuna he ahua hua kia ataahua ake.",
    },
    /**
     * @see BProductFlowErrorsVendor
     */
    vendor_errors: {
      title: "tūemi kaihoko",
      subtitle:
        "Ko tenei hua kaore he kaihoko hono. Mo tetahi hua ka taea te hoko i te kainga hokohoko, me kotahi te kaihoko.",
      manage_vendors: "tūemi kaihoko",
    },

    /**
     * @see BProductFlowErrorsFile
     */
    file_errors: {
      title: "Nga Kōnae Ngaro",
      subtitle:
        "I tenei wa, kaore he konae i tukuna mo tenei hua. Tukuna koa nga konae kia haere tonu.",
      manage_files: "Whakahaere Kōnae",
    },

    /**
     * @see BProductFlowRowPricing
     */
    pricing: {
      title: "Te utu",
      no_product_price_msg: "Ko te utu hua taunoa kaore ano kia whakatauhia!",
      listing_pricing_msg: "rārangi Te utu:",
      has_valuation_msg: "He uara te hua.",
      subscription_pricing_msg:
        "tūemi Hua tūemi {count} Ohaurunga Te utu Mahere.",
      has_no_subscription_pricing_msg:
        "tūemi Hua tūemi kahore Ohaurunga Te utu Mahere.",
    },
    /**
     * @see BProductFlowErrorsSubscription
     */
    subscription_errors: {
      title: "tūemi Ohaurunga Te utu Mahere",
      subtitle:
        "Ko tenei hua kaore he mahere utu ohaurunga. Me taapiri koe kia kotahi te iti rawa o te mahere utu ohaurunga kia watea ai tenei hua mo te hoko ohaurunga.",
      pricing_plans: "Te utu Mahere",
    },

    /**
     * @see BProductFlowRowLocation
     */
    location: {
      title: "tūemi Tauwāhi tūemi ● {status}",
      status: {
        has_restriction: "Kei te waatea i {count} tūemi",
        no_restriction: "Ehara i te here",
      },
      available_countries_msg:
        "Ko te hokohoko puta noa i te ao kua herea ki {count} whenua. Ko enei whenua ko {countries}.",
      disable_for_all_countries_msg:
        "Kua monoa te hokohoko mo nga whenua katoa i to toa> tautuhinga waahi.",
      location_is_not_in_permitted_shop_locations_msg:
        "Ka whakatauhia e koe a {country} hei waahi hoko engari i te taumata toa kaore koe e whakaae ki tenei whenua. Tena tirohia o tautuhinga taumata toa.",
      shop_locations: "Toa Nga waahi",
      product_locations: "Hua Nga waahi",
    },

    /**
     * @see BProductFlowErrorsLocation
     */
    location_errors: {
      title: "Kei te waatea Nga waahi tūemi",
      no_location_error_subtitle:
        "Kua whakahohehia e koe he here waahi mo te hoko i te hua, engari i tenei wa, karekau he whenua kei raro i tenei here. Ko te mutunga, kaore tenei hua e waatea mo te hoko e tetahi.",
      invalid_location_error_subtitle:
        "He hapa kei roto i nga tautuhinga here tauwāhi mo to hua: ko etahi waahi kua whakarārangihia kaore he waehere ZIP e hono ana. No reira, kaore e taea e nga kaiwhakamahi te kowhiri i tetahi waehere ZIP, ka kore e waatea te hua mo te hoko.",

      manage_locations: "tūemi Nga waahi",
    },

    /**
     * @see BProductFlowRowTax
     */
    tax: {
      title: "Ohaurunga Taake",
      default_shop: "Taunoa Toa Taake",
      tax_profiles: "Taake Kōtaha",
      description: {
        subscription_tax_msg:
          "Ka taea e taatau te tono taake anake i runga i te waahanga hanga utu.",
        dedicated: "tūemi",
        is_disabled: "tūemi Taake kōtaha to tūemi!",
        tax_is_based_on_location: "Taake: Tauwāhi tūemi",
        shipping_tax_is_based_on_location: "Te tuku: Tauwāhi tūemi",
        included_in_price: "tūemi i utu.",
      },
    },

    /**
     * @see BProductFlowRowVendor
     */
    vendor: {
      vendor_owner_msg: "tūemi Hua tūemi Ki <b>{vendor}</b>.",
      has_vendors_msg: "tūemi to {count} kaihoko mō tūemi Hua.",
      no_vendor_msg:
        "Kare ano koe i tautapa he kaihoko ki tenei hua, na reira kaore e taea e nga kaihoko te hoko.",
      add_vendors: "tāpiri kaihoko",
    },
  },

  /**
   * @see BPageProductTemplate
   */
  product_template: {
    title: "Hua whārangi tauira",
    subtitle:
      "Ka taea e koe te whakauru i tetahi wharangi ki roto i to wharangi hua mo te whakaaturanga whakarei ake. Ko te huarahi tino pai ko te hanga i nga wharangi e whakaatu ana i te papamuri marama me te 1 ki te 3 nga waahanga kua whakaritea mo ia waahanga hua. Na, tautapa tetahi wharangi ki nga hua maha mo te whakaatu rite tonu me te whakahihiri.",
    edit_page: "Whakatikatika whārangi",
    list_of_pages: "Rarangi o Nga wharangi",
  },
  /**
   * @see BPageProductEmbed
   */
  product_embed: {
    subtitle:
      "Ka taea e koe te whakauru ngawari i tetahi hua, rarangi hua ranei ki to rangitaki, ki etahi atu wharangi paetukutuku ma te kape me te whakapiri i nga waehere ka tukuna e matou. Ahakoa kei te timatanga tonu tenei ahuatanga, kei te whakapau kaha matou ki te whakawhānui ake i ana mahi ki tua atu i nga mea e waatea ana ki etahi atu waahi. Ahakoa he iti noa te kaha o tenei wa, kei a matou he mahere ki te whakauru i etahi atu tauira me nga whiringa whakaritenga a muri ake nei.",
    card: {
      title: "tūemi",
      description:
        "Waihangahia he waehere HTML hei taapiri i nga kaari hua ki etahi atu papaaho me nga wharangi HTML.",
    },
    iframe: {
      title: "tūemi",
      description:
        "Hangaia te waehere iframe hei whakaatu i nga taipitopito hua matawhānui ki te matapihi whakauru.",
    },
  },

  /**
   * @see BPermissionStaffAddDialog
   */
  permission_staff_add: {
    title_new: "Hou Kaimahi",
    title_edit: "Whakatikatika Kaimahi",

    staff: {
      title: "tāpiri Kaimahi & Urunga",
      subtitle:
        "Ka taea e nga kaiwhakamahi katoa e uru ana nga kaimahi ki te takiuru ki to papatohu me te kite i nga korero putea. Hei taapiri i nga kaiwhakamahi whai waahi iti, tautuhia te Turanga.",
      go_to_roles: "tūemi Ki Ngā tūranga",
    },
    access: {
      customization_tips:
        "<b>Whakaritenga:</b> Paatohia ia mea hei whakarite i te urunga panui/tuhi taunoa. Ko ia rarangi e tohu ana i te taumata uru e rite ana ki te papatohu toa matua.",
      simplify_tips:
        "<b>He mea nui!</b> Ka whakangawarihia te mana uru mai i te 300 neke atu ki raro i te 10 taumata. Ka whakaputahia nga taumata whai kiko me nga mea hou ki roto i nga tuhinga.",
      multi_permission_tips:
        "<b>Tepe ritenga?</b> Ka taea e koe te taapiri atu ki te kaiwhakamahi kotahi, no reira i muri i te taapiri i te uru tuatahi, hanga he urunga taapiri me nga hikoinga ano.",
      view_data: "Tirohanga Raraunga",
      apply_changes: "Hoatu Huringa",
    },

    inputs: {
      email: {
        placeholder: "Īmēra",
        label: "kaiwhakamahi Īmēra wāhi noho",
      },
      level: {
        label: "Urunga tūemi",
        placeholder: "tūemi he tūemi...",
      },
    },

    actions: {
      add: "tāpiri Kaimahi",
    },
    notifications: {
      add_success: {
        title: "Urunga tūemi",
        message: "Kua tutuki pai te whakaurunga hou.",
      },
      update_success: {
        title: "Urunga tūemi",
        message: "Kua pai te whakahou i nga whakaaetanga.",
      },
    },
  },

  /**
   * @see BCategoryEngineEditor
   */
  category_engine_editor: {
    title: "20% Hua rārangi",
    subtitle:
      "Whakaatuhia nga hua mai i nga momo waahanga. Kei a koe te whiringa ki te whiriwhiri i nga waahanga maha hei whakaatu i nga hua, i nga hua tātari ranei ma nga tohu.",
    aut_add_sub_categories_tips:
      "Ka taea e koe te taapiri aunoa i nga waahanga iti katoa ki te waahanga o naianei ma te paatene i te paatene kei raro.",
    inputs: {
      categories: {
        label: "Ngā Kāwai",
        messages: "Ko nga hua kei roto i enei waahanga ka whakaatuhia.",
        placeholder: "tūemi Ngā Kāwai...",
      },
      tags: {
        label: "Hua Tohutohu",
        messages: "Ko nga hua me enei tohu ka whakaatuhia.",
        placeholder: "Tohu waea ki konei ka pehi i te tomo. ex. kohinga hou",
      },
    },
    actions: {
      save_engine: "Tiaki tūemi",
      auto_add_subcategories: "tūemi tāpiri tūemi-Ngā Kāwai",
    },
  },

  /**
   * @see SProductsSortView
   */
  products_sort_view: {
    search_tips: {
      title: "rapu tūemi",
      normal:
        "<b>tūemi rapu: </b>rapu mā Taitara, MPN, SKU, Waitohu, te Tohutohu.",
      product:
        "<b>Tou ID Hua: </b>Whakamahia P+Huatanga ID ki te kimi hua motuhake, hei tauira, <b class=\"text-green\">P</b><i class=\"text-yellow\">360</i>.",
      quotes:
        "<b>Te Kïwaha Tika: </b>Whakamahia nga tohu kïanga hei rapu i te kïanga tika, hei tauira, <b class=\"text-green\">\"</b><i class=\"text-yellow\">to tuhinga ki konei</i><b class=\"text-green\">\"</b>.",
      tax: "<b>Tax Profile: </b>Rapuhia nga hua whai tohu taake motuhake ma te whakamahi \"taake:\", hei tauira, <b class=\"text-green\">tax:</b><i class=\"text-yellow\">tax ingoa kōtaha</i>.",
      new_products:
        "<b>Huatanga Hou: </b>Kimihia nga hua kua taapirihia i roto i te awhe ra motuhake, hei tauira, <b class=\"text-green\">hou~</b><i class=\"text-yellow\">2023-08-10</i><b class=\"text-green\">~</b><i class=\"text-yellow\">2023-08-20</i>.",
    },
  },

  /**
   * @see LAugmentForm
   */
  augment_form: {
    title: "tūemi",
    subtitle:
      "Whakauruhia te taviri ka hurihia me tona uara e rite ana ki te ihirangi wharangi. Kia 32 te teitei o nga taviri.",
    add_caption: "tāpiri Hou tūemi",
    add_sub_caption: "tāpiri Ritenga tūemi-Uara tūemi.",
    add_dialog: {
      title: "tūemi Whakauru Momo",
      html: {
        title: "tūemi & Html",
        subtitle: "Ko te uara o te tuemi ka taea te tohu hei tuhinga, hei HTML ranei.",
      },
      image: {
        title: "Whakaahua",
        subtitle: "Ka taea e koe te tuku pikitia.",
      },
    },
    help_dialog: {
      title: "Me pehea te whakamahi i nga ihirangi hihiri",
      how_it_works_tips:
        "I roto i nga wharangi taunga, kei a koe te ngawari ki te tohu i nga waahi hihiri mo nga tuhinga me nga whakaahua. Ka whai waahi nui enei kaipupuri waahi ki te whakarite i nga ihirangi kia rite ki te horopaki.   Ko te ahua hihiri o enei waahi ka taea e ratou te whakakii i nga uara whakanui, ka rereke pea i runga i nga ahuatanga rereke. Hei tauira, he rereke pea nga uara whakanui mo ia hua, waahanga ranei. Ko te tino painga o tenei ahuatanga ka uru mai ina he maha nga taonga kei a koe me nga huanga rereke. Ka taea e koe te whakamahi tika i enei waahi waahi hihiri ki te whakawhaiaro me te whakarei ake i te whakaaturanga o ia taonga, na reira ka hangaia he wheako ahurei me te taunekeneke ki o wharangi tauranga.",
      assign_tips:
        "Hei tautapa i nga uara hihiko ki nga whakaahua, pawhiria te paatene whangai kei te taha maui o te waahanga.",
      set_dynamic_image_tips:
        "Kei a koe inaianei te kaha ki te whakarereke i te URL ahua me te tautapa he uara hihiri.",
      enter_values_tips:
        "Mo ia hua, kei a koe te kaha ki te tautapa i nga takirua uara-matua i whakatapua.",
      result_tips:
        "Koinei te putanga whakamutunga o te wharangi, kua whakakapihia nga kaipupuri waahi ki nga uara whakanui ake.",
    },
  },

  /**
   * @see BLogisticProfileEditor
   */
  logistic_profile_editor: {
    languages: {
      title: "tūemi reo",
      subtitle:
        "Koinei te rarangi o nga reo ihirangi. Ka taea e koe te whakarite i nga ihirangi rereke mo te kotaha logistics i ia reo.",
      add_caption: "tāpiri Hou Tuhinga",
      no_more_language: "kahore Ētahi atu.. reo!",
    },
    content: {
      title: "tūemi",
      subtitle:
        "Ka taea e koe te whakarite i nga ihirangi mo te kotaha logistics i roto i nga reo maha.",

      language_input: "tūemi Tuhinga reo",
      delete_article: "Mukua Tuhinga",
      menu_tooltip: "Whakamaori aunoa / Muku tuhinga",
    },
    translate_to: "tūemi Ki",
    notifications: {
      translate: {
        message: "Tuhinga whakamāoritanga Kua oti tūemi.",
      },
      save_article: {
        message: "Kua tiakina pai te tuhinga.",
      },
      delete_article: {
        message: "Tuhinga tūemi te Kua mukua tūemi.",
      },
    },
    delete_dialog: {
      message: "Me muku tonu tenei tuhinga mo ake tonu atu?",
      action: "Ae, Muku Inaianei",
    },
  },

  /**
   * @see LogisticProfilePage_Products
   */
  logistic_profile_products: {
    title: "Hua",
    subtitle: "Rarangi o nga hua e hono ana ki tenei korero.",
  },
  /**
   * @see LogisticProfilePage_Setting
   */
  logistic_profile_setting: {
    title: "Nga korero whanui",
    inputs: {
      name: {
        message:
          "E whakaatu ana tenei ingoa ki a koe, e kore e hurahia ki te marea.",
      },
      accept_delete: {
        true_description:
          "Ma tenei mahi ka tango i nga tuhinga katoa me nga hononga hononga.",
        true_title: "Manatoko ahau ki te tango i tenei hononga.",
      },
    },
    actions: {
      remove: "Tango kōtaha & tūemi",
    },
  },

  /**
   * @see BPageShopLogisticProfiles
   */
  shop_logistic_profiles: {
    title: "tūemi Kōtaha",
    add_caption: "tāpiri Hou kōtaha",
    subtitle:
      "Ka taea e koe te hanga me te whakatika i te whakamana me te whakahoki kaupapa here mo nga hua i konei ka tautapa ki etahi hua. Ka taea e koe te tautuhi i nga kaupapa here raihana rereke mo nga momo hua rereke.",
    add_dialog: {
      title: "Waihanga Hou tūemi kōtaha",
      config: {
        title: "Nga korero whanui",
        subtitle:
          "Kōwhiria te momo kōtaha. Ka taea te waiho hei kaiarahi, whakamana, etahi atu ihirangi me nga rawa e tiritiria ana i roto i nga hua maha.",
      },
      article: {
        title: "Tuhinga",
        subtitle:
          "Ka taea e koe te taapiri atu i etahi atu reo i muri i te waihanga i tenei whaarangi korero.",
      },
      inputs: {
        name: {
          message:
            "E whakaatu ana tenei ingoa ki a koe, e kore e hurahia ki te marea.",
          placeholder: "tūemi he Ingoa mō te kōtaha...",
        },
      },
    },
  },

  /**
   * @see BPageShopLogisticIncludes
   */
  shop_logistic_includes: {
    title: "tūemi Nga taonga whakahaere",
    add_caption: "tāpiri Hou tūemi",
    subtitle:
      "I konei, ka taea e koe te whakahaere i nga taonga kei roto i o hua. Hei whakarite i te ahua kohakore mo to toa, whakamahia nga taonga me nga tauira ahua rite, penei i nga waea waea, puta noa i nga hua katoa.",
    empty_list_msg: "Rarangi o tūemi Nga taonga...",
    notifications: {
      delete: {
        message: "Kua mukua te tuemi.",
      },
    },
    delete_dialog: {
      title: "Mukua nga mea kua whakauruhia",
      message:
        "Kei te tino hiahia koe ki te muku i tenei mea? Kia mahara ka tangohia mai i nga hua katoa e pa ana!",
      action: "āe, Mukua tūemi",
    },
    actions: {
      edit_linked_page: "Whakatikatika tūemi whārangi",
    },
  },

  /**
   * @see BIncludeItemAdd
   */
  include_item_add: {
    dialog_title_edit: "Whakatikatika i nga mea kua whakauruhia",
    dialog_title_add: "Waihangahia nga mea hou kua whakauruhia",

    title: "Tuemi kei roto",
    subtitle:
      "Kōwhirihia he taitara poto, me 2 ki te 3 kupu te nuinga. Whakamahia nga whakaahua tapawha, na te mea ka whakarereke-aunoa i nga pikitia kua tukuna ki te whakatakotoranga tapawha 256x256. Mo te ahua pai puta noa i nga tauira katoa, whakamahia nga whakaahua iti me nga papamuri maamaa me te pupuri i te tauira rite puta noa i o hua.",
    inputs: {
      title: {
        placeholder: "Ko te taitara poto kua whakaatuhia ki te wharangi hua...",
      },
      code: {
        placeholder: "Waehere ahurei, ex. waea_waea_usb",
        hint: "Whakamahia tenei waehere hei whakamaarama i te tukanga rapu mo te kimi taonga.",
      },
      description: {
        placeholder: "He korero poto mo tenei mea...",
        hint: "Ka whakamahia te whakaahuatanga mo SEO ka kitea pea e nga kaiwhakamahi i runga i etahi whakatakotoranga ritenga i te wharangi hua.",
      },
      image: {
        message: "tūemi Whakaahua tūemi: 1MB",
      },
      path: {
        label: "whārangi tūemi",
      },
    },
    mode: {
      no_link: {
        title: "kahore Hononga",
      },
      external_link: {
        title: "Hononga o waho",
        description: "tūemi he Hononga Ki he tūemi url.",
        tips: "Ka taea e koe te taapiri i tetahi URL ki te taonga. Ka paatohia e nga kaiwhakamahi te mea kei te wharangi hua, ka tukuna atu ki taua hononga.",
      },
      internal_link: {
        title: "tūemi Hononga",
        description:
          "Waihangahia te wharangi hihiri ma te whakarato i nga raraunga taapiri mo tenei mea.",
        tips: "Ka taea e koe te hono i tetahi wharangi taunga me te whaarangi me te whakarite i tetahi ara ritenga me nga ihirangi hihiri mo taua mea. Ka hangaia e Seldone he hononga hihiri mo te taonga.",
      },
    },
    notifications: {
      add: {
        message: "Kua tutuki pai te taapiri i te mea kua whakauruhia.",
      },
      edit: {
        message: "Kua pai te whakahōu i te tūemi kua whakauruhia.",
      },
    },
  },
  /**
   * @see BPageShopLogisticReturns
   */
  shop_logistic_returns: {
    title: "Kua hoki mai Nga ota",
    subtitle:
      "Ka taea e o kaihoko te whakahoki mai i a raatau ota tinana, ka taea e koe te tiro i nga tono whakahoki mai i te wharangi tukatuka ota, ka uru atu ranei ki te raarangi katoa o nga ota kua whakahokia mai ki konei.",
  },
  /**
   * @see OReferralFees
   */
  referral_fees: {
    title: "tūemi",
    subtitle:
      "Ko te rarangi o nga komihana katoa kua riro mai i a koe mai i o tohutoro. Kia 30 ra te roa mo te komihana ka whakaaetia ka whakawhiti ki to putea.",
  },
  /**
   * @see OReferralWallets
   */
  referral_wallets: {
    title: "Pukoro",
    subtitle:
      "I konei ka kite koe i nga utu komihana kua riro ia koe. Ka taea e koe te whakawhiti i to utu komihana ki to putea putea, ki to putea Seldone ranei.",
    no_commission_yet: "Kare ano koe kia whiwhi komihana.",
    no_wallet: "kahore Pukoro!",

    withdraw_dialog: {
      title: "tūemi Komihana tūemi Fee",

      need_bank_info_message: "Tena koa whakauruhia nga korero putea i te tuatahi.",

      to_wallet: {
        title: "Ki Pukoro",
        subtitle:
          "Ma tenei whiringa, ka taea e koe te whakawhiti utu komihana ki to putea Seldone. Me mohio kei te hono tetahi putea ki to putea koha, me te mahi me te moni kotahi.",
      },
      to_bank: {
        title: "Ki Peeke",
        subtitle:
          "Ma tenei whiringa, ka taea e koe te whakawhiti utu komihana ki to putea Seldone. Me mohio kei te hono tetahi putea ki to putea koha, me te mahi me te moni kotahi.",
      },
      inputs: {
        verify_to_wallet: {
          true_description:
            "E hiahia ana ahau ki te whakawhiti i taku komihana ki taku putea Seldone.",
        },
        verify_to_bank: {
          true_description:
            "Ka whakamanahia e au aku korero peeke ka hiahia ahau ki te whakawhiti i taku komihana ki taku putea putea.",
        },
      },
      actions: {
        withdraw_to_wallet: "tūemi Ki Pukoro",
        withdraw_to_bank: "tūemi Ki Peeke",
      },
    },
    notifications: {
      request_transfer_to_bank_success:
        "Kua tutuki pai to tono, ka arotakehia e matou.",
    },
  },

  /**
   * @see OPageMonetizeReferral
   */
  monetize_referral: {
    copy_box: {
      message: "Ka taea e koe te taapiri <b>?ref={code}</b> ki tetahi url hoko.",
    },
    intro: {
      title: "tūemi te Selldone Hononga tūemi",
      message:
        "He maamaa, he maamaa hoki te Kaupapa Hononga a Selldone, i hangaia mo koe ki te whiwhi moni ngawari. Tohaina to hononga hono ahurei ki te tono i nga kaiwhakamahi hou ki a Selldone, ka whiwhi koe i te komihana mai i ia tauwhitinga ka mahia e ratou—mo ake tonu atu. Kei roto i tenei ko nga moni whiwhi mai i nga ohaurunga, utu ratonga, whakamohoatanga toa, me era atu moni moni katoa. Tīmatahia te whiwhi i tetahi wahanga o nga moni whiwhi kaore he herenga mo te wa, mo nga moni whiwhi ranei. Hono mai ki a maatau ka huri to whatunga ki te whiwhi moni tuturu!",
      why_join_selldone_affiliate: "He aha tenei he waahi ahurei?",
      reasons: [
        "Ka mau tonu mo ake tonu atu!",
        "Ka kapi katoa nga utu ohaurunga me nga utu tauwhitinga. Ko nga utu katoa a te kaiwhakamahi i runga i a Selldone.",
        "Ka tukuna he putea $99 kore utu ki te tangata ka rehita ma te whakamahi i to hononga.",
        "Ka piki ake to taumata, ka whiwhi koe i te reiti hou mai i nga kaiwhakamahi katoa i tukuna i mua.",
      ],
    },
  },

  /**
   * @see LMenuLeftAssets
   */
  l_menu_left_assets: {
    no_asset_uploaded: "Karekau he rawa i tukuna ake.",
  },

  /**
   * @see LMenuTopHome
   */
  l_menu_top_home: {
    hot_keys: "tūemi",
    click_to_view_all: "tūemi Ki Tirohanga katoa...",
    clone_tooltip: {
      title: "tūemi Kāhua",
      subtitle:
        "Whakamahia tenei taputapu ki te tarai i nga momo penei i nga momotuhi, papamuri, tawhē me nga taitapa. A pee i enei kaupae hei taarua i nga ahua:",
      step_1:
        "Whakahohehia te taputapu ka whakaparohia to kiore ki runga i te kuputuhi, nga pou, nga whakaahua ranei; ka huri te pehu ki te pipette.",
      step_2: "Patohia te huānga e hiahia ana koe ki te kape.",
      step_3:
        "Ka huri te pehu ki te papatae; paatohia te huānga e hiahia ana koe ki te tono i te ahua kua kapea.",
    },
    show_side_menu_tooltip: {
      subtitle:
        "Takahurihia te tirohanga o te tahua taha, kei roto nga waahanga waahanga katoa mo te hanga i to wharangi.",
    },
    sort_tooltip: {
      subtitle:
        "Whakahohehia tenei waahanga kia ngawari te whakariterite me te whakarite i nga waahanga ma te toia.",
    },
    edit_mode_tooltip: {
      title: "Whakatikatika / Arokite aratau",
      edit_mode_title: "Whakatikatika aratau",
      edit_mode_msg: "Ka whakaatuhia nga taputapu whakatika katoa.",
      view_mode_title: "Arokite aratau",
      view_mode_msg:
        "Tirohia to wharangi ka kitea e nga kaiwhakamahi ma te huna i etahi atu taputapu whakatika me nga waahi kuputuhi kore.",
    },
    repository_tooltip: {
      title: "Nga Waahanga Hanga",
      subtitle:
        "Takahurihia te whakaaturanga o nga waahanga i hangaia i mua. Ka hangaia enei waahanga ma te whakamahi i nga waahanga paerewa e waatea ana i te tahua maui, e whakaatu ana i te ngawari o tenei kaihanga wharangi.",
    },
    live_view: {
      title: "Ora Arokite",
      subtitle: "Tirohia nga whakahōutanga i te wa-tūturu i a koe e whakatika ana i to wharangi.",
    },
    hotkeys_dialog: {
      title: "Pokatata Papapātuhi",
      augment_tips:
        "Ka taea e koe te uta i nga ihirangi hihiri ki runga i to wharangi ka whakakapi ki nga uara <b>augment</b>. Ka whakamahia te whakanui mo nga wharangi hihiko ritenga penei i <b>Huatanga</b>, <b>Categories</b>, me <b>Include Items</b>. Hei tautuhi i tetahi atahanga ki te uta tere, pawhiria te <b>Feeder</b> ka kowhiria te <b>Pūtake Hihiko</b> mo taua atahanga.",
    },
    hotkeys: {
      ctrl_b: "Takahuri maia ki runga/whakaweto mo te kuputuhi kua tohua | <b>Kia maia ahau</b>",
      ctrl_i: "Takahurihia te whakatitaha ki runga/whakaweto mo te kuputuhi kua tohua | <i>Whakairihia ahau</i>",
      ctrl_l:
        "Takahurihia te whiu ki runga/whakaweto mo te kuputuhi kua tohua | <s>Pahia ahau</s>",
      ctrl_u: "Takahurihia te raina raro ki runga/whakaweto mo te kuputuhi kua tohua | <u>Tararo ahau</u>",
      ctrl_1:
        "Whakaitihia te rahi momotuhi o te kuputuhi kua tohua | <small>Whakaitihia ahau</small>",
      ctrl_2: "Whakanuia te rahi momotuhi o te kuputuhi kua tohua | <big>Whakanuia ahau</big>",
      ctrl_r: "Tango whakahōputu katoa mai i te kuputuhi kua tīpakohia | Horoia nga ahua katoa",
      ctrl_s: "Tiaki te whārangi",
      tab: "Hurihia i waenga i nga momo tauine",
      ctrl_e: "Whakahohe, whakawetohia ranei te aratau kāhua clone",
    },
  },

  /**
   * @see LPageEditorRepository
   */
  l_page_editor_repository: {
    title: "Toia & Whakataka",
    subtitle: "Nga waahanga kua oti te hanga",
    add_dialog: {
      title: "tāpiri Ritenga tūemi",
      configuration: {
        title: "Tautuhinga",
        subtitle:
          "Ka taea e koe te tiaki i nga waahanga kua hoahoatia hei whakamahi a muri ake nei ki o hoahoa wharangi. Ko enei waahanga kua tiakina ka uru ki nga kaiwhakahaere katoa o tenei toa.",
      },
      image: {
        title: "Whakaahua",
        subtitle: "Tūmatanui Whakaahua tūemi i te Rarangi.",
      },
      code: {
        title: "Waehere",
        subtitle:
          "Ka taea e koe te kape me te whakapiri i te waehere huānga mai i nga waahanga taha maui o te kaihanga wharangi.",
      },
      inputs: {
        title: {
          label: "Taitara",
          message: "tūemi to te Tūmatanui Taitara.",
        },
        tags: {
          label: "Tohutohu",
          message: "Ka whakamahia mo te rapu me te whakarōpū i nga huānga.",
        },
        section: {
          message: "Tāruatia te wāhanga mai i te kaihanga whārangi.",
        },
      },
    },
  },

  /**
   * @see BPageMarketplaceVendors
   */
  marketplace_vendors: {
    title: "kaihoko Rarangi",
    subtitle:
      "Tāpirihia nga kaihoko ki konei, ka tohua ki nga hua kei te Papatohu Hua > Kaihoko.",
    add_new_vendor_action: "tāpiri Hou kaihoko",
    status_tooltip: {
      title: "kaihoko Tūnga",
      ACCEPTED: "Ka taea e te kaihoko te uru atu ki o raatau roopu.",
      REJECTED: "Kua whakakorehia te tono kaihoko.",
      PENDING: "te tūemi to E tatari ana he tūemi Mai te kaihoko.",
    },
    number_of_products: "tūemi o Hua",
    invited: "tūemi",
    no_pricing: "kahore Te utu",
    no_pricing_tooltip: {
      title: "Taunoa Te utu Mahere",
      subtitle: "Whakatakotoria he tauira utu taunoa mo te kaihoko.",
    },
    updated_products_tooltip: {
      title: "Ko nga hua kua whakahoutia i roto i nga haora 24 kua hipa",
    },
    added_products_tooltip: {
      title: "Nga hua kua taapirihia i roto i nga haora 24 kua hipa",
    },
    access_tooltip: {
      title: "tūemi Urunga",
      subtitle:
        "Ka taea e te kaihoko te uru atu ki o raatau roopu. Ka taea e koe te whakatika i tenei i te Kaihoko> Ripa Uru.",
    },
    reject_by_user_tooltip: {
      title: "Kua whakakorehia mā kaiwhakamahi",
      subtitle:
        "I whakakorehia e te kaiwhakamahi te tono kia noho hei kaihoko me te uru atu ki tenei kaihoko.",
    },
    download_all_vendors: "tikiake katoa kaihoko",
    bulk_import: "Kawemai nui (Excel)",
  },

  /**
   * @see BShopLicenseNeedUpdateOverlay
   */
  shop_license_need_update_overlay: {
    title: "Me Whakahou Raihana!",
    subtitle:
      "Ko to raihana o naianei kaore e tutuki i nga whakaritenga o to tauira pakihi kua tohua. Whakahoutia to raihana ki te mahere Hinonga.",
  },

  /**
   * {@see BOrderChatBox}
   */
  b_order_chat_box: {
    title: "tūemi Nga Mahi",
    subtitle:
      "Ka taea e koe te tuku imeera whakahou mana ki te kaihoko me te paato noa. I tua atu, ka taea e koe te tautuhi i nga karere noa i konei hei whakamahi ano, no reira kaore koe e tuhi ano i ia wa.",
    send_update_email_action: "Tukua Whakahou Īmēra",
    add_template_action: "tāpiri Karere tauira",

    notifications: {
      add_chat_success_message: "Kua pai te taapiri i te karere.",
    },

    send_update_email: {
      title: "🛍️ Ota Whakahou Īmēra",
      need_set_service_alert: {
        message:
          "Whakaritea to kaiwhakarato ratonga imeera kia tukuna te imeera.",
        action: "tūemi Ki Tautuhinga",
      },
      send_action: "Tukua Whakahou Īmēra",

      notifications: {
        send_success_message: "Īmēra tūemi te Tukuna tūemi.",
      },
    },
    add_template: {
      edit_title: "Whakatikatika Karere tauira",
      add_title: "tāpiri Karere tauira",
      message:
        "Me whakarite e koe he taitara me te tinana karere ki konei. Ko te taitara anake ka whakamahia mo to tohutoro kaore e tukuna ki te kaihoko.",

      save_action: "Tiaki tauira",
      add_action: "tāpiri tauira",

      notifications: {
        save_success_message: "Kua tiakina pai te tauira.",
      },

      inputs: {
        title: {
          label: "tauira Taitara",
          placeholder: "tūemi he tūemi Taitara...",
        },
        body: {
          label: "tauira Karere",
          placeholder: "tūemi to Karere tūemi...",
          messages:
            "Ka taea e koe te whakarite uara hihiri ma {order_id} me {buyer_name}.",
        },
      },
    },
  },

  /**
   * {@see BOrderShare}
   */
  b_order_share: {
    title: "🛍️ Ota Hononga",
    subtitle: "Ka taea e koe te tiri i te hononga ota ki te kaihoko.",
    guest_checkout_message:
      "Ka taea e nga kaihoko manuhiri te tiro aunoa i a raatau ota i roto i te tirotiro kotahi.Heoi, mena he manuhiri te kaihoko ka hiahia koe ki te tiri i nga korero ota, ka taea e koe te hanga hononga haumaru mo ratou.",
    authentication: "tūemi",
    guest: "tūemi",
    click_to_create: "tūemi Ki Waihanga...",
    valid_for_30_days: "tūemi mō 30 30 ra",
    notifications: {
      secure_link_created: "Kua oti te hanga hononga haumaru.",
    },
  },

  /**
   * {@see BPageShopCrmReviews}
   */
  shop_reviews: {
    title: "Nga arotake tūemi",
    add_source_action: "tāpiri Arotake tūemi",
    subtitle:
      "Me ngawari te hono atu i nga papaahi o waho ki te kohikohi me te tātari i nga arotake mo to pakihi mai i nga puna penei i a Facebook, Google, me etahi atu. Ka whakataurite aunoa i nga kaihoko ki o raatau arotake mo te pai ake o te wheako kaihoko. Ka taea hoki e koe te whakaatu i enei arotake ki o wharangi taunga me o wharangi hua.",
  },

  /**
   * {@see BPageShopLogisticPropertySets}
   */
  shop_logistic_property_set: {
    add_caption: "Tāpirihia he huinga taonga hou",
    title: "Nga huinga taonga",
    subtitle:
      "Ka taea e koe te hanga me te whakahaere i nga huinga taonga i konei. Ka whakamahia nga huinga rawa ki te tautuhi i nga ahuatanga o nga hua me nga ratonga. Ka taea e koe te tautapa i enei huinga taonga ki nga hua kei te Papatohu Hua > Whakatika.",
  },

  /**
   * {@see BProductProfilePropertySet}
   */
  product_property_set: {
    profile: {
      title: "Tautuhinga Taonga - Nga rereke Ritenga",
      subtitle:
        "Ka taea e koe te whakarite i nga ingoa rereke mo tenei hua, ka whakakapi i nga ingoa taunoa.",
    },

    subtitle:
      "Ka taea e koe te whakarite i nga ingoa rereke, tohu, me o raatau ota i konei. Mo nga hua me nga momo rereke ka puta ki tua atu i te ono taunoa, ka taea e koe te whakarite i nga momo rereke kua whakaritea mo ia hua. Me tautuhi noa i nga huinga taonga kei te ripa Logistic> Tautuhinga Taonga ka tautapa ki etahi maha o nga hua ka hiahiatia.",
    select_property_set: "Tīpakohia te Tautuhinga Taonga",
    add_property_set: "Tāpiri Tautuhinga Taonga",
  },

  /** {@see BPageShopLogisticInventory} **/
  transportation_boxes: {
    title: "tūemi",
    add_action: "tāpiri tūemi",
    subtitle:
      "Waihangahia nga rahi pouaka ka taea te whakamahi kia tere ake te hoko tapanga me te whakarite i nga reiti kaipuke tika. Ina whakatutukihia he ota, tohua he pouaka kua tiakina, ka whakakiia aunoa ona rahi.",
    empty: "Kaore ano he pouaka. Paatohia te \"Taapiri pouaka\".",
    edit: "Whakatikatika tūemi",
    new: "Hou tūemi",
    default_weight: "Taunoa Taumaha",
    updated: "Pouaka kua whakahoutia.",
    created: "Pouaka hanga.",
    deleted: "tūemi Kua mukua.",
  },

  /** {@see BBoxInput} **/
  transportation_boxes_input: {
    select_box: "tūemi he tūemi",
    define_package: "tūemi he tūemi",
    define_package_hint: "Waihangahia, whakatikahia ranei o rahinga kua tautuhia e koe",
  },























  /** {@see BPageShopListing} **/
  listing:{
    name:"rārangi",
    message:"Waihangahia he whaiaronga rapu, whakaōrite mō ngā momo rārangi—taonga, hōtera, tākuta, ratonga, aha atu."
  },

  shop_listing: {
    name: "rārangi",
    message:
      "Waihangahia he whaiaronga rapu, whakaōrite mō ngā momo rārangi—taonga, hōtera, tākuta, ratonga, aha atu.",

    items: {
      title: "Nga taonga",
      sub_title:
        "Waihanga, whakatika, whakahaere i nga mea whakararangi. Mana mana, utu, korero whakapā, me te pāpāho.",

      empty_title: "kahore Nga taonga tūemi",
      empty_subtitle:
        "Waihangahia to rarangi ingoa tuatahi hei timata ki te hanga i to raarangi.",

      add_title: "tāpiri tūemi",
      edit_title: "Whakatikatika tūemi",

      price_to: "utu Ki",
      price_unit: "utu Waeine",
      is_featured: "tūemi",

      meta_json: "Meta JSON",
      links_json: "Hononga JSON",
      opening_hours_json: "Nga haora whakatuwhera JSON",

      delete_title: "Mukua tūemi",
      delete_message: "Kei te tino hiahia koe ki te muku i tenei mea?",
      delete_success: "I tutuki pai te mukunga.",
      restore_success: "I momoho te whakaora i te taonga.",

      opening_hours:"Nga haora whakatuwhera",
      form_fields:"puka tūemi",

      company: "Kamupene",
      badges: "tūemi",

    },

    categories: {
      title: "Ngā Kāwai",
      sub_title:
        "Whakaritehia to whaiaronga me nga waahanga me nga waahanga iti. Ka pai ake te whakatere me te tātari.",

      empty_title: "kahore Ngā Kāwai tūemi",
      empty_subtitle:
        "Waihangahia nga waahanga hei hanga i to raarangi me te whakapai ake i te tirotiro.",

      add_title: "Tāpiri Kāwai",
      edit_title: "Whakatika kāwai",

      parent_filter: "Tātari mā tūemi",
      parent: "Te waahanga matua",

      price_format: "utu tūemi",
      icon: "Tohu",
      design_id: "Design ID",

      delete_title: "Muku kāwai",
      delete_message: "Kei te tino hiahia koe ki te muku i tenei waahanga?",
      delete_success: "Kāwai Kua mukua tūemi.",
      form_schema: "Kāwai puka tūemi",
    },

    options: {
      submission_title: "tūemi",
      submission_enabled: "Whakahohe tūemi",

      moderation_title: "tūemi",
      moderation_mode: "tūemi aratau",
      require_first_approved_before_more: "Me whakaae tuatahi i mua i etahi atu",

      limits_title: "Nga rohe",
      max_pending_per_user: "tūemi Tarewa ana tūemi kaiwhakamahi",
      max_items_per_user: "tūemi Nga taonga tūemi kaiwhakamahi",
      limits_hint: "Nga rohe tūemi",

      templates_title: "tūemi",
      card_template: "Kaari tauira",
      details_template: "Nga korero tauira",

      form_schema_title: "puka tūemi",
      settings_title: "20% Tautuhinga"
    },

    supervision: {
      title: "tūemi",
      sub_title:
        "Arotakehia nga mea kua tukuna me te whakatau he aha nga mea ka whai ake: whakaae, whakaputa, paopao, whakapuranga, tautuhi ranei ki te tatari.",

      empty_title: "kahore Nga taonga Ki Arotake",
      empty_subtitle:
        "Ka tukuna e nga kaiwhakamahi nga mea hou, ka puta mai ki konei mo te whakatikatika.",

      review_title: "Arotake tūemi",
      approve_publish: "tūemi & Whakaputa",

      submitted_at: "tūemi",
      quick_actions: "tūemi Nga Mahi",

      open_review: "tūemi Arotake",
      reset_to_pending: "tūemi Ki Tarewa ana",
      items_title: "Nga taonga",

    },

    moderation: {
      approve_success: "I tutuki pai te taonga.",
      publish_success: "tūemi Whakaputaina tūemi.",
      unpublish_success: "Kua oti te whakaputanga o te taonga.",
      archive_success: "I momoho te purangatanga o nga taonga.",
      reject_success: "tūemi Kua whakakorehia tūemi.",
      penalty_success: "I tutuki pai te whiunga.",

      reject_title: "Whakakahore tūemi",
      reject_reason: "Te take whakakore",

      penalty: "tūemi",
      penalty_title: "tūemi",
      penalty_points: "Nga tohu whiu",
      penalty_note: "tūemi Tuhipoka"
    },

    media: {
      upload_images: "tukuake Whakaahua",
      url_type: "Momo",
      url: "URL",
      is_main: "Whakaritea hei matua"
    },






    /** {@see BShopListingDesignsList, BShopListingDesignUpsert} **/
    designs: {
      title: "tūemi",
      sub_title:
        "Waihangahia nga huinga hoahoa maha (kaari + tauira tauira) ka tohua ki nga waahanga. Ka taea e nga waahanga te whakamahi i nga hoahoa rereke.",

      empty_title: "Kaore ano kia hoahoa",
      empty_subtitle:
        "Tāpirihia to huinga hoahoa tuatahi ki te tiimata ki te whakarite i te ahua o nga waahanga.",

      add_title: "tāpiri tūemi",
      edit_title: "Whakatikatika tūemi",

      card_template: "Kaari tauira",
      details_template: "Nga korero tauira",

      notifications: {
        add_success: "I momoho te waihanga hoahoa.",
        edit_success: "I pai te whakahou i te hoahoa.",
        delete_success: "Design Kua mukua tūemi.",
      },

      delete_title: "Mukua tūemi",
      delete_message:
        "Mukua tenei hoahoa? Ko nga waahanga e whakamahi ana ka ngaro aunoa design_id.",
    },





    badges: {
      title: "tūemi",
      sub_title:
        "Waihanga me te whakahaere tohu toa toa. Ka taea te tohu tohu ki te whakararangi i nga mea hei whakanui i nga huanga motuhake.",

      empty_title: "Kaore ano he tohu",
      empty_subtitle:
        "Tāpirihia to tohu tuatahi ki te tiimata ki te miramira i nga mea whakarārangi.",

      add_title: "tāpiri tūemi",
      edit_title: "Whakatikatika tūemi",

      notifications: {
        add_success: "I momoho te hanga tohu.",
        edit_success: "I momoho te whakahou o te tohu.",
        delete_success: "tūemi Kua mukua tūemi.",
      },

      delete_title: "Mukua tūemi",
      delete_message: "Kei te tino hiahia koe ki te muku i tenei tohu?",
    },

    companies: {
      title: "Kamupene",
      sub_title:
        "Waihanga me te whakahaere kamupene. Ka taea te hono atu ia mea rarangi ingoa ki tetahi kamupene.",

      empty_title: "kahore Kamupene tūemi",
      empty_subtitle:
        "tāpiri to tūemi Kamupene Ki Hononga tūemi Ki rārangi Nga taonga.",

      add_title: "tāpiri Kamupene",
      edit_title: "Whakatika kamupene",

      notifications: {
        add_success: "I angitu te hanga a te kamupene.",
        edit_success: "I angitu te whakahou a te kamupene.",
        delete_success: "Kamupene Kua mukua tūemi.",
      },

      delete_title: "Mukua Kamupene",
      delete_message:
        "Mukua tenei kamupene? Ko nga taonga e whakamahi ana ka ngaro aunoa i te hononga kamupene.",
    },



    messages: {
      title: "tūemi",
      sub_title: "Whakahaerehia nga karere i tukuna mai e nga kaiwhakamahi mai i nga raarangi korero.",
      empty_title: "kahore tūemi",
      empty_subtitle: "Ka puta nga karere ka tukuna e nga kaiwhakamahi ki konei.",

      sender: "tūemi",
      item: "tūemi",
      has_reply: "tūemi",
      has_reply_yes: "He whakautu",
      has_reply_no: "kahore tūemi",

      dialog_title: "Karere",
      owner_reply: "Rangatira tūemi",
      reply: "tūemi",
      reply_hint: "Tuhia he whakautu ki te kaiwhakamahi. Ka taea hoki te tohu i te karere hei panui/spam mai i nga mahi.",

      mark_read: "Tohua kua panuitia",
      mark_unread: "Tohua kaore ano kia panuitia",
      archive: "tūemi",
      unarchive: "tūemi",
      mark_spam: "Tohua hei spam",
      mark_not_spam: "Ehara i te pāme",

      delete_confirm: "Mukua tūemi Karere?",
    },

    enable: {
      title: "rārangi",
      subtitle: "Whakahohe, monohia ranei to wharangi rarangi ingoa whanui. Ina whakahohea mo te wa tuatahi, ka waihanga aunoa a Selldone i te whirihoranga raarangi.",
      true_description: "Kua whakahohea te raarangi. Ka taea e nga kaihoko te tirotiro i nga rarangi raarangi.",
      false_description: "Kua monoa te rarangi ingoa. Kua aukatihia nga rarangi rarangi me nga tukunga.",
      open_listing: "tūemi rārangi",
      note: "Ka taea e koe te whirihora i te whakaōrite, ngā tepe, ngā puka, me te hoahoa i roto i ngā tautuhinga Rārangi.",
      vendor_prereq_text:
        "Kei te pirangi nga kaihoko ki te taapiri i nga raarangi taonga mai i a raatau Paewhiri Kaihoko? Tuatahi, whakawhitihia to tauira pakihi ki te Marketplace.",
      vendor_prereq_button: "Whakaritea te tauira pakihi mo te maakete",

      vendor_access_text:
        "Inaianei ka taea e koe te whakatau mena ka taea e nga kaihoko te uru ki nga ahuatanga Rarangi (tuemi + kamupene). Whakahohea ki nga tautuhinga Marketplace.",
      vendor_access_button: "Whakahohe rārangi mō kaihoko",

      more_options_text:
        "Kei te waatea etahi atu mana whakahaere i roto i nga tautuhinga Rarangi. Ka taea e koe te whirihora i nga whakaurunga, UI/ahua, whakaōrite, rohe, me nga whiringa moni.",
      more_options_button: "tūemi Ki rārangi Tautuhinga",

      updated: "rārangi tūemi.",
      update_failed: "tūemi Ki Whakahou rārangi.",
    },


    search: {
      title: "rapu Tautuhinga",
      subtitle: "Hoahoa pehea te rapu a nga kaihoko i to raarangi. Whakahohehia te rapu i runga i te waahi me te hanga i nga whiriwhiringa mai i o mara puka ritenga.",

      location_title: "Tauwāhi rapu",
      location_subtitle: "Tukua nga kaihoko ki te kowhiri i tetahi waahi ka kitea nga hua tata i roto i te radius whirihora.",
      location_enabled_true: "Kua whakahohehia te tātari tauwāhi. Ka taea e nga kaihoko te rapu ma te tawhiti.",
      location_enabled_false: "Kua monoa te tātari tauwāhi. Ko nga hua e kore e tātarihia e te tawhiti.",

      radius_default: "Taunoa Rorohiko (tūemi)",
      radius_default_hint: "Ka whakamahia ina kowhiria e te kaihoko tetahi waahi engari kaore e kowhiri i te radius.",
      radius_max: "tūemi Rorohiko (tūemi)",
      radius_max_hint: "Ka rawhihia te radius o mua o te toa ki tenei uara.",
      radius_options: "Rorohiko tūemi (tūemi)",
      radius_options_hint: "Ka taea te whakaatu i enei uara hei whiringa tere i roto i te UI storefront.",

      filters_title: "rapu Tātari",
      filters_subtitle: "Taapirihia nga whiriwhiringa e panui ana i nga uara mai i nga waahanga meta tuemi (to rarangi rarangi / waahanga waahanga). Whakamahia te Hanga-Aunoa ki te kohikohi i nga whiringa tuuturu mai i nga mea kua whakaputaina.",
      no_filters: "Kaore ano kia taapirihia he tātari.",
      auto_generate: "Kōwhiringa hanga-aunoa",

      filter_field: "Kī mara",
      filter_label: "Tapanga",
      filter_type: "Momo",
      filter_options: "tūemi",
      filter_options_hint: "Mo nga whiriwhiringa whiriwhiri, ka taea te mahi a-ringa, ka hangaia ranei.",
      multiple: "tūemi",
      single: "tūemi",
      kv_mode: "tūemi/Uara aratau",

      requires_listing_active: "Kaore i te hohe te raarangi. Whakahohehia te Rarangi i te tuatahi ki te whakamahi i runga i te toa.",


    },


  },















  /**
   * Auto fill suggestions
   */
  suggestions: {
    /**
     * Vendor Payment
     */
    vendor_payment: {
      note: [
        "Kua tutuki nga utu mo nga hoko",
        "Te Whakawhiwhinga Komihana Kaihoko.",
        "Kua Oti Momo te Whakawhitiwhiti.",
        "Remittance mo nga hoko tata nei.",
        "Whakawhiti Angitu mo nga Ratonga Kaihoko.",
      ],
      /**
       * Reverse fund
       */
      reverse_fund_reason: [
        "Whakahoki Moni: Whakawhiti mai i te peeke ki te putea a te kaihoko.",
        "Whakakii Pukoro Kaihoko: Whakahokia nga moni mai i te putea putea.",
        "Tauhokohoko Whakamuri: Peeke ki te kaihoko te whakawhiti putea putea.",
        "Whakahoki: Whakahokia te putea putea ki te putea a te kaihoko.",
        "Whakamuri Putea: Te whakakii i te putea a te kaihoko mai i te peeke.",
      ],
    },

    /**
     * Email Marketing
     */
    shop_email: {
      name: [
        "Te Huka Hupe Hupepa Tiaki Nui",
        "Kua hurahia nga utu o roto",
        "To urunga VIP ki nga utu utu utu",
        "Te Hoko Whakanuia o te Tau",
        "Wewetehia to Pararaiha Hoko",
        "Ko te Carnival a te Kaihoko Nui",
        "Spring Fling Shopping Spree",
        "Nga Mahi Ngahuru E kore e taea e koe te Atete",
        "Nga mea whakamiharo o te takurua: nga utu mahana mo nga ra makariri",
        "Tatauria Whakamutunga o te Raumati",
        "Black Friday Bonanza: Nga Manu Moata",
        "Kia Ora Hararei: To Aratohu Koha me etahi atu",
        "Nga Kitea Hou: Nga Taunga Hou Mau anake",
        "Ka Utuhia te Whakapono: Nga utu motuhake o roto",
      ],
      subject: [
        "⏰ Wā iti te tuku! Hopukia Ō Makau Ināianei",
        "🌼 Te Wa Hou, Nga Kaahua Hou - Tirotiro To Tatou Kohinga Hou",
        "👑 Hoko VIP Motuhake Mau anake",
        "💃 Huri Nga Mahunga me o Tatou Taunga Hou",
        "🔚 Waahanga Whakamutunga ki te pai ki te 20% te utu mo nga mea katoa",
        "🎁 He Koha Mau: $10 Whiwhinga Kei Roto!",
        "☀️ Kia Riri mo te Raumati me a maatau utu wera",
        "🛍️ Whakapai ake i to kakahu - Ka tika koe",
        "👀 Sneak Peek: Hua Hou Ka Tata Mai!",
        "🎂 Kia ora whanau! Kei te tatari to koha motuhake",
        "🖤 Kua Riri koe mo nga Whakaaetanga Paraire Pango?",
        "🎯 Whiriwhiringa Runga Mau anake - He wheako hokohoko Whaiaro",
        "🍹 Te wa ki te atawhai i a koe ano - Hoko Kohiko i te wiki!",
        "🧹 Te Whakamutunga o te Wa Whakakore - Me haere nga mea katoa!",
        "🚀 Kua Whiwhi koe! Kore utu mo to Ota Panuku",
        "📢 Kaua e ngaro - Kei te hokona nga taonga o to raarangi hiahia inaianei",
        "🏡 Kawea mai te Harikoa ki To Kainga me Ta matou Kohinga Whakapaipai Whare",
        "🎉 He Ohorere Mou anake! Wewetehia to utunga ngaro",
      ],
    },

    /**
     * Discount code
     */
    discount_code: {
      code: [
        "Nau Mai10",
        "RAUMA20",
        "tūemi30 IP",
        "tūemi15",
        "tūemi ID",
        "tūemi",
        "WHAKATOKANGA KOREUTU IP",
        "HOKO KORERO",
        "WHAKAWAHI",
        "tūemi IR",
        "ĀKONGA10 DE",
        "TE HOKI25",
        "KOANGA15",
        "HUNGA20",
        "Kaihoko pai10",
        "HAERE30 ID",
        "tūemi20",
        "NGA WAEHA",
        "NGA PAPA",
        "WHAKAMUTU",
      ],
      title: [
        "Hoko Kohiko: 20% Whakatau!",
        "He utu mo te urunga moata VIP",
        "Te Mahi mo te wiki: Tiaki Inaianei!",
        "Kia pai te 15% Off i tenei ra",
        "Te Hokonga Raumati: He 10% Whakatau",
        "Hoko Whakakore! Tiaki Nui",
        "Tuku Motuhake: Kore utu",
        "2-mo-1: I tenei ra anake!",
        "Hararei Motuhake: 25% Off",
        "He utu 30% motuhake",
        "Ohorere! 20% Whakatau",
        "Nga Tiaki Waa Ki Roto",
        "Porangi Wawaenga: 10% Whakatau",
        "Ka Mutu Ake: 15% Whakahekea Nga Katoa",
        "Whānau & Hoa: 20% Off",
        "Nga Tiaki Nui I tenei wiki",
        "Paraire Pango: 40% Whakatau",
        "Hekenga Tirohanga: 30% Whakahekea",
      ],
      description: [
        "Tonoa to tuku motuhake! He wa iti noa.",
        "Kia tere! He penapena motuhake kei roto i to kaata.",
        "He paato noa te penapena moni!",
        "Whakamahia tenei mahi i mua i te ngaro.",
        "He utu motuhake e tatari ana! Kaua e ngaro.",
        "Whakanuia to wairua hokohoko me ta maatau tuku!",
        "Ko to ota ka whai ake ka iti ake i to whakaaro!",
        "Koinei te wa tino pai ki te hopu i taua mea i hiahia koe.",
        "He mea iti kia marama ai to ra.",
        "Kua reri ki te penapena? He hekenga ohorere kei te tatari!",
        "Na te mea he mea motuhake koe, he utu motuhake tenei.",
        "Kua whai kanohi koe ki tetahi mea? Anei he nuku!",
        "Karekau he wa pera i tenei wa hei whakaora.",
        "Kaua e ngaro tenei mahi! He pai rawa kia pahemo.",
        "He rongo pai! Kua iti ake to hokohoko.",
        "Kia tere, ka mutu to utu utu motuhake!",
        "Anei he mihi iti ki a koe mo to pono.",
      ],
    },
    /**
     * Coupon
     */
    coupon: {
      title: [
        "Nga Tiaki Kua Whakarewa",
        "Wewete i te hekenga",
        "Super Tiaki",
        "Whakaaetanga Motuhake",
        "Tiaki Takoha",
        "Hekenga VIP",
        "Tuku Motuhake",
        "Tiaki Apiti",
        "Matohi utu",
        "Putea Pai",
        "Ohanga Delight",
        "Uara Bonanza",
        "Tapahia te Utu",
        "Ka hari te penapena",
        "Tiaki Atu, Hoko Atu",
        "Pakihi Flash",
        "Hokohoko Bounty",
        "Tiaki Inamata",
        "Whakaaetanga o te Ra",
        "Tahae kaupeka",
      ],
      description: [
        "Kia pai te mahi reka ki a matou! Tiaki mo to hokonga e whai ake nei.",
        "Ka whakanuia e matou o taatau kaihoko - tangohia tenei tuku motuhake.",
        "Kia iti ake nga mea e arohaina ana e koe. He utu ki a matou!",
        "Kua reri ki te penapena? He tuku motuhake kei te tatari!",
        "No te mea he mea motuhake koe! Kia pai te utu mo to ota ka whai ake.",
        "Mahia koe ki te hokohoko hokohoko ka penapena!",
        "Kia rite ki te hokohoko kia taka ra ano koe - ka penapena!",
        "Whakanuia te wa me te utu motuhake.",
        "Kei te mihi matou ki o taatau kaihoko pono me te utu utu motuhake.",
        "Wewetehia nga penapena miiharo mo to hokonga ka whai ake.",
        "Tirohia o moni penapena motuhake.",
        "Tirohia to utu ohorere ki a matou!",
        "Whakapai ake i to wheako hokohoko me ta maatau tuku motuhake.",
        "Timatahia to penapena me ta maatau tuku motuhake.",
        "Whakanuia koe ki tetahi mea iti - he utu motuhake.",
        "Ko to haerenga hokohoko e whai ake nei ka nui ake te utu!",
        "No te mea he pai ake te hokohoko me te utu utu.",
      ],
    },

    /**
     * Offer
     */
    offer: {
      title: [
        "Paihere & Tiaki Nui!",
        "Hokona Atu, Tiaki Atu!",
        "Nga Mahi Ataata mo nga Kaihokohoko Ataata",
        "Matohi utu utu! Paihere Tiaki",
        "Tuku Paheko - Ake mo te Iti",
        "Tiaki Motuhake mo nga Paihere",
        "Tikina atu mo to Moni!",
        "Tiaki Ina Paihere koe",
        "Nga Moni Motuhake mo nga Hiko",
        "He penapena nui kei te Toa!",
        "Paihere Uara Pai",
        "Nga Waahi Paihere Whakaiti",
        "Nga utu nui i runga i nga Hiko",
        "Tiaki Ataata i runga i nga mea kua tohua",
        "Nga Whakaaetanga Whakakore i runga i nga Paihere",
        "E waatea ana nga utu maha-hoko",
        "Whakapaihia mo te Iti Inaianei",
        "Hoko Nui = Tiaki Nui",
      ],

      description: [
        "Whiwhi moni penapena nui ina paihere koe i nga hua kua tohua. Kia tere, he wa iti te tuku!",
        "Kei te tatari nga mahi atamai! Tiaki ake i te wa e hoko tahi ana i a maatau taonga kua tohua.",
        "Tirohia te huarahi mohio ki te penapena! Paihere i o tino pai ka matakitaki i te hekenga o te utu.",
        "Kia pai nga utu motuhake ina hoko mai i nga mea kua tohua i roto i te paheko. Mahi inaianei!",
        "Hokona atu, penapena ake! Nga utu kore e taea te whakatau i runga i a maatau paihere hua motuhake.",
        "Hokona atamai me a maatau tuku motuhake. Nga utu whakaheke mo nga paihere hua kua tohua.",
        "Tikina ake mo te iti ake! Nga utu whakamiharo ina hoko koe i a maatau paihere kua tohua.",
        "Tiaki nui ina paihere koe! Nga utu motuhake mo nga mea kua tohua mo te wa poto.",
        "Whakakotahitia ka penapena! Nga utu motuhake ina hoko tahi koe i a maatau hua kua tohua.",
        "Ko te nui ake o to hoko, ka nui ake te penapena! Kia mohio ki a maatau paihere motuhake.",
      ],
    },

    /**
     * Gift card type
     */
    gift_card_type: {
      title: [
        "Kia atawhai koe!",
        "Mou anake!",
        "Ka harikoa te hokohoko!",
        "Kia pai ki a matou!",
        "Whakanuia he iti",
        "No te mea he motuhake koe",
        "He Miharo iti",
        "Hanga to Ra",
        "Tenei ki a koe",
        "Horahia te harikoa",
        "He Koha Aroha",
        "Hokona me te Delight",
        "He Tohu Mihi",
        "Te wa ki te Splurge",
        "Whakanuia te Momeniti",
        "Kimihia Ō Makau",
        "Nau te Kōwhiringa",
        "Whakaatuhia koe",
        "Tirohia te Maama",
        "Wewetehia to ahua",
      ],
    },

    /**
     * Refund vendor order
     */
    vendor_refund_order: {
      note: [
        "Kua tangohia nga putea mai i to putea mariko ka whakahokia ki te putea hokohoko.",
        "Kua tukuna he moni whakahoki, e whakaiti ana i to toenga putea mariko.",
        "Kua oti te whakahoki. Kua whakakiia te putea maakete mai i to putea mariko.",
        "Kua nama to putea mariko mo te moni whakahoki ki te putea hokohoko.",
        "Kua whakatikahia o putea putea mariko na te whakahokinga moni tata.",
        "Kua whakahaerehia he moni whakahoki, ka tangohia te moni mai i to putea mariko.",
        "Kua whakahekehia to toenga putea mariko na te moni whakahoki ki te putea hokohoko.",
        "Kua puta he tauwhitinga whakahoki, kua nekehia nga moni mai i to putea mariko ki te kainga hokohoko.",
      ],
    },

    /**
     * Product
     */
    product: {
      warranty: [
        "24-Marama Gold Guarantee",
        "Tiaki Platinum mo nga tau e rua",
        "Whakapumau Taimana mo nga Marama 36",
        "Hiriwa Hiriwa: Kotahi Tau Rarangi",
        "2-Tau Whakaaetanga Rangatira",
        "Tiaki Moni mo te 24 Marama",
        "Pūrua Parahi: Pūtāhui Kotahi Tau",
        "3-Tau Whakaaetanga Whakaaetanga Rawa",
        "24-Marama Taputapu Ratonga Haumaru",
        "36-Marama Te rangimarie o te Hinengaro Tiaki",
      ],
      pros_name: [
        "Whakarei ake te kaha",
        "Utu-Painga Rongoa",
        "Kounga Whakapumautia",
        "Kaiwhakamahi-hoa",
        "Tiaki Wā",
        "Whakapumautia te Whakapono",
        "Whakamahinga Whanui",
        "Hangarau Matapihi",
        "Taiao",
        "Mahinga Rawa",
      ],

      pros_value: [
        "Ka whakaora wa nui me nga mahi tere me te whai hua.",
        "Ka whakamahi i nga hangarau matatau mo nga mahi kore e rite.",
        "Ka whakaiti i nga utu me te whakanui i te hua.",
        "He atanga ratarata-kaiwhakamahi mo te whakatere ngawari.",
        "Ka whakamanahia te mahi pono me te riterite.",
        "Ka whakarato i nga tono maha mo nga momo mahi.",
        "Ka whakamahi i nga rauemi me nga tukanga e pai ana ki te taiao.",
        "I hangaia me te hangarau hou mo nga hua pai ake.",
        "Ko te hoahoa-kounga teitei te whakarite kia mau tonu te roa.",
        "Ka tuku tautoko kiritaki huri noa i te karaka.",
      ],

      cons_name: [
        "Utu Utu",
        "Hototahi iti",
        "Tatūnga Matatini",
        "Me Whakangungu",
        "Mahinga Raro",
        "Kare he Mahinga",
        "Tautoko Iti",
        "Tiaki auau",
        "Te Whakamahi Pungao Nui",
        "Roa Ora Poto",
      ],

      cons_value: [
        "He nui ake te haumi tuatahi ki te whakataurite ki etahi atu.",
        "Kaore pea i te hototahi ki nga punaha me nga taputapu katoa.",
        "He tino uaua te mahi whakarite.",
        "Ka hiahiatia etahi atu whakangungu mo te whakamahi.",
        "Kaore pea nga mahi e tutuki i nga tumanako katoa.",
        "He iti te mahi, he kore noa iho ranei te mahi.",
        "Kaore pea te tautoko a nga kaihoko i te wa katoa.",
        "Ka hiahiatia te tiaki i nga wa katoa, ka piki ake nga utu.",
        "Ka pau te nui o te kaha, ka pa ki nga utu whaipainga.",
        "Ko te roa o te oranga o te hua kare pea e rite ki te mea e tumanakohia ana.",
      ],
    },

    /**
     * Blog Category
     */
    blog_category: {
      category: [
        "Hangarau Hangarau",
        "Ka kimi Kai Kai",
        "Tales Haereere",
        "Tohutohu Hauora",
        "Fashion Faves",
        "Buzz Pakihi",
        "Nga Korero Tikanga",
        "Ngā Whakahōu Hākinakina",
        "Nga Tohu Toi",
        "Nga Tohu Matua",
      ],
      description: [
        "Tirohia nga tuhinga mohio mo nga kaupapa rereke.",
        "Tirotirohia ta maatau kohinga o nga panui rangitaki korero.",
        "Kia noho mohio me o maatau waahanga kaupapa blog.",
        "Kimihia te hiringa na roto i a maatau tuhinga waahanga blog whakaohooho.",
        "Whakawhanuihia to mohiotanga me o maatau momo kaupapa kaupapa momo blog.",
        "Kia mau tonu nga korero me o maatau waahanga blog whakahihiri.",
        "Rukuhia ki roto i te hohonutanga o ta maatau momo momo tuhinga whakahihiri.",
        "Hurihia nga korero nui i roto i a maatau puranga momo tuhinga.",
        "Akohia tetahi mea hou mai i ta maatau waahanga rangitaki kua ata whakaritea.",
        "Whakauru atu ki a maatau tuhinga momo tuhinga me nga panui whakarangatira.",
      ],
    },

    /**
     * Partner Plan
     */
    partner_plan: {
      name: [
        "Painga Whakamata",
        "Mahere Tino",
        "Roopu Ngaio",
        "Putanga hinonga",
        "Te taumata teitei",
        "Mōkī Premium",
        "Otinga Whakamutunga",
        "Taumata whiriwhiri",
        "Whakatere Tupu",
        "Te Mana Platinum",
      ],
    },

    /**
     * Tax Profile
     */
    tax_profile: {
      name: [
        "Nga kai",
        "Pūrere Hiko",
        "Te kakahu me te kakahu",
        "Pukapuka & Tuhituhi",
        "Hua Hauora me te Ataahua",
        "Nga taonga",
        "Taonga Hakinakina",
        "Taonga Taonga me nga Taakaro",
        "Nga whakapaipai me nga taonga",
        "Nga Waahanga Waka",
        "Maara me waho",
        "Nga taputapu kihini",
        "Whakapaipai Whare",
        "Taonga Waiata",
        "Nga Taonga Pet",
        "Huhu",
        "Peeke & Tueke",
        "Nga Taonga Tari",
        "Utauta taputapu",
        "Nga hoko kai",
      ],

      label: [
        "VAT", // Value-Added Tax
        "GST", // Goods and Services Tax
        "tūemi", // Provincial Sales Tax
        "tūemi", // Harmonized Sales Tax
        "tūemi", // Quebec Sales Tax
        "tūemi", // Central Sales Tax
        "Taake Taake", // Excise Tax
        "Kawemai", // Import Duty
        "Taake Ratonga", // Service Tax
        "Taake papai", // Luxury Tax
      ],
    },
    /**
     * Cluster
     */
    cluster: {
      name: [
        "Nga Tohu Hoko",
        "Waehere Whakatairanga",
        "Nga Whakatairanga Whakaaturanga",
        "Tau Hou",
        "Kaihoko Pai",
        "Nga Taonga Motuhake",
        "Nga Tukunga Iti",
        "Nga Mahi Takahanga",
        "VIP Motuhake",
        "Nga Mea Whakakore",
        "Paihere Hararei",
        "Kohinga Kaupapa",
        "Nga Whakaaturanga Waitohu",
        "Nga Makau Mema",
        "Nga utu utu",
        "Hoko Tino",
        "Rauemi Mamati",
        "Whakaaro Koha",
        "Ko nga whiriwhiringa taiao",
        "Nga Tohutohu Kaimahi",
      ],
    },

    /**
     * Popup
     */
    popup: {
      title: [
        "Toha Motuhake!",
        "Tau Hou",
        "Wā iti",
        "Matohi utu utu!",
        "Nga Mahi Motuhake",
        "Hoko wera!",
        "Matohi Whakakore",
        "He utu kore utu",
        "Koha i Roto!",
        "Riro Tohu",
        "Nga Whakaaetanga Mema",
        "Hoko Kohiko",
      ],
    },

    /**
     * Campaign
     */
    campaign: {
      name: [
        "Hokona 'Til You Drop",
        "Pokarekare Kaata",
        "Fiesta Hoko Kohiko",
        "Patohia, Hoko, Tiaki",
        "Hokona Bonanza",
        "E-Deal Extravaganza",
        "Mega Markdown Madness",
        "Cyber ​​Savings Spree",
        "Nga Ra Whakatau Mamati",
        "E-Commerce Carnival",
        "Putanga Ipurangi Oasis",
        "Rere Retail",
        "Te Hokonga Whare Taonga Tukutuku",
        "Pararaiha a E-Shopper",
        "Mariko Voucher Venture",
        "Mamati Doorbusters",
        "Whakawātea E-Kaata",
        "Whakaaturanga Hoko Ipurangi",
        "tūemi Hokonga tūemi",
        "E-hoko Blitz",
      ],
    },

    /**
     * Valuation
     */
    valuation: {
      title: [
        "Utu Whakaritenga T-Shirt",
        "Te Whakataunga Taonga Taonga",
        "Utu Hutu Whaiaro",
        "Whakaritenga Toi Aromatawai",
        "Utu Hutu Whakaritea",
        "Utu Whakaritenga Taputapu Hiko",
        "Te Whakataunga Whakapaipai Whaiaro",
        "Utu Puke Ritenga",
        "Te Aromatawai kakahu",
        "Utu Utu Puka Whaiaro",
        "Whakaritenga Whakataunga Taonga",
        "Te utu o te Maataki Maataki",
        "Te Whakataunga Takaroi Whaiaro",
        "Te utu o te Potae kua whakaritea",
        "Te Whakataunga Hua Tiaki kiri Ritenga",
        "Te utu mo te whakapaipai i te whare",
        "Te Aromatawai Taputapu Whakaritea",
        "Utu Tuhituhi Whaiaro",
        "Te Whakataunga Whakataunga Huruhuru",
        "Utu Taonga Taakaro Ritenga",
      ],
      structure: {
        title: [
          "Hoahoa Tā T-hāte",
          "Momo whakarewa whakapaipai Meta",
          "Kōwhiringa Rauemi Hu",
          "Rahi Anga Toi",
          "Te Kowhiringa Hutu kakahu",
          "Kōwhiringa Tae Taputapu",
          "Kotaha kakara kakara",
          "Rawa Puke me te Rahi",
          "Te roa o te kakahu me te ahua",
          "Rauemi Uhi Pukapuka",
          "Momo Rakau Taonga",
          "Matakitaki Rauemi Taataki",
          "Taonga Taonga Taonga Taonga",
          "Rahi Potae me te Tae",
          "Manakohanga Kai Tiaki kiri",
          "Kaupapa Whakapaipai Whare",
          "Rauemi Taonga Kihini",
          "Momo Pepa Tuhituhi",
          "Rauemi Taonga Huruhuru",
          "Rahi Taputapu Taakaro",
        ],
      },
    },

    /**
     * Lottery
     */
    lottery: {
      title: [
        "Hurihuri & Whiwhi Tohu Nui!",
        "Hukarere Wira Fortune",
        "Waimarie Spin Bonanza",
        "Hurihia ki te Win Rewards",
        "Wira o te Taonga",
        "Waimarie Lottery Extravaganza",
        "Maere Miro Mega",
        "Whakataetae Wira Tohu",
        "Taonga Miro Mīharo",
        "Hurihia te Maama Wira",
        "Waimarie Wheel Galore",
        "Te manaakitanga a Fortune",
        "tūemi o tūemi",
        "Hurihia mo te Taonga",
        "Prize Spinner Delight",
        "Lottery Spin koura",
        "tūemi-he-tūemi",
        "Wira o te Wairau",
        "Whakaaturanga Waimarie Waimarie",
        "Utu Mimio Mutunga",
      ],
      description: [
        "Hurihia te wira mo te whai waahi ki te wikitoria i nga utu motuhake!",
        "Whakamātauria to waimarie - kei te tatari nga taonga whakamiharo i roto i to maatau wira!",
        "Riro nui ki ia miro - he utu whakahihiri hei hopu!",
        "Hurihia kia toa! Ko nga taonga ohorere me nga tuku i nga wa katoa.",
        "Ko to tupono ki te piro nui - huri i te wira mo nga taonga inamata!",
        "Nga utu motuhake me ia miro - ko koe te tangata waimarie?",
        "Miro ka wini! Hurihia nga taonga huna i roto i te wira o to maatau toa.",
        "Kia waimarie me ta maatau wira - nga taonga, nga utu, me etahi atu!",
        "He toa nga miro katoa – hopu i to taonga motuhake i tenei ra!",
        "Hurihia te wira mo te ohorere - he aha koe ka toa?",
        "Wetekina te waimarie me te hurihanga – kei te tatari nga utu toa whakahihiri.",
        "Hurihia hei whakaatu i to taonga - utu, koha, me etahi atu!",
        "Ma te miro i te ra ka mau te hikaka i roto i te keemu - wini taonga ahurei!",
        "Hurihia to maatau wira mo nga taonga toa motuhake.",
        "Ka haria mai he toa waimarie - tikina to taonga inaianei!",
        "Ka taea e to miro te whakatuwhera i nga utu hokohoko whakamiharo!",
        "Hurihia kia whai waahi ki te putea i nga utu me nga tuku motuhake.",
        "Kei te hurihuri te wira o te waimarie – hopukia to taonga waimarie!",
        "Miro, wikitoria, me te ataata - nga utu ohorere i ia hurihanga.",
        "Hurihia ka waiho ma te waimarie e whakatau to taonga!",
      ],
    },

    /**
     * Subscription Price
     */
    subscription_price: {
      title: [
        "tūemi Urunga",
        "Mōkī tūemi",
        "Mōkī Koura",
        "tūemi Mahere",
        "tūemi Ohaurunga",
        "Te taumata hinonga",
        "Paihere Whakamata",
        "He mutunga kore tuku",
        "Whānau Pack",
        "Roopu Ngaio",
      ],
      description: [
        "He pai mo nga tangata takitahi e torotoro ana i nga kaupapa.",
        "Urunga Ki katoa Mōkī tūemi ka tūemi.",
        "Nga painga motuhake me te tautoko matua.",
        "He putea taurite mo nga kaiwhakamahi auau.",
        "Te urunga mutunga me nga painga o runga.",
        "Nga otinga ritenga mo nga whakahaere nui.",
        "He kōwhiringa taumata-urunga utu.",
        "Kaore he rohe mo te whakamahi, mo te kaiwhakamahi hiko.",
        "He mahere motuhake mo nga whanau, tae atu ki te 4 mema.",
        "I hangaia mo nga tohunga e rapu ana i nga taputapu matatau.",
      ],
    },

    spec: {
      group: [
        "Whakatakotoranga Hangarau",
        "tūemi Mōkī",
        "Mahi tūemi",
        "Kōwhiringa Hononga",
        "Nga Whakaritenga Mana",
        "Material ka Hanga tūemi",
        "Tae ka Whakaoti tūemi",
        "Storage ka tūemi",
        "Hototahitanga Pūnaha Whakahaere",
        "tūemi ka tūemi",
        "Ihirangi mokete",
        "tūemi ka Tautoko",
        "Waitohu ka tūemi",
        "Te Taiao",
        "Pungao Pungao",
        "tūemi ka tūemi",
        "Kōwhiringa Whakaritenga",
        "Nga taputapu kei roto",
        "kaiwhakamahi tūemi ka Nga arotake",
        "Te utu ka tūemi",
      ],
    },

    avocado: {
      title: [
        "Hokonga Rihiti",
        "Hokonga tūemi",
        "🛍️ Ota nama",
        "💳 Utu nama",
        "kiritaki Rihiti",
        "Tauwhitinga tūemi",
        "Te nama tūemi",
        "Takitaki Rihiti",
        "🛍️ Ota tūemi",
        "Hokonga 🛍️ Ota nama",
        "Hokonga Rihiti",
        "tūemi nama",
        "E-Rihiti",
        "te nama",
        "Hua nama",
        "💳 Utu Rihiti",
        "Whakaaetanga Whakatau",
        "Te nama nama",
        "Takitaki nama",
        "kiritaki nama",
      ],
      message: [
        "Mauruuru koe mo to hoko!",
        "Ka mihi matou ki to pakihi.",
        "Kei te haere mai to ota!",
        "Nga mihi mo te hokohoko ki a matou.",
        "Kei te titiro whakamua ki te mahi ano ki a koe.",
        "Ko to pai ki a maatau te kaupapa matua.",
        "Kaua e wareware ki te whakatau i to wheako.",
        "Mauruuru koe mo te tautoko i to maatau toa.",
        "Ko te tumanako ka koa koe ki to hokonga hou!",
        "Tiakina to nama mo te tohutoro a meake nei.",
        "Haerenga us tūemi mō Ētahi atu.. tūemi Hua.",
        "tūemi Kua hono mō tūemi Nga tuku.",
        "Mauruuru koe mo te kaihoko nui.",
        "Kei konei matou ki te awhina mena ka hiahia awhina koe.",
        "to Urupare tūemi us tūemi.",
        "Kia pai te utu motuhake mo to hokonga ka whai ake.",
        "He pai to whiringa!",
        "Mauruuru koe mo to whakawhirinaki ki a matou.",
        "Kei te titiro whakamua koe ki to urupare.",
        "He mihi aroha mai i ta matou roopu.",
      ],
    },

    /**
     * Map Tag
     */
    map_tag: {
      title: [
        "Whakaaturanga Taonga",
        "kaihoko tūemi",
        "Nga Whare Whakaaturanga",
        "Maakete rohe",
        "Tau Hou",
        "Nga Maara Whenua",
        "Takiwa hokohoko",
        "Toi Alley",
        "Nga Taonga papai",
        "Te Tari Reti",
        "Nga Whare Tuwhera",
        "Hangarau Hangarau",
        "Huaahua huarahi",
        "Kokonga Mahi",
        "Kooti Kai",
        "Puka Nook",
        "Te whakapaipai tūemi",
        "Te huarahi tawhito",
        "Rohe whakapakari tinana",
      ],
    },

    /**
     * Marketplace Documents (For vendors)
     */
    marketplace_document: {
      title: [
        "Tiwhikete Rehita Pakihi",
        "Tau Tautuhi Taake (TIN)",
        "kaihoko tūemi puka",
        "Hua tūemi",
        "Te utu Rarangi",
        "Peeke Pūkete Nga korero",
        "tūemi o wāhi noho",
        "Tiwhikete Inihua",
        "Kamupene kōtaha",
        "Tiwhikete Whakapumau Kounga",
        "Rau Raraunga Haumaru (SDS)",
        "Nga Korero Whakaputanga Hua",
        "📦 Hoki mai tūemi",
        "Nga Tikanga Tukunga me te Whakahaere",
        "kiritaki te tūemi",
        "Te hokohoko ka tūemi Mahere",
        "Hononga Paapori",
        "Tohutoro Tauhokohoko",
        "tūemi Waehere o tūemi",
        "Whakaaetanga Whakaatu-kore (NDA)",
      ],
    },

    /**
     * Expert Contract
     */
    expert_contract: {
      title: [
        "Te kainga hokohoko toa Ratonga tūemi",
        "Kirimana Whirihoranga Toa",
        "Whakaaetanga Ratonga Taa-I-Tuinga",
        "tūemi Te hokohoko te tūemi",
        "Whakaaetanga Tatūnga Paerewa E-tauhokohoko",
        "kirimana Tohunga Tohunga mo nga Kaipupuri Pakihi",
        "te tūemi mō Te kainga hokohoko tūemi",
        "Toa toa ka tūemi Ratonga tūemi",
        "Whakaaetanga Whakarato Ratonga POD",
        "Whakaaetanga Whakatinana Rautaki Tauhokohoko",
        "Kirimana Ratonga Whakawhanake Pakihi",
        "E-tūemi toa toa tūemi",
        "Whakaaetanga Whakarato Ratonga Tohunga",
        "Nga Whakataunga Pakihi me te Whakatau kirimana",
        "Ritenga Toa toa Ratonga tūemi",
        "Te kainga hokohoko tūemi te tūemi",
        "tūemi ka toa Ratonga tūemi",
        "POD tūemi Ratonga tūemi",
        "Te hokohoko ka tūemi Ratonga tūemi",
        "Whakaaetanga Ratonga Whakawhanui Pakihi",
      ],
    },

    /**
     * Account
     */
    account: {
      account_name: [
        "tūemi Pukoro",
        "tūemi Pūkete",
        "Whaiaro Pukoro",
        "tūemi Pukoro",
        "tūemi Pūkete",
        "Paraimere Pukoro",
        "tūemi Pūkete",
        "tūemi Pukoro",
        "Tauwhitinga Pukoro",
        "tūemi Pūkete",
      ],
    },

    /**
     * Cashback
     */
    cashback: {
      title: [
        "Hoko & Whiwhi Moni Moni",
        "Ka utua e te pono te hokinga moni",
        "Ko nga Whakahoki Moni Motuhake",
        "Kaihoko Moni",
        "Nga utu mo te whakahoki moni inamata",
        "Tiaki Moni Moni Moni",
        "Nga Ra Whakahoki Moni Rua",
        "Te hokinga moni mo ia hokonga",
        "Putea Putea",
        "Kaupapa Whakahoki Moni VIP",
        "Wā Te wa tūemi",
        "Te Karapu Utu Utu Utu",
        "Toa Ētahi atu.., Tiaki Ētahi atu..",
        "Utu Utu Utu Whakamutunga",
        "Whakahoki Moni Moni",
      ],
      description: [
        "Whiwhi moni mo ia hoko!",
        "Whiwhi utu mo to pono.",
        "Ko nga utu utu utu motuhake mo koe anake.",
        "Kia pai ki te penapena moni me nga utu putea.",
        "He hokinga moni inamata mo o taonga tino pai katoa.",
        "Whakaorangia ake me nga utu utu mo te tau.",
        "Whakaruatia to putea i nga ra motuhake!",
        "Whiwhi moni whakahoki i nga wa katoa e hoko ana koe.",
        "Hono atu ki te hikaka i tenei ra!",
        "He pai ki nga mema VIP te hokinga moni motuhake.",
        "tūemi! Wā Te wa tūemi Nga tuku.",
        "Hono mai ki ta matou Karapu Utu Moni inaianei.",
        "te Ētahi atu.. tūemi Toa, te Ētahi atu.. tūemi Tiaki.",
        "Wewete i nga utu utu nui.",
        "Ka wheako i nga utu tino pai mo te whakahoki moni huri noa.",
      ],
    },

    /**
     * Page
     */
    page: {
      prompts: [
        "Nau mai ki {shop} - Tirotiro To Tatou Awhe Tuemi Motuhake",
        "{shop} - To Puna Mutunga mo nga Taonga Kounga me te Ratonga Tino Pai",
        "Nga Mahi Motuhake me te Tiaki Tiaki anake i {shop} - Hopukia te Faingamālie!",
        "Kia mohio koe ki a {shop} ki nga mahi atawhai me nga tikanga matatika",
        "Me noho hei Waahanga o te Whanau {shop} - Ohauru ki ta matou Panui mo nga Whakahoutanga",
        "Rukuhia ki nga Kowhiringa Rawa me nga Taonga Whakataunga Tino i {shop}",
        "Whakaatuhia nga whakaaro me nga whakaaro auaha mo nga huihuinga katoa i {shop}",
        "Ko nga Taataki Kaore he Taatete me nga Tohu Waahi-waa i {shop} - Kia Tere!",
        "Nga Tohutohu Hoko Matawhānui a {shop} me nga Aromātai Hua - Hoko ma te Maia",
        "Totoro atu ki {shop} - Ka whakapau kaha matou ki te tuku i tetahi wheako hokohoko kore e rite",
      ],
    },

    /**
     * Cross-Selling
     */
    cross_selling: {
      message: [
        "Whakaotia to ahua, kapohia te huinga!",
        "Tiaki nui me a maatau putea paihere!",
        "Kaua e ngaro i a maatau takirua tino pai!",
        "Whakanuia to hokonga, tatari ana nga taapiri!",
        "Wewetehia nga penapena motuhake, hoko tahi!",
        "Nga taonga taapiri, nga utu motuhake!",
        "Tikina ake, whakapau iti - paihere!",
        "Wā-Te wa tuku: tūemi Hua!",
        "Whakapai ake i to wheako, utu paheko!",
        "Tirohia a maatau hiato kua tohua, tiakina inaianei!",
        "Whakahoahoa mo te paheko mutunga!",
        "Toa tūemi, tūemi ka Tiaki!",
        "Whakaruahia te uara, whakaruarua te ngahau!",
        "Kei konei to whakataetae tino pai - hokohoko inaianei!",
        "Nga utu paihere motuhake mo koe anake!",
        "Whakanuia to penapena me a maatau hiato!",
        "Whakanuia to kohinga me enei whiringa!",
        "Kua whakaritea mo koe: whakahoa me te penapena!",
        "Ko te takirua tino pai mo te ra tino pai!",
        "tūemi to Makau mō 20% tūemi!",
      ],
    },

    /**
     * Cross-Selling
     */
    vendor_pricing: {
      title: [
        "Taonga Mamati",
        "tūemi",
        "tūemi",
        "tūemi",
        "Kainga tūemi",
        "tūemi Hua",
        "Taputapu Takaro",
        "Taonga Taonga me nga Taakaro",
        "Nga hoko kai",
        "Nga taonga",
        "Nga Waahanga Waka",
        "Te whakapaipai",
        "tūemi & tūemi",
        "Nga Taonga Pet",
        "Nga Taonga Tari",
        "tūemi & tūemi",
        "tūemi & TV tūemi",
        "Pūmanawa",
        "tūemi & tūemi",
        "Taputapu o waho",
      ],
    },

    /**
     * Template Message
     */
    template_message: {
      title: [
        "Whakaaetanga Whakatau",
        "🛍️ Ota Ka tukuna",
        "🛍️ Ota Kua tukuna",
        "💳 Utu Kua riro",
        "🛍️ Ota tūemi",
        "Whakaroa Tukatuka Tono",
        "Te Ota Whakamau",
        "🛍️ Ota tūemi mō Tukunga",
        "📦 Hoki mai tūemi",
        "Whakahokinga i tukuna",
        "tūemi Whakamuri i tūemi",
      ],
      body: [
        "Kia ora {buyer_name}, kua whakamanahia to ota #{order_id}! Kei te tukatukahia e matou ka whakamohio atu ki a koe ina rite ana ki te tuku.",

        "He rongo pai, {buyer_name}! Kua tukuna to ota #{order_id}. Ka taea e koe te whai i roto i to putea. Nga mihi mo te hokohoko me matou!",

        "Kia ora {buyer_name}, kua tutuki pai to ota #{order_id}. Ko te tumanako ka koa koe ki to hoko. Mauruuru koe!",

        "Kia ora {buyer_name}, kua riro mai te utu mo te ota #{order_id}. Kei te tukatukahia to ota inaianei ka tukuna wawe.",

        "E {buyer_name}, e pouri ana matou ki te whakamohio atu kua whakakorehia to ota #{order_id}. Mēnā he pātai āu, whakapā atu ki te tautoko.",

        "E {buyer_name}, kei te roa to ota #{order_id}. E mihi ana matou mo te raruraru, kei te mahi ki te whakatika wawe tonu.",

        "Kia ora {buyer_name}, kei te puritia to ota #{order_id} i tenei wa. Tena koa tirohia to putea, waea atu ranei ki te tautoko mo etahi atu korero.",

        "Kia ora {buyer_name}, kua puta to ota #{order_id} mo te tukunga ka tae wawe atu ki a koe. Mauruuru koe mo to manawanui!",

        "E {buyer_name}, kua tukatukahia e matou te whakahoki mo to ota #{order_id}. Mēnā he pātai āu, whakapā atu ki te tautoko.",

        "Kia ora {buyer_name}, kua tukuna to moni whakahoki mo te ota #{order_id}. Me kite koe i nga moni i roto i to putea apopo.",

        "Kia ora {buyer_name}, kua hoki mai te taonga mai i to ota #{order_id} ki roto i nga taonga! Ka taea e koe te whakatakoto ota hou inaianei mena kei te pirangi tonu koe.",
      ],
    },

    /**
     * Property Set
     */
    property_set: {
      title: [
        "tūemi",
        "Taputapu Mamati",
        "tūemi",
        "Nga kai",
        "Nga taonga",
        "tūemi",
        "tūemi Hua",
        "tūemi",
        "Taputapu Takaro",
        "tūemi",
      ],
      description: [
        "Huanga mo nga taonga kakahu penei i te rahi, te tae, me te papanga.",
        "Nga whakaritenga mo nga taputapu penei i te waitohu, tauira, me nga ahuatanga.",
        "Nga reka, te rahi, me nga momo inu.",
        "Nga korero mo nga kai, paramanawa, me nga hua kai.",
        "Te papanga, te rahi, me nga momo hoahoa.",
        "Kaituhi, momo, me te tau whakaputa.",
        "Momo kiri, nga kai, me nga korero whakakakahu.",
        "Te reanga pakeke, nga rawa, me nga paerewa haumaru.",
        "Momo, tūemi, ka Mahi tūemi.",
        "tūemi, tūemi, ka Waitohu Nga korero.",
      ],
    },

    /**
     * Boxes
     */
    boxes: {
      name: [
        "tūemi XS",
        "tūemi S",
        "tūemi M",
        "tūemi L",
        "tūemi XL",
        "Pouaka XXL",
        "Pouaka Pikitia",
        "Pouaka Whanui",
        "Pouaka Taarahi",
        "Pouaka Papatahi",
        "Pouaka Pouaka",
        "Pouaka Pouaka",
        "Pouaka Whatiwhati",
        "Pouaka Pounamu Karaihe",
        "Pouaka Pouaka Maama", // flexible mailer
        "Pouaka Papaki", // bubble/padded
        "Pouaka Wewete", // thermal
        "Pouaka-Taumaha",
      ],
    },
  },
};
